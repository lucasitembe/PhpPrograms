...
$src = 'drill_acc=' . $this->input->get('drill_acc');
?>
<div class="row">
    <div class="col-lg-12">
        <ol class="breadcrumb">
            ...
            <?php if ($isLast) { ?>
                <li><i class="fa fa-laptop"></i><a href="<?= site_url("gledger/trialbalance?$src&acc_code=$ref_code&start_date=$start_date&end_date=$end_date") ?>"> Trial Balance</a></li>
            <?php } else { ?>
                <li><i class="fa fa-laptop"></i><a href="<?= site_url("gledger/trialbalance?start_date=$start_date&end_date=$end_date") ?>"> Trial Balance</a></li>	
            <?php } ?>
...
<div class="row">
    <div class="col-lg-12">
        <div id='table-options' class="pull-right">
            <?php if (!empty($src)) { ?>  
                <a href="<?= base_url('Gledger/trialbalance?report&' . $_SERVER['QUERY_STRING']) ?>" id="ajaxUpdateContainerPdf" class="btn btn-primary" target="_blank"><i class="fa fa-print"></i> Pdf</a> 
            <?php } else { ?>
                    ...
                    <div class="col-sm-12">
                        <div class="table-responsive"> 
                            ...
                            <table class="table table-hover" style="margin-bottom:0;"> 
                                <thead> 
                                    <tr>
                                        <th colspan="4"><h3><?= $this->input->get('drill_acc') ?></h3></th>
                                    </tr>
                                    <tr> 
                                        <th>Ledger Name</th> 
                                        <th>GL Account</th> 
                                ...
                                <tbody class="tbody-backc-color"> 
                                    ...
                                    if (count($ledgersTransactions)) {
                                        ...
                                        foreach ($ledgersTransactions as $val) {
                                            ...
                                            $hrefclose = '</a>';
                                            if (!$isLast) {
                                                $href = '<a href="' . base_url("Gledger/trialbalance?$src&ref_code=" . $this->input->get('acc_code') . "&start_date=$start_date&end_date=$end_date&id=" . $val->ledger_id) . '">';
                                            }
                                            ?>
                                            <tr>
