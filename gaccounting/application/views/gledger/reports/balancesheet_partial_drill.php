...
$src = '';
if (strtolower($source) == 'assets')
    $src = 'drill_asset_account';
if (strtolower($source) == 'liablilities')
    $src = 'drill_liab_account';
?>
<div class="row">
    <div class="col-lg-12">
        <ol class="breadcrumb">
            ...
            <?php if ($isLast) { ?>
                <li><i class="fa fa-laptop"></i><a href="<?= site_url("gledger/balancesheet?$src&acc_code=$ref_code&start_date=$start_date&end_date=$end_date") ?>"> Balance Sheet</a></li>
            <?php } else { ?>
                <li><i class="fa fa-laptop"></i><a href="<?= site_url("gledger/balancesheet?start_date=$start_date&end_date=$end_date") ?>"> Balance Sheet</a></li>	
            <?php } ?>
                                ...
                                <tbody class="tbody-backc-color"> 
                                    ...
                                    if (count($ledgersTransactions)) {
                                        ...
                                        foreach ($ledgersTransactions as $val) {
                                            ...
                                            $hrefclose = '</a>';
                                            if (!$isLast) {
                                                $href = '<a href="' . base_url("Gledger/balancesheet?$src&ref_code=".$this->input->get('acc_code')."&start_date=$start_date&end_date=$end_date&id=" . $val->ledger_id) . '">';
                                            }
                                            ?>
                                            <tr>
