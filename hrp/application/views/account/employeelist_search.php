<html>
<head>
    <title>Search Employee List</title>
    <script type="text/javascript">
        var base_url='<?php echo base_url(); ?>';
    </script>
<link type="text/css" href="<?php echo base_url(); ?>media/css/style2.css" rel="stylesheet" media="all" rel="stylesheet" />
        <link type="text/css" href="<?php echo base_url(); ?>media/css/superfish.css" rel="stylesheet" media="all" rel="stylesheet" />        
        <link type="text/css" href="<?php echo base_url(); ?>media/css/calendar.css" rel="stylesheet" media="all" rel="stylesheet" />        
        <script type="text/javascript" src="<?php echo base_url(); ?>media/js/jquery.js" ></script>
        <script type="text/javascript" src="<?php echo base_url(); ?>media/js/superfish.js" ></script>
        <script type="text/javascript" src="<?php echo base_url(); ?>media/js/calendar.js" ></script>
        <script type="text/javascript" src="<?php echo base_url(); ?>media/js/chain.js" ></script>
       
        <link type="text/css" href="<?php echo base_url(); ?>media/css/style.css" rel="stylesheet" media="all" rel="stylesheet" />
      
        <script type="text/javascript">
            
            function select_item(item){
                
window.opener.document.getElementById('<?php echo $input ?>').value=item;
top.close();
return false;
}
    </script>
</head>
      <body>
         
<div class="formdata" style="width: 98%;">
    <div class="formdata-title" >
        Employee Information
    </div>
    
    <div class="formdata-content" style="margin: 0px;   padding-top: 0px; ">
        
        <?php 
        $ipt= (isset ($input) ? $input : '');
        
        echo form_open('account/employeelist_search/'.$ipt,'style="width:95%;"'); ?>
        
        <table border="0" style="width: 100%;">
             <?php 
 
            if($this->session->flashdata('message') !=''){
            	?>
            	<tr>
            	<td colspan="2">
            	<div class="message">
            	<?php echo $this->session->flashdata('message'); ?>
            	</div>
            	</td>
            	</tr>
            	<?php
            }else if(isset($error_in)){
            	?>
            	<tr>
            	<td colspan="2" >
            	<div class="message">
            	<?php echo $error_in; ?>
            	</div>
            	</td>
            	</tr>
            	
            	<?php 
            }
            ?>
                <tr>
                    <td>First Name</td>
                    <td><input type="text" name="fname" value="<?php echo set_value('fname') ;?>"/></td>
                    <td>Last Name</td>
                    <td><input type="text" name="lname" value="<?php echo set_value('lname') ;?>"/></td>
                    <td>Employee ID</td>
                    <td><input type="text" name="employee" value="<?php echo set_value('employee') ;?>"/></td>
                    
                </tr>
                <tr>
                    <td>Gender</td>
                    <td><select name="sex">
                            <option value=""></option>
                            <option value="M">Male</option>
                            <option value="F">Female</option>
                        </select></td>
                    <td>Position</td>
                    <td><select name="position">
                            <option value=""></option>
                            <?php
                                        foreach ($position as $key => $value) { ?>
                            <option value="<?php echo $value->id; ?>"><?php echo $value->Name; ?></option>
                                        <?php }  ?>
                        </select></td>
                    <td>Department</td>
                    <td><select name="department">
                             <option value=""></option>
                            <?php
                                        foreach ($department as $key => $value) { ?>
                            <option value="<?php echo $value->id; ?>"><?php echo $value->Name; ?></option>
                                        <?php }  ?>
                        </select></td>
                    
                </tr>
              
                
                <tr class="submit">
                    <td colspan="6"><div>
                            <input type="submit" value="Search" name="Search"/>
                             
                        </div></td>
                </tr>
        </table>
        <?php echo form_close() ?>
    </div>
</div>



<div class="table_list" style="padding-top: 5px; width: 98%;">
    <div style="text-align: right; margin-right: 40px; color: brown; font-weight: bold;">Total Record Entered: <?php echo $employeetotalnumber; ?></div>
    <table class="view_data" cellspacing="0" cellpadding="0" style="width: 100%;">
        <th style="width:50px;">S/No</th>
        <th style="width:100px;">EmployeeID</th>
        <th>First Name</th>
        <th>Middle Name</th>
        <th>Last Name</th>
        <th>Sex</th>
        <th>Mobile</th>
        <th>Email</th>
</tr>

<?php 
$i=1;
if(count($employeelist) > 0){
 foreach ($employeelist as $key => $value){ 
?>
<tr <?php echo ($i%2 == 0 ? 'class="even_tr"':''); ?> >
    <td align="center"> <?php echo $i++; ?></td>
    <td><?php echo anchor('#',$value->EmployeeId,'onclick="return select_item(\''.$value->EmployeeId.'\')"'); ?></td>
    <td><?php echo $value->FirstName; ?></td>
    <td><?php echo $value->MiddleName;?></td>
    <td><?php echo $value->LastName; ?></td>
     <td><?php echo $value->Sex;?></td>
     <td><?php echo $value->Mobile;?></td>
    <td><?php echo $value->Email;?></td>
    
    
 </tr>   
<?php
}
}else{

 echo '<tr ><td colspan="8" align="center">No data found</td></tr>';
}
?>
 </table>
 <?php echo $links; ?> 
</div>
      </body>
</html>