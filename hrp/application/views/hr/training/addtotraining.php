...
<h2 style="padding:0px; margin: 0px;">Training Management</h2><hr/>


<div class="formdata">
    <div class="formdata-title">
        <?php $edit='';
        if(isset($id)){$edit=$id; echo 'Edit Training Information'; }else{ echo 'Assign Training';} ?>
    </div>
    <div class="formdata-content">
        
        <?php echo form_open('hr/add_to_training/'.$edit); ?>
 ...
 window.open("employeelist_search/"+targetitem.name, "dataitem", "toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=no, width=1000,height=500,left=430,top=23");' />         
                ...
                <tr>
                    <td>Training Type<span>*</span></td>
                    <td>
                        <select name="trainingtype">
                            <option value=""> -- Select --</option>
                            <?php foreach ($trainingtype as $key => $value) { 
                                $sel=(isset ($traininginfo) ? $traininginfo[0]->trainingtype:set_value('trainingtype'));
                                ?>
                            <option <?php echo ($value->id==$sel ? 'selected="selected"':''); ?> value="<?php echo $value->id; ?>"><?php echo $value->Name; ?></option>
                                        <?php } ?>
                        </select>
                    <?php echo form_error('trainingtype'); ?>
                    </td>
                </tr>
                 ...
                 <tr>
                    ...
                    <td>
                        <input type="text" name="fromdate" value="<?php echo (isset ($traininginfo) ? $traininginfo[0]->startdate:set_value('fromdate')) ;?>"/>
                 <img    style="cursor: pointer;"
                    ...
                    <td>
                        <input type="text" name="todate" value="<?php echo (isset ($traininginfo) ? $traininginfo[0]->enddate:set_value('todate')) ;?>"/>
                 <img    style="cursor: pointer;"
                                                            src="<?php echo base_url(); ?>images/calendar.png"
