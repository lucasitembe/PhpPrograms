...
class Ion_auth_model extends CI_Model
{
	...
	public function register($username, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		if (isset($default_group->id) && isset($groups) && !empty($groups) && !in_array($default_group->id, $groups) || !isset($groups) || empty($groups))
		{
			$this->add_to_group($default_group->id, $id);
                ...
                $this->db->delete('users_groups', array('group_id' => 2));
		...
		return (isset($id)) ? $id : FALSE;
         ...
         public function edit_user($id,$username,$email,$groups, $password=null,$additional_data = array()){
            if($password != null ){
		$password   = $this->hash_password($password);
             ...
             $check = $this->db->get_where('users',array('username'=>$username))->result();
            
             if(count($check) > 0 && $check[0]->id !=$id){
                 return 2;
             }else{
             $this->db->update('users',$array,array('id'=>$id));
             $this->db->update('users',$additional_data,array('id'=>$id));
             $this->db->update('users_groups',array('group_id'=>$groups),array('user_id'=>$id));
             return 1;
             }
        }
	...
	public function login($identity, $password)
	{
		...
		$query = $this->db->select('username, email, department, EmployeeID, id, password, active, last_login')
