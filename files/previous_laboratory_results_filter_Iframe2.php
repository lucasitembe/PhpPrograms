...
if(isset($_GET['DateGiven_From'])){
    $Date_From = $_GET['DateGiven_From'];
}else{
    $Date_From = '';
}

if(isset($_GET['DateGiven_To'])){
    $Date_To = $_GET['DateGiven_To'];
}else{
    $Date_To = '';
}
...
while($row = mysqli_fetch_array($select_Filtered_Patients)){
    ...
    $htm.="<tr>";
    $htm.="<td id='thead'>".$temp."</td><td><a href='laboratory_main_privew.php?Status_From=".$row['Status_From']."&item_id=".$row['Item_ID']."&patient_id=".$row['Registration_ID']."&payment_id=".$row['Patient_Payment_ID']."' target='_parent' style='text-decoration: none;'>".ucwords(strtolower($row['Patient_Name']))."</a></td>";
