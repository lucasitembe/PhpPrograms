...
//    if($consultationvalue=='All'){
      $sqlIterms="SELECT Item_ID,Product_Name,Item_Subcategory_ID,tis.Item_category_ID  FROM tbl_items ti JOIN tbl_item_subcategory tis ON ti.Item_Subcategory_ID=tis.Item_Subcategory_ID WHERE tis.Item_category_ID='".$row['Item_category_ID']."'";   
 echo $sqlIterms.'<br />';   
//      }else{
...
//   $grandTotal=0;
     ...
     $data.="<br/><table width = '80%'>
           ...
           ";
   $sponsor=$rowSponsors['Sponsor_ID'];
   while ($row = mysqli_fetch_array($categoryRs)) {
    ...
    //retreive the Subcategory
      ...
      $sql="SELECT tpp.Patient_Payment_ID,itm.Item_Subcategory_ID,tpl.Item_ID,tpp.Registration_ID,tpp.Supervisor_ID,tpp.Employee_ID, tpp.Sponsor_ID,tpp.Patient_Payment_ID, tpp.Receipt_Date,tpp.Billing_Type,tpp.Transaction_status,tpl.Patient_Payment_Item_List_ID,tpl.Item_ID,tpl.Price,tpl.Quantity,tpl.Discount,tpp.Payment_Date_And_Time,tpl.Sub_Department_ID,itc.Item_category_ID,tpl.Process_Status
             FROM tbl_patient_payments as tpp JOIN tbl_patient_payment_item_list AS tpl ON tpp.Patient_Payment_ID=tpl.Patient_Payment_ID
            JOIN tbl_items AS itm ON itm.Item_ID=tpl.Item_ID JOIN tbl_item_subcategory as itsb ON itsb.Item_Subcategory_ID=itm.Item_Subcategory_ID JOIN tbl_item_category as itc ON itc.Item_category_ID=itsb.Item_category_ID WHERE itc.Item_category_ID='".$CategoryID."' $Billing_Type AND tpp.Sponsor_ID='".$sponsor."' $consultationvalue $dataEntry $filter";
     ...
     //Retreive Iterms and filter
       ...
       $subcategoryRs=mysqli_query($conn,$sqlSubcategory) or die(mysqli_error($conn));
           if(mysqli_num_rows($subcategoryRs)>0){
              ...
              $u=1;
//              echo $sql.'<br /><br />';
//              echo 'mmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm'.'<br />';
//              
              $queryResult=  mysqli_query($conn,$sql) or die(mysqli_error($conn));
              while ($rowTpl = mysqli_fetch_array($queryResult)) {   
              if($rowTpl['Billing_Type']=='Inpatient Credit' || $rowTpl['Billing_Type']=='Outpatient Credit'){
...
<?php 
$q='';
    if(isset($_POST['showSponsorData'])){//$query_string
        $q='<a href="revenuegroupprintSummary.php?'.$printString.'" target="_blank" style=""><button type="button" class="art-button-green">Print</button></a>';
          echo '<a href="revenuebysponsors.php?Section=GeneralLedger&RevenueCollectionReport=RevenueCollectionReportThisForm" target="" style=""><button type="button" class="art-button-green">Back</button></a>';
 
   }else{
       $q='<a href="revenueprint.php?'.$printString.'" target="_blank" style=""><button type="button" class="art-button-green">Print</button></a>';
     echo '<a href="revenue.php" target="" style=""><button type="button" class="art-button-green">Back</button></a>';
   }
   echo $q;
...
//    var value=$(this).attr('ppval');
//     if(value>0){
//         
//     }else{
//     var removing=$('#hide'+itemID);
//     removing.html('');
//     removing.remove();
//  
//     }   
