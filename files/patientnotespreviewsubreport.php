<?php
	...
	//get Registration_ID, Admission_ID, Consultation_ID
	$select = mysqli_query($conn,"select Registration_ID, Consultation_ID, Admision_ID from tbl_nurse_notes where Notes_ID = '$Notes_ID'") or die(mysqli_error($conn));
	...
	$htm .= '<table width="100%">
	            <tr><td><span style="font-size: x-small;">NURSING NOTES</span></td></tr>
	            <tr>
		...
		$select = mysqli_query($conn,"select pp.Notes_Date_Time, pp.Notes, emp.Employee_Name
								from tbl_nurse_notes pp, tbl_employee emp where
								emp.Employee_ID = pp.Employee_ID and
								pp.Admision_ID = '$Admision_ID' and
								Notes_ID = '$Notes_ID' and
		...
		if($num > 0){
			while ($data = mysqli_fetch_array($select)) {
				$htm .= '<table width="100%" border=1 style="border-collapse: collapse;">
						<tr>
							<td  width="60%" colspan="3"><span style="font-size: x-small;">'.++$temp.'<b>. Date And Time : </b>'.@date("d F Y H:i:s",strtotime($data['Notes_Date_Time'])).'</td>
							<td width="40%" style="text-align: left;" colspan="2"><b><span style="font-size: x-small;">Nurse Name : </span></b>'.ucwords(strtolower($data['Employee_Name'])).'</td>
						</tr>';
				$htm .=	'<tr>
								<td width="14%" style="text-align: right; vertical-align: top;"><span style="font-size: x-small;">Nursing Notes</span></td>
								<td colspan="4"><span style="font-size: x-small;">'.$data['Notes'].'</span></td>
							</tr>
				</table><br/><br/>';
			}
		}
