...
<?php
	...
	$Date_To='';
        ...
        $today= mysqli_fetch_assoc(mysqli_query($conn,"SELECT NOW() AS NOWDATETIME"))['NOWDATETIME'];
...
<fieldset style='overflow-y:scroll; height:440px' >
    ...
    <table width='69%'>
        <tr>
            <td style="text-align: center" valgn="middle"><b>From</b></td>
	    <td style="text-align: center">
		<input type='text' name='Date_From' id='date_From' required='required'>    
	    </td>
<!--            <td style="text-align: center"><b>Diagnosis Type</b></td>
	    <td style="text-align: center">
                <select name="diagnosis_type" id="diagnosis_type" style="padding:5px;font-size:16px;width:97%   ">
                    <option value="all">All diagnosis</option>
                    <option value="finaldgno">Final Diagnosis</option>
                    <option value="provdgno">Provisional Diagnosis</option>
                    <option value="diffdgno">Differential Diagnosis</option>
                </select>
	    </td>-->
       ...
       <center>
		...
		echo "<thead><tr>
			...
			$employeeName=$select_doctor_row['Employee_Name'];
                       ...
                       // echo "SELECT ch.consultation_ID FROM tbl_consultation_history ch JOIN tbl_employee e ON ch.employee_ID=e.employee_ID WHERE ch.cons_hist_Date BETWEEN '$Date_From' AND '$Date_To' AND ch.employee_ID='$employeeID'";exit;
                        $patient_no_number=0;
