<?php
	...
	$select = mysqli_query($conn,"select pp.Check_In_ID, sp.Guarantor_Name, pp.Patient_Bill_ID, pr.Date_Of_Birth, pr.Registration_ID, pp.Patient_Payment_ID, sum((price - discount)*quantity) as Amount, pr.Patient_Name, pp.Folio_Number, pp.Sponsor_ID, pp.Claim_Form_Number
							...
							GROUP BY  pp.Registration_ID, pp.Folio_Number, pp.Patient_Bill_ID order by pp.Folio_Number limit 1200") or die(mysqli_error($conn));
	...
	if($num > 0){
		while ($data = mysqli_fetch_array($select)) {
			...
			$details = mysqli_query($conn,"select Phone_Number,Patient_Name, Date_Of_Birth, Member_Number, Gender from tbl_patient_registration where Registration_ID = '$Registration_ID'") or die(mysqli_error($conn));
			...
			<table width="100%">
				...
				<tr>
					...
					<td width="12%"><b>MEMBER NUMBER ~ <?php echo $Member_Number; ?></b></td>
                                        <td width="12%"><b>PHONE NUMBER ~ <?php echo $Member_Number; ?></b></td>
					<td width="15%"><b>AGE ~ </b><?php echo $age; ?></td>
				</tr>
				<tr>
				...
				<?php
					...
					$slct = mysqli_query($conn,"select ic.Item_category_ID, ic.Item_Category_Name from 
											...
											$Filter_Billing and
                                                                                        pp.receipt_date between '$Start_Date' and '$End_Date' and
											pp.Registration_ID = '$Registration_ID' group by ic.Item_Category_ID ORDER BY pp.receipt_date ASC
