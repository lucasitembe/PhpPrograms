...
//            
	...
	if ($Sub_Department_ID == 'Others') {
            $Sub_Department_ID = 'NULL';
        }
    
		
    if($Employee_ID != 0 && $Registration_ID != 0 && $Item_ID != 0 && $Quantity != '' && $Employee_ID .' '. $Billing_Type != '' && ($Sub_Department_ID > 0 || $Sub_Department_ID == 'NULL')){
		...
		//echo $Employee_ID .' '. $Registration_ID .' '.  $Item_ID .' '.  $Quantity .' '.  $Employee_ID .' '.  $Billing_Type .' '.  $Sub_Department_ID;exit;
    //validate data entered then proceed
        ...
        $insert_data = mysqli_query($conn,"INSERT INTO tbl_departmental_items_list_cache(
                                    ...
                                    values('$Claim_Form_Number','$Billing_Type','$Guarantor_Name',
                                        '$Sponsor_ID','$Item_ID','$Price',$Discount,'$Quantity',
                                        '$Consultant_ID','$Employee_ID','$Registration_ID','$Comment','$Sub_Department_ID','$Type_Of_Check_In','$Fast_Track','$Clinic_ID','$finance_department_id','$clinic_location_id')") or die(mysqli_error($conn).'fff');
    }
?>
...
<?php
    ...
    $select_Transaction_Items = mysqli_query($conn,
        "select Item_Cache_ID, Product_Name, Price,Discount,Quantity,Registration_ID, Comment, Sub_Department_ID, Type_Of_Check_In 
