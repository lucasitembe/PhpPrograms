...
<br/>
<fieldset>
    <legend align=right><b>OPTICAL APPROVAL</b></legend>
    <center>
        <table width=100%> 
            <tr> 
                <td>
                    <table width=100%>
                        ...
                        <tr>
                            ...
                            <?php
                            //get Billing Type
                            $slct = mysqli_query($conn,"select Billing_Type from tbl_payment_cache where Payment_Cache_ID = '$Payment_Cache_ID'") or die(mysqli_error($conn));
                            $nmz = mysqli_num_rows($slct);
                            if ($nmz > 0) {
                                while ($dt = mysqli_fetch_array($slct)) {
                                    $Billing_Type = substr($dt['Billing_Type'], 0, 3);
                                }
                            } else {
                                $Billing_Type = 'out';
                            }
...
<fieldset style="display:none">
    <table width="100%" id="Removed_Area">
        <tr>
            <td style="text-align: right;">
                <b>TOTAL : <?php echo number_format($Grand_Total); ?></b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            </td>
        </tr>
    </table>
</fieldset>
...
<script>
    ...
    function update_discount_price(Payment_Item_Cache_List_ID){
       ...
       $.ajax({
           type:"GET",
           url:"change_discount_price_for_approval_glasses_optical.php",
...
<script type="text/javascript">
    function make_Payments(Payment_Cache_ID) {
        ...
        if (Confirm_Message == true) {
            document.location = 'Approve_Selected_Transaction_Optical.php?' + Section_Link + 'Payment_Cache_ID=' + Payment_Cache_ID + '&Registration_ID=' + Registration_ID + '&Billing_Type=' + Billing_Type;
        }
    }
</script>
<script>
