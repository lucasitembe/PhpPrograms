...
<script type="text/javascript">
	...
	if(patientlist=='Outpatient cash'){
	    document.location = "./rchcashpatientlist.php";
...
<!--<br/>-->
<fieldset>  
            <legend align=right><b><?php if(isset($_SESSION['Rch'])){ echo $_SESSION['Rch']; } ?></b></legend>
        <center>
            <table width=100%> 
                <tr> 
                    <td>
				...
				<td style='text-align: right'>Receipt Date & Time</td>
					...
					<option selected='selected'>Rch</option> 
				    </select>
				</td>
				...
				<?php
				    if(isset($_SESSION['Rch_Supervisor'])) {
					if(isset($_SESSION['Rch_Supervisor']['Session_Master_Priveleges'])){
					    if($_SESSION['Rch_Supervisor']['Session_Master_Priveleges'] = 'yes'){
						$Supervisor = $_SESSION['Rch_Supervisor']['Employee_Name'];
					    }else{
...
<!--					     <a href='Send_To_Cashier.php?Payment_Cache_ID=<?php echo $Payment_Cache_ID; ?>&Transaction_Type=<?php echo $Transaction_Type; ?>&Sub_Department_Name=<?php echo $Sub_Department_Name; ?>&Registration_ID=<?php echo $Registration_ID; ?>' class='art-button-green' onclick='return approved_Message2();'>
		...
		if($Transaction_Status_Title != 'APPROVED' && $Transaction_Status_Title != 'PAID'){
			...
			if(strtolower($Departmental_Collection) == 'yes'){
				...
				echo "<a href='Departmental_Patient_Billing_Rch_Page.php?Payment_Cache_ID=".$Payment_Cache_ID."&Transaction_Type=".$Transaction_Type."&Sub_Department_ID=".$Sub_Department_ID."&Registration_ID=".$Registration_ID."&Billing_Type=".$Billing_Type."' class='art-button-green'>
					Make Payment
				    </a>";
			    }
			}
...
<?php if($Transaction_Status_Title != 'DISPENSED'){ 
	...
	<?php if(strtoupper($Transaction_Status_Title) == 'PAID'){ ?>
	    <a href='rchcashpatientlist.php?Payment_Cache_ID=<?php echo $Payment_Cache_ID; ?>&Transaction_Type=<?php echo $Transaction_Type; ?>' class='art-button-green'>Back To Main Page</a>
	<?php }else{ ?>
...
<?php }else{ ?>
	...
	<a href='rchcreditpatientlist.php?Payment_Cache_ID=<?php echo $Payment_Cache_ID; ?>&Transaction_Type=<?php echo $Transaction_Type; ?>&Registration_ID=<?php echo $Registration_ID; ?>&Billing_Type=<?php echo $Billing_Type; ?>' class='art-button-green'>Back To Main Page</a>
    <?php } ?>
<?php }} ?>
		<?php
...
<fieldset>   
		...
		<tr>
		    <td>
			<iframe src='rchitems_Iframe.php?Transaction_Type=<?php echo $Transaction_Type; ?>&Payment_Cache_ID=<?php echo $Payment_Cache_ID; ?>' width='100%' height=200px></iframe>
		    </td>
		</tr> 
	    </table>
        </center>
