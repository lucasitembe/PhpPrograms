...
if(isset($_POST['action'])){
    if($_POST['action']=='save'){
      ...
      $mtaa_jina = mysqli_real_escape_string($conn,$_POST['mtaa_jina']);

      $birth_dates = date_format(date_create($birth_date),'Y-m-d');
      $hudhurio_dates = date_format(date_create($hudhurio_date),'Y-m-d');

        $insert=mysqli_query($conn,"UPDATE tbl_postnal SET RCH_4_Card_No='$rchNo',mtaa_jina = '$mtaa_jina',Reg_Date='$postnatal_Date',Para='$para',Tarehe_ya_kujifungua='$birth_dates',Mahali_Alipojifungulia='$alipojifungulia',
          Kada_ya_mzalishaji='$mzalishaji_kada',Hali_ya_mama='$hali_mama',Hali_ya_mtoto='$hali_mtoto',Unyonyeshaji_withn_1_hr='$saa_1_nyonyesha',Hali_ya_VVU='$vvu_hali',Kipimo_VVU='$vvuPostnatal',
          Hudhurio='$Hudhurio',Hudhurio_Date='$tarehe_hudhurio',BP='$BP',HB='$HB',Matiti_hali='$Matiti',Uzazi_tumbo='$uzazi_tumbo',Rangi='$Lochia',Msamba_hali='$msamba',Fistula='$fistula',
          Akili_Timamu='$Akili',Aina_Dawa='$dawa_aina',Idadi_Dawa='$idadi_dawa',Idadi_Vitamin='$vitamin_A',Chanjo_TT='$ttchanjo',Family_planing='$uzazi_mpango',Hudhurio_la_mtoto='$mtoto_hudhurio',
          tareh_ya_hururio_la_mtoto='$hudhurio_dates',Mtoto_gender='$jinsi_mtoto',Joto='$mtoto_jina',Chanjo='$chanjo',Uzito_wa_mtoto='$uzito_mtoto',mtoto_HB='$HB_mtoto',KMC='$kmc',Jaundice='$Jaundice',
          Uambukizo_Mkali='$uambukizo_mkali',Kitovu='$kitovu',Ngozi='$Ngozi',Mdomo='$mdomo',Macho='$macho',ARV='$ARV',ARV_Muda='$muda',Ulishaji_wa_mtoto='$ulishaje_mtoto',Rufaa_To='$alikopelekwa',
          Rufaa_from='$alikotokea',Rufaa_reason='$rufaa_sababu' WHERE Mother_ID='$motherID'");
        if($insert){
            echo 'Data Updated successfully';
        }  else {
            echo 'Data updating error';
        }
    }
