...
if(mysqli_num_rows($sql_select_finance_department_name_result)>0){
   $finance_department_name=mysqli_fetch_assoc($sql_select_finance_department_name_result)['finance_department_name']; 
	...
	//generate filter
    $filter = "pp.Payment_Date_And_Time between '$Start_Date' and '$End_Date' and i.Item_ID = '$Item_ID' and ";
   ...
   $Clinic_ID = mysqli_real_escape_string($conn,$_GET['clinic']);
    ...
    if($Patient_Type == 'Inpatient'){
        ...
        if(strtolower($Section) == 'cash'){
            ...
            $filter .= "((Billing_Type = 'Outpatient Cash'Cash' and Pre_Paid = '0') or Billing_Type = 'Outpatient Credit') and Transaction_status <> 'cancelled' and ";
        }
    }else{
        if(strtolower($Section) == 'cash'){
...
<fieldset style='overflow-y: scroll; height: 320px; background-color: white;' id=''>
	<?php
	    if(strtolower($Section) == 'cashcredit'){
	        include("./General_Items_details_Cash_Credit.php"); //Cash & Credit
	    }else if(strtolower($Section) == 'cash'){ // Cash Only
	        include("./General_Items_details_Cash.php");
	    }else if(strtolower($Section) == 'credit'){
	        include("./General_Items_details_Credit.php");
	    }else if(strtolower($Section) == 'cancelled'){
	        include("./General_Items_details_Cancelled.php");
	    }else{
	        include("./General_Items_details_Credit.php");
	    }
	?>
    ...
    <table width="100%">
        <tr>
        	<td width="25%" style="text-align: right;">
                <input type="button" class="art-button-green" value="PREVIEW REPORT" onclick="Preview_Items_Display('<?php echo strtolower($Section); ?>',<?php echo $Item_ID; ?>)">
                <input type="button" class="art-button-green" value="BACK" onclick="Close_Items_Display()">
            </td>
        </tr>
    </table>
</fieldset>