...
<?php
echo "<a href='otherscredits.php?" . $Section_Link . "InvestigationCredit=InvestigationCreditThisPage' class='art-button-green'>BACK</a>";
?>
...
<br/>
<fieldset>
    <legend align=right><b>OUTPATIENT OTHERS BILL APPROVAL</b></legend>
    <center>
        <table width=100%> 
            <tr> 
                <td>
                    ...
                    <tr>
                        ...
                        <td style="text-align: right;" id="td1<?= $Payment_Item_Cache_List_ID ?>"><?php
                            if($Transaction_type=="Cash"){
                                ?>
                            <input type="text"style="width:50%"readonly="readonly" <?= $discount ?>onkeyup="update_discount_price('<?php echo $Payment_Item_Cache_List_ID ?>')" value="<?php echo $data['Discount']; ?>" id="<?= $Payment_Item_Cache_List_ID  ?>"/>
                                    <?php
                            }else{
...
<!--                        <td>
            ...
            <tr><td colspan="6" style="text-align: right;"><b>TOTAL</b></td><td style="text-align: right;" id="grand_total_td"><b><?php echo (($_SESSION['systeminfo']['price_precision'] == 'yes') ? number_format($Grand_Total) : number_format($Grand_Total)); ?></td></tr>
        </table>
    </center>
</fieldset>
<?php
    ...
    <script>
        function create_out_patient_bill(){
            ...
            if(confirm("Are you sure you want to create bill for this patient?")){
               $.ajax({
                   type: 'GET',
                    url: "create_and_approve_out_patient_bill_Others.php",
                    data: {Registration_ID:Registration_ID,Payment_Cache_ID:Payment_Cache_ID,Sponsor_ID:Sponsor_ID},
                    success: function (data) {
                        //alert(data)
                       // window.location = window.location.href;
                       if(data!="fail"&&data!="0"){
                          document.location="previewapprovedtransaction.php?Section=Revenue&Patient_Payment_ID="+data+"&Registration_ID="+Registration_ID+"&PreviewApprovedTransaction=PreviewApprovedTransactionThisPage";
                       }else{
