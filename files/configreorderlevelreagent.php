...
<center> 
        <center>
            <fieldset>
                    <legend align="center" ><b>REAGENTS RE-ORDER LEVEL CONFIGURATION</b></legend>
		    <br/>
		    <fieldset>
				...
				<tr>
					...
					<select name='Sub_Department_ID' id='Sub_Department_ID' required='required'>
						...
						$sql_select = mysqli_query($conn,"select tsd.Sub_Department_ID, tsd.Sub_Department_Name from
									    tbl_sub_department tsd, tbl_reagent_items_balance ib where
										tsd.Sub_Department_ID = ib.Sub_Department_ID group by ib.Sub_Department_ID") or die(mysqli_error($conn));
			...
			<table width=100%>
			    <tr>
					...
					<b>Category : </b>
						...
						<option selected='selected'></option>
						<?php
						    $data = mysqli_query($conn,"select Reagent_Name, Reagent_Category_ID from tbl_reagents_category") or die(mysqli_error($conn));
						...
						<tr>
							...
							<select name='Sub_Department_ID2' id='Sub_Department_ID2' required='required' onchange='Get_Previous_Value()'>
								...
								if($num > 0){
								    while($row = mysqli_fetch_array($sql_select)){
							    ?>
									<option value='<?php echo $row['Sub_Department_ID']; ?>'><?php echo ucwords($row['Sub_Department_Name']); ?></option>
							    <?php
...
<script>
	...
	myObject.open('GET','Get_List_Of_Reagents_Filtered_Items.php?Item_Category_ID='+Item_Category_ID+'&Item_Name='+Item_Name,true);
	myObject.send();
    }	
</script>
...
<script>
	...
	myObject.open('GET','Get_List_Of_Reagents_Items_List.php?Item_Category_ID='+Item_Category_ID,true);
	myObject.send();
    }
</script>
...
<script>
	...
	if (Sub_Department_ID != '' && Sub_Department_ID != null && Re_Order_Value != null && Re_Order_Value != '') {
		...
		if (Filter_Option) {
		    myObject.open('GET','Update_General_Re_Order_Level.php?Sub_Department_ID='+Sub_Department_ID+'&Re_Order_Value='+Re_Order_Value+'&Filter_Option=True',true);
		    myObject.send();
		}else{
		    myObject.open('GET','Reagent_Update_General_Re_Order_Level.php?Sub_Department_ID='+Sub_Department_ID+'&Re_Order_Value='+Re_Order_Value,true);
		    myObject.send();
		}
	    }
	}else{
...
<script>
	...
	if (Sub_Department_ID2 != null && Sub_Department_ID2 != '' && Item_ID != null && Item_ID != '') {
		...
		myObjectGetPreviousValue.open('GET','Reagent_Get_Previous_Re_Order_Level_Value.php?Sub_Department_ID='+Sub_Department_ID2+'&Item_ID='+Item_ID,true);
...
<script>
	...
	if (Sub_Department_ID2 != '' && Sub_Department_ID2 != null && New_Re_Order_Level_Value != null && New_Re_Order_Level_Value != '' && Item_ID != null && Item_ID != '' && Item_Name != '' && Item_Name != null) {
		...
		myObject.open('GET','Reagent_Update_Specific_Re_Order_Level.php?Sub_Department_ID2='+Sub_Department_ID2+'&New_Re_Order_Level_Value='+New_Re_Order_Level_Value+'&Item_ID='+Item_ID,true);
		myObject.send();
	    }
	}else{
	    if(Sub_Department_ID2 == '' || Sub_Department_ID2 == null){
   ...
   <?php
		$select = mysqli_query($conn,"select Sub_Department_ID, Sub_Department_Name, Department_Location from 
                                        tbl_sub_department sd, tbl_department dep where 
                                                sd.Department_ID = dep.Department_ID
                                                    order by sd.Sub_Department_Name") or die(mysqli_error($conn));
...
<script>
    function Update_Item_List(Sub_Department_ID,Sub_Department_Type){
	    ...
	    myObjectUpdateItemList.open('GET','Reagent_Refresh_Item_List.php?Sub_Department_ID='+Sub_Department_ID+'&Sub_Department_Type='+Sub_Department_Type,true);
