...
if (isset($_POST['action'])) {
    ...
    if ($_POST['action'] == 'view') {
		...
		pr.sponsor_id = sp.sponsor_id and pn.Mother_ID=pr.Registration_ID and Hudhurio_Date between '$fromDate' and '$toDate'  limit 500") or die(mysql_error());
...
//AGE FUNCTION
		...
		pr.sponsor_id = sp.sponsor_id and fp.Patient_ID=pr.Registration_ID and Visiting_Date between '$fromDate' and '$toDate'  limit 500") or die(mysql_error());
...
//		$age .= $diff->d." D";
           ...
           <th><b>Tarehe</b></th>
            ...
            $select_Filtered_Patients = mysql_query(
                "select pr.Patient_Name, pr.Date_Of_Birth, pr.Gender, pr.Registration_ID, pr.Phone_Number, pr.Member_Number,tw.* from tbl_patient_registration pr,tbl_wajawazito tw where tw.Patient_ID=pr.Registration_ID and hudhurio_tarehe between '$fromDate' and '$toDate'  limit 500") or die(mysql_error());
...
//                echo "<td><a href='powercharts_Wajawazito_real_edit.php?pn=" . $row['Registration_ID'] . "&PatientBilling=PatientBillingThisForm' target='_parent' style='text-decoration: none;'>" . $row['Gender'] . "</a></td>";
           ...
           <td><b>Tarehe</b></td>
            ...
            $select_Filtered_Patients = mysql_query(
                "select * from tbl_watoto tw where Tarehe between '$fromDate' and '$toDate'  limit 500") or die(mysql_error());
...
//                 UKUAJI
		...
		pr.sponsor_id = sp.sponsor_id and tw.Patient_No=pr.Registration_ID and admission_date between '$fromDate' and '$toDate' limit 500") or die(mysql_error());
               //tbl_wazazi
            while ($row = mysql_fetch_array($select_Filtered_Patients)) {
