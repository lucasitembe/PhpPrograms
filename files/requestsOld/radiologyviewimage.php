...
$consultation_query_result = mysql_query($consultation_query) or die(mysql_error());

if (@mysql_num_rows($consultation_query_result) > 0) {
    $row = mysql_fetch_assoc($consultation_query_result);
...
if (mysql_num_rows($result) > 0) {
    ...
    while ($row = mysql_fetch_array($result)) {
        ...
        $data.= "<td><input type='checkbox' id='close_id' onclick='remove_img(" . $row['Pic_ID'] . ",this,\"".$_SERVER['QUERY_STRING']."\",\"".$row['Product_Name']."\")'></td><td><img height='150' alt='' class='art-lightbox' src='RadiologyImage/" . $Radiology_Image . "' onclick='SelectViewer(\"$Radiology_Image\")' alt='" . $Radiology_Image . "'></td>";
    }
    $data.= '</tr></table>';
} else {
