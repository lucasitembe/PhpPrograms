...
if ($consultType == 'Radiology') { //check for radiology result
    ...
    $select_patients_qry = mysql_query($qr) or die(mysql_error());
    $sn = 1;

    while ($patient = mysql_fetch_assoc($select_patients_qry)) {
        ...
        $rs = mysql_query("SELECT Patient_Payment_ID FROM tbl_patient_payment_item_list WHERE Patient_Payment_Item_List_ID='$Patient_Payment_Item_List_ID'") or die(mysql_error());

        $ppID = mysql_fetch_assoc($rs);
        ...
        $select_radi_qry = mysql_query($select_radi) or die(mysql_error());
        if (mysql_num_rows($select_radi_qry) > 0) {
            while ($radist = mysql_fetch_assoc($select_radi_qry)) {
        ...
        $select_sono_qry = mysql_query($select_sono) or die(mysql_error());
        if (mysql_num_rows($select_sono_qry) > 0) {
            while ($sonog = mysql_fetch_assoc($select_sono_qry)) {
        ...
        $select_docomments_qry = mysql_query($select_docomments) or die(mysql_error());

        if (mysql_num_rows($select_docomments_qry) > 0) {
            while ($docom = mysql_fetch_assoc($select_docomments_qry)) {
    ...
    if (mysql_num_rows($QueryResults) > 0) {
        while ($row = mysql_fetch_assoc($QueryResults)) {
            ...
            $totalParm = mysql_num_rows($RS);
            $result = "";

            $postvQry = mysql_query("SELECT result FROM tbl_tests_parameters_results AS tpr WHERE result = 'positive' AND ref_test_result_ID='" . $row['test_result_ID'] . "'")or die(mysql_error());
            $positive = mysql_num_rows($postvQry);

            $negveQry = mysql_query("SELECT result FROM tbl_tests_parameters_results AS tpr WHERE result = 'negative' AND ref_test_result_ID='" . $row['test_result_ID'] . "'")or die(mysql_error());
            $negative = mysql_num_rows($negveQry);

            $abnormalQry = mysql_query("SELECT result FROM tbl_tests_parameters_results AS tpr WHERE result = 'abnormal' AND ref_test_result_ID='" . $row['test_result_ID'] . "'")or die(mysql_error());
            $abnormal = mysql_num_rows($abnormalQry);

            $normalQry = mysql_query("SELECT result FROM tbl_tests_parameters_results AS tpr WHERE result = 'normal' AND ref_test_result_ID='" . $row['test_result_ID'] . "'")or die(mysql_error());
            $normal = mysql_num_rows($normalQry);

            $highQry = mysql_query("SELECT result FROM tbl_tests_parameters_results AS tpr WHERE result = 'high' AND ref_test_result_ID='" . $row['test_result_ID'] . "'")or die(mysql_error());
            $high = mysql_num_rows($highQry);

            $lowQry = mysql_query("SELECT result FROM tbl_tests_parameters_results AS tpr WHERE result = 'low' AND ref_test_result_ID='" . $row['test_result_ID'] . "'")or die(mysql_error());
            $low = mysql_num_rows($lowQry);
            ...
            $query = mysql_query("select * from tbl_attachment where Registration_ID='" . $Registration_ID . "' AND item_list_cache_id='" . $row['Payment_Item_Cache_List_ID'] . "'");
            $attach = mysql_fetch_assoc($query);
            $image = '';
    ...
    $select_surgery = mysql_query("select pon.*, i.Product_Name, emp.Employee_Name from tbl_post_operative_notes pon, tbl_items i, tbl_item_list_cache ilc, tbl_employee emp where
										...
										i.Item_ID = ilc.Item_ID") or die(mysql_error());
    $num_surgery = mysql_num_rows($select_surgery);
    if ($num_surgery > 0) {
        $temp = 0;
        while ($dt = mysql_fetch_array($select_surgery)) {
            ...
            <fieldset style="background-color: white;">
                ...
                <?php
                $Number = 0;
                $select_post = mysql_query("select  d.disease_code, d.disease_name
											...
											pod.Diagnosis_Type = 'Preoperative Diagnosis'") or die(mysql_error());
                $no = mysql_num_rows($select_post);
                if ($no > 0) {
                    ?>
                    <table width="100%">
                        ...
                        <?php
                        while ($data = mysql_fetch_array($select_post)) {
                            echo "<tr>
                    ...
                    $select_post = mysql_query("select  d.disease_code, d.disease_name
											...
											d.disease_ID = pod.disease_ID and
											pod.Post_operative_ID = '$Post_operative_ID' and
											pod.Diagnosis_Type = 'Postoperative Diagnosis'") or die(mysql_error());
                    $no = mysql_num_rows($select_post);
                    if ($no > 0) {
                        ?>
                        <table width="100%">
                        ...
                        $select = mysql_query("select pop.Employee_Type, emp.Employee_Name from 
											tbl_post_operative_participant pop, tbl_employee emp where
											emp.Employee_ID = pop.Employee_ID and
											pop.Post_operative_ID = '$Post_operative_ID'") or die(mysql_error());
                        ...
                        if ($num > 0) {
                            ...
                            $select = mysql_query("select * from tbl_post_operative_external_participant where Post_operative_ID = '$Post_operative_ID'") or die(mysql_error());
                            $num = mysql_num_rows($select);
                            if ($num > 0) {
                                while ($data = mysql_fetch_array($select)) {
                                ...
                                /* //get post operative details
                                  $get_details = mysql_query("") or die(mysql_error());
                                  $nm_details = mysql_num_rows($get_details);
                                  if($nm_details > 0){
                                  while ($dtails = mysql_fetch_array($get_details)) {
	  ...
	  ilc.Status = 'served' AND ilc.Check_In_Type='Procedure' 
	   ...
	   WHERE Check_In_Type='Pharmacy' AND pc.Registration_ID='$Registration_ID' AND pc.consultation_id ='$consultation_ID' AND ilc.Status='dispensed'";
                        ...
                        //echo $subqr;exit;
                        $result = mysql_query($subqr) or die(mysql_error());
                        ...
                        if (mysql_num_rows($result) > 0) {
                            ...
                            while ($row = mysql_fetch_assoc($result)) {
                                $qr = mysql_query("SELECT Employee_Name FROM tbl_employee WHERE Employee_ID='" . $row['Dispensor'] . "'")or die(mysql_error());
                                $Disponsor = mysql_fetch_assoc($qr)['Employee_Name'];
