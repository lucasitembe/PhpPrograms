...
//    }
   $temp = 1;
    echo '<center>
      ...
      $select_Filtered_Patients ="SELECT 'cache' as Status_From,pr.Patient_Name,pc.Sponsor_Name,pr.Date_Of_Birth, emp.Employee_Name,
                                ...
                                pc.Payment_Cache_ID as payment_id,il.Process_Status as Status,il.Consultant as Doctors_Name,il.Transaction_Date_And_Time as Transaction_Date_And_Time,il.romoving_staff,il.removing_date
                                 ...
                                 JOIN tbl_patient_registration AS pr ON pr.Registration_ID =pc.Registration_ID JOIN tbl_employee as emp ON il.romoving_staff=emp.Employee_ID
...
//                                $getsatffname=mysql_query("SELECT * FROM tbl_employee WHERE Employee_ID='".$row['romoving_staff']."'");
//                                $result=  mysql_fetch_assoc($getsatffname);
                                echo "<td><a class='viewDetails' href='laboratory_sample_collection_details.php?Status_From=".$row['Status_From']."&patient_id=".$row['registration_number']."&payment_id=".$row['payment_id']."&Required_Date=".$row['Required_Date']."' target='_parent' style='text-decoration: none;'>".$row['Employee_Name']."</a></td>";
                                //}
                                echo"<td><input type='button' class='returnptnt' id='".$row['payment_id']."' value='Return Patient'></td>";
                                $temp++;
...
<script type="text/javascript"> 
	...
	$('.returnptnt').bind('click',function(){
            var payID=$(this).attr('id');
            if(confirm('Are you sure you want to return this patient to specimen list?')){
                   $.ajax({
                    type:'POST',
                    url:"requests/Search_Inpatient.php",
                    data:"returnptnlToList=returning&payID="+payID,
                    success:function(html){
                     //alert(html);
                    window.location.href='removedspecimenCollectionPatients.php';
