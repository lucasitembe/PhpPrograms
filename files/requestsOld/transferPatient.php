...
foreach ($dti as $data) {
    ...
    // $cnslRs=mysql_query("SELECT * FROM tbl_consultation WHERE Patient_Payment_Item_List_ID ='$ppil'") or die(mysql_error());
    // $consultation_id=mysql_fetch_assoc($cnslRs)['consultation_ID'];
...
//    $pcid = mysql_query("SELECT * FROM tbl_item_list_cache ilc LEFT JOIN tbl_payment_cache pc ON ilc.Payment_Cache_ID=pc.Payment_Cache_ID WHERE pc.consultation_id='$consID' AND pc.Registration_ID='$reg_id'") or die(mysql_error());
//
//    while ($row = mysql_fetch_array($pcid)) {
//
//        $qr = mysql_query("INSERT INTO  tbl_item_list_cache_transfer SELECT * FROM tbl_item_list_cache WHERE Payment_Item_Cache_List_ID='" . $row['Payment_Item_Cache_List_ID'] . "'") or die(mysql_error());
//        $qr = mysql_query("UPDATE  tbl_item_list_cache SET Consultant_ID='$transfers_To' WHERE Payment_Item_Cache_List_ID='" . $row['Payment_Item_Cache_List_ID'] . "'") or die(mysql_error());
//    }
   ...
   // echo "UPDATE tbl_patient_payment_item_list SET $transferToId,Patient_Direction='$patient_direction' WHERE Patient_Payment_Item_List_ID='$ppil'";exit;
    $qr = mysql_query("INSERT INTO  tbl_patient_payment_item_list_transfer SELECT * FROM tbl_patient_payment_item_list WHERE Patient_Payment_Item_List_ID='$ppil'") or die(mysql_error());
    $qr = mysql_query("UPDATE tbl_patient_payment_item_list SET $transferToId,Patient_Direction='$patient_direction' WHERE Patient_Payment_Item_List_ID='$ppil'") or die(mysql_error());

	
    $insertatin_querry = mysql_query("INSERT INTO tbl_transfer(Patient_Payment_Item_List_ID,Employee_ID_Do_Transfer,Registration_ID,Transfer_From,Transfer_To,Transfer_Date,
											Reason)
				VALUES ('$ppil'," . $Employee_ID . ",'$reg_id','$transfers_From','$transfers_To',NOW(),'$reason') ") or die(mysql_error());
    //exit;
...
$Select_patients = mysql_query($qr) or die(mysql_error());

//  $result = mysql_query($Select_patients);



$no = mysql_num_rows($Select_patients);
$datad = '';
...
if ($no > 0) {
    ...
    $temp = 1;
    while ($row = mysql_fetch_array($Select_patients)) {
        ...
        $select_checkin_qry = mysql_query($select_checkin) or die(mysql_error());

        $date = date('Y-m-d', strtotime($row['Transaction_Date_And_Time']));
        if (mysql_num_rows($select_checkin_qry) > 0) {
