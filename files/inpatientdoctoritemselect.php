<?php
	...
	if(strtolower($Consultation_Type)=='procedure'){
	    ...
	    (d.Department_Location='Family Planning'))";
	...
	if(@mysqli_numrows($Ppcid_result)>0){
...
<?php
	...
	if(mysqli_query($conn,$insert_query2)){
		$url = "inpatientdoctoritemselect.php?Consultation_Type=$Consultation_Type&Patient_Payment_Item_List_ID=$Patient_Payment_Item_List_ID&Round_ID=$Round_ID&Registration_ID=$Registration_ID&payment_cache_ID=$payment_cache_ID&Patient_Payment_ID=$Patient_Payment_ID&SearchListDoctorsPagePatientBilling=SearchListDirectCashPatientBillingThisPage";
		?>
...
<script type="text/javascript" language="javascript">
	...
	<script type='text/javascript'>
		...
		document.getElementById('doctordetaileddiagnosisselect_Iframe').src ='./inpatientdoctordetaileddiagnosisselect_Iframe.php?disease_name='+disease_name+'&disease_category_ID='+disease_category_ID+'&subcategory_ID='+subcategory_ID+'&Round_ID=<?php echo $Round_ID;?>&Consultation_Type=<?php echo $Consultation_Type;?>&Registration_ID=<?php echo $Registration_ID; ?>&Payment_Cache_ID=<?php echo $payment_cache_ID;?>&SearchListDoctorsPagePatientBilling=SearchListDirectCashPatientBillingThisPage';
	    }
...
<!--filtering services against categories-->
	<script type="text/javascript" language="javascript">
	    ...
	    function searchItem() {
		    ...
		    echo $Patient_Payment_ID;
		    ?>&Round_ID=<?php
		    echo $Round_ID;?>&Consultation_Type=<?php
	...
	<form method='post' action=''>
	 <table style='width: 100%;'>
		...
		<table>
			...
			<td>
			    <textarea id='comments' name='comments' required='required' rows='4' cols='100' ></textarea>
			</td>
			<?php
		...
		<tr>
			...
			<script type='text/javascript'>
				...
				if (Item_Name == '' || Item_Name == null) {
				    ...
				    var Service_Date_And_Time = document.getElementById('Service_Date_And_Time').value;
				    var Discount = document.getElementById('Discount').value;
					...
					if (bill_type!='' && Sub_Department_ID!='') {
					    ...
					    myObject.open('GET','InpatientsendOrRemove.php?action='+action+'&quantity='+quantity+'&Patient_Payment_ID='+Patient_Payment_ID+'&Consultation_Type=<?php echo $_GET['Consultation_Type'];?>&Sub_Department_ID='+Sub_Department_ID+'&Round_ID=<?php echo $Round_ID; ?>&Sponsor_ID=<?php echo $Sponsor_ID; ?>&Item_ID='+Item_ID+'&bill_type='+bill_type+'&Guarantor_Name='+Guarantor_Name+'&Registration_ID='+Registration_ID+'&comments='+comments+'&Discount='+Discount+'&Priority='+Priority+'&Service_Date_And_Time='+Service_Date_And_Time,true);
					    myObject.send();
				...
				document.getElementById('doctoritemcache').src="./inpatientdoctoritemcache.php?Round_ID=<?php echo $Round_ID;?>&Consultation_Type=<?php echo $Consultation_Type;?>&Registration_ID=<?php echo $Registration_ID; ?>&Payment_Cache_ID=<?php echo $payment_cache_ID;?>&Guarantor_Name=<?php echo $Guarantor_Name?>&SearchListDoctorsPagePatientBilling=SearchListDirectCashPatientBillingThisPage";
					...
					<td>
					    <fieldset>
						    <table width="100%">
							 <tr>
							     <td>
								  <iframe name='itemIframe' src="./inpatientdoctordetaileditemselect_Iframe.php?Item_Subcategory_ID=All&Item_Category_ID=All&Consultation_Type=<?php echo $Consultation_Type;?>&Patient_Payment_ID=<?php echo $Patient_Payment_ID;
								     ?>&Round_ID=<?php echo $Round_ID;?>&Patient_Payment_Item_List_ID=<?php echo $Patient_Payment_Item_List_ID;?>&Patient_Payment_ID=<?php echo $Patient_Payment_ID;?>&Registration_ID=<?php echo $Registration_ID; ?>&Payment_Cache_ID=<?php echo $payment_cache_ID;?>&Guarantor_Name=<?php echo $Guarantor_Name?>&provisional_diagnosis=<?php echo $_GET['provisional_diagnosis'];?>&SearchListDoctorsPagePatientBilling=SearchListDirectCashPatientBillingThisPage" width='100%' height='380px' id='frmaesearch'></iframe>
							     </td>
							 </tr>
						     </table>
					    </fieldset>	
