...
if (isset($_GET['type']) && $_GET['type'] == 'credit') {
    ...
    $data.="<table width = '80%'>
           <tr>
             <th>S/N</th><th>ITEM NAME</th><th style='text-align:right'>QTY</th><th style='text-align:right'>CREDIT</th><th style='text-align:right'>TOTAL</th>
           </tr>
            <tr>
             <td colspan='5'> <hr style='width:100%'/></td>
           </tr>
...
if (isset($_GET['type']) && $_GET['type'] == 'canceled') {
    ...
    $data.="<table width = '80%'>
            <tr>
             <th>S/N</th><th>ITEM NAME</th><th style='text-align:right'>QTY</th><th style='text-align:right'>CASH</th><th style='text-align:right'>CREDIT</th><th style='text-align:right'>TOTAL</th>
           </tr> 
            <tr>
...
while ($rowitem = mysqli_fetch_array($getItemID)) {
        ...
        while ($rowTpl = mysqli_fetch_array($queryResult)) {
            ...
            $total+=$totalsubCash + $totalsubCredit;
            // echo $i.'. Category_ID='.$row['Item_Category_ID'].' Category_Name='.$row['Item_Category_Name'].' Subcategory_ID='.$row['Item_Subcategory_ID'].'<br/>';
            $data.='<tr><td>' . $i . '</td><td>' . $rowTpl['Product_Name'] . '</td><td style="text-align:right">' . $TotalItem . '</td><td style="text-align:right">' . number_format($totalsubCash ). '</td><td style="text-align:right">' . number_format($totalsubCredit) . '</td><td style="text-align:right">' . number_format($totalsubCash + $totalsubCredit) . '</td></tr>';
            $totalsubCredit = 0;
            $totalsubCash = 0;
            $SuperTotalItems += $TotalItem; //$TotalItem
            $TotalItem = 0;
...
<a href="individualrevenue.php?<?php echo $query_string; ?>" style=""><button type="button" class="art-button-green">Back</button></a>
