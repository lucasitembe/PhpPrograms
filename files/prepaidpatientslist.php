...
<?php
    if(strtolower($Section) != 'revenue'){
        echo "<a href='./patientslist.php?Section=".$Section."&NewVisitors=NewVisitorsThisPage' class='art-button-green'>NEW VISITORS</a>";
    }
    
    if(strtolower($Section) == 'visitor'){
        echo "<a href='./visitorform.php?Section=".$Section."&VisitorForm=VisitorFormThisPage' class='art-button-green'>BACK</a>";
    }else if(strtolower($Section) == 'reception'){
        echo "<a href='./receptionworkspage.php?Section=".$Section."&ReceptionWork=ReceptionWorkThisPage' class='art-button-green'>BACK</a>";
    }else if(strtolower($Section) == 'revenue'){
        echo "<a href='./departmentpatientbillingpage.php?Section=".$Section."&DepartmentPatientBilling=DepartmentPatientBillingThisPage' class='art-button-green'>BACK</a>";
    }else{
        echo "<a href='./visitorform.php?Section=".$Section."&VisitorForm=VisitorFormThisPage' class='art-button-green'>BACK</a>";
    }
?>
...
<fieldset style="overflow-y: scroll; height: 400px;" id="Patient_Area">
    <legend align="left"><b>PRE / POST PAID PATIENTS</b></legend>
    <table width = "100%">
<?php
    ...
    $select = mysqli_query($conn,"select pr.Patient_Name, pr.Date_Of_Birth, pr.Gender, pr.Registration_ID, pr.Phone_Number, pr.Member_Number, sp.Guarantor_Name
                            from tbl_patient_registration pr, tbl_sponsor sp, tbl_prepaid_details pd where
                            pd.Registration_ID = pr.Registration_ID and pd.Status = 'pending' and
                            pr.Sponsor_ID = sp.Sponsor_ID order by Registration_ID Desc limit 200") or die(mysqli_error($conn));
    $num = mysqli_num_rows($select);
    if($num > 0){
        while ($data = mysqli_fetch_array($select)) {
            $link = '<a href="postpaidrevenuecenter.php?Registration_ID='.$data['Registration_ID'].'&Section='.$Section.'&PostPaidRevenueCenter=PostPaidRevenueCenterThisForm" style="text-decoration:none;">';
...
<script type="text/javascript">
    function Search_Patient(parameter){
        ...
        if(parameter == 'Patient_Name'){
            myObjectSearch.open('GET', 'Pre_Paid_List_Search_Patients.php?Sponsor_ID='+Sponsor_ID+'&Patient_Name='+Patient_Name+'&Section='+Section, true);            
        }else if(parameter == 'Patient_Number'){
            myObjectSearch.open('GET', 'Pre_Paid_List_Search_Patients.php?Sponsor_ID='+Sponsor_ID+'&Patient_Number='+Patient_Number+'&Section='+Section, true);
        }
        myObjectSearch.send();
    }
</script>
...
<script type="text/javascript">
    function Filter_Patient(){
        ...
        myObjectPr.open('GET', 'Pre_Paid_List_Search_Patients.php?Sponsor_ID='+Sponsor_ID+'&Section='+Section+'&Patient_Number='+Patient_Number+'&Patient_Name='+Patient_Name, true);            
        myObjectPr.send();
    }
</script>
<?php
