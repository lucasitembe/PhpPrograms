        ...
        <table  class="hiv_table" style="width:100%;margin-top:5px;">
            <tr> 
                <td style="width: 20px;text-align:center ">
                    ...
                    Category: <select id="procedureSubCatID" style='text-align: center;padding:4px; width:15%;display:inline'>
                        <?php 

                            $procedureQuery = mysqli_query($conn,"SELECT sb.Item_Subcategory_ID,sb.Item_Subcategory_Name FROM `tbl_item_subcategory` sb INNER JOIN tbl_items i ON i.`Item_Subcategory_ID` = sb.`Item_Subcategory_ID` WHERE i.Consultation_Type='Procedure' GROUP BY i.`Item_Subcategory_ID` ORDER BY sb.Item_Subcategory_Name ") or die(mysqli_error($conn));
                            $dataSubCategory = '';
                            echo '<option value="All">All Subcategory</option>';

                            while ($row = mysqli_fetch_array($procedureQuery)) {
                    ...
                    <input type="button" name="filter" value="FILTER" class="art-button-green" onclick="Filter_Procedure();"> 
                </td>
            </tr>
...
<script type="text/javascript">
    function Filter_Procedure(){
        var fromDate=$("#date_From").val();
        var toDate=$("#date_To").val();
        var SubCategory=$("#procedureSubCatID").val();
        if(fromDate.trim()!=='' && toDate.trim()!==''){
                ...
                $.ajax({
                    url:'government_procedure_reports.php',
                    type:'post',
                    data:{Filter_Category:'yes',fromDate:fromDate,toDate:toDate,SubCategory:SubCategory},
...
<script type="text/javascript">
        ...
        var SubCategory=$("#procedureSubCatID").val();
        if(fromDate.trim()!=='' && toDate.trim()!==''){
            window.location.href='print_procedure_excel_report.php?fromDate=' + fromDate + '&toDate=' + toDate + '&Sponsor=All' + '&SubCategory=' + SubCategory;
        }else{
            alert('FILL THE START DATE AND END DATE');
        }
    }
