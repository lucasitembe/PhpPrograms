...
<center><b>Radiology Service List</b></center><link rel="stylesheet" type="text/css" href="css/jquery.datetimepicker.css"/>
...
if($no > 0){
	...
	<?php
	    echo "<td><input type='text' name='Sub_Total' id='Sub_Total' readonly='readonly' value='".number_format(($row['Price'] - $row['Discount']) * $Quantity)."' style='text-align:right;'></td>";	
	    echo "<td style='text-align: right;' width=8%><a href='RemoveRadiologyItem.php?Payment_Item_Cache_List_ID=".$row['Payment_Item_Cache_List_ID']."&Payment_Cache_ID=".$row['Payment_Cache_ID']."' class='art-button-green' target='_Parent'>Remove</a></td>";
	    $total = $total + (($row['Price'] - $row['Discount']) * $Quantity);
	    $temp++;
	}   echo "</tr>";
	...
	if($No_Of_Items > 0){
	    echo "<tr><td colspan=5 style='text-align: right;'><b> TOTAL : ".number_format($total)."</b></td><td colspan=2 style='text-align: right;'>
		    <a href='radiologyitems_Iframe2.php?Transaction_Type=".$Transaction_Type."&Payment_Cache_ID=".$Payment_Cache_ID."' class = 'art-button'>View Removed Services</a>
			</td></tr>";
	}else{
	    echo "<tr><td colspan=6 style='text-align: right;'><b> TOTAL : ".number_format($total)."</b></td><td></td></tr>";
	}
