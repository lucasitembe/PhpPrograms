<?php
	...
	if(isset($_GET['Sub_Category_Name'])){
		$Sub_Category_Name = str_replace(" ", "%", $_GET['Sub_Category_Name']);
	}else{
		$Sub_Category_Name = 0;
	}
...
<table width="100%">
<?php
	$select = mysqli_query($conn,"select Item_Category_ID, Item_Category_Name from tbl_item_category $filter order by Item_Category_Name") or die(mysqli_error($conn));
...
?>
			...
			<tr>
				<td>
			<?php
				$slct = mysqli_query($conn,"select * from tbl_item_subcategory where Item_Category_ID = '$Item_Category_ID' and Item_Subcategory_Name like '%$Sub_Category_Name%'") or die(mysqli_error($conn));
