<?php
    ...
    if($Employee_ID != 0 && $Registration_ID != 0 && $Item_ID != 0 && $Quantity != '' && $Employee_ID != 0 && $Billing_Type != ''){
        ...
        $select = "select * from tbl_pharmacy_inpatient_items_list_cache
        ...
        if($no > 0){
            $delete_details = mysqli_query($conn,"delete from tbl_pharmacy_inpatient_items_list_cache where Employee_ID = '$Employee_ID'") or die(mysqli_error($conn));
        }
        if(strtolower($Transaction_Mode) == 'fast track transaction'){
            $Fast_Track = '1';
        }else{
        ...
        $insert_data = mysqli_query($conn,"INSERT INTO tbl_pharmacy_inpatient_items_list_cache(
