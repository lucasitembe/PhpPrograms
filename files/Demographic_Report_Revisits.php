					...
					if($no > 0){
						while ($row = mysqli_fetch_array($get_patients)) {
							...
							if($num_verify > 1){
								...
								$verify_again = mysqli_query($conn,"select Check_In_ID from tbl_check_in where Check_In_ID < '$Initial_Check_In_ID' and Registration_ID = '$Registration_ID' group by Registration_ID") or die(mysqli_error($conn));
