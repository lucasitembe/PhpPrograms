...
<br>
 ...
 <fieldset style="width:80%;height:360px;overflow:scroll;">
  ...
  $select_Food=mysqli_query($conn,"SELECT * 
			...
			WHERE 
				ft.Food_Time='BREAKFAST'  AND 
					DATE(Trans_Date_Time)=DATE_SUB('".DATE("Y-m-d")."',INTERVAL 0 DAY) AND
						(TIME(Trans_Date_Time) BETWEEN '00:00:01' AND '08:30:30' ) 	AND
							ft.Food_Menu_ID=fm.Food_Menu_ID AND 
								pr.Registration_ID=ft.Registration_ID 
									GROUP BY  fm.Food_Menu_ID") or die (mysqli_error($conn));
					
			$Check = mysqli_num_rows($select_Food);
	  ...
	  $temp = 1;
		$select_Name=mysqli_query($conn,"SELECT * 
					...
					WHERE 
						ft.Food_Time='BREAKFAST'  AND 
							DATE(Trans_Date_Time)=DATE_SUB('".DATE("Y-m-d")."',INTERVAL 1 DAY) AND
									(TIME(Trans_Date_Time) BETWEEN '00:00:01' AND '08:30:30' ) 	AND
										pr.Registration_ID=ft.Registration_ID AND 
											ft.Food_Menu_ID=fm.Food_Menu_ID AND
												fm.Food_Menu_ID='$Food_Menu_ID' AND 
													fm.Food_Name='$Food_Name' ") or die (mysqli_error($conn));
...
echo "<td><input type='text' value='".$row['Food_Standard']."'  name='Doctor_Comment'  readonly='readonly'></td>";
 ...
 $temp++;
		...
		echo'<br><br><br><br><br><br>';
		 echo'<b style="color:red;">NO FOOD TRANSACTION FOR BREAKFAST</b>';
		}
