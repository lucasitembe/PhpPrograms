        ...
        <table  class="hiv_table" style="width:100%;margin-top:5px;">
            <tr> 
                <td style="width: 20px;text-align:center ">
                    ...
                    <b>Report Type:</b> 
                    <select name="radiology_report_type" id="radiology_report_type"  style='padding:4px;'>
                        <option value="attendance">Patient Attendance</option>
                        <option value="frequency">Examination Frequency</option>
                    </select>
                    <b>Category:</b><select id="SubCategory" style='padding:4px;'>
                        <?php 
                    ...
                    <input type="button" name="filter" value="FILTER" class="art-button-green" onclick="Filter_Radiology();"> 
                </td>
            </tr>
...
<script type="text/javascript">
    ...
    function Filter_Radiology(){
        var fromDate=$("#date_From").val();
        var toDate=$("#date_To").val();
        var SubCategory=$("#SubCategory").val();
        var radiology_report_type=$("#radiology_report_type").val();
        if(fromDate.trim()!=='' && toDate.trim()!==''){
                ...
                $.ajax({
                    url:'government_radiology_reports.php',
                    type:'post',
                    data:{Filter_Category:'yes',fromDate:fromDate,toDate:toDate,radiology_report_type:radiology_report_type,SubCategory:SubCategory},
       ...
       function checkType(type){
        ...
        if(type==='frequency')
            page='print_radiology_frequency_excel_report.php';
        if(type==='')
            page='print_excel_file.php';
        return page;
    }
</script>
<script type="text/javascript">
        ...
        if(fromDate.trim()!=='' && toDate.trim()!==''){
        window.location.href=checkType($("#radiology_report_type").val())+'?fromDate=' + fromDate + '&toDate=' + toDate + '&Sponsor=All'+ '&SubCategory=' + SubCategory; 
        }else{
            alert('FILL THE START DATE AND END DATE');
        }
    }
