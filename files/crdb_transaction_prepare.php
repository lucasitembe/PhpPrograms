<?php
	...
	$select = mysqli_query($conn,"select sum((Price - Discount) * Quantity) as Amount
								from tbl_patient_payments_cache ppc, tbl_patient_payment_item_list_cache ppi
		...
		if($insert){
			...
			if($Transaction_ID != 0){
				...
				if($mynum > 0){
					...
					if($update){
						//get items
						$insert_cache = mysqli_query($conn,"select ppc.Patient_Payment_Cache_ID, Check_In_Type, Billing_Type, Sponsor_ID, Patient_Direction, Price, Discount, Quantity, Consultant, Consultant_ID, ppc.Registration_ID, Item_ID
								from tbl_patient_payments_cache ppc, tbl_patient_payment_item_list_cache ppi
								where ppc.Patient_Payment_Cache_ID = ppi.Patient_Payment_Cache_ID and
								Employee_ID = '$Employee_ID' and
								Registration_ID = '$Registration_ID' and
						...
						if($mn > 0){
							while ($row2 = mysqli_fetch_array($insert_cache)) {
								...
								$move = mysqli_query($conn,"insert into tbl_bank_items_detail_cache(
													...
													Transaction_Date_And_Time, Transaction_Date)

													VALUES ('$Check_In_Type','$Billing_Type','$Sponsor_ID',
															'$Item_ID','$Discount','$Price','$Quantity',
															'$Patient_Direction','$Consultant','$Consultant_ID',
															'$Employee_ID','$Registration_ID','$Transaction_ID',
															(select now()), (select now()))") or die(mysqli_error($conn));
							}
							if($move){
