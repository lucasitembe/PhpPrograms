   ...
   echo '<center><table width =100%>';
    ...
    $select_Filtered_Patients = mysqli_query($conn,
		...
		from tbl_patient_registration pr, tbl_sponsor sp where
		    pr.sponsor_id = sp.sponsor_id and Diseased='yes' order by Registration_ID Desc limit 200") or die(mysqli_error($conn));
	...
	//AGE FUNCTION
	 $age = floor( (strtotime(date('Y-m-d')) - strtotime($row['Date_Of_Birth'])) / 31556926)." Years";
	   // if($age == 0){
        ...
        echo "<tr><td width ='2%' id='thead'>".$temp."<td><a href='visitorform.php?Registration_ID=".$row['Registration_ID']."&PatientBilling=PatientBillingThisForm' target='_parent' style='text-decoration: none;'>".ucwords(strtolower($row['Patient_Name']))."</a></td>";
