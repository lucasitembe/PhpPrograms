 <?php
    ...
    if($Item_ID != 0 && $Item_ID != '' && $Store_Need != '' && $Quantity != 0 && $Store_Issue != 0){
        ...
        $select_details = mysqli_query($conn,"select * from tbl_reagents_requisition
                                        where Store_Need = '$Store_Need'
        ...
        $num = mysqli_num_rows($select_details);
            if($num > 0){
                ...
                $_SESSION['Reagents_Requisition_ID'] = $Requisition_ID;
                $insert_data2 = mysqli_query($conn,"insert into tbl_reagents_requisition_items(
                        ...
                        if($insert_data2){
                            ...
                            $select_Transaction_Items = mysqli_query($conn,"select itm.Product_Name, rqi.Quantity_Required, rqi.Item_Remark, rqi.Requisition_Item_ID
                                                                        from tbl_reagents_requisition_items rqi, tbl_items itm where
                ...
                $insert_data = mysqli_query($conn,"insert into tbl_reagents_requisition(
                                            ...
                                            values('$Requisition_Description',(select now()),(select now()),
                                                '$Store_Need','$Store_Issue','$Employee_ID','$Branch_ID')") or die(mysqli_error($conn));
                
                if($insert_data){
                    //select the current requisition id
                    $select_Requisition_ID = mysqli_query($conn,"select Requisition_ID from tbl_reagents_requisition
                    ...
                    if($no > 0 ){
                        while($row = mysqli_fetch_array($select_Requisition_ID)){
                            $Requisition_ID = $row['Requisition_ID'];
                            $_SESSION['Reagents_Requisition_ID'] = $row['Requisition_ID'];
                        }
                        $_SESSION['Reagents_Requisition_ID'] = $Requisition_ID;
                    }else{
                        $Requisition_ID = 0;
                    ...
                    if($Requisition_ID != 0){
                        //insert item into tbl_reagents_requisition_items table
                        $insert_data2 = mysqli_query($conn,"insert into tbl_reagents_requisition_items(
                                                        Requisition_ID,Quantity_Required,Item_Remark,Item_ID)
