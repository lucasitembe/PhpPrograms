...
<fieldset style='overflow-y: scroll; height: 400px; background-color:white' id='Previous_Fieldset_List'>
 ...
 <legend style='background-color:#006400;color:white;padding:5px;' align=right><b>Previous requisitions</b></legend>
	<?php
	    ...
	    echo '<tr><td colspan="9"><hr></td></tr>';
	    $sql_select = mysqli_query($conn,"select rq.Requisition_Description, rq.Store_Need, rq.Requisition_ID, rq.Created_Date_Time, rq.Store_Issue, rq.Sent_Date_Time, sd.Sub_Department_Name from
									...
									rq.requisition_status = 'submitted' order by rq.Requisition_ID desc limit 200") or die(mysqli_error($conn));
	    ...
	    if($num > 0){
			while($row = mysqli_fetch_array($sql_select)){
				...
				<td style="text-align: center;"><input type="button" value="Preview" onclick="Preview_Requisition_Report('.$row['Requisition_ID'].')" class="art-button-green">&nbsp;</td><td><a href="Control_Requisition_Sessions.php?Requisition_ID='.$row['Requisition_ID'].'&Previous_Requisition=True&Requisition=RequisitionThisPage" class="art-button-green">Edit</a></td></tr>';
				$temp++;
			}
	    }
	    echo '</table>';
