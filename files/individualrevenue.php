<?php
 ...
 if(isset($_GET['type']) && $_GET['type']=='cash'){
     ...
     $totalsubCash=0;
        ...
        // echo $i.'. Category_ID='.$row['Item_Category_ID'].' Category_Name='.$row['Item_Category_Name'].' Subcategory_ID='.$row['Item_Subcategory_ID'].'<br/>';
         $data.='<tr><td>'.$i.'</td><td><a href="individualItemsrevenue.php?category_id='.$category_id.'&category_name='.$category.'&sub_category_id='.$subCatID.'&sub_category_name='.$rowSubcat['Item_Subcategory_Name'].'&'.$query_string.'">'.$rowSubcat['Item_Subcategory_Name'].'</a></td><td style="text-align:right">'.number_format($TotalItem).'</td><td style="text-align:right">'.number_format($totalsubCash).'</td><td style="text-align:right">'.number_format($totalsubCash).'</td></tr>';
         $totalsubCash=0;
 ...
 if(isset($_GET['type']) && $_GET['type']=='credit'){
     ...
     $totalsubCredit=0;
       ...
       //retreive tde category
         $querySubcategory=  mysqli_query($conn,"SELECT Item_Subcategory_ID,Item_Subcategory_Name FROM tbl_item_subcategory WHERE Item_Category_ID='".$category_id."'");
         //die("SELECT Item_Subcategory_ID,Item_Subcategory_Name FROM tbl_item_subcategory WHERE Item_Category_ID='".$category_id."'");
         while ($rowSubcat = mysqli_fetch_array($querySubcategory)) {
          $subCatID=$rowSubcat['Item_Subcategory_ID'];
         //echo $subCatID;
         //exit;
         $sql="SELECT tpp.Patient_Payment_ID,itm.Item_Subcategory_ID,tpl.Item_ID,tpp.Registration_ID,tpp.Supervisor_ID,tpp.Employee_ID, tpp.Sponsor_ID,tpp.Patient_Payment_ID, tpp.Receipt_Date,tpp.Billing_Type,tpp.Transaction_status,tpl.Patient_Payment_Item_List_ID,tpl.Item_ID,tpl.Price,tpl.Quantity,tpl.Discount ,tpp.Payment_Date_And_Time,tpl.Sub_Department_ID,tpl.Process_Status
          ...
          $total+=$totalCredit;
        // echo $i.'. Category_ID='.$row['Item_Category_ID'].' Category_Name='.$row['Item_Category_Name'].' Subcategory_ID='.$row['Item_Subcategory_ID'].'<br/>';
         $data.='<tr><td>'.$i.'</td><td><a href="individualItemsrevenue.php?category_id='.$category_id.'&category_name='.$category.'&category_id='.$category_id.'&sub_category_id='.$subCatID.'&sub_category_name='.$rowSubcat['Item_Subcategory_Name'].'&'.$query_string.'">'.$rowSubcat['Item_Subcategory_Name'].'</a></td><td style="text-align:right">'.number_format($TotalItem).'</td><td style="text-align:right">'.number_format($totalsubCredit).'</td><td style="text-align:right">'.number_format($totalsubCredit).'</td></tr>';
         $totalsubCredit=0;
         $SuperTotalItems += $TotalItem;//$TotalItem
         $TotalItem=0;
         $i++;
 ...
 if(isset($_GET['type']) && $_GET['type']=='credit_cash'){
     ...
     $data.="<table width = '80%'>
           <tr>
             <th>S/N</th><th>SUBCATEGORY</th><th style='text-align:right'>QTY</th><th style='text-align:right'>CASH</th><th style='text-align:right'>CREDIT</th><th style='text-align:right'>TOTAL</th>
           </tr>   
            <tr>
     ...
     $totalsubCash=0;
        ...
        // echo $i.'. Category_ID='.$row['Item_Category_ID'].' Category_Name='.$row['Item_Category_Name'].' Subcategory_ID='.$row['Item_Subcategory_ID'].'<br/>';
         $data.='<tr><td>'.$i.'</td><td><a href="individualItemsrevenue.php?category_id='.$category_id.'&category_name='.$category.'&category_id='.$category_id.'&sub_category_id='.$subCatID.'&sub_category_name='.$rowSubcat['Item_Subcategory_Name'].'&'.$query_string.'">'.$rowSubcat['Item_Subcategory_Name'].'</a></td><td style="text-align:right">'.number_format($TotalItem).'</td><td style="text-align:right">'.number_format($totalsubCash).'</td><td style="text-align:right">'.number_format($totalsubCredit).'</td><td style="text-align:right">'.number_format($totalsubCash+$totalsubCredit).'</td></tr>';
...
//         //retreive tde category
 ...
 ?>
<a href="revenueindividualprint.php?<?php echo $query_string?>" target="_blank" style=""><button type="button" class="art-button-green">Print</button></a>
<a href="revenuesummary.php?<?php echo $query_string ;?>" style=""><button type="button" class="art-button-green">Back</button></a>
