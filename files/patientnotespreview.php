<?php
	...
	$htm .= '<table width="100%">
		...
		$select = mysqli_query($conn,"select pp.Notes_Date_Time, pp.Notes, emp.Employee_Name
								...
								Registration_ID = '$Registration_ID' order by Notes_ID desc limit 1") or die(mysqli_error($conn));
		$num = mysqli_num_rows($select);
		$temp = 0;
		if($num > 0){
			while ($data = mysqli_fetch_array($select)) {
				$htm .= '<table width="100%" border=1 style="border-collapse: collapse;">
						<tr>
							<td  width="50%" colspan="5"><span style="font-size: x-small;">'.++$temp.'<b>. Date And Time : </b>'.@date("d F Y H:i:s",strtotime($data['Notes_Date_Time'])).'
							&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>Nurse Name : </b>'.ucwords(strtolower($data['Employee_Name'])).'</td>
						</tr>';
				$htm .=	'<tr>
								<td width="14%" style="text-align: right;"><span style="font-size: x-small;">Nursing Notes</span></td>
