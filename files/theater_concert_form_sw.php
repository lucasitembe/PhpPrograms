...
<font color='blue;'> <b> Sasa unaweza Kuchukua Saini wa Wahusika kwa ajili ya Ridhaa</b></font>
 ...
 ";
 $patient_button="
 <a target='_blank' href='../esign/signature.php?Registration_ID=".$Registration_ID." 'class='art-button-green'>CHUKUA SAINI YA MLEZI</a>
 ...
 ";
 $printbtn="
 <a target='_blank' href='printrtheater.php?Registration_ID=".$Registration_ID."' class='art-button-green'>ONESHA NA CHAPISHA</a>
...
<body>
<a href="theaterconcertform.php?Theater_Consent_Form=Theater_Consent_Form" class='art-button-green'>BACK</a>
<center>
                              <div class="button_container">
                                    <div class="button_alignment">
                                          <!-- <a href='theater_concert_form.php?Registration_ID=<?php echo $Registration_ID; ?>&PatientBilling=PatientBillingThisForm'> -->
                                              <button style='width: 100%; height: 100%' onclick="history.go(-1)">
                                                  ENGLISH VERSION
                                              </button>
                                          </a>
                                    </div>
...
<td colspan="4"> 
<p>
Ninatoa ridhaa/ruhusa kwa madaktari na/au kwa kushirikiana na wahudumu wengine ili kutoa tiba au kufanya upasuaji ufuatao ambao unahusu:  <input type="text" class="text" name='Procedure_named' id='Procedure_named' value="<?php  echo $Product_Name;  ?>" style='width: 700px; border-top: none; border-left: none; text-align: center; font-weight: bold; border-right: none; border-bottom: 2px solid #000;'>
<p>
Namna na lengo la tiba/upasuaji limeelezwa vyema. Na zaidi faida, matokeo na madhara ya huo upasuaji, yameelezwa vyema kwangu na nimeelewa na kuridhia binafsi bila shuruti:  <input type="text" class="text" name='maelekezo' id='maelekezo' value="<?php if($Consent_ID > 0){ echo $maelekezo; } ?>" style='width: 700px; border-top: none; border-left: none; border-right: none; border-bottom: 2px solid #000;'>
</td>
</tr>
<tr>
<td colspan="4">
...
<input class="form_group" name="presense_of_students" value="Agree" type="radio" id="presense_of_students" <?php if($presense_of_students == "Agree") { echo "checked";}?> > Ninaruhusu
<input class="form_group" name="presense_of_students" value="Disagree" type="radio" id="presense_of_students" <?php if($presense_of_students == "Disagree") {echo "checked";}?> > Siruhusu
</td>
...
<input class="form_group" name="photography_on_surgery" value="Agree" type="radio" id="photography_on_surgery" <?php if($photography_on_surgery == "Agree") { echo "checked"; } ?> > Ninaruhusu
<input class="form_group" name="photography_on_surgery" value="Disagree" type="radio" id="photography_on_surgery" <?php if($photography_on_surgery == "Disagree") { echo "checked"; } ?> > Siruhusu
</td>
</tr>
<tr>
<td colspan='4'><hr></td>
...
<tr>
<td colspan='4'>
<p><input class="form_group" name="consent_amputation" value="Agree" type="radio" id="consent_amputation" <?php if($consent_amputation == "Agree") { echo "checked"; } ?> > <b>Ninakubali</b>
<input class="form_group" name="consent_amputation" value="Disagree" type="radio" id="consent_amputation" <?php if($consent_amputation == "Disagree") { echo "checked"; } ?> > <b>Sikubali</b>: Kuondolewa/kukatwa kiungo changu cha mwili ambacho ni: <input type="text" class="text" name='Amptutation_of' id='Amptutation_of' value="<?php  if ($datazangu){ echo $Amptutation_of; } ?>" style='width: 700px; border-top: none; border-left: none; border-right: none; border-bottom: 2px solid #000;'> 
</td>
</tr>
<tr>
<td colspan='4'><hr></td>
...
<label>Imeridhiwa na:  </label><input type="radio" id="vehicle" name='vehicle' value="Mgonjwa">
<label> Mgonjwa</label><input type="radio" id="vehicle" name='vehicle' value="Mlezi">
<label>Mlezi</label><input type="radio" id="vehicle" name='vehicle' value="Director">
...
Daktari Bingwa/Daktari wa Zamu: </b>
                ...
                if ($Consent_ID > 0){ echo "<input type='text'  style='width: 370px; border-top: none; border-left: none; border-right: none; border-bottom: 2px solid #000; text-align: center; font-weight:bold;' value=".$surgeon2.">"; }else{ ?>
                <b><select id='surgery_doctor2'><option value="<?php echo $Current_Employee_ID ?>" selected><?php echo $Current_Employee_Name ?></option><?php echo $select; ?> </select></b>
                <?php } ?></td>
                ...
                <?php if($Consent_ID > 0){ echo $doctor_button; } ?> 
                </td>
                <td   colspan="2"style="text-align:left" ><b>Uhusiano (kama sio mgonjwa mwenyewe):</b>
                <input type="text" name="relation" id='relation' placeholder="Relationship of Guardian/Proxy..." value="<?php if($Consent_ID > 0) {  echo $relation; }?> ">
                </td>
                </tr>
                <tr>
...
<td colspan="2"style="text-align:left" ><b>Jina la Shahidi wa Mgonjwa: </b>
                ...
                <?php if($Consent_ID > 0) { echo $witnes_button; }?>
                </td>
                </tr>
                <tr>
    ...
    function save_consent_data2(){
        ...
        if(Registration_ID != '0'){
            if(consent_amputation === 'Agree' && Amptutation_of === ''){
                alert("UMEKUBALI Kuondolewa/Kukatwa Kiungo cha mwili, Tafadhali Ainisha ni kiungo gani kitakachoondolewa");
                $("#Amptutation_of").css("border","2px solid red");
                $("#Amptutation_of").focus();
                exit();
            }else if(vehicle === undefined){
                alert("Tafadhali, Chagua Mtu anayesaini hii fomu ya ridhaa kabla ya kuhifadhi taarifa ulizojaza");
                exit();
            }else{
                if (confirm("Una hakika taarifa ulizojaza ni sahihi na unataka kuendelea?")) {
                    $.ajax({
                        ...
                        type: "post",
                        data: {Procedure_named:Procedure_named,behalf:behalf,relation:relation,maelekezo:maelekezo,designation:designation,patient_witness:patient_witness,Registration_ID:Registration_ID,presense_of_students:presense_of_students,Amptutation_of:Amptutation_of,surgery_doctor2:surgery_doctor2,photography_on_surgery:photography_on_surgery,consent_amputation:consent_amputation,vehicle:vehicle,Language:Language,Consultation_ID:Consultation_ID,Payment_Item_Cache_List_ID:Payment_Item_Cache_List_ID},
                        cache: false,
                        success: function(responce){
                            alert(responce);
                            location.reload(); 
