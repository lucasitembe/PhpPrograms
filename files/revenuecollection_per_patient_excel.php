...
$htm.= "<tr><td colspan='16' style='background:green; color:white; text-align:center;'>INPATIENT RESULT</td></tr>";
    ...
    $htm.= "</tr>";
        ...
        $htm.= "<tr><td colspan='16' style='color:red;'>No result found since $start_date to $end_date</td></tr>";
    }
    //===============+++ TOTAL FOR INPATIENT +++==============
        ...
        while($rwdt = mysqli_fetch_assoc($sql_payments_amaount)){
            ...
            if (mysqli_num_rows($sql_select_cash_pays) > 0) {
                while ($rws = mysqli_fetch_assoc($sql_select_cash_pays)) {
                    $subtotal_cash_bill_created =$rws['Amount'];
                }
            }
            ...
            if(($Consultation ==0) && ($total_Radiology ==0) && ($total_lab_test ==0) && ($total_dawa ==0)&& ($total_Procedure ==0) && ($total_upasuaji ==0) && ($total_Others ==0)){
                $background="style='background:yellow; color:black; font-size:15px;'";
            ...
            $Grand_total_cash_bill2 +=($subtotal_cash_bill_created);
    ...
    $htm.= "
                <td $background  style=''>$Employee_Names</td>
                ...
                <td $background>".number_format($subtotal_cash_bill_created)."</td>";
    ...
    $htm.= "<tr><td colspan='11' style='background:yellow; color:black; font-size:20px;' ><h5><b>TOTAL DEBIT BILLS PAID BY CASH DEPOSIT </b></h5></td><td colspan='5' style='background:yellow; color:black; font-size:20px;'><h5><b>".number_format($Waliolipamadeni, 2)."</b></h5></td><td></tr>";
...
header("content-Disposition: attachement; filename= Revenuecollection.xls");
echo $htm;