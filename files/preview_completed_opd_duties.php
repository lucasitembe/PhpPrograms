...
$no = mysqli_num_rows($get_details);
    ...
    while ($data2 = mysqli_fetch_assoc($get_details)) {
        ...
        $ward = mysqli_fetch_assoc(mysqli_query($conn,"SELECT Clinic_Name FROM tbl_clinic WHERE Clinic_ID='$duty_ward'"))['Clinic_Name'];
    ...
    $data .= '
        <div style="padding:5px; width:99%;font-size:12px;border:1px solid #000;  background:#ccc;text-align:center  " id="outpatient">
            <b align="center">NURSE HANDLING DUTY REPORT FOR '.strtoupper($ward).' CLINIC - '.strtoupper($Ward_Type).' WING - '.strtoupper($select_round).'</b>
        </div>
        <div style="border:1px solid #000">
            <table class="userinfo" border="0" style="border:none !important" width="100%" style="margin-left:2px">
                <tr>
                    ...
                    <td style="width:10%;text-align:right; font-size: 10px;"><b>Working Clinic</b></td style="font-size: 10px;"><td colspan="" style="font-size: 10px;">' . $ward . '</td>
                </tr>
                <tr>
