...
require_once './functions/items.php';
	if(isset($_GET['Section'])){
        $Section_Link = "Session=".$_GET['Section']."&";
	...
	foreach($items as $Payment_Item_Cache_List_ID){
    	...
    	if(mysqli_num_rows($select_items) > 0){			
			...
			while ($rm = mysqli_fetch_array($select_items)) {
				...
				$Price  = $rm['Price'];
				$Treatment_Authorization_No= $rm['Treatment_Authorization_No'];
				$Consultant_ID = $rm['Consultant_ID'];
				$Billing_Type = $rm['Billing_Type'];
				$Treatment_Authorizer = $rw['Treatment_Authorizer'];
				$Today = date('Y-m-d');	
			
				$select_patient_payment= mysqli_query($conn, "SELECT pp.Patient_Payment_ID, Payment_Date_And_Time FROM tbl_patient_payments pp, tbl_patient_payment_item_list ppl WHERE Registration_ID='$Registration_ID' AND pp.msamaha_Items='$msamaha_Items' AND Employee_ID='$Employee_ID' AND Check_In_ID='$Check_In_ID' AND pp.Patient_Payment_ID=ppl.Patient_Payment_ID $checkintype  AND Receipt_Date='$Today' ORDER BY pp.Patient_Payment_ID DESC LIMIT 1") or die(mysqli_error($conn));
