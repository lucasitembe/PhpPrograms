...
if (isset($_POST['process_procedure'])) {
    $_POST = sanitize_input($_POST, array('queryString', 'paymentItermCache'));
...
if (($billing_type == 'outpatient cash' && $transaction_type == "cash") || ($billing_type == 'outpatient cash' && $transaction_type == "credit")|| ($billing_type == 'outpatient credit' && $transaction_type == "cash") || ($billing_type == 'inpatient cash' && $payment_type == "pre" && $status_pro == 'paid') || ($billing_type == 'outpatient credit' && $status_pro == 'paid' && $require_approve == 'mandatory') || ($billing_type == 'inpatient cash' && $pre_paid == '1' && $payment_type=='pre')) {
            if ($status !== 'Select progress') {
                $checkPAymentStatus = "SELECT ilc.Sub_Department_ID,Item_ID,Registration_ID FROM tbl_item_list_cache as ilc INNER JOIN tbl_payment_cache as pc ON ilc.Payment_Cache_ID=pc.Payment_Cache_ID WHERE Payment_Item_Cache_List_ID='" . $value . "'";
            ...
            if ($status !== 'Select progress') {
                if ($status == 'served') {
                    ...
                    $checkPAymentStatus = "SELECT ilc.Sub_Department_ID,Registration_ID,Billing_Type,Folio_Number,Sponsor_ID,Sponsor_Name,Item_ID,Quantity,Price,Discount,Consultant,Consultant_ID FROM tbl_item_list_cache as ilc INNER JOIN tbl_payment_cache as pc ON ilc.Payment_Cache_ID=pc.Payment_Cache_ID WHERE Payment_Item_Cache_List_ID='" . $value . "'";
                    ...
                    if ($insert) {
                        ...
                        if ($Patient_Payment_ID != 0 && $Receipt_Date != '') {
                            $insert = mysqli_query($conn,"insert into tbl_patient_payment_item_list(Check_In_Type,Item_ID,Discount,Price,Quantity,Patient_Direction,Consultant,Consultant_ID,status,Patient_Payment_ID,Transaction_Date_And_Time,remarks,ServedDateTime,ServedBy,ItemOrigin,Sub_Department_ID,finance_department_id)
                                                        values('Radiology','$Item_ID','$Discount','$Price','$Quantity','others','$Consultant','$Consultant_ID','$status','$Patient_Payment_ID',(select now()),'" . $_POST['remarks_' . $value] . "',NOW(),'" . $_SESSION['userinfo']['Employee_ID'] . "','Doctor','$Sub_Department_ID','$finance_department_id')") or die(mysqli_error($conn));
                            if ($insert) {
    ...
    echo "<script type='text/javascript'>
                                ...
                                window.location='emergency_radiologypatientinfo.php?" . $queryString . "'
                            </script>";
}
...
$filter_room = '';
$Date_FromBack = filter_input(INPUT_GET, 'Date_From');
$Date_ToBack = filter_input(INPUT_GET, 'Date_To');
...
if(!empty($room)){
  $filter_room = " AND ilc.Atendace_Number='$room'";
}
...
if (isset($_GET['from'])) {
    ...
    echo"
    <a href='emergencyclinicalnotes.php?consultation_ID=$consultation_ID&Patient_Payment_ID=$Patient_Payment_ID&Patient_Payment_Item_List_ID=$Patient_Payment_Item_List_ID&Registration_ID=$Registration_ID' class='art-button-green'>
        BACK
    </a>";
        ...
        <table width=100%>
            <tr>
                ...
                <td style='text-align: right;'>
                    <?php
                    echo "<button class='art-button-green' onclick='viewPatientPhoto(".$Registration_ID.")'>VIEW PATIENT PHOTO</button>";
                    ...
                    if (isset($_GET['section']) && ($_GET['section'] == 'doctor' || $_GET['section'] == 'Doctorlist')) {
                        ...
                        ?>
                        <a href="prview_rad_test.php?source=new&<?php echo $_SERVER['QUERY_STRING'] ?>&billtype=<?php echo $Billing_Type ?>" target="_blank" id="preview" class="art-button-green">PREVIEW</a>
                ...
                <tr>
                    <td>
                        ...
                        <div id="patientItemsList" style='height:200px;overflow-x:hidden; overflow-y:scroll;  '>
                            <?php include "radiology_iframe.php"; ?>
                        </div>
...
<script>
  function open_echorcadiorgram(Registration_ID,Payment_Item_Cache_List_ID,Product_Name){
        $.ajax({
            type:'POST',
            ...
            success:function(data){
                ...
                $("#echorcadiorgram").dialog({
                        title: 'ECHOCARDIOGRAM FOR :-'+Product_Name,
                        width: '90%',
                        height: 650,
                        modal: true,
                    });
...
//
  ...
  function send_patient_result_to_ehms_mr_online(){
    var Registration_ID = <?= $Registration_ID ?>;
    var Payment_Item_Cache_List_ID_arry=[];
    $(".Procedureprogress").each(function () {
        var status = $(this).val();
        var id = $(this).attr('id');
        Payment_Item_Cache_List_ID_arry.push(id);
    });
//    alert(Registration_ID);
    $.ajax({
            type:'POST',
            url:'ajax_send_patient_radiology_result_to_ehms_mr_online.php',
            data:{Payment_Item_Cache_List_ID_arry:Payment_Item_Cache_List_ID_arry,Registration_ID:Registration_ID},
            success:function(data){
                console.log(data);
               // alert(data);
               document.getElementById('myForm').submit();
...
<script>
    function SaveComments(PPILI, Item_ID, Registration_ID) {
        ...
        datastring = datastring.replace("&", "and");
