...
while ($row = mysqli_fetch_array($select_Transaction_Items_Active)) {
    ...
    } elseif (($billing_Type == 'inpatient cash' && $status == 'active') || ($billing_Type == 'inpatient cash' && $status == 'removed') || ($billing_Type == 'inpatient credit' && $status == 'active' && $transaction_Type == "cash")) {
        if ($pre_paid == '1') {
    ...
    //echo $billing_Type.' '.$status.' '.$payment_type;

    if ($transStatust) {
        $displ = '<td style="text-align:center" colspan="7"><h5 style="color:#FF0E7B;font-size:20px" class="notpaid">' . $msg . '</td>';
    } else {
        ...
        $displ.= '<td>' . $add_parameters . $commentsDescription . '';
    }
