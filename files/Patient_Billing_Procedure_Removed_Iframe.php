   ...
   // echo $Transaction_Status_Title;
	...
	if($no > 0){
		...
		echo "<td><input type='text' name='Sub_Total' id='Sub_Total' readonly='readonly' value='".number_format(($row['Price'] - $row['Discount']) * $Quantity)."' style='text-align:right;'></td>";
                echo "<td><input type='checkbox' class='procedurepayment' id='".$row['Payment_Item_Cache_List_ID']."'></td>";
	    ...
	    if($No_Of_Items > 0){
		 $dataAmount= "<td style='text-align: right;'><b> TOTAL : ".number_format($total)."</b>
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<button type='button' class='paymentProcedure art-button'>Make Payment</button>   
                       </td>";
	    }else{
//		 $dataAmount= "<td style='text-align: right;'><b> TOTAL : ".number_format($total)."</b></td>";
    ...
    $(document).on('click','.paymentProcedure',function(){
      var locate=$(location).attr('href');
     var datastring='';
     var num=1;
     var n = $(".procedurepayment:checked").length;
    if(n>0){
    if(confirm("Are you sure you want to perform this transaction ?")){
    $('.procedurepayment').each(function(){
    var id=$(this).attr('id');
