        ...
        $num = mysqli_num_rows($select_details);
            if($num > 0){
                while($row = mysqli_fetch_array($select_details)){
					...
					$_SESSION['Pharmacy_Requisition_ID'] = $Requisition_ID;
                }
                $_SESSION['Pharmacy_Requisition_ID'] = $Requisition_ID;
                        $Selling_Price = Get_Selling_Price($Item_ID,$Store_Need_ID);
                ...
                $insert_data2 = mysqli_query($conn,"insert into tbl_requisition_items(
                                                        Requisition_ID,Quantity_Required,Item_Remark,Item_ID,Container_Qty,Items_Qty,Last_Buying_Price,Selling_Price)
                                                            
                                                    values('$Requisition_ID','$Quantity','$Item_Remark','$Item_ID','$Cont_Quantity','$Items_Quantity','$Last_Buy_Price','$Selling_Price')") or die(mysqli_error($conn));
                        if($insert_data2){
                                echo '<center><table width = 100% border=0>';
                                echo '<tr><td width=4% style="text-align: center;"><b>Sn</b></td>
                                            <td><b>Item Name</b></td>
                                            <td width=7% style="text-align: center;"><b>Containers</b></td>
                                            <td width=7% style="text-align: center;"><b>Items</b></td>
                                            <td width=7% style="text-align: center;"><b>Quantity</b></td>
                                            <td width=5% style="text-align: center;"><b>Pharmacy Balance</b></td>
                                            <td width=20% style="text-align: center;"><b>Remark</b></td><td><b>Remove</b></td></tr>';
                                ...
                                while($row = mysqli_fetch_array($select_Transaction_Items)){ 
                                    ...
                                    echo "<td><input type='text' value='".$row['Item_Balance']."' style='text-align: center;'></td>";
                ...
                if($insert_data){
                    ...
                    if($no > 0 ){
                        while($row = mysqli_fetch_array($select_Requisition_ID)){
                            $Requisition_ID = $row['Requisition_ID'];
                            $_SESSION['Pharmacy_Requisition_ID'] = $row['Requisition_ID'];
                        }
                        $_SESSION['Pharmacy_Requisition_ID'] = $Requisition_ID;
                    }else{
                        $Requisition_ID = 0;
