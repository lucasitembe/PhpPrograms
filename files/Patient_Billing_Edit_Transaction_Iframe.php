...
$select_Transaction_Items = mysqli_query($conn,
        "select bl.e_bill_delivery_status,ppi.Check_In_Type, ppi.Patient_Direction, t.Product_Name, ppi.Price, ppi.Discount, ppi.Quantity,ppi.Patient_Payment_Item_List_ID,pp.Patient_Payment_ID
	   FROM tbl_patient_payment_item_list ppi LEFT JOIN tbl_patient_payments pp ON pp.Patient_Payment_ID = ppi.Patient_Payment_ID 
	   JOIN tbl_items t ON t.item_id = ppi.item_id LEFT JOIN tbl_bills bl ON bl.Bill_ID = pp.Bill_ID
	   WHERE
            pp.Patient_Payment_ID = '$Patient_Payment_ID' and ppi.Status<>'removed' 
            and Patient_Payment_Item_List_ID <> '$Patient_Payment_Item_List_ID'") or die(mysqli_error($conn));

// die( "select ppi.Check_In_Type, ppi.Patient_Direction, t.Product_Name, ppi.Price, ppi.Discount, ppi.Quantity,ppi.Patient_Payment_Item_List_ID,pp.Patient_Payment_ID
// FROM tbl_patient_payment_item_list ppi LEFT JOIN tbl_patient_payments pp ON pp.Patient_Payment_ID = ppi.Patient_Payment_ID 
// JOIN tbl_items t ON t.item_id = ppi.item_id 
// WHERE
...
while ($row = mysqli_fetch_array($select_Transaction_Items)) {
    $deliveryStatus = $row['e_bill_delivery_status'];
    if ($deliveryStatus == "1") {
        ...
        echo "<td  style=''><input style='text-align:left;width:100px'  type='text' name='Quantity' id='Quantity_$pilc' disabled='true' value='" . ($row['Quantity'] ? number_format($row['Quantity']) : 0) . "'></td>";
        ...
        echo "<td  style='text-align: right;width:60px'><input style='text-align: left;'  type='text' name='TotPrice' id='Sub_total_$pilc' disabled='disabled' value='" . number_format(($row['Price'] - $row['Discount']) * $row['Quantity']) . "'></td>";
        if ($num_rows > 0) {
            echo "<td style='text-align: right;width:180px'><input type='button' value='Edit' class='art-button EditItem' pp='$Patient_Payment_ID' id='$ppil'> <input type='button' value='Remove' class='art-button RemoveItem' id='$ppil'></td>";
        } else {
            echo "<td style='text-align: right;width:100px'><input type='button' style='width:70px' value='Edit' class='art-button EditItem' pp='$Patient_Payment_ID' id='$ppil'></td>";
        }
        $total = $total + (($row['Price'] - $row['Discount']) * $row['Quantity']);
        $temp++;
    }
