...
<?php
	...
	$select_Filtered_Patients = mysqli_query($conn,
		...
		from tbl_patient_registration pr, tbl_sponsor sp, tbl_patient_payment_item_list ppl, tbl_patient_payments pp where
		pr.sponsor_id = sp.sponsor_id and
		pp.Patient_Payment_ID = ppl.Patient_Payment_ID and
		pp.Registration_ID = pr.Registration_ID and
		ppl.Check_In_Type = 'RCH' and
		pr.Patient_Name like '%$Patient_Name%' and
		pr.Registration_ID like '%$Patient_Number%' and
		pr.Phone_Number like '%$Phone_Number%' order by pp.Receipt_Date Desc limit 500") or die(mysqli_error($conn));
	...
	$select_Filtered_Patients = mysqli_query($conn,
		...
		pp.Registration_ID = pr.Registration_ID and
		ppl.Check_In_Type = 'RCH' and
		pr.Patient_Name like '%$Patient_Name%' order by pp.Receipt_Date Desc limit 500") or die(mysqli_error($conn));
	...
	$select_Filtered_Patients = mysqli_query($conn,
		...
		pp.Registration_ID = pr.Registration_ID and
		ppl.Check_In_Type = 'RCH' and
		pr.Registration_ID like '%$Patient_Number%' order by pp.Receipt_Date Desc limit 500") or die(mysqli_error($conn));
	...
	//AGE FUNCTION
	 $age = floor( (strtotime(date('Y-m-d')) - strtotime($row['Date_Of_Birth'])) / 31556926)." Years";
	   // if($age == 0){
        ...
        echo "<tr><td width ='2%' id='thead'>".$temp."</td><td><a href='powercharts_antinatal_real.php?pn=".$row['Registration_ID']."&PatientBilling=PatientBillingThisForm' target='_parent' style='text-decoration: none;'>".ucwords(strtolower($row['Patient_Name']))."</a></td>";
	echo "<td><a href='powercharts_antinatal_real.php?pn=".$row['Registration_ID']."&PatientBilling=PatientBillingThisForm' target='_parent' style='text-decoration: none;'>".$row['Registration_ID']."</a></td>";
        ...
        echo "<td><a href='powercharts_antinatal_real.php?pn=".$row['Registration_ID']."&PatientBilling=PatientBillingThisForm' target='_parent' style='text-decoration: none;'>".$row['Gender']."</a></td>";
        echo "<td><a href='powercharts_antinatal_real.php?pn=".$row['Registration_ID']."&PatientBilling=PatientBillingThisForm' target='_parent' style='text-decoration: none;'>".$row['Phone_Number']."</a></td>";
        echo "<td><a href='powercharts_antinatal_real.php?pn=".$row['Registration_ID']."&PatientBilling=PatientBillingThisForm' target='_parent' style='text-decoration: none;'>".$row['Member_Number']."</a></td>";
		echo "<td><a href='powercharts_antinatal_real.php?pn=".$row['Registration_ID']."&PatientBilling=PatientBillingThisForm' target='_parent' style='text-decoration: none;'>Add to rch</a></td>";
	$temp++;
    }    echo "</tr>";
?></table></center>
