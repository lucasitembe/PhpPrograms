	...
	if(isset($_SESSION['userinfo']['Hiv_Works'])){
		...
		header("Location: ./index.php?InvalidPrivilege=yes");
                    ...
                    if(!isset($_SESSION['Hiv_Supervisor'])){ 
                        header("Location: ./deptsupervisorauthentication.php?SessionCategory=Hiv&InvalidSupervisorAuthentication=yes");
                    }
            }
	}else{
	    header("Location: ./index.php?InvalidPrivilege=yes");
...
<?php
	...
	if(isset($_GET['Payment_Cache_ID']) && isset($_GET['Payment_Item_Cache_List_ID'])){
	    ...
	    $Supervisor_ID = $_SESSION['Hiv_Supervisor']['Employee_ID'];
		...
		$select_from_cache = "SELECT * FROM tbl_item_list_cache il, tbl_items i
			...
			document.location = "hivclinicalnotes.php?Hivclinicalnotes=HivPageclinicalnotes&Patient_Payment_ID=<?php echo $Patient_Payment_ID;?>&Registration_ID=<?php echo $Registration_ID;?>";
		    </script>
		    <?php
	    }   
	}
...
<script type="text/javascript">
	...
	if(patientlist=='Outpatient cash'){
	    ...
	    document.location = "hivcreditpatientlist.php?HivCreditPatientlist=HivCreditPatientlistThispage";
...
<fieldset><legend align='right'><b>Hiv Works</b></legend>
	<table width=100%>
		<tr>
...
<a href="powercharts_hiv.php" class="art-button-green">Enter Patient Data</a>
<?php
    if(isset($_GET['Patient_Payment_ID'])){
	?>
