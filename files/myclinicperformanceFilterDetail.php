...
<?php
    if(isset($_SESSION['userinfo'])){
        if($_SESSION['userinfo']['Doctors_Page_Outpatient_Work'] == 'yes'){ 
?>
    <a href='./myclinicperformanceFilter.php?Employee_ID=<?php echo $_GET['Employee_ID']?>&Date_From=<?php echo $_GET['Date_From'];?>&Date_To=<?php echo $_GET['Date_To'];?>&DoctorsWork=WorkThisPage' class='art-button-green'>
        BACK
    </a>
<?php  } } ?>
...
<fieldset>
    <center>
	<form action='doctorsclinicperformancefilterdetailsafterfilter.php?DoctorsClinicPerformanceReportThisPage=ThisPage' method='post' name='myForm' id='myForm' onsubmit="return validateForm();" enctype="multipart/form-data">
    ...
    ?>
    <legend align=center><b>CLINIC PERFORMANCE DETAILS FOR DOCTOR </b><b style="color:blue;"><?php echo strtoupper($doctorName)?></b> <b>FROM </b><b style="color: blue;"><?php echo date('j F, Y H:i:s',strtotime($Date_From))?></b><b> TO </b><b style="color: blue;"><?php echo date('j F, Y H:i:s',strtotime($Date_To))?></b></legend>
        <iframe src="myclinicperformancefilterdetail_Iframe.php?Employee_ID=<?php echo $doctorID?>&Date_From=<?php echo $Date_From?>&Date_To=<?php echo $Date_To?>" width="100%" height="320px"></iframe>
<center><table align='center' width='60%'>
	...
	$employeeID=mysqli_real_escape_string($conn,$_GET['Employee_ID']);
			    ...
			    while($select_doctor_query_row=mysqli_fetch_array($select_doctor_query)){//return employee details
                                ...
                                $select_patient_item_list=mysqli_query($conn," SELECT SUM((ppl.Price - ppl.Discount) * Quantity) AS TotalRevenue FROM tbl_consultation co,tbl_clinic c,tbl_clinic_employee ce,tbl_patient_payment_item_list ppl,tbl_patient_payments pp 
...
//print result
		echo "<tr><hr>
			...
			<a href='PrintIndividualClinicPerformanceFilterDetails.php?Employee_ID=$doctorID&Date_From=$Date_From&Date_To=$Date_To&PrintIndividualDoctorPerformanceFilterDetailsThisPage=ThisPage' class='art-button-green' target='_blanck'>Print</a>
		    </td>
		</tr>";   
?>
