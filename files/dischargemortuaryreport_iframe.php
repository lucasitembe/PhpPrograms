...
if (isset($Date_To) && !empty($Date_To) && isset($Date_From) && !empty($Date_From)) {
    $filter = "  AND Admission_Status='Discharged' AND a.Discharge_Date_Time BETWEEN '" . $Date_From . "' AND '" . $Date_To . "' AND TIMESTAMPDIFF($diagnosis_time,pr.Date_Of_Birth,CURDATE()) BETWEEN '" . $start_age . "' AND '" . $end_age . "'";
}
...
//    }
    ...
    echo '<tr><td>' . $temp . '</td>';
    echo "<td style='font-size:13px;'><a target='_blank' href='mortuary_discharge_report.php?intent=print&Registration_ID=".$row['Registration_ID']."' title='Click to go to discharge patient Summer'>" . ucwords($row['Patient_Name']) . "</a></td>";
