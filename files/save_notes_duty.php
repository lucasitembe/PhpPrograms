...
$Ward_Type=$_GET['Ward_Type'];
        ...
        $duty_ID = mysqli_fetch_assoc(mysqli_query($conn, "SELECT duty_ID FROM tbl_nurse_duties WHERE Ward_Type = '$Ward_Type' AND select_round = '$select_round' AND duty_ward = '$duty_ward' AND Process_Status = 'pending' AND DATE(duty_handled) = CURDATE()"))['duty_ID'];
            ...
            if($duty_ID > 0){
                $Update_Nurse = mysqli_query($conn, "UPDATE tbl_nurse_duties SET duty_nurse = '$duty_nurse', major_round = '$major_round', Employee_Submitted = '$current_nurse', Process_Status = 'Submitted', Saved_Date_Time = NOW() WHERE duty_ID = '$duty_ID'");
            }
