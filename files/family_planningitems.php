<?php
    ...
    if(isset($_SESSION['userinfo'])){
	if(isset($_SESSION['userinfo']['Family_Planning_Works'])){
	    if($_SESSION['userinfo']['Family_Planning_Works'] != 'yes'){
		header("Location: ./index.php?InvalidPrivilege=yes");
	    }else{
		@session_start();
		if(!isset($_SESSION['Family_Planning_Supervisor'])){ 
		    header("Location: ./deptsupervisorauthentication.php?SessionCategory=Family_Planning&InvalidSupervisorAuthentication=yes");
		}
	    }
	}else{
	    header("Location: ./index.php?InvalidPrivilege=yes");
...
<script type="text/javascript">
	...
	if(patientlist=='Outpatient cash'){
	    document.location = "./family_planningcashpatientlist.php";
...
<!--Approved message-->
	...
	function approved_Message(){
	    alert('    Successfully Approved! Please notify PATIENT to go to CASHIER for payment and then return for Service ');   
	}
	
	function approved_Message2(){
	    alert('    The Bill is already APPROVED! if not yet, please notify PATIENT to go to CASHIER for payment then return for Service ');   
	}
...
<script language="javascript" type="text/javascript">
    function seafamily_planningPatient(Patient_Name){
        document.getElementById('Seafamily_planning_Iframe').innerHTML = "<iframe width='100%' height=100% src='viewpatientsIframe.php?Patient_Name="+Patient_Name+"'></iframe>";
    }
</script>
...
<!--<br/>-->
<fieldset>  
            <legend align=right><b><?php if(isset($_SESSION['Family_Planning_Department_Name'])){ echo $_SESSION['Family_Planning_Department_Name']; }?></b></legend>
        <center>
            <table width=100%> 
                <tr> 
                    <td>
				...
				<td style='text-align: right'>Receipt Date & Time</td>
					...
					<option selected='selected'>Family Planning</option> 
				    </select>
				</td>
				...
				<?php
					...
					if(isset($_SESSION['Family_Planning_Supervisor']['Session_Master_Priveleges'])){
						...
						$Supervisor = $_SESSION['Family_Planning_Supervisor']['Employee_Name'];
					    }else{
		...
		<tr>
		   <td style='text-align: center;'>
			<?php
				...
				$Payment_Cache_ID = '';
			    }if(isset($_SESSION['Family_Planning_Department_Name'])){
				$Sub_Department_Name = $_SESSION['Family_Planning_Department_Name'];
			    }else{
				$Sub_Department_Name = '';
			    }
...
<!--					     <a href='Send_To_Cashier.php?Payment_Cache_ID=<?php echo $Payment_Cache_ID; ?>&Transaction_Type=<?php echo $Transaction_Type; ?>&Sub_Department_Name=<?php echo $Sub_Department_Name; ?>&Registration_ID=<?php echo $Registration_ID; ?>' class='art-button-green' onclick='return approved_Message2();'>
		...
		if($Transaction_Status_Title != 'APPROVED' && $Transaction_Status_Title != 'PAID'){
			...
			if(strtolower($Departmental_Collection) == 'yes'){
				...
				echo "<a href='Departmental_Patient_Billing_Family_Planning_Page.php?Payment_Cache_ID=".$Payment_Cache_ID."&Transaction_Type=".$Transaction_Type."&Sub_Department_ID=".$Sub_Department_ID."&Registration_ID=".$Registration_ID."&Billing_Type=".$Billing_Type."' class='art-button-green'>
					Make Payment
				    </a>";
			    }
			}
...
<?php if($Transaction_Status_Title != 'DISPENSED'){ 
	...
	<?php if(strtoupper($Transaction_Status_Title) == 'PAID'){ ?>
	    <a href='family_planningcashpatientlist.php?Payment_Cache_ID=<?php echo $Payment_Cache_ID; ?>&Transaction_Type=<?php echo $Transaction_Type; ?>' class='art-button-green'>Back To Main Page</a>
	<?php }else{ ?>
...
<?php }else{ ?>
	...
	<a href='family_planningcreditpatientlist.php?Payment_Cache_ID=<?php echo $Payment_Cache_ID; ?>&Transaction_Type=<?php echo $Transaction_Type; ?>&Registration_ID=<?php echo $Registration_ID; ?>&Billing_Type=<?php echo $Billing_Type; ?>' class='art-button-green'>Back To Main Page</a>
    <?php } ?>
<?php }} ?>
		<?php
		    if($Patient_Payment_ID != '' && ($Transaction_Status_Title == 'PAID' || $Transaction_Status_Title == 'DISPENSED')){
			echo "<a href='individualpaymentreportindirect.php?Patient_Payment_ID=".$_GET['Patient_Payment_ID']."&IndividualPaymentReport=IndividualPaymentReportThisPage' class='art-button-green' target='_blank'>
			    Print Receipt
			</a>";
		    } 		    
		?>
...
<fieldset>   
		...
		<tr>
		    <td>
			<iframe src='family_planningitems_Iframe.php?Transaction_Type=<?php echo $Transaction_Type; ?>&Payment_Cache_ID=<?php echo $Payment_Cache_ID; ?>' width='100%' height=200px></iframe>
		    </td>
		</tr> 
	    </table>
        </center>
