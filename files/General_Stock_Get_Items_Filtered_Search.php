...
<?php
  ...
  if($Item_Category_ID == 0 && $Item_Subcategory_ID == 0){
    $select = mysqli_query($conn,"select Item_ID, Product_Name, Last_Buy_Price from tbl_items where Item_Type = 'Pharmacy' and Product_Name like '%$Search_Value%' order by Product_Name") or die(mysqli_error($conn));
  }else{
    if($Item_Subcategory_ID == 0){
      $select = mysqli_query($conn,"select Item_ID, Product_Name, Last_Buy_Price from tbl_items i, tbl_item_subcategory isc, tbl_item_category ic where 
                              ...
                              ic.Item_Category_ID = '$Item_Category_ID' and Product_Name like '%$Search_Value%' order by Product_Name") or die(mysqli_error($conn));
    }else{
      $select = mysqli_query($conn,"select Item_ID, Product_Name, Last_Buy_Price from tbl_items where Item_Type = 'Pharmacy' and Item_Subcategory_ID = '$Item_Subcategory_ID' and Product_Name like '%$Search_Value%' order by Product_Name") or die(mysqli_error($conn));
    }
  }
  $num = mysqli_num_rows($select);
  if($num > 0){
