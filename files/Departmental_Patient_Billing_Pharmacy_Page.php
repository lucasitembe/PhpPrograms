...
if (strtolower($_SESSION['systeminfo']['Inpatient_Prepaid']) == 'yes' && $Billing_Type == 'Inpatient Cash') {
    $payment_type = 'pre';
} else {
    $payment_type = 'post';
}
...
while ($row = mysqli_fetch_array($result)) {
    ...
    $slct_folio = mysqli_query($conn,"select Folio_Number from tbl_patient_payments where Registration_ID = '$Registration_ID' order by Patient_Payment_ID desc limit 1") or die(mysqli_error($conn));
    $nm_folio = mysqli_num_rows($slct_folio);
    if($nm_folio){
        while ($dt = mysqli_fetch_array($slct_folio)) {
        ...
        if(strtolower($Billing_Type) == 'inpatient cash' || strtolower($Billing_Type) == 'inpatient credit'){
            ...
            $insert = "insert into tbl_patient_payments(
                        Registration_ID,Supervisor_ID,Employee_ID,
                            Payment_Date_And_Time,Folio_Number,Claim_Form_Number,Check_In_ID,
                                Sponsor_ID,Sponsor_Name,Billing_Type,Patient_Bill_ID,
                                    Receipt_Date,Transaction_status,Transaction_type,Branch_ID,payment_type,Hospital_Ward_ID,terminal_id,auth_code,manual_offline)
                                    
                    values('$Registration_ID','$Supervisor_ID','$Employee_ID',
                                (select now()),'$Folio_Number','$Claim_Form_Number','$Check_In_ID',
                                    '$Sponsor_ID','$Sponsor_Name','$Billing_Type','$Patient_Bill_ID',
                                        (select now()),'$Transaction_status','$Transaction_type','$Branch_ID','$payment_type','$Hospital_Ward_ID','$terminal_id','$auth_code','$manual_offline')";
        }else{
            $insert = "insert into tbl_patient_payments(
                        Registration_ID,Supervisor_ID,Employee_ID,
                            Payment_Date_And_Time,Folio_Number,Claim_Form_Number,
                                Sponsor_ID,Sponsor_Name,Billing_Type,
                                    Receipt_Date,Transaction_status,Transaction_type,Branch_ID,payment_type,Pre_Paid,terminal_id,auth_code,manual_offline)
                                    
                    values('$Registration_ID','$Supervisor_ID','$Employee_ID',
                                (select now()),'$Folio_Number','$Claim_Form_Number',
                                    '$Sponsor_ID','$Sponsor_Name','$Billing_Type',
                                        (select now()),'$Transaction_status','$Transaction_type','$Branch_ID','$payment_type','$Pre_Paid','$terminal_id','$auth_code','$manual_offline')";
        }
            ...
            $select_medication = mysqli_query($conn,"select * from tbl_item_list_cache where
                                                    payment_cache_id = '$Payment_Cache_ID' and 
                                                        Transaction_Type = '$Transaction_Type' and
                                                            ...
                                                            status in('active','approved') and Check_In_Type='$Check_In_Type'");
                ...
                $Insert_Data_To_tbl_patient_payment_item_list = "insert into tbl_patient_payment_item_list(
                                    check_In_type,item_id,discount,
                                        price,quantity,patient_direction,
                                            consultant,consultant_id,patient_payment_id,Transaction_Date_And_Time,Clinic_ID,Sub_Department_ID)
                                    values(
                                        '$Check_In_Type','$Item_ID','$Discount',
                                            '$Price','$Quantity','$Patient_Direction',
                                                '$Consultant','$Consultant_ID','$Patient_Payment_ID',(select now()),'$Clinic_ID','$Sub_Department_ID')";
                if (!mysqli_query($conn,$Insert_Data_To_tbl_patient_payment_item_list)) {
                    $HAS_ERROR = true;
                } else {
                    $upt1 = mysqli_query($conn,"update tbl_item_list_cache set status = 'paid',
...
if (!$HAS_ERROR) {
   ...
   // header("Location: ./pharmacyworkspage.php?section=Pharmacy&Registration_ID=$Registration_ID&Transaction_Type=$Transaction_Type&Payment_Cache_ID=$Payment_Cache_ID&NR=True&Billing_Type=$Billing_Type&Patient_Payment_ID=$Patient_Payment_ID&Payment_Date_And_Time=$Payment_Date_And_Time&Sub_Department_ID=$Sub_Department_ID&PharmacyWorks=PharmacyWorksThisPage");
    header("Location: ./Dispense_Medication.php?Payment_Cache_ID=$Payment_Cache_ID&Transaction_Type=$Transaction_Type&Registration_ID=$Registration_ID&Check_In_Type=$Check_In_Type"); 
} else {
