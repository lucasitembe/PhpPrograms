...
?>
  <a href='radiotherapy_parameter_patientlist.php?Registration_ID=<?= $Registration_ID ?>&Patient_Payment_ID=<?= $Patient_Payment_ID ?>&Patient_Payment_Item_List_ID=<?= $Patient_Payment_Item_List_ID ?>&SearchListDirectCashPatientBilling=SearchListDirectCashPatientBillingThisPage&from_consulted=<?= $from_consulted ?>' class='art-button-green'>
       BACK
    </a>
<br/>
<fieldset>  
    <legend align=center><b>RADIATION PARAMETER CALCULATION</b></legend>
...
?>
    ...
    <div class="box box-primary" style="height: 500px;overflow-y: scroll;overflow-x: hidden">
       ...
       <?php
        ...
        $select_data_simulation = mysqli_query($conn,"SELECT ps.field_ID, pm.techniques,ps.field_name,pm.Comment, pm.position_immobilization_ID, pm.number_phases, ps.f_s_on_skin,ps.Skin_length,ps.f_s_on_tumour,ps.ssd_sad,ps.depth,ps.gantry_angle,ps.coll_angle,ps.cough_angle FROM tbl_fields_position ps,tbl_position_immobilization pm WHERE ps.position_immobilization_ID=pm.position_immobilization_ID AND pm.Radiotherapy_ID='$Radiotherapy_ID' AND ps.field_ID NOT IN(SELECT field_ID FROM tbl_calculation_parameter)");
...
?> 
            ...
            <table>
                <tr>
                    <td style='width: 14%; text-align: right;'><h4>Simulation Comment</h4></td>
                    <td style='width: 86%;'><textarea name="Comments_calculation" id="Comments_calculation" cols="200" rows="3" disabled='disabled' style='resize: none;'><?php echo $Comment ?></textarea></td>
                </tr>
                <tr>
                    <td style='width: 10%; text-align: right;'><h4>Comment</h4></td>
                    <td style='width: 90%;'><textarea name="Comments_calculation" id="Comments_calculation" cols="200" rows="3" style='resize: none;'></textarea></td>
                </tr>
            </table>
</fieldset>
