		...
		while ($row=mysqli_fetch_assoc($QueryResults)){
			...
			//retrieve attachment info
			  ...
			  if($attach['Attachment_Url'] !=''){
				$image="<a href='patient_attachments/".$attach['Attachment_Url']."' class='fancybox12'  target='_blank' rel='gallery' target='_blank' title='".$row['Product_Name']."'><img src='patient_attachments/".$attach['Attachment_Url']."' width='40' height='20' /></a>";
...
<?php
	    ...
	    $qry="SELECT tit.Product_Name,ilc.Doctor_Comment,ilc.remarks,em.Employee_Name,em.Employee_Type,ilc.Transaction_Date_And_Time AS DateTime,ServedDateTime,
                (SELECT Employee_Name FROM JOIN tbl_employee em WHERE tbl_employee em ON em.Employee_ID=ilc.Consultant_ID) AS Sent_By,
                (SELECT Employee_Name FROM JOIN tbl_employee em WHERE tbl_employee em ON em.Employee_ID=ilc.ServedBy) AS Served_By,
	   ...
	   $rs=mysqli_query($conn,$qry)or die(mysqli_error($conn));
	    echo '<table width="100%">';
			...
			echo '<td>SENT BY</td>';	
                        echo '<td>SENT ON</td>';
			echo '<td>Processed By</td>';
                        echo '<td>Processed On</td>';
			echo '</tr>';
		$sn=1; 
		...
		if(mysqli_num_rows($rs)>0){
			while ($row=mysqli_fetch_assoc($rs)){
			  ...
			  echo '<tr style="text-align:left">';	
					...
					echo '<td>'.$Sent_By.'</td>';	
					echo '<td>'.$DateTime.'</td>';	
					echo '<td>'.$Served_By.'</td>';
                                        echo '<td>'.$ServedDateTime.'</td>';
					
				echo '</tr>';;			
				$sn++;
			}
		}else{
			 echo '<tr><td colspan="7" style="text-align:center;font-size:20px;color:red">This Patient has no procedure done on this date</td></tr>';
		 }
	  //echo 'Procedure';
	...
	while($row = mysqli_fetch_assoc($result)){
	 ...
	 if($disepnsedQty>0){
		...
		$sn++;	
			 ...
			 echo '<tr><td colspan="5" style="text-align:center;font-size:20px;color:red">This Patient has no medication given on this date</td></tr>';
		 }
  }
