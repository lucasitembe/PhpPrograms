...
if (!$isReceipt) {  //Not receipt
    ...
    $sql_select_dublicate_item_result=mysqli_query($conn,"SELECT MAX(Patient_Payment_Item_List_ID) AS Patient_Payment_Item_List_ID FROM `tbl_patient_payment_item_list` WHERE `Patient_Payment_ID`='$Patient_Payment_ID' GROUP BY Item_ID,`Quantity` HAVING COUNT(Item_ID)>1") or die(mysqli_error($conn));
            ...
            <div id="page-wrap"  style="font-weight:bold">
                ...
                <div id="identity">
                    ...
                    $select_Transaction_Items = mysqli_query($conn,
                            "select pp.Pre_Paid,pp.Payment_Code,pp.Employee_ID,pp.Billing_Type,pp.Payment_Date_And_Time,preg.Patient_Name,preg.Registration_ID,preg.Old_Registration_Number,Guarantor_Name, sp.Exemption from  tbl_patient_payment_item_list ppl INNER JOIN tbl_patient_payments pp ON pp.Patient_Payment_ID = ppl.Patient_Payment_ID
