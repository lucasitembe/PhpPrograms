...
$is_string_needed = false;

if (isset($_GET['Return'])) {
    $is_string_needed = true;
}
...
if ($is_string_needed) {
    ...
    $select_patients = "SELECT 0 as Patient_Payment_Item_List_ID, 0 as Patient_Payment_ID,Employee_Name, c.consultation_ID,ch.employee_ID, ch.cons_hist_Date,consultation_histry_ID,ch.course_of_injuries, c.Registration_ID
			    ...
			    c.Patient_Payment_Item_List_ID = ppl.Patient_Payment_Item_List_ID and
                            c.consultation_ID=ch.consultation_ID and
                            e.Employee_ID=ch.employee_ID and
			    c.Registration_ID = '$Registration_ID'  ORDER BY c.consultation_ID DESC LIMIT 100";
...
//          $doctorsID=$doctorsInfo['Employee_Name'];
    ...
    $Procedure = "";
    $Surgery = "";
    if (@mysql_numrows($cache_result) > 0) {
    ...
    if (@mysql_numrows($consultation_result) > 0) {
...
if (!empty($maincomplain)) {
    ...
    if (!empty($maincomplain)) {
        ...
        $data .= ' <tr><td width="15%" style="text-align:right;">First Date Of Symptoms</td><td>' . $firstsymptom_date . '></td></tr>';
    }if (!empty($opt)) {
        ...
        $data .= '<tr><td width="15%" style="text-align:right;">History Of Present Illness</td><td>' . $history_present_illness . '</td></tr>';
    ...
    if (!empty($general_observation)) {
        ...
        $data .= '<tr><td width="15%" style="text-align:right;">Differential Diagnosis</td><td>' . $diferential_diagnosis . '</td>';
    }
    $data .= '</table>
        </div>
         <div id="investigation">
            <table width=100% style="border: 0px;">';
    if (!empty($Laboratory)) {
        $data .= ' <tr><td width="15%" style="text-align:right;">Laboratory</td><td>' . $Laboratory . '</tr>';
        ...
        $data .= '<tr><td width="15%" style="text-align:right;">Radiology</td><td>' . $Radiology . '</tr>';
    ...
    $data .= ' </table>
        ...
        <div id="diagnosis_treatment">
            <table width=100% style="border: 0px;">';
    if (!empty($diagnosis)) {
        $data .= ' <tr><td width="15%" style="text-align:right;"><b>Final Diagnosis </b></td><td>' . $diagnosis . '>';
    }if (!empty($Procedure)) {
        $data .= ' <tr><td width="15%" style="text-align:right;">Procedure</td><td>' . $Procedure . '</tr>';
    ...
    if (!empty($Surgery)) {
        $data .= '<tr><td width="15%" style="text-align:right;">Surgery</td><td>' . $Surgery . '</tr>';
    ...
    if (!empty($Pharmacy)) {
        $data .= '<tr><td width="15%" style="text-align:right;">Pharmacy</td><td>' . $Pharmacy . '';
    }
    $data .= ' </table>
        </div>
        <div id="remarks">
...
//          $doctorsID=$doctorsInfo['Employee_Name'];
        ...
        <div id="diagnosis_treatment">
            <table width=100% style="border: 0px;">';
        if (!empty($diagnosis)) {
            $data .= ' <tr><td width="15%" style="text-align:right;"><b>Final Diagnosis </b></td><td>' . $diagnosis . '</td>';
        }if (!empty($Procedure)) {
        ...
        if (!empty($Pharmacy)) {
            $data .= '<tr><td width="15%" style="text-align:right;">Pharmacy</td><td>' . $Pharmacy . '</td>';
        }
        $data .= ' </table>
        </div>
        <div id="remarks">
