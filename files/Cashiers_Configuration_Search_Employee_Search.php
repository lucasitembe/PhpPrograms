<?php
	...
	if(isset($_GET['Unselected_Employee'])){
		$Unselected_Employee = str_replace(' ','%',$_GET['Unselected_Employee']);
	}else{
		$Unselected_Employee = '';
	}

	if(isset($_GET['Selected_Employee'])){
		$Selected_Employee = str_replace(' ','%',$_GET['Selected_Employee']);
	}else{
		$Selected_Employee = '';
	}
		...
		<table width="100%">
			...
			<?php
				...
				$select = mysqli_query($conn,"select emp.Employee_ID, emp.Employee_Name from tbl_employee emp, tbl_employee_performance_cache epc where
										emp.Employee_ID = epc.Employee_ID and emp.Employee_Name like '%$Selected_Employee%' order by Employee_Name limit 100") or die(mysqli_error($conn));
			...
			<tr>
				...
				<?php
					...
					$select = mysqli_query($conn,"select Employee_ID, Employee_Name from tbl_employee where
											Employee_ID NOT IN(select Employee_ID from tbl_employee_performance_cache) and
											Employee_Name like '%$Unselected_Employee%' order by Employee_Name limit 100") or die(mysqli_error($conn));
					$num = mysqli_num_rows($select);
					if($num > 0){
						while($row = mysqli_fetch_array($select)){
				?>
