...
$filter = "";
 if(isset($_GET['patient_id'])){
    $id=$_GET['patient_id'];
  $filter = "AND pc.Registration_ID='$id'";
}

$sql = "SELECT DISTINCT pc.Payment_Cache_ID,pc.Registration_ID,ilc.Item_ID,ilc.Approved_By,ilc.Approval_Date_Time 
        FROM tbl_payment_cache pc 
        JOIN `tbl_item_list_cache` ilc 
        ON pc.Payment_Cache_ID = ilc.Payment_Cache_ID 
        WHERE ilc.Category='indirect cash' 
        AND DATE(ilc.Approval_Date_Time)='$today' $filter 
        GROUP BY  ilc.Payment_Cache_ID  DESC LIMIT 20";
