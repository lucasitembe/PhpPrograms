...
<style>
    ...
    .cash_credit_bill_tbl, .cash_credit_bill_tbl tr,.cash_credit_bill_tbl td{
...
<span id="print_discharge_summary_btn" style="display:none">
    ...
    ?>
    <a class="art-button-green" href="print_patient_discharge_summery.php?consultation_ID=<?= $Consultation_ID ?>&Registration_ID=<?= $Registration_ID ?>&Admission_ID=<?= $Admision_ID ?>" target="_blank" >PRINT PATIENT DISCHARGE SUMMARY</a>
</span>  
          <?php
...
?>
<?php // if ($Patient_Status=="mortuary") {?><fieldset>
    <table width="100%">
        <tr>
...
//    $Today = $new_Date;
//    
//    //date("oW", strtotime("Admission_Date"));
//    //
//   }
...
//kupata hours MICHAEL YONGO CODING FROM GPITG-
//$date1 = new DateTime($Today);
...
$inalala_bilakulala=$data['inalala_bilakulala'];
        ...
        ?>
             ...
             <td width='13%' style='text-align:right;'><font color="red">CHANGE STATUS&nbsp;:&nbsp;MWILI UNALALA AU HAULALI? : </font></td>
						<td width='16%'>
                                                    
							<select name='inalala_bilakulala' id='inalala_id'  style='width:100%;padding: 5px' onchange="update_morgue_condition();"   >
								      <option value="inalala" <?php if($inalala_bilakulala=='inalala'){ echo 'selected';}?> >Maiti inayolala</option>
                                                                        <option value="bilakulala" <?php if($inalala_bilakulala=='bilakulala'){ echo 'selected';}?> >Maiti inayochukuliwa bila kulala</option>
...
?>
                 ...
                 //echo array_sum($totalmorgueprice);
                // $totalmorgueprice=$ubtotal_price_total;
                 //$totalmorgueprice=$subtotal_price_detail + $subtotal_price_detail;
                 //$diff2=($diff2==-1?"Within 24 hrs":$diff2=$diff2); 
                                                                ?>
                ...
                <fieldset style='overflow-y: scroll; height: 340px; background-color: white;'>
                   ...
                   <table width="100%">
                                ...
                                <tr>
                                    ...
                                    <!--<td width="40%" style="text-align: left;">AGE FROM</td>
                                    <td width="40%" style="text-align: left;">AGE TO </td>
                                    <td width="40%" style="text-align: left;">AGE STATUS</td>-->
...
$sql_select_all_auto_add_item_result=mysqli_query($conn,"SELECT it.Item_ID,ip.Items_Price FROM tbl_items it,tbl_item_price ip WHERE it.Item_ID=ip.Item_ID AND daily_add_automaticaly_in_inpatient_bill='yes' AND ip.Sponsor_ID='$Sponsor_ID' AND Items_Price<>'0' AND it.Consultation_Type <> 'Pharmacy'") or die(mysqli_error($conn));
if(mysqli_num_rows($sql_select_all_auto_add_item_result)>0){
    ...
    while($item_rows=mysqli_fetch_assoc($sql_select_all_auto_add_item_result)){
        ...
        if(mysqli_num_rows($sql_check_if_auto_item_already_charged_result)<=0){
            ...
            $sql_save_auto_add_item_for_current_date_result=mysqli_query($conn,"INSERT INTO tbl_patient_payments(Registration_ID,Payment_Date_And_Time,Check_In_ID,Sponsor_ID,Sponsor_Name,Billing_Type,Receipt_Date,payment_type,branch_id,Patient_Bill_ID,Folio_Number) VALUES('$Registration_ID',NOW(),'$Check_In_ID','$Sponsor_ID','$Sponsor_Name','$Billing_Type',CURDATE(),'post','$Branch_ID','$Patient_Bill_ID','$Folio_Number')") or die(mysqli_error($conn));
            ...
            $sql_save_item_result=mysqli_query($conn,"INSERT into tbl_patient_payment_item_list(
                                                            Check_In_Type, Item_ID, Price,
                                                            Quantity, Patient_Direction,
                                                            Patient_Payment_ID,Transaction_Date_And_Time,Hospital_Ward_ID,Sub_Department_ID,finance_department_id)
                                                            
                                                            values ('Others','$Item_ID','$Items_Price',
                                                            '1','others',
                                                           '$Patient_Payment_ID_1', (select now()),'$Hospital_Ward_ID','$Sub_Department_ID','$bill_working_department')") or die(mysqli_error($conn));
        
            }
            }else{
            
            $Patient_Payment_ID=mysqli_fetch_assoc($sql_check_if_auto_item_already_charged_result)['Patient_Payment_ID'];
           // mysqli_query($conn,"UPDATE tbl_patient_payment_item_list SET Quantity='$lapsed_time_from_admision_date' WHERE Item_ID='$Item_ID' AND Patient_Payment_ID='$Patient_Payment_ID'") or die(mysqli_error($conn));
        }
    }
}
?>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            if (strtolower($Payment_Method) == 'cash') {
                                ...
                                ?>
                                         <input type="button" name="Approval_Button" id="Approval_Button" class="art-button-green" value="CLEAR BILL" onclick="alert('Access Denied!you dont have privilege to approve bill')">
...
<div id="No_Enough_Payments">
    ...
    <table class="table table-bordered">
        <tr><th colspan="3">CLEAR BILL ANYWAY</th></tr>
        <tr>
            <td>
            ...
            <td id="approve_bill_status_out">
                <select id="approve_bill_status" style="width:100%" onchange="check_if_bill_excemption_status()">
                    ...
                    <option value="bill_excemption">BILL EXCEMPTION</option>
              ...
              <td>
                <input type="button" class="art-button-green pull-right"onclick="force_bill_approval(<?php echo $Patient_Bill_ID; ?>,<?php echo $Folio_Number; ?>,<?php echo $Sponsor_ID; ?>,<?php echo $Check_In_ID; ?>,<?= $Admision_ID ?>)"value="APPROVE BILL">
            </td>
        </tr>
        <tr style="display:none" id="bill_excemption_details">
            <td>
                <input type="text" id="excemption_number" placeholder="Enter Excemption Number" />
            </td>
            <td style="text-align:right">
                Select Excemption Attachment
            </td>
            <td>
...
<script>
    $(document).ready(function () {
        $("#MessageAlert").dialog({autoOpen: false, width: 600, height: 110, title: 'eHMS 4.0 ~ Alert Message', modal: true});
    });
</script>
...
<script>
    $(document).ready(function () {
        $("#Approval_Warning_Message").dialog({autoOpen: false, width: "70%", height: 450, title: 'eHMS 4.0 ~ Alert Message', modal: true});
    });
</script>

<script>
    $(document).ready(function () {
        $("#Not_Ready_To_Bill").dialog({autoOpen: false, width: "50%", height: 100, title: 'eHMS 4.0 ~ Alert Message', modal: true});
    });
</script>
<script>
    $(document).ready(function () {
...
<script>
    $(document).ready(function () {
        $("#Body_Not_Ready_To_Bill").dialog({autoOpen: false, width: "40%", height: 110, title: 'eHMS 4.0 ~ Alert Message', modal: true});
    });
</script>
<script>
    $(document).ready(function () {
        $("#change_sponsor_first").dialog({autoOpen: false, width: "40%", height: 110, title: 'eHMS 4.0 ~ Alert Message', modal: true});
    });
</script>
<script>
    $(document).ready(function () {
        $("#Zero_Price_Alert").dialog({autoOpen: false, width: "40%", height: 110, title: 'eHMS 4.0 ~ Alert Message', modal: true});
    });
</script>
<script>
    $(document).ready(function () {
        $("#No_Items_Found").dialog({autoOpen: false, width: "40%", height: 110, title: 'eHMS 4.0 ~ Alert Message', modal: true});
    });
</script>
<script>
    $(document).ready(function () {
        $("#Something_Wrong").dialog({autoOpen: false, width: "40%", height: 110, title: 'eHMS 4.0 ~ Alert Message', modal: true});
    });
</script>
<script>
    $(document).ready(function () {
        $("#Zero_Price_Or_Quantity_Alert").dialog({autoOpen: false, width: "40%", height: 110, title: 'eHMS 4.0 ~ Alert Message', modal: true});
    });
</script>

<script>
    $(document).ready(function () {
        $("#Patient_Already_Cleared").dialog({autoOpen: false, width: "40%", height: 110, title: 'eHMS 4.0 ~ Alert Message', modal: true});
    });
</script>

<script>
    $(document).ready(function () {
        $("#Refund_Required").dialog({autoOpen: false, width: "40%", height: 110, title: 'eHMS 4.0 ~ Alert Message', modal: true});
    });
</script>

<script>
    $(document).ready(function () {
        $("#No_Enough_Payments").dialog({autoOpen: false, width: "60%", height: 260, title: 'eHMS 4.0 ~ Alert Message', modal: true});
    });
</script>

<script>
    $(document).ready(function () {
        $("#Error_During_Process").dialog({autoOpen: false, width: "40%", height: 110, title: 'eHMS 4.0 ~ Alert Message', modal: true});
    });
</script>

<script>
    $(document).ready(function () {
        $("#Credit_Bill").dialog({autoOpen: false, width: "40%", height: 110, title: 'eHMS 4.0 ~ Alert Message', modal: true});
    });
</script>
...
<script>
    $(document).ready(function () {
        $("#Successful_Dialogy").dialog({autoOpen: false, width: "40%", height: 110, title: 'eHMS 4.0 ~ Alert Message', modal: true});
    });
</script>

<script>
    $(document).ready(function () {
        $("#Unsuccessful_Dialogy").dialog({autoOpen: false, width: "40%", height: 110, title: 'eHMS 4.0 ~ Alert Message', modal: true});
    });
</script>
...
//        alert(Patient_Bill_ID)
       if(confirm("Are you sure you want to discharge this patient?")){
          $.ajax({
           ...
           data:{Registration_ID:Registration_ID,Admision_ID:Admision_ID,Patient_Bill_ID:Patient_Bill_ID},
           success:function(data){
               alert(data)
               if(data=="pending"){
...
//        form_data.append("file",property);
    ...
    function force_bill_approval(Patient_Bill_ID,Folio_Number,Sponsor_ID,Check_In_ID,Admision_ID){
        var imbalance_approval_reason=$("#imbalance_approval_reason").val();
        var approve_bill_status=$("#approve_bill_status").val();
        var excemption_number=$("#excemption_number").val(); 
        var excemtion_attachment=$("#excemtion_attachment").val();
        ...
        if(userText4==""&&approve_bill_status=="bill_excemption"){
            $("#excemtion_attachment").css("border","2px solid red");
            $("#excemtion_attachment").focus();
            exit;
        }else{
           $("#excemtion_attachment").css("border",""); 
        }
        if(approve_bill_status=="bill_excemption"){
            ...
            $.ajax({
              type:'GET',
              url:'force_imbalance_bill_approval.php',
              data:{Admision_ID:Admision_ID,imbalance_approval_reason:imbalance_approval_reason,approve_bill_status:approve_bill_status,excemption_number:excemption_number,excemtion_attachment:excemtion_attachment},
              success:function(data){
    ...
    function update_morgue_condition() {
        ...
        var Patient_id = '<?php echo $Registration_ID; ?>';
           ...
           $.ajax({
            ...
            data:{inalala_bilakulala:inalala_bilakulala,patient_id:Patient_id},
            success:function(data){
               location.reload();
            }
        });
