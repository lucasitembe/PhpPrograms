...
<?php
	...
	if(isset($_GET['Requisition_ID'])){
		if($_SESSION['userinfo']['Admission_Works'] == 'yes'){
			echo "<a href='admission_submit_this_requisition.php?Requisition_ID=".$_GET['Requisition_ID']."' class='art-button-green'>SUBMIT THIS REQUISITION</a>";
		}
	}
	
	if(isset($_SESSION['userinfo'])){
		if($_SESSION['userinfo']['Admission_Works'] == 'yes'){ 
			echo "<a href='admissionworkspage.php?AdmissionWorkPage=AdmissionWorkPageThisPage' class='art-button-green'>BACK</a>";
		}
	}
...
<br/>
<fieldset> 
	<legend align='right'><b>Requisition<?php if(isset($_SESSION['Admission_ID'])){ echo ' ~ '.$Sub_Department_Name; }?></b></legend>  
        <table width=100%>
		<tr>
			<td width='12%' style='text-align: right;'>Requisition Number</td>
			<td width=5%>
				<?php if(isset($_SESSION['Admission_Requisition_ID'])){ ?>
					<input type='text' name='Requisition_Number' size=6 id='Requisition_Number' readonly='readonly' value='<?php echo $_SESSION['Admission_Requisition_ID']; ?>'>
				<?php }else{ ?>
		...
		<tr>
                  ...
                  <td width='16%'>
                    <select name='Store_Need' id='Store_Need'>
                      <?php
                        if(isset($_SESSION['Admission_ID'])){
                                echo '<option selected="selected" value="'.$Sub_Department_ID.'">'.$Sub_Department_Name.'</option>';
                        }else{
		...
		<td id='Store_Issue_Area'>			
			<?php
				if(isset($_SESSION['Admission_Requisition_ID']) && $_SESSION['Admission_Requisition_ID'] != '' && $_SESSION['Admission_Requisition_ID'] != null){
					$Requisition_ID = $_SESSION['Admission_Requisition_ID'];
					...
					$Sub_Department_ID = $_SESSION['Admission_ID'];
					$select_Supplier = mysqli_query($conn,"select * from tbl_sub_department where Sub_Department_ID <> '$Sub_Department_ID'");
...
<script>
	function Get_Item_Name(Item_Name,Item_ID){
		...
		myObjectGetItemName.open('GET','Admission_Check_Item_Selected.php?Item_ID='+Item_ID,true);
...
<script>
	function Get_Balance(){
		...
		myObjectGetBalance.open('GET','Admission_Get_Item_Expected_Balance.php?Item_ID='+Item_ID,true);
...
<script>
	function updateStoreIssueMenu2() {
		...
		myRequisitionObject.open('GET','Admission_Get_Store_Issued.php',true);
...
<script>
	function updateStoreIssueMenu() {
	   /*	var Requisition_ID = '<?php echo $_SESSION['Admission_Requisition_ID']; ?>';
...
<script>
	function updateRequisitionNumber(){
		...
		myObjectUpdateRequisition.open('GET','Admission_Update_Requisition_Number.php',true);
...
<script>
	function updateRequisitionDescription(){
		...
		myObjectUpdateDescription.open('GET','Admission_Update_Requisition_Description.php',true);
		myObjectUpdateDescription.send();
	}
</script>
<script>
	function updateRequisitionCreatedDate(){
		...
		myObjectUpdateCreatedDate.open('GET','Admission_Update_Requisition_Created_Date.php',true);
...
<script>
	function updateSubmitArea(){
		...
		myObjectUpdateSubmitArea.open('GET','Admission_Update_Submit_Area.php',true);
...
<script>	
	function Get_Selected_Item(){
		...
		if (Item_ID != '' && Item_ID != null && Quantity != '' && Quantity != null && Quantity!= '' && Store_Issue != '' && Store_Issue != null) {
			...
			my_Object_Get_Selected_Item.open('GET','Admission_Requisition_Add_Selected_Item.php?Item_ID='+Item_ID+'&Quantity='+Quantity+'&Item_Remark='+Item_Remark+'&Store_Issue='+Store_Issue+'&Store_Need='+Store_Need+'&Requisition_Description='+Requisition_Description,true);
...
<script>
	function Confirm_Remove_Item(Item_Name,Requisition_Item_ID){ 
		...
		if (Confirm_Message == true) {
			...
			My_Object_Remove_Item.open('GET','Admission_Requisition_Remove_Item_From_List.php?Requisition_Item_ID='+Requisition_Item_ID,true);
				...
				<table width=100%>
					...
					<script type='text/javascript'>
						function Submit_Requisition_Function(Requisition_ID){				
						    document.location = 'Admission_Submit_Requisition.php?Requisition_ID='+Requisition_ID;
						}
					</script>
					<script>
						function Confirm_Submit_Requisition(){
							...
							myObjectCheckItemNumber.open('GET','Admission_Requisition_Check_Number_Of_Items.php',true);
