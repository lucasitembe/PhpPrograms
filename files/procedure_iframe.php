...
while ($row = mysqli_fetch_array($select_Transaction_Items_Active)) {
    ...
    echo '<td style="text-align:center">';
      ...
      if(!$transStatust){
            ...
            <input type="button" name="Bronchoscopy_Button" id="Bronchoscopy_Button" class="art-button-green" value="BRONCHOSCOPY" onclick="Bronchoscopy_Process('.$row["Payment_Item_Cache_List_ID"].')">
            <input type="button" name="echorcadiogram_button" id="echorcadiogram_button" class="art-button-green" value="ECHOCARDIOGRAM" onclick="open_echocardiogram('.$row["Payment_Item_Cache_List_ID"].')">
		<input type="button" name="btn_refraction" id="btn_refraction" class="art-button-green" value="REFRACTION" onclick="open_refraction_procedure()">';
        echo "<a href='audiology_form.php?section=Procedure&Registration_id=".$Registration_ID."&Date_From=".$Date_From."&Date_To=".$Date_To."&Sponsor=".$Sponsor."&Sub_Department_ID=".$row['Sub_Department_ID']."&Payment_Item_Cache_List_ID=".$row['Payment_Item_Cache_List_ID']."' class='art-button-green' target='_parent'>AUDIOLOGY</a>";
      }
