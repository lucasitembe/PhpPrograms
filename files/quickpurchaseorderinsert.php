<?php
    ...
    if(isset($_SESSION['Storage']) && $Employee_ID != 0){
    ...
    while($row = mysqli_fetch_array($select_order_items)){
        $Item_ID = $row['Item_ID'];
        mysqli_query($conn,"insert into tbl_quick_purchase_order(Sub_Department_ID,Item_ID,Employee_ID) values('$Sub_Department_ID','$Item_ID','$Employee_ID')") or die(mysqli_error($conn));
    }
