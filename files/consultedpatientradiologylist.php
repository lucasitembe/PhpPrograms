...
?>
<input class="hide" type='button' name='patient_outpatient' id='patient_outpatient' value='DIRECT DEPT - OUTPATIENT' onclick='outpatient()' class='art-button-green' />
<input class="hide" type='button' name='patient_inpatient' id='patient_inpatient' value='DIRECT DEPT - INPATIENT' onclick='inpatient()' class='art-button-green' />
<a href='searchpatientradiologylist.php?from=consultedRadiology' class='art-button-green'>
        UN CONSULTED PATIENTS
    </a>

<?php
    if(isset($_GET['from']) && $_GET['from'] == "unconsultedRadiology") {
?>
    <a href='searchpatientradiologylist.php' class='art-button-green'>
        BACK
    </a>
<?php
    } else {
...
<fieldset style='margin-top:30px;'>
    <legend id="resultsconsultationLablist" style="font-weight: bold;font-size: 16px;background-color: #006400; padding:10px; color:white;width: auto;" align="right">CONSULTED RADIOLOGY PATIENTS LIST</legend>
    <center>
        <hr width="100%">
    </center>
        ...
        <table  class="hiv_table" style="width:100%">
            <tr>
                ...
                <td style="text-align: center">
                    <input type="text" autocomplete="off" style='text-align: center;width:10%;display:inline' id="Date_From" placeholder="Start Date" value="<?php echo $Date_From ?>"/>
                    <input type="text" autocomplete="off" style='text-align: center;width:10%;display:inline' id="Date_To" placeholder="End Date" value="<?php echo $Date_To ?>"/>&nbsp;
                    <select id="subCatID" style='text-align: center;padding:4px; width:10%;display:inline'>
                        <?php echo $dataSubCategory ?>
                    </select>
                    <select id="sponsorID" style='text-align: center;padding:4px; width:10%;display:inline'>
                        <?php echo $dataSponsor ?>
                    </select>
                    <input type="button" value="Filter" class="art-button-green" onclick="filterLabpatient()">
                    <input type='text' autocomplete="off" name='Search_Patient' id='Search_Patient' style='text-align: center;width:20%;display:inline' oninput="filterLabpatient()"  placeholder='~~~~~~~~~~~~~  Global Search Patient Name  ~~~~~~~~~~~~~~~~~~~'>
                    <input type='text' autocomplete="off" name='Patient_Number' id='Patient_Number' style='text-align: center;width:20%;display:inline' oninput="filterLabpatient()"  placeholder='~~~~~~~~~~~~~  Global Search Patient Number  ~~~~~~~~~~~~~~~~~~~'>
                </td>
            </tr>
            <tr>
                <td id='Search_Iframe'>
                    <div id="iframeDiv" style="height: 460px;overflow-y: auto;overflow-x: hidden">
                        <?php include 'radiologylistconsulted_iframe.php'; ?>
                    </div>
                </td>
            </tr>
        </table>
...
<script>
    function filterLabpatient() {
        ...
        mm.open('GET', 'radiologylistconsulted_iframe.php?filterlabpatientdate=true&Date_From=' + Date_From + '&Date_To=' + Date_To + '&Sponsor=' + Sponsor+ '&Patient_Name=' + Patient_Name+ '&SubCategory=' + SubCategory + '&Patient_Number=' + Patient_Number, true);
        mm.send();
    }
