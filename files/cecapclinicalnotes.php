...
<?php
	...
	if(isset($_GET['Payment_Cache_ID']) && isset($_GET['Payment_Item_Cache_List_ID'])){
	    ...
	    $Supervisor_ID = $_SESSION['Cecap_Supervisor']['Employee_ID'];
		...
		$select_from_cache = "SELECT * FROM tbl_item_list_cache il, tbl_items i
			...
			document.location = "cecapclinicalnotes.php?Cecapclinicalnotes=CecapPageclinicalnotes&Patient_Payment_ID=<?php echo $Patient_Payment_ID;?>&Registration_ID=<?php echo $Registration_ID;?>";
		    </script>
		    <?php
	    }   
	}elseif(isset($_GET['Patient_Payment_ID'])){
	    
	  //do action for paid customer here (set status served)
...
<script type="text/javascript">
	...
	if(patientlist=='Outpatient cash'){
	    ...
	    document.location = "cecapcreditpatientlist.php?CecapCreditPatientlist=CecapCreditPatientlistThispage";
...
<fieldset><legend align='right'><b>Cecap Works</b></legend>
	<table width=100%>
		<tr>
