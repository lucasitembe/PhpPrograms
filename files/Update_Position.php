<?php
	...
	if(isset($_GET['Position_Value'])){
		$Position_Value = mysqli_real_escape_string($conn,$_GET['Position_Value']);
	}else{
		$Position_Value = '';
	}
	...
	if($Employee_ID != '' && $Payment_Item_Cache_List_ID != '' && $consultation_ID != 0){
	    ...
	    $update = mysqli_query($conn,"update tbl_post_operative_notes set Position = '$Position_Value' where Post_operative_ID = '$Post_operative_ID'") or die(mysqli_error($conn));
	    if($update){
	    	echo $Position_Value;
	    }
	}
