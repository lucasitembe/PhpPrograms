...
//declare Claim_Form_Number
		...
		$select_items = mysqli_query($conn,"select ilc.Quantity,ilc.Clinic_ID, ilc.Edited_Quantity, ilc.Price, ilc.Discount, ilc.Payment_Item_Cache_List_ID, ilc.Check_In_Type, Consultant, 
		    	...
		    	if($num_rec > 0){
		    		...
		    		while ($rm = mysqli_fetch_array($select_items)) {
		    			...
		    			$sql = "insert into tbl_patient_payment_item_list(Check_In_Type, Item_ID, Discount, Price, Quantity,
		    											...
		    											Transaction_Date_And_Time,Clinic_ID) 

		    										values('".$rm['Check_In_Type']."','".$rm['Item_ID']."','".$rm['Discount']."','".$rm['Price']."','".$Quantity."',
		    											'Others','".$consultant."','".$rm['Consultant_ID']."',".$Patient_Payment_ID.",(select now()),'$Clinic_ID')";
