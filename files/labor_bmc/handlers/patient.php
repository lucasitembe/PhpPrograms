...
{
    if (isset($patient_id) && $patient_id != 0) {
        $select_patient_details = "SELECT pr.Sponsor_ID, pr.Member_Number, pr.Patient_Name, pr.Registration_ID, pr.Region, pr.District, pr.Gender, sp.Guarantor_Name, pr.Date_Of_Birth, pr.Tribe, pr.ten_cell_leader_name
                                        FROM
                                            tbl_patient_registration pr,
                                            tbl_sponsor sp
                                        WHERE
                                            sp.Sponsor_ID = pr.Sponsor_ID AND pr.Registration_ID = ?";
        ...
        mysqli_stmt_bind_result($stmt, $Sponsor_ID, $Member_Number, $Patient_Name, $Registration_ID, $region, $district, $Gender, $Guarantor_Name, $DOB, $tribe, $ten_cell);

        mysqli_stmt_fetch($stmt);

        $age = date_diff(date_create($DOB), date_create('today'))->y;

        return array($Sponsor_ID, $Member_Number, $Patient_Name, $Registration_ID, $region, $district, $Gender, $Guarantor_Name, $age, $tribe, $ten_cell);
    }
}
