...
<?php
echo "<a href='./doctorsindivinpatientperform.php?" . $_SERVER['QUERY_STRING'] . "' class='art-button-green'>
        BACK
    </a>
 <br/><br/>";
?>
...
//echo "SELECT pr.Patient_Name,pr.Phone_Number,c.Registration_ID,c.consultation_ID,ppl.Patient_Payment_Item_List_ID,ppl.Patient_Payment_ID ,e.Employee_Name,ch.employee_ID FROM tbl_consultation_history ch LEFT JOIN tbl_consultation c ON c.consultation_ID=ch.consultation_ID JOIN tbl_employee e ON ch.employee_ID=e.employee_ID JOIN tbl_patient_registration pr ON pr.Registration_ID=c.Registration_ID JOIN tbl_patient_payment_item_list ppl ON c.Patient_Payment_Item_List_ID = ppl.Patient_Payment_Item_List_ID WHERE ch.cons_hist_Date BETWEEN '$Date_From' AND '$Date_To' AND ch.employee_ID='$employee_ID'";
	 ...
	 ") or die(mysqli_error($conn));
                        ...
                        echo "<td style='text-align:center'><span onclick='Show_Patient_File(" . $Registration_ID . "," . $consultation_ID . ")' class='linkstyle' >" . ($Laboratory > 0 ? 'Yes' : 'No') . "</span></td>";

                        echo "<td style='text-align:center'><span onclick='Show_Patient_File(" . $Registration_ID . "," . $consultation_ID . ")' class='linkstyle' >" . ($Radiology > 0 ? 'Yes' : 'No') . "</span></td>";

                        echo "<td style='text-align:center'><span onclick='Show_Patient_File(" . $Registration_ID . "," . $consultation_ID . ")' class='linkstyle' >" . ($Pharmacy > 0 ? 'Yes' : 'No') . "</span></td>";

                        echo "<td style='text-align:center'><span onclick='Show_Patient_File(" . $Registration_ID . "," . $consultation_ID . ")' class='linkstyle' >" . ($Procedur > 0 ? 'Yes' : 'No') . "</span></td>";
