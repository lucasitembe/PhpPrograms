<?php
    ...
    $pdf.="<table width ='100%'>
    <tr>
        <td><center><b>IPD CREDIT BILL</center></b></td>
    </tr>
    <tr>
    ...
    $pdf.= '<tr><td width=5%>
		...
		</td>   <td width="10%"><span style="font-size: x-small;"><b>Category</b></span></td>
                        <td width="15%"><span style="font-size: x-small;"><b>Item</b></span></td>
			<td width="10%"><span style="font-size: x-small;"><b>Date</b></span></td>
    ...
    $Select_payments = "SELECT pp.Payment_Date_And_Time, pp.Patient_Payment_ID AS receipt_number,
                        pp.Transaction_status,pp.Transaction_type,pl.Item_ID,
                        ((pl.price-pl.Discount)*pl.Quantity) AS Amount,pl.Category,pl.Item_Name,pl.Item_ID
                        FROM tbl_patient_payments pp,tbl_patient_payment_item_list pl
                        WHERE pp.Registration_ID=$Registration_ID AND pp.Folio_Number = $folio AND
                        pp.Patient_Payment_ID = pl.Patient_Payment_ID
			AND (Billing_Type = 'Outpatient Credit' OR Billing_Type = 'Inpatient Credit')";
	...
	if($row['Transaction_type'] == 'Direct cash'){
	 ...
	 $Item_Name = $row['Item_Name'];
         ...
         $amount_p =0;
	 $inactive_amount = 0;
	 $select_itemName = "SELECT Product_Name FROM tbl_items WHERE Item_ID = ".$row['Item_ID'];
	 $nameresult = mysqli_query($conn,$select_itemName);
	 $namerow = mysqli_fetch_assoc($nameresult);
	 $Item_Name = $namerow['Product_Name'];
        }
	}else{
	...
	$pdf.= "<tr><td colspan=9><hr height = '3px'></td></tr>";
	$pdf.= '<tr><td><span style="font-size: x-small;">'.$temp.'</span></td>';
        $pdf.= "<td><span style='font-size: x-small;'>".ucfirst($row['Category'])."</span></td>";
        $pdf.= "<td><span style='font-size: x-small;'>".ucfirst($Item_Name)."</span></td>";
    ...
    $pdf.= "<tr><td colspan=9><hr></td></tr>";
    $pdf.= "<tr><td colspan=5 style='text-align: right;'><b> TOTAL :&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</b></td>
    ...
    $pdf.= "<tr><td colspan=9><hr></td></tr></table></center>";
