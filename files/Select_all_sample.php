          ...
          if($paymentstatus == "Not Billed" || $paymentstatus == "Paid" || $paymentstatus == "Approved"){
              ...
              if(mysqli_num_rows($sql_result)>0){
                  ...
                  if(mysqli_num_rows($sql_check_if_exist) > 0){
                             $specimenRejectionQr=mysqli_query($conn,"UPDATE tbl_specimen_results SET collection_Status='collected',TimeCollected=NOW(),specimen_results_Employee_ID='".$Employee_ID."' WHERE payment_item_ID='$Patient_Payment_Item_List_ID' AND Specimen_ID='$ref_specimen_ID'")or die(mysqli_error($conn));
                    }else{
                            $specimenRejectionQr=mysqli_query($conn,"INSERT INTO tbl_specimen_results (payment_item_ID,Specimen_ID,collection_Status,specimen_results_Employee_ID,TimeCollected,BarCode) VALUES ('$Patient_Payment_Item_List_ID','$ref_specimen_ID','collected','$Employee_ID',NOW(),'')") or die(mysqli_error($conn));
                  }
