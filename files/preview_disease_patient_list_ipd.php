...
if($diagnosis_type!='all'){
	...
	// if($age_group=="less"){
        ...
        //                             d.disease_ID=$disease_ID  $filterDiagnosis $filter and c.Consultation_Date_And_Time BETWEEN '$fromDate' and '$toDate' and TIMESTAMPDIFF(YEAR,Date_Of_Birth,CURDATE()) BETWEEN $start_age AND $end_age ORDER BY TIMESTAMPDIFF(YEAR,Date_Of_Birth,CURDATE()) ASC") or die(mysqli_error($conn));
    // }
//     elseif($patient_type=="Inpatient"){
        $select_patients=mysqli_query($conn,"select pr.Registration_ID, pr.patient_name, TIMESTAMPDIFF(YEAR,Date_Of_Birth,CURDATE()) as age,pr.Gender,wd.Round_Disease_Date_And_Time from tbl_ward_round_disease wd, tbl_disease d,tbl_ward_round wr,tbl_patient_registration pr where d.disease_ID = wd.disease_ID and wr.Round_ID = wd.Round_ID and wr.Registration_ID = pr.Registration_ID and diagnosis_type = 'diagnosis' and d.disease_ID=$disease_ID    and wd.Round_Disease_Date_And_Time between '$fromDate' and '$toDate' and TIMESTAMPDIFF(YEAR,Date_Of_Birth,CURDATE()) BETWEEN $start_age AND $end_age GROUP BY pr.Registration_ID ORDER BY TIMESTAMPDIFF(YEAR,Date_Of_Birth,CURDATE()) ASC");
...
//}
  ...
  // cl.Clinic_ID=c.Clinic_ID $filterIn and wd.Round_Disease_Date_And_Time between '$fromDate' and '$toDate' and TIMESTAMPDIFF(YEAR,pr.Date_Of_Birth,CURDATE())	
    ...
    $htm .= "<tr>";
    $htm .= "<td style='text-align: center;'><h3> Patients List From ".$fromDate." To ".$toDate."<br/> With Age From ".$start_age." --  ".$end_age." (Yrs)</h3></td>";
    $htm .= "</tr>";
    $htm .= "</table><br/>";
    ...
    if(mysqli_num_rows($select_patients)==0){
    	$htm.="<tr><td colspan='5'><center><br><br><br><br><b>No Patient Found</b><br><br><br><br><br></center></td></tr>";
    }
    while ($row=mysqli_fetch_assoc($select_patients)) {
		$htm.= "<tr><td>".$count."</td><td>".ucwords(strtolower($row['patient_name']))."</td></td><td style='text-align:center;'>".$row['age']."</td><td  style='padding-left:10px;'>".ucfirst($row['Gender'])."</td><td style='text-align:center;width:20%;'>".strtoupper($row['Round_Disease_Date_And_Time'])."</td></tr>";
		$count++;
	}
    $htm .= "</table>";
    //$htm.=$title;
