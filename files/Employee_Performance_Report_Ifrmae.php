   ...
   $transaction_channel=$_GET['transaction_channel'];
    if($transaction_channel != "All"){
       if($transaction_channel=="CRDB"){
        ...
        //    $transaction_channel_filter="AND pp.payment_direction IN('to_nmb','NMB', 'to-nmb')";  
       }else{
...
//}
    ...
    echo '<center><table class="fixTableHead" width =100% border="1" id="dtTableperformancedetails" class="display" style="background-color:white;">';
...
<?php
    if($Transaction_Type == 'All'){
        $select_Filtered_Employees = mysqli_query($conn, 
                "SELECT pr.Patient_Name, pp.Patient_Payment_ID, pp.Payment_Date_And_Time, pp.Billing_Type, pp.Transaction_status,pp.Cancel_transaction_reason, sum((price*quantity)-(discount*quantity)) as Total, pp.payment_type, pp.Pre_Paid from 
        ...
        $select_Filtered_Employees = mysqli_query($conn, 
               "select pp.payment_direction,pr.Patient_Name, pp.Patient_Payment_ID, pp.Payment_Date_And_Time, pp.Billing_Type, pp.Transaction_status,pp.Cancel_transaction_reason, sum((price*quantity)-(discount*quantity)) as Total, pp.payment_type, pp.Pre_Paid
        ...
        $select_Filtered_Employees = mysqli_query($conn, 
                "select pr.Patient_Name, pr.Patient_Name, pp.Patient_Payment_ID, pp.Payment_Date_And_Time, pp.Billing_Type, pp.Transaction_status,pp.Cancel_transaction_reason, sum((price*quantity)-(discount*quantity)) as Total, pp.payment_type, pp.Pre_Paid 
                    from tbl_patient_payments pp, tbl_patient_payment_item_list ppl, tbl_patient_registration pr
                    where pp.Patient_Payment_ID = ppl.Patient_Payment_ID and
                    pp.employee_id = '$Employee_ID' and
                    pr.Registration_ID = pp.Registration_ID and
    ...
    while($row = mysqli_fetch_array($select_Filtered_Employees)){
		if((strtolower($row['Billing_Type']) == 'outpatient cash' && $row['Pre_Paid'] == '1')){
			continue;
		}
        $payment_type = $row['payment_type'];
        echo "<tr><td id='thead1' style='text-align: center;'>".$temp."</td>";
        ...
        }elseif(((strtolower($row['Billing_Type']) == 'outpatient credit') || (strtolower($row['Billing_Type']) == 'inpatient credit') || (strtolower($row['Billing_Type']) == 'inpatient cash' && strtolower($payment_type) == 'post')) && (strtolower($row['Transaction_status']) == 'active')){
            echo "<td style='text-align: right;'>0</td>";
            echo "<td style='text-align: right;'>".(($_SESSION['systeminfo']['price_precision'] == 'yes') ? number_format($row['Total'], 2) : number_format($row['Total']))."</td>"; 
            echo "<td style='text-align: right;'>0</td>";
            echo  "<td>".$row['Cancel_transaction_reason']."</td>";
    ...
    echo "<tr><td colspan=4 style='text-align: right;'><b>Sub Total</b></td>";
