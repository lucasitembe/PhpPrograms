...
if(isset($_SESSION['userinfo']['Employee_Name'])){
    ...
    if($transaction_channel != "All"){
        if($transaction_channel == 'crdb_pos'){
            $transaction_channel_filter =" AND (pp.Payment_Code != '' AND (pp.payment_mode= 'CRDB' OR pp.payment_mode= 'CRDB..' ))";
        }else if($transaction_channel == 'to_crdb'){
            $transaction_channel_filter = " AND pp.payment_direction = 'to_crdb' AND pp.auth_code !=''";
        }else if($transaction_channel == 'to_nmb'){
            $transaction_channel_filter = " AND pp.payment_direction = 'to_nmb' AND pp.auth_code !=''";
...
// 
    ...
    if($Sponsor_ID == 0){
        if($Employee_ID == 0){    
            if($Billing_Type_Value == 'All'){
                    $select_details = mysqli_query($conn,"SELECT emp.Phone_Number,emp.Employee_ID,ts.Exemption, emp.Employee_Name, pp.payment_type,pp.Payment_Code,pp.manual_offline from tbl_patient_payment_item_list ppl, tbl_patient_payments pp, tbl_employee emp,tbl_sponsor as ts where
        ...
        if($Employee_ID == 0){    
            ...
            if($num > 0){
                while($row = mysqli_fetch_array($select_details)){
                    ...
                    if($Sponsor_ID == 0){
                        if($Billing_Type_Value == 'All'){
                            $select = mysqli_query($conn,"SELECT pp.Billing_Type, pp.auth_code, pp.Transaction_status,pp.Pre_Paid,ts.Exemption, pp.payment_type, sum((ppl.Price - ppl.Discount) * ppl.Quantity) as Amount 
                        ...
                        while($data = mysqli_fetch_array($select)){
                           ...
                           $total = $data['Amount']; //(($Price - $Discount) * $Quantity);
                            ...
                            // if((strtolower($Billing_Type) == 'outpatient cash'&& $payment_type == 'pre') || strtolower($Billing_Type) =="patient from outside"||(strtolower($Billing_Type) == 'inpatient cash' && strtolower($payment_type) == 'pre' && (!empty($auth_code)))){
                                // if(((strtolower($Billing_Type) == 'outpatient cash'&& $Pre_Paid==0) ||(strtolower($Billing_Type) =="patient from outside"  && $data['Pre_Paid'] == '0') || (strtolower($Billing_Type) == 'inpatient cash' && $payment_type == 'pre')) && strtolower($Transaction_status) != 'cancelled'  && $auth_code != ''){
            ...
            $htm .= "<tr><td colspan='2' style='text-align: left;'>
                        ...
                        <td style='text-align: right;'><b>".number_format($grand_total_msamaha)."</b></td>
    ...
    header("content-disposition: attachment; filename=cashandcreditreport.xls");
    echo $htm;
