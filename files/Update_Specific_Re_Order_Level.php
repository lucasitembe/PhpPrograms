<?php
    ...
    if($Sub_Department_ID != null && $Sub_Department_ID != '' && $New_Re_Order_Level_Value != null && $New_Re_Order_Level_Value != '' && $Item_ID != '' && $Item_ID != null){
        if($Sub_Department_ID != 0){
            $sql_update = mysqli_query($conn,"UPDATE tbl_items_balance set Reorder_Level = '$New_Re_Order_Level_Value', Reorder_Level_Status = 'specific', Employee_ID = '$Employee_ID', Update_Date_Time = NOW() where
                                        Sub_Department_ID = '$Sub_Department_ID' and
                                            Item_ID = '$Item_ID'") or die(mysqli_error($conn));
        }else{
            $sql_update = mysqli_query($conn,"UPDATE tbl_items_balance set Reorder_Level = '$New_Re_Order_Level_Value', Reorder_Level_Status = 'specific', Employee_ID = '$Employee_ID', Update_Date_Time = NOW() where  Item_ID = '$Item_ID'") or die(mysqli_error($conn));
        }
    }
    if($sql_update){
