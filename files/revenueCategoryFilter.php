...
?>
<?php
//receive the category id and use it to filter data
if(isset($_GET['categoryID'])){
    $catID=$_GET['categoryID'];
//echo $catID;
}
?>
<form action='revenueCategoryFilterWithFilterBefore.php?catID=<?php echo $catID;?>&RevenueCategoryFilterDetails=ThisForm' method='post' name='myForm' id='myForm' onsubmit="return validateForm();" enctype="multipart/form-data">
    <table width=80%>
        <tr>
	    ...
	    <td style='border: 0;'>
		<input type="hidden" name="categoryID" value="<?php if(isset($_GET['categoryID'])){echo $categoryID=$_GET['categoryID'];}?>"/>
	    </td>
            <!--<td style='text-align: center;'>
...
<script src="css/jquery.js"></script>
	...
	<table width=100%>
		...
		<?php
		    ...
		    $query="SELECT * FROM tbl_item_category WHERE Item_Category_ID='{$_GET['categoryID']}'";
...
<?php
    ...
    ?>    
<fieldset>  
            <legend align=center><b>REVENUE SUMMARY FOR <b style="color: blue;font-size: 14px"><?php echo $categoryName?></b> CATEGORY</b></legend>
        <center>
            <?php
    ...
    echo "<tr>
		...
		if($Branch == 0 && $sponsorID ==0){
		    ...
		    //RUN THE QUERY
					...
					ic.Item_Category_ID=isc.Item_Category_ID AND ic.Item_Category_ID=ic2.Item_Category_ID AND ic.Item_Category_ID='$catID'
					) 
					as cash,
					(
					select SUM((ppl.Price-ppl.Discount)*quantity) from tbl_patient_payments pp,tbl_patient_payment_item_list ppl,tbl_items it,tbl_item_subcategory isc,tbl_item_category ic WHERE ppl.Item_ID=it.Item_ID AND
					...
					FROM tbl_item_category ic2 WHERE ic2.Item_Category_ID='$catID'					 
				 ";
		    $select_Revenue_Details = mysqli_query($conn,$query) or die();
		}//END IF
		elseif($Branch ==0 && $sponsorID !=0){
		    //run the query if branch is set to all and sponsor is not set to all
			$query="select ic2.Item_Category_ID,ic2.Item_Category_Name,
					(
					select SUM((ppl.Price-ppl.Discount)*quantity) from tbl_patient_payments pp,tbl_patient_payment_item_list ppl,tbl_items it,tbl_sponsor sp,tbl_item_subcategory isc,tbl_item_category ic WHERE ppl.Item_ID=it.Item_ID AND
					pp.Patient_Payment_ID=ppl.Patient_Payment_ID and
					it.Item_Subcategory_ID=isc.Item_Subcategory_ID AND
					pp.Sponsor_ID=sp.Sponsor_ID AND
					pp.Payment_Date_And_Time BETWEEN '$Date_From' AND '$Date_To' AND
					pp.Sponsor_ID='$sponsorID' AND
					(pp.Billing_Type='Outpatient Cash' or pp.Billing_Type='Inpatient Cash') AND
					ic.Item_Category_ID=isc.Item_Category_ID AND ic.Item_Category_ID=ic2.Item_Category_ID AND ic.Item_Category_ID='$catID'
					) 
					as cash,
		    ...
		    elseif($Branch !=0 && $sponsorID ==0){
			//run the query when branch is not set to all and sponsor is set all
			$query="select ic2.Item_Category_ID,ic2.Item_Category_Name,
					(
					select SUM((ppl.Price-ppl.Discount)*quantity) from tbl_patient_payments pp,tbl_patient_payment_item_list ppl,tbl_items it,tbl_sponsor sp,tbl_branches br,tbl_item_subcategory isc,tbl_item_category ic WHERE ppl.Item_ID=it.Item_ID AND
					pp.Patient_Payment_ID=ppl.Patient_Payment_ID and
					it.Item_Subcategory_ID=isc.Item_Subcategory_ID AND
					pp.Sponsor_ID=sp.Sponsor_ID AND
					pp.branch_id=br.Branch_ID AND
					pp.Payment_Date_And_Time BETWEEN '$Date_From' AND '$Date_To' AND
					pp.branch_id='$Branch' AND
					(pp.Billing_Type='Outpatient Cash' or pp.Billing_Type='Inpatient Cash') AND
					ic.Item_Category_ID=isc.Item_Category_ID AND ic.Item_Category_ID=ic2.Item_Category_ID AND ic.Item_Category_ID='$catID'
					) 
					as cash,
		    ...
		    elseif($Branch !=0 && $sponsorID !=0){
			//run the query to select data if both branch and sponsor are not set to all
			$query="select ic2.Item_Category_ID,ic2.Item_Category_Name,
					(
					select SUM((ppl.Price-ppl.Discount)*quantity) from tbl_patient_payments pp,tbl_patient_payment_item_list ppl,tbl_items it,tbl_sponsor sp,tbl_branches br,tbl_item_subcategory isc,tbl_item_category ic WHERE ppl.Item_ID=it.Item_ID AND
					pp.Patient_Payment_ID=ppl.Patient_Payment_ID and
		    ...
		    //RUN THE QUERY
					...
					pp.Patient_Payment_ID=ppl.Patient_Payment_ID and
					it.Item_Subcategory_ID=isc.Item_Subcategory_ID AND
					pp.Payment_Date_And_Time='CURDATE()' AND
					(pp.Billing_Type='Outpatient Cash' or pp.Billing_Type='Inpatient Cash') AND
					ic.Item_Category_ID=isc.Item_Category_ID AND ic.Item_Category_ID=ic2.Item_Category_ID AND ic.Item_Category_ID='$catID'
					) 
					as cash,
	...
	$cancelled=$row['cancelled'];
        echo "<tr><td>".($i+1)."</td>";
        echo "<td><a href='revenueCategoryFilterDetails.php?categoryID=$catID&Branch=$Branch&Date_From=$Date_From&Date_To=$Date_To&sponsorID=$sponsorID&RevenueCategoryDetails=RevenueCategoryDetailsThisForm'>".$row['Item_Category_Name']."</a></td>";
