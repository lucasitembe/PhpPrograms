...
//        die($Admission_Status );
        ...
        if ($Admission_Status == 'Dicharged') {
            $result_disc =  mysqli_query($conn, "SELECT Discharge_Date_Time,Employee_Name,Discharge_Reason FROM tbl_admission ad,  tbl_discharge_reason dr, tbl_employee e, tbl_check_in_details cd  WHERE dr.Discharge_Reason_ID = ad.Discharge_Reason_ID AND e.Employee_ID=ad.Discharge_Employee_ID AND cd.Admission_ID = ad.Admision_ID   WHERE consultation_ID = '$consultation_ID' AND ad.Registration_ID='$Registration_ID' AND cd.Admit_Status='admitted' ") or die(mysqli_error($conn));
...
$data .='<div class="row" style="height:70vh; overflow-x:scroll;overflow-y:scroll">
            <div class="col-md-12">
                <div class="row" style="padding:5px; width:99%;font-size:larger;border:0px solid #000;  background:#ccc;text-align:center " id="inpatient">
                    <h3><b align="center">INPATIENT DETAILS</b></h3>
                </div>
                <div >
                    <table class="table" >
                        <tr>
    ...
    ';
        ...
        $rsDoc = mysqli_query($conn,"SELECT e.Employee_Name,e.Employee_ID,wr.Findings,wr.investigation_comments,wr.Ward_Round_Date_And_Time,wr.remarks FROM tbl_ward_round wr JOIN tbl_employee e ON wr.Employee_ID=e.Employee_ID WHERE wr.consultation_ID='" . $consultation_ID . "' AND wr.Registration_ID=$Registration_ID ") or die(mysqli_error($conn));

        if (mysqli_num_rows($rsDoc) > 0) {
            $data .='<div  class="row" id="findings-remarks">
                    <div class="col-md-12">
                        <h3><b>FINDINGS AND REMARKS</b></h3>
                    </div>
                    ';
            $data .= '<div class="col-md-12" ><table class="table table-bordered">
            <tr style="background:#E8E8E8;">
                <td>#</td><td><b>Doctor\'s Name</b></td><td><b>Findings</b></td><td><b>Investigation comments</b></td><td><b>Remarks</b></td><td><b>Date Consulted</b></td>
            </tr>';
        ...
        $results = mysqli_query($conn,"SELECT DISTINCT(wr.employee_ID),e.Employee_Name,e.Employee_Type FROM tbl_ward_round_disease wd INNER JOIN tbl_ward_round wr ON wr.Round_ID = wd.Round_ID  JOIN tbl_employee e ON e.Employee_ID = wr.Employee_ID WHERE wr.consultation_ID='$consultation_ID'") or die(mysqli_error($conn));
        $sn = 1;
        if (mysqli_num_rows($results) > 0) {
            $data .="
    <div class='row' id='diagonsis'>
        <div class='col-md-12'>
            <h3><b> DIAGNOSIS </b></h3>
        </div>";
            ...
            while ($row = mysqli_fetch_array($results)) {
                ...
                $data .= '<br/><table id="diagonsis" class="table">
		    ...
		    ";
                ...
                $data .= ' <table id="diagonsis" class="table">
                     <tr style="background:#E8E8E8;"><td>#</td><td><b>Provisional Diagnosis</b></td><td><b>Differential Diagnosis</b></td><td><b>Final Diagnosis</b></td><td style="text-align:center"><b>Disease Code</b></td><td><b>Date Consulted</b></td></tr>';
                ...
                while ($attach = mysqli_fetch_array($query)) {
                    ...
                    if ($attach['Attachment_Url'] != '') {
                        if ($resultLab == "No result" && empty($attach['Description'])) {
                            ...
                            $image .= "<a href='patient_attachments/" . $attach['Attachment_Url'] . "' class='fancybox2' rel='gallery' target='_blank' title='" . $row['Product_Name'] . "'> View";
                            
                            //<img src='patient_attachments/attachement.png' width='50' height='50' alt='Not Image File' />
                            $image .="</a>&nbsp;&nbsp;";
                        }
                    }
...
//                $data .= "</tr></table>";
            ...
            $hide_btn="";
                ...
                if (empty($resultLab)) {
                   // $selectQuery = "SELECT * from tbl_items item,tbl_item_list_cache ilc,tbl_payment_cache as tpc, tbl_test_results tr, tbl_tests_parameters tp, tbl_parameters p, tbl_tests_parameters_results tpr where item.Item_ID=ilc.Item_ID AND ilc.Payment_Cache_ID=tpc.Payment_Cache_ID AND ilc.Payment_Item_Cache_List_ID = tr.payment_item_ID and ilc.Item_ID = tp.ref_item_ID and p.parameter_ID = tp.ref_parameter_ID and tpr.parameter = p.parameter_ID and tr.test_result_ID = tpr.ref_test_result_ID and ilc.Item_ID = '" . $item_ID . "' AND tr.payment_item_ID='" . $ppil . "'  AND Registration_ID='" . $Registration_ID . "' AND Validated = 'Yes' AND Parameter_Name !='default_parameter'  GROUP BY PARAMETER_NAME ORDER BY PARAMETER_NAME";
                    ...
                    $selectQuery = "SELECT tr.test_result_ID,tr.payment_item_ID,higher_value, result, Parameter_Name,p.unit_of_measure,p.parameter_ID, lower_value, operator,Saved, result_type, Submitted,Validated from tbl_items item,tbl_item_list_cache ilc,tbl_payment_cache as tpc, tbl_test_results tr, tbl_tests_parameters tp, tbl_parameters p, tbl_tests_parameters_results tpr where item.Item_ID=ilc.Item_ID AND ilc.Payment_Cache_ID=tpc.Payment_Cache_ID AND ilc.Payment_Item_Cache_List_ID = tr.payment_item_ID and ilc.Item_ID = tp.ref_item_ID and p.parameter_ID = tp.ref_parameter_ID and tpr.parameter = p.parameter_ID and tr.test_result_ID = tpr.ref_test_result_ID and ilc.Item_ID = '" . $item_ID . "' AND tr.payment_item_ID='" . $ppil . "'  AND Registration_ID='" . $Registration_ID . "' AND Validated = 'Yes' AND Parameter_Name !='default_parameter'  GROUP BY PARAMETER_NAME ORDER BY PARAMETER_NAME";
        ...
        <div style='padding:5px; width:99.3%;font-size:larger;  background:#ccc;text-align:left  '>
            ...
            <tr><td colspan="8"><b>' . $temp++ . '.</b> <b>' . $row['Product_Name'] . '</b></td></tr>
            <tr>
               <td><b>Doctor\'s Ordered</b></td><td><b>Doctor\'s Notes</b></td><td><b>Lab Remarks</b></td><td><b>Attachment</b></td>' . ((empty($display)) ? '<td><b>Status</b></td>' : '') . '<td><b>Performer</b></td><td><b>Validator</b></td><td><b>Date</b></td>
            ...
            $hide_btn="";
                ...
                if(mysqli_num_rows($sql_select_patient_lab_intergrate_result)<=0){
                    ...
                    $selectQuery = "SELECT  tr.payment_item_ID, result, Parameter_Name, p.unit_of_measure, modified, Validated, Submitted,operator,higher_value, Saved,result_type  from tbl_items item,tbl_item_list_cache ilc,tbl_payment_cache as tpc, tbl_test_results tr, tbl_tests_parameters tp, tbl_parameters p, tbl_tests_parameters_results tpr where item.Item_ID=ilc.Item_ID AND ilc.Payment_Cache_ID=tpc.Payment_Cache_ID AND ilc.Payment_Item_Cache_List_ID = tr.payment_item_ID and ilc.Item_ID = tp.ref_item_ID and p.parameter_ID = tp.ref_parameter_ID and tpr.parameter = p.parameter_ID and tr.test_result_ID = tpr.ref_test_result_ID and ilc.Item_ID = '" . $item_ID . "' AND tr.payment_item_ID='" . $ppil . "'  AND Registration_ID='" . $Registration_ID . "'  GROUP BY PARAMETER_NAME ORDER BY PARAMETER_NAME";
        ...
        $tbrad = " <table id='radiology' border='1' width='100%' style='margin-left:2px'>
            ...
            // rpt.Date_Time,Radiologist_ID,Sonographer_ID,Patient_Payment_Item_List_ID,i.Item_ID,ilc.Transaction_Date_And_Time FROM	tbl_radiology_patient_tests rpt INNER JOIN tbl_items i ON rpt.Item_ID = i.Item_ID JOIN tbl_item_list_cache ilc ON ilc.Payment_Item_Cache_List_ID=rpt.Patient_Payment_Item_List_ID  JOIN tbl_payment_cache pc ON ilc.Payment_Cache_ID=pc.Payment_Cache_ID WHERE rpt.Registration_ID = '$Registration_ID' AND 	pc.consultation_id ='$consultation_ID' AND Billing_Type LIKE '%Inpatient%'";
        $qr = "SELECT rpt.Status,pc.Registration_ID,i.Product_Name,rpt.Remarks,Payment_Item_Cache_List_ID, rpt.Date_Time,Radiologist_ID,Sonographer_ID,Patient_Payment_Item_List_ID,i.Item_ID,ilc.Transaction_Date_And_Time FROM	tbl_radiology_patient_tests rpt, tbl_items i, tbl_item_list_cache ilc,tbl_payment_cache pc WHERE  rpt.Item_ID = i.Item_ID AND ilc.Payment_Item_Cache_List_ID=rpt.Patient_Payment_Item_List_ID  AND ilc.Payment_Cache_ID=pc.Payment_Cache_ID AND rpt.Registration_ID = '$Registration_ID' AND 	pc.consultation_id ='$consultation_ID' AND Billing_Type LIKE '%Inpatient%'";

        $qrnotdone = "SELECT ilc.Status,ilc.Transaction_Date_And_Time,ilc.Doctor_Comment,pc.Registration_ID,i.Product_Name, Payment_Item_Cache_List_ID,i.Item_ID FROM 	tbl_item_list_cache ilc, tbl_payment_cache pc, tbl_items i WHERE ilc.Payment_Cache_ID=pc.Payment_Cache_ID AND   ilc.Item_ID = i.Item_ID AND  ilc.Status != 'notsaved' AND  pc.Registration_ID = '$Registration_ID' AND pc.consultation_id ='$consultation_ID' AND Billing_Type LIKE '%Inpatient%' AND ilc.Check_In_Type='Radiology'";
        //  die($qrnotdone);
            ...
            while ($patient = mysqli_fetch_assoc($select_patients_qry)) {
                ...
                if (mysqli_num_rows($result) > 0) {
                    ...
                    while ($row = mysqli_fetch_array($result)) {
                        ...
                        //imagefiles
                        //<img height='50' alt=''  src='RadiologyImage/" . $Radiology_Image . "'  alt=''/>
                         if (preg_match('/\.(jpg|jpeg|png|gif)(?:[\?\#].*)?$/i', $Radiology_Image)) {
                            $imaging .= "<a href='RadiologyImage/" . $Radiology_Image . "' title='" . $test_name . "' class='fancyboxRadimg' target='_blank'>View Image</a>";
                        } else {
                            $imaging .= "<a href='RadiologyImage/" . $Radiology_Image . "' title='" . $test_name . "' class='fancyboxRadimg' target='_blank' >View Image</a>";
                        }
                    }
                ...
                $comm = "<a class='no_color' href='RadiologyTests_Print.php?RI=" . $Registration_ID . "&II=" . $Item_ID . "&PPILI=" . $patient['Payment_Item_Cache_List_ID'] . "' title='Click to view comment added by radiologist' target='_blank'>View Image</a>";
...
//				$commentsDescription='<button style="width:74%;" class="art-button-green" onclick="commentsAndDescription(\''.$href.'\',\''.$test_name.'\')">Report</button>';
    ...
    <div width='100%' id='departments_comments'>
        <div style='padding:5px; width:99.3%;font-size:larger;  background:#ccc;text-align:left  '>
            ...
            while ($patient = mysqli_fetch_assoc($select_qr)) {
                ...
                if (strtolower($Status) == 'served') {
                    $st = '<span style="color:blue;text-align:center;">Done</span>';
                    $comm = "<a class='no_color' href='previewpostoperativereport.php?Registration_ID=$Registration_ID&Payment_Item_Cache_List_ID=$Payment_Item_Cache_List_ID' title='Click to view comment added by Post Operative Notes' target='_blank'>View Image</a>";
                } else {
        ...
        $qry = "SELECT tit.Product_Name,ilc.Status,ilc.Doctor_Comment,ilc.remarks,em.Employee_Name,em.Employee_Type,ilc.ServedDateTime,ilc.ServedBy,ilc.Transaction_Date_And_Time AS DateTime,'doctor' as origin FROM tbl_item_list_cache ilc, tbl_payment_cache pc,tbl_items tit, tbl_employee em, tbl_consultation tc WHERE  ilc.Payment_Cache_ID=pc.Payment_Cache_ID AND tit.Item_ID=ilc.Item_ID AND em.Employee_ID=ilc.Consultant_ID	AND tc.consultation_ID=pc.consultation_id 		AND   ilc.Status != 'notsaved' AND  pc.consultation_id ='$consultation_ID' AND Billing_Type LIKE '%Inpatient%' AND ilc.Check_In_Type='Procedure'";
...
//            }
        ...
        $subqr = "SELECT ilc.Employee_Created,ilc.Status,Product_Name,Quantity,Edited_Quantity,Dispense_Date_Time,Employee_Name,Doctor_Comment,Transaction_Date_And_Time,Dispensor FROM tbl_item_list_cache ilc, tbl_payment_cache pc, tbl_items i, tbl_employee em, tbl_consultation tc WHERE ilc.Payment_Cache_ID = pc.Payment_Cache_ID AND i.item_ID = ilc.item_ID  AND em.Employee_ID=ilc.Consultant_ID  AND tc.consultation_ID=pc.consultation_id   AND  Check_In_Type='Pharmacy' AND pc.Registration_ID='$Registration_ID' AND pc.consultation_id ='$consultation_ID' AND Billing_Type LIKE '%Inpatient%'";
