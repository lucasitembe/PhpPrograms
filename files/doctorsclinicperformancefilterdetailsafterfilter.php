...
<?php
    if(isset($_SESSION['userinfo'])){
        if($_SESSION['userinfo']['Doctors_Page_Outpatient_Work'] == 'yes'){ 
?>
    <a href='./myclinicperformance.php?FinanceWork=FinanceWorkThisPage' class='art-button-green'>
        BACK
    </a>
<?php  } } ?>
...
<fieldset>
    <center>
	<form action='doctorsclinicperformancefilterdetailsafterfilter.php?DoctorsPerformanceReportThisPage=ThisPage' method='post' name='myForm' id='myForm' onsubmit="return validateForm();" enctype="multipart/form-data">
    ...
    <legend align=center><b>CLINIC PERFORMANCE DETAILS FOR DOCTOR </b><b style="color:blue;"><?php echo strtoupper($doctorName)?></b> <b>FROM </b><b style="color: blue;"><?php echo date('j F, Y H:i:s',strtotime($Date_From))?></b><b> TO </b><b style="color: blue;"><?php echo date('j F, Y H:i:s',strtotime($Date_To))?></b></legend>
        <iframe src="doctorsclinicperformancefilterdetailsafterfilter_Iframe.php?Employee_ID=<?php echo $doctorID?>&Date_From=<?php echo $Date_From?>&Date_To=<?php echo $Date_To?>" width="100%" height="340px"></iframe>
			</center>
