...
?>
	...
	<a href='bloodworkpage.php?Usage=UsageThisPage'  class='art-button-green'>
        BACK
	</a>
...
<?php
    if(isset($_POST['submittedAddNewIssueForm'])){
        ...
        $Blood_Used= mysqli_real_escape_string($conn,$_POST['Blood_Used']);
        $Reason = mysqli_real_escape_string($conn,$_POST['Reason']);
        $Date_Taken = mysqli_real_escape_string($conn,$_POST['Date_Taken']);
		$Patient_Given=mysqli_real_escape_string($conn,$_POST['Patient_Given']);
	...
	$data = mysqli_query($conn,"select now() as Registered_Date_And_Time");
        ...
        $Insert_Sql = "insert into tbl_blood_checked(
                    Blood_Group,Blood_Batch,BloodRecorded,Blood_Status,Employee_ID,Blood_ID,Patient_Given,
                         	Reason,Date_Taken,Registered_Date_And_Time)
	
                    values('$Blood_Group','$Blood_Batch',' $Blood_Used','$Blood_Status','$Employee_ID',
			    '$Blood_ID','$Patient_Given','$Reason',
		...
		if(!mysqli_query($conn,$Insert_Sql)){ 
		 ...
		 echo "<script type='text/javascript'>
			    alert('RECORDED');
			    document.location = './bloodissue.php';
			</script>";
		}
...
<script type="text/javascript" language="javascript">
    function getBatches(Batch_Name) {
	    ...
	    mm2.open('GET','GetBloodId.php?Batch_Name='+Batch_Name,true);
	    mm2.send();
	}
    function AJAXP2() {
...
<script>
	...
	var a = parseInt(document.getElementById('Blood_Used').value);
	var b = parseInt(document.getElementById('sumblood').value);
	    if (a>b) {
		alert ("exceed total value");
			document.getElementById('Blood_Used').value='';
	    }
	 }  
</script>
...
<center>
	...
	<legend align=right><b>BLOOD ISSUED</b></legend><br/>
        <center>
	    <form action='#' method='post' name='myForm' id='myForm' onsubmit="return validateForm();" enctype="multipart/form-data">           
            ...
            <table width=90%>
		<tr>
		    <td style="text-align:right;"><b>Donated Patient</b></td>
		    <?php
			if(isset($_GET['Registration_ID'])){
				$Registration_ID = $_GET['Registration_ID'];
				$Patient_qr = "SELECT Patient_Name FROM tbl_patient_registration WHERE Registration_ID=$Registration_ID";
				$Patient_result = mysqli_query($conn,$Patient_qr);
			...
			<input type='text'  name='Patient_Given' readonly='readonly' placeholder='Search the Patient Name' value='<?php
				if(isset($Patient_Name)){
					echo $Patient_Name;
				}
			    ?>'  required='required' style='width: 320px;'>
			<input type='hidden' id='Registration_ID' name='Registration_ID' value='<?php echo $Registration_ID; ?>'>
		    </td>
		    <td>      
			<a href='searchpatient.php' id='search' class='art-button-green'>SEARCH</a>
		    </td>
		</tr>
		<tr>
		    ...
		    <td>
			<select name='Blood_ID' id='Blood_ID' required='required' onchange='BloodVolume();BloodGroup()'>
		...
		<tr>
		    ...
		    <td style="text-align: right;width:10%;"><input type='text' name='Blood_Group' id='Blood_Group' ></td> 
			      
	
		    <td style="text-align: right;width:15%;"><b>Total Volume</b></td>
		   <td><input type="text" id="sumblood" name="sumblood" value="" placeholder='In milliliter'></td>
                </tr> 
                <tr>

	                              
		    <td style="text-align: right;width:40%"><b>Blood Volume Used</b></td>
		    <td  colspan='3'><input type='text' name='Blood_Used' id='Blood_Used' onkeyup='Bloodcompare()' placeholder='In milliliter'></td>
	        </tr>
						   
                            
		<tr>
		    <td style='text-align:right;'><b>Reason for Using</b></td>
		    <td colspan='3'><textarea  name='Reason' id='Reason'></textarea></td>
		</tr>
								   
		<tr>
		     <td style="text-align: right;width:40%"><b>Date Used</b></td>
		...
		<tr>
			...
			<input type='hidden' name='submittedAddNewIssueForm' value='true'/> 
		    </td>
		</tr>
	    </table>
                    </td>
