				...
				if(isset($_SESSION['Pharmacy_Requisition_ID']) && $_SESSION['Pharmacy_Requisition_ID'] != '' && $_SESSION['Pharmacy_Requisition_ID'] != null){
					...
					if($Store_Issue != 0){
						...
						$select_sub_department = mysqli_query($conn,"select Sub_Department_Name from tbl_Sub_Department where
											Sub_Department_ID = '$Store_Issue' $filter_sub_d") or die(mysqli_error($conn));
					...
					?>
						<select name='Store_Issue' id='Store_Issue' required='required' style="width:100%">
					...
					$select_Supplier = mysqli_query($conn,"select * from tbl_sub_department where Sub_Department_Name <> '$Current_Sub_Department' $filter_sub_d");
					echo "<select name='Store_Issue' id='Store_Issue' required='required' style='width:100%'>";
...
<script>	
	function Get_Selected_Item(){
		...
		var Store_Need_ID = '<?= $_SESSION['Pharmacy_ID'] ?>';
...
//					alert("Item Added Successfully");
			...
			my_Object_Get_Selected_Item.open('GET','Pharmacy_Requisition_Add_Selected_Item.php?Item_ID='+Item_ID+'&Quantity='+Quantity+'&Item_Remark='+Item_Remark+'&Store_Issue='+Store_Issue+'&Store_Need='+Store_Need+'&Requisition_Description='+Requisition_Description+'&Cont_Quantity='+Cont_Quantity+'&Items_Quantity='+Items_Quantity+'&Store_Need_ID='+Store_Need_ID,true);
						...
						<td>
							<table width = 100%>
								...
								<tr>
									...
									<td width="6%">
										<input type='text' name='Quantity' id='Quantity' autocomplete='off' size=10 placeholder='Quantity' onchange='numberOnly(this); Clear_Quantity();' onkeyup='numberOnly(this); Clear_Quantity();' onkeypress='numberOnly(this); Clear_Quantity();' oninput="Clear_Quantity();" required='required' style="text-align: center;">
									</td>
									<td width="7%">
