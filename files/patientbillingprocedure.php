...
<script type="text/javascript">
	...
	if(patientlist=='OUTPATIENT CASH'){
	    document.location = "revenuecenterprocedurelist.php?Billing_Type=OutpatientCash&RadiologyList=RadiologyListThisForm";
	}else if (patientlist=='OUTPATIENT CREDIT') {
	    document.location = "revenuecenterprocedurelist.php?Billing_Type=OutpatientCredit&RadiologyList=RadiologyListThisForm";
	}else if (patientlist=='INPATIENT CASH') {
	    document.location = "revenuecenterprocedurelist.php?Billing_Type=InpatientCash&RadiologyList=RadiologyListThisForm";
	}else if (patientlist=='INPATIENT CREDIT') {
	    document.location = "revenuecenterprocedurelist.php?Billing_Type=InpatientCredit&RadiologyList=RadiologyListThisForm";
	}else if (patientlist=='PATIENT FROM OUTSIDE') {
	    document.location = "revenuecenterprocedurelist.php?Billing_Type=PatientFromOutside&RadiologyList=RadiologyListThisForm";
	}else{
...
<!--<br/>-->
<fieldset>  
            <legend align=right><b>PATIENT PAYMENTS ~ PROCEDURES</b></legend>
        <center>
            <table width=100%> 
                <tr> 
                    <td>
				...
				<td style="text-align:right;">Receipt Date & Time</td>
					...
					<option selected='selected' >Procedure</option> 
				    </select>
				</td>
			...
			<?php
			    if($Transaction_Status_Title == 'NOT PAID'){
                    ...
                    <?php }  if(isset($_SESSION['configData']) && $_SESSION['configData']['ShowCreateEpaymentBillOrMakePaymentButton']=='makepayment'){ ?>
                        ...
                        <?php }//end of check show button visibility
                    }
...
?>
        <tr><input type="text" id="total_txt" hidden="hidden" value="<?php echo $total; ?>"/>
...
<table width="100%">
    <tr>
        ...
        <td style="text-align: right;" id="Grand_Total"><b>GRAND TOTAL : <?php echo number_format($total); ?></b>&nbsp;&nbsp;&nbsp;</td>
    </tr>
</table>
<!--Dialog div-->
<div id="addTests" style="width:100%;overflow:hidden" >
    ...
    <fieldset>
        <!--<legend align='right'><b><a href='#' class='art-button-green'>LOGOUT</a></b></legend>-->
            <center>
                ...
                <tr>
                    <td width="40%" style="text-align: center"><input type="text" name="search" id="search_medicene" placeholder="-----------------------------------------Search Item-------------------------------------------" onkeyup="searchMedicene(this.value,<?php echo $_GET['Payment_Cache_ID'];?>)"></td><td width="50%" style="text-align: center"><button style="width:90%;font-size:20px; " name="submitadded" class="art-button-green" type="button" onclick="submitAddedItems()">Add Item(s)</button></td>
                </tr>   
                <tr>
...
<script type="text/javascript">
    function Add_New_Items(){
        ...
        myObjectAddItem.open('GET','Patient_Billing_Procedure_Add_New_Item.php?Payment_Cache_ID='+Payment_Cache_ID+'&Sub_Department_ID='+Sub_Department_ID+'&Transaction_Type='+Transaction_Type+'&Guarantor_Name='+Guarantor_Name,true);
...
<script>
      ...
      $.ajax({
          ...
          data:{Registration_ID:Registration_ID,Payment_Cache_ID:Payment_Cache_ID,Sub_Department_ID:Sub_Department_ID,Transaction_Type:Transaction_Type,Patient_Name:Patient_Name,Check_In_Type:'Procedure'},
...
<script type="text/javascript">
    function Refresh_Total(Payment_Cache_ID,Sub_Department_ID,Transaction_Type){
        ...
        myObjectRefresh.open('GET','Patient_Billing_Procedure_Refresh_Total.php?Payment_Cache_ID='+Payment_Cache_ID+'&Sub_Department_ID='+Sub_Department_ID+'&Transaction_Type='+Transaction_Type,true);
...
<script type="text/javascript">
    function Refresh_Removed_Button(Payment_Cache_ID,Sub_Department_ID,Transaction_Type){
        ...
        myObjectRefresh.open('GET','Patient_Billing_Procedure_Refresh_Removed_Button.php?Payment_Cache_ID='+Payment_Cache_ID+'&Sub_Department_ID='+Sub_Department_ID+'&Transaction_Type='+Transaction_Type,true);
        myObjectRefresh.send();
    }
</script>
...
<script type="text/javascript">
    function Preview_Removed_Items(Payment_Cache_ID,Sub_Department_ID,Transaction_Type){
        ...
        myObjectPreview.open('GET','Patient_Billing_Procedure_Preview_Removed_Items.php?Payment_Cache_ID='+Payment_Cache_ID+'&Sub_Department_ID='+Sub_Department_ID+'&Transaction_Type='+Transaction_Type,true);
...
<script type="text/javascript">
    function Re_Add_Item(Product_Name,Payment_Item_Cache_List_ID){
        ...
        if (Confirm_Message == true) {
            ...
            myObjectReAdd.open('GET','Patient_Billing_Procedure_Re_Add_Item.php?Payment_Cache_ID='+Payment_Cache_ID+'&Sub_Department_ID='+Sub_Department_ID+'&Transaction_Type='+Transaction_Type+'&Payment_Item_Cache_List_ID='+Payment_Item_Cache_List_ID,true);
            myObjectReAdd.send();
        }
    }
</script>
...
<script>
   $(document).ready(function(){
      $("#Add_New_Item").dialog({ autoOpen: false, width:'80%',height:450, title:'ADD PROCEDURE ITEMS',modal: true});
   });
</script>
...
<script type="text/javascript">
    ...
    function get_terminals(trans_type){
        ...
        if(trans_type.value=="Manual"){
            ...
            if(result){
                     document.location = 'Patient_Billing_Procedure.php?Transaction_Type='+Transaction_Type+'&Payment_Cache_ID='+Payment_Cache_ID+'&Sub_Department_ID='+Sub_Department_ID+'&Billing_Type='+Billing_Type+'&manual_offline=manual';
      ...
      function offline_transaction(amount_required,reg_id){
        ...
        var uri = '../epay/patientbillingprocedureOfflinePayment.php';
        var Payment_Cache_ID = '<?php echo $Payment_Cache_ID; ?>';
        var Sub_Department_ID = '<?php echo $Sub_Department_ID; ?>';
        var Transaction_Type = '<?php echo $Transaction_Type; ?>';
        var Billing_Type = '<?php echo $Temp_Billing_Type2; ?>';
...
<script type="text/javascript">
    function Make_Payment_Laboratory(){
        ...
        myObjectVerify.onreadystatechange = function (){
            ...
            if (myObjectVerify.readyState == 4) {
                ...
                if (feedback == 'yes'){
                  ...
                  //  if(r == true){
                       // document.location = 'Patient_Billing_Procedure.php?Transaction_Type='+Transaction_Type+'&Payment_Cache_ID='+Payment_Cache_ID+'&Sub_Department_ID='+Sub_Department_ID+'&Billing_Type='+Billing_Type;
                    //}
                }else{
        ...
        myObjectVerify.open('GET','Patient_Billing_Procedure_Verify.php?Payment_Cache_ID='+Payment_Cache_ID+'&Sub_Department_ID='+Sub_Department_ID+'&Transaction_Type='+Transaction_Type,true);
        myObjectVerify.send();
    }
</script>
...
<script>
    function removeitem(Item_Name,Item_Location){
        ...
        if(check == true){
            ...
            myObjectRemove.open('GET','Patient_Billing_Procedure_Remove.php?Item_Location='+Item_Location+'&Payment_Cache_ID='+Payment_Cache_ID+'&Sub_Department_ID='+Sub_Department_ID+'&Transaction_Type='+Transaction_Type,true);
            myObjectRemove.send();
        }
    }
</script>
<script>
 function vieweRemovedItem(){
     ...
     $.ajax({
         type: 'POST',
         url: "change_items_info_procedure.php",
...
function searchMedicene(search,Payment_Cache_ID){
   if(search !==''){ 
    $.ajax({
         type: 'GET',
         url: "search_item_for_test.php",
         data: "section=<?php echo $_GET['section']?>&search_word="+search+"&Payment_Cache_ID=" + Payment_Cache_ID,
         success: function (data) {
            if(data !==''){
...
<script>
    function Pay_Via_Mobile_Function(Payment_Cache_ID){
        ...
        myObjectGetDetails.open('GET','ePayment_Patient_Details_Departmental.php?Section=Procedure&Employee_ID='+Employee_ID+'&Registration_ID='+Registration_ID+'&Payment_Cache_ID='+Payment_Cache_ID+'&Sub_Department_ID='+Sub_Department_ID,true);
...
<script type="text/javascript">
    function Confirm_Create_ePayment_Bill(){
        ...
        myObjectConfirm.open('GET','Confirm_ePayment_Patient_Details_Departmental.php?Section=Procedure&Payment_Cache_ID='+Payment_Cache_ID+'&Sub_Department_ID='+Sub_Department_ID,true);
        ...
        if(Amount <= 0 || Amount == null || Amount == '' || Amount == '0'){
            ...
            if(Payment_Mode != null && Payment_Mode != ''){
                if(Payment_Mode == 'Bank_Payment'){
                    ...
                    if (Confirm_Message == true) {
                        document.location = 'Departmental_Bank_Payment_Transaction.php?Section=Procedure&Registration_ID='+Registration_ID+'&Payment_Cache_ID='+Payment_Cache_ID+'&Sub_Department_ID='+Sub_Department_ID+'&Billing_Type='+Billing_Type;
                    }
