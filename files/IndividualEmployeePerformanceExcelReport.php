<?php
   ...
   if($transaction_channel != "All"){
    ...
    $output.= "<tr><td><b>Employee Name : </b>".$Employee_Name."</td></tr>
            <tr><td><b>Transaction Type : </b>".$Transaction_Type."</td></tr>
    	    <tr><td><b>Date From : </b>".$Date_From."</td></tr>
    	    <tr><td><b>Date To : </b>".$Date_To."</td></tr></table>";
?> 
<?php
    $output.="<table width=100%><tr><td colspan=8><hr></td></tr>
		<tr><td width='5%'>Sn</td>
    ...
    while($row = mysqli_fetch_array($select_Filtered_Employees)){
        ...
        $output.= "<td>".$row['Payment_Date_And_Time']."</td>";
        $output.= "<td>".ucwords(strtolower($row['Patient_Name']))."</td>";
        $output.= "<td style='text-align: left;'>".$row['Patient_Payment_ID']."</td>";
        if(((strtolower($row['Billing_Type']) == 'outpatient cash' && $row['Pre_Paid'] == '0' || (strtolower($row['Billing_Type']) == 'patient from outside' && $row['Pre_Paid'] == '0')) or (strtolower($row['Billing_Type']) == 'inpatient cash'  && strtolower($row['payment_type']) == 'pre')) and (strtolower($row['Transaction_status']) == 'active')){
            $output.= "<td style='text-align: right;'>".number_format($row['Total'])."</td>";
    ...
    $output.= "<tr><td colspan=8><hr></td></tr>";
    $output.= "<tr><td colspan=4 style='text-align: right;'><b> Sub Total</b></td>";
    $output.= "<td style='text-align: right;'><b>".number_format($Cash_Total)."</b></td>";
    $output.= "<td style='text-align: right;'><b>".number_format($Credit_Total)."</b></td>";
    $output.= "<td style='text-align: right;'><b>".number_format($Cancelled_Total)."</b></td></tr>";
    $output.= "<tr><td colspan=8><hr></td></tr>";
    $output.= "<tr><td colspan=8 style='text-align: right;'><b>Grand Total ".number_format($Cash_Total + $Credit_Total)."</b></td></tr>";
    $output.= "<tr><td colspan=8><hr></td></tr></table>";
    ...
    if($category_no > 0){
    	...
    	$output.= '<br/><br/><br/><table width="100%">';
    	$output.= '<tr><td width="5%"><b>SN</b></td>';
    	$output.= '<td width="55%"><b>SUB CATEGORY NAME</b></td>';
    	$output.= '<td width="12%" style="text-align: right;"><b>CASH</b></td>';
    	$output.= '<td width="12%" style="text-align: right;"><b>CREDIT</b></td>';
    	$output.= '<td width="15%" style="text-align: right;"><b>CANCELLED</b></td></tr><tr><td colspan=5><hr></td></tr>';
    	while($data = mysqli_fetch_array($get_category)){    	    
               ...
               if($nm > 0){
                    while ($dt = mysqli_fetch_array($get_sub_categories)) {
                        ...
                        $output.= '<tr><td>'.++$temp2.'</td>';
                        $output.= '<td>'.ucwords(strtolower($dt['Item_Subcategory_Name'])).'</td>';
                        $output.= '<td style="text-align: right;">'.number_format($Cash_Total).'</td>';
                        $output.= '<td style="text-align: right;">'.number_format($Credit_Total).'</td>';
                        $output.= '<td style="text-align: right;">'.number_format($Cancelled_Total).'</td></tr>';
                        $Cash_Total = 0;
                        $Credit_Total = 0;
                        $Cancelled_Total = 0;
                    }
	...
	$output.= '<tr><td colspan=2 style="text-align: left;"><b>TOTAL</b></td><td style="text-align: right;"><b>'.number_format($Grand_total_cash).'</b></td><td style="text-align: right;"><b>'.number_format($Grand_total_credit).'</b></td><td style="text-align: right;"><b>'.number_format($Grand_total_cancelled).'</b></td></tr>';
	$output.= '<tr><td colspan=5><hr></td></tr>';
	$output.= '<tr><td colspan=3 style="text-align: left;"><b>GRAND TOTAL</td><td colspan=2  style="text-align: right;"><b>'.number_format($Grand_total_credit + $Grand_total_cash).'</b></td></tr>';
	$output.= '<tr><td colspan=5><hr></td></tr>';
    }
