...
<?php
	...
	if(isset($_SESSION['userinfo'])){
		if($_SESSION['userinfo']['Storage_And_Supply_Work'] == 'yes'){ 
			echo "<a href='Control_Requisition_Sessions.php?New_Requisition=True' class='art-button-green'>NEW REQUISITION</a>";
		}
	}
...
<br/>
<fieldset> <legend style="background-color:#006400;color:white" align='right'><b>Requisition ~ <?php if(isset($_SESSION['Storage'])){ echo $_SESSION['Storage']; }?></b></legend>  
 ...
 <style>
	...
	<table style="margin-top:5" width=100%>
		<tr>
			...
			<td width=5%>
				<?php if(isset($_SESSION['Requisition_ID'])){ ?>
					<input type='text' name='Requisition_Number' size=6 id='Requisition_Number' readonly='readonly' value='<?php echo $_SESSION['Requisition_ID']; ?>'>
				<?php }else{ ?>
		...
		<td id='Store_Issue_Area'>			
			<?php
				if(isset($_SESSION['Requisition_ID']) && $_SESSION['Requisition_ID'] != '' && $_SESSION['Requisition_ID'] != null){
					$Requisition_ID = $_SESSION['Requisition_ID'];
					...
					$select_Supplier = mysqli_query($conn,"select * from tbl_sub_department where Sub_Department_Name <> '$Current_Sub_Department' order by Sub_Department_Name");
...
<script>
	function Get_Item_Name(Item_Name,Item_ID){
		...
		myObjectGetItemName.open('GET','Check_Item_Selected.php?Item_ID='+Item_ID,true);
...
<script>
	function updateStoreIssueMenu2() {
		...
		myRequisitionObject.open('GET','Get_Store_Issued.php',true);
...
<script>
	function updateStoreIssueMenu() {
	   /*	var Requisition_ID = '<?php echo $_SESSION['Requisition_ID']; ?>';
...
<script>
	function updateRequisitionNumber(){
		...
		myObjectUpdateRequisition.open('GET','Update_Requisition_Number.php',true);
...
<script>
	function updateRequisitionDescription(){
		...
		myObjectUpdateDescription.open('GET','Update_Requisition_Description.php',true);
		myObjectUpdateDescription.send();
	}
</script>
<script>
	function updateRequisitionCreatedDate(){
		...
		myObjectUpdateCreatedDate.open('GET','Update_Requisition_Created_Date.php',true);
...
<script>
	function updateSubmitArea(){
		...
		myObjectUpdateSubmitArea.open('GET','Update_Submit_Area.php',true);
...
<script>	
	function Get_Selected_Item(){
		...
		if (Item_ID != '' && Item_ID != null && Quantity != '' && Quantity != null && Quantity!= '' && Store_Issue != '' && Store_Issue != null) {
			...
			my_Object_Get_Selected_Item.open('GET','Requisition_Add_Selected_Item.php?Item_ID='+Item_ID+'&Quantity='+Quantity+'&Item_Remark='+Item_Remark+'&Store_Issue='+Store_Issue+'&Store_Need='+Store_Need+'&Requisition_Description='+Requisition_Description+'&Cont_Quantity='+Cont_Quantity+'&Items_Quantity='+Items_Quantity,true);
...
<script>
	function Confirm_Remove_Item(Item_Name,Requisition_Item_ID){ 
		...
		if (Confirm_Message == true) {
			...
			My_Object_Remove_Item.open('GET','Requisition_Remove_Item_From_List.php?Requisition_Item_ID='+Requisition_Item_ID,true);
				...
				<table width=100%>
							...
							<fieldset style='overflow-y: scroll; height: 299px; background-color:silver' id='Items_Fieldset_List'>
								<?php
									...
									echo '<tr><td width=4% style="text-align: center; background-color:silver;color:black">Sn</td>
											...
											<td width=7% style="text-align: center; background-color:silver;color:black">Containers</td>
					...
					<script type='text/javascript'>
						function Submit_Requisition_Function(Requisition_ID){				
						    document.location = 'Submit_Requisition.php?Requisition_ID='+Requisition_ID;
						}
					</script>
					<script>
						function Confirm_Submit_Requisition(){
							...
							myObjectCheckItemNumber.open('GET','Requisition_Check_Number_Of_Items.php',true);
