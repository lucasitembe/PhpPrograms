...
if (isset($_SESSION['userinfo'])) {
    if ($_SESSION['userinfo']['Revenue_Center_Works'] == 'yes') {
        if (isset($_GET['location']) && $_GET['location'] == 'otherdepartment') {
            ...
            echo "<a href='patient_direct_item_payment.php?DepartmentPatientBilling=DepartmentPatientBillingThisPage' class='art-button-green'>BACK</a>";
        }
    }
}
?>
...
<fieldset style='overflow-y: scroll; height: 400px;background-color:white;margin-top:20px;' id='Patient_Fieldset_List'>
    ...
    <table width=100% border=1>
        ...
        <?php
        //select patients
        $select = mysqli_query($conn,"SELECT pr.Registration_ID, pr.Patient_Name, pr.Gender, pr.Date_Of_Birth, sp.Guarantor_Name, pr.Phone_Number, cid.Check_In_ID, pr.Member_Number, ad.Admision_ID from
...
<script type="text/javascript">
    function Search_Patients() {
        ...
        myObjectSearchPatient.open('GET', 'Inpatient_search_all_patient.php?<?php echo $location; ?>Sponsor_ID=' + Sponsor_ID, true);
...
<script type="text/javascript">
    function Search_Patients_Via_Name() {
        ...
        myObjectSearchPatient.open('GET', 'Inpatient_search_all_patient.php?<?php echo $location; ?>Sponsor_ID=' + Sponsor_ID + '&Patient_Name=' + Patient_Name, true);
        myObjectSearchPatient.send();
    }
</script>
...
<script type="text/javascript">
    function Search_Patients_Via_Number() {
        ...
        myObjectSearchPatient.open('GET', 'Inpatient_search_all_patient.php?<?php echo $location; ?>Sponsor_ID=' + Sponsor_ID + '&Patient_Number=' + Patient_Number, true);
        myObjectSearchPatient.send();
    }
</script>
