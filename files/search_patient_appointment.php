...
//}
    ...
    $clinic=  mysqli_real_escape_string($_POST['clinic']);
    $doctor=  mysqli_real_escape_string($_POST['doctor']);
    $regID=  mysqli_real_escape_string($_POST['regID']);
    $PatientName=  mysqli_real_escape_string($_POST['PatientName']);
...
//    regID:regID,PatientName:PatientName
    ...
    $numberSpecimen="SELECT ap.doctor,ap.Set_BY,ap.date_time,emp.Employee_Name,reg.Patient_Name,ap.appointment_reason,cl.Clinic_Name,ap.appointment_id FROM tbl_appointment ap,tbl_employee emp,tbl_patient_registration reg,tbl_clinic cl WHERE ap.patient_No=reg.Registration_ID AND (reg.Patient_Name LIKE '%$PatientName%' OR reg.Registration_ID LIKE '%$regID%') AND ap.doctor=emp.Employee_ID $filterdoctor AND ap.Clinic=cl.Clinic_ID $filterclinic AND ap.Status='1' AND date_time BETWEEN '$fromDate' AND '$toDate' LIMIT 200  ";     
   echo "inafika";
