        ...
        <b>
            <?php 
                if(strtolower($B_Type) == 'inpatient cash'){
                    echo strtoupper('Laboratory Payments ~ inpatient cash'); 
                }else{
                    echo strtoupper('Laboratory Payments ~ outpatient cash'); 
                }
            ?> 
        </b>
    </legend>
<?php
        ...
        echo "<tr><td colspan='10'><hr></tr>";
            $qr="
SELECT pc.Registration_ID, pc.Transaction_Status AS General_Transaction_Status, pc.Payment_Cache_ID,preg.Patient_Name, pc.Sponsor_Name, preg.Date_Of_Birth, preg.Gender, preg.Phone_Number, preg.Member_Number FROM tbl_payment_cache pc, tbl_patient_registration preg WHERE preg.registration_id = pc.registration_id AND (pc.billing_type = 'Outpatient Cash' OR pc.Billing_Type = 'Outpatient Credit') AND pc.Payment_Date_And_Time BETWEEN '$Start_Date' AND '$End_Date' ORDER BY pc.Payment_Date_And_Time DESC ";


        $select_Filtered_Patients = mysqli_query($conn,$qr) or die(mysqli_error($conn));
        //die('tis '.mysqli_num_rows($select_Filtered_Patients));
        while($row = mysqli_fetch_array($select_Filtered_Patients)){
            $Payment_Cache_ID=$row['Payment_Cache_ID'];
            $select2=mysqli_query($conn,"SELECT ilc.Transaction_Date_And_Time, ilc.Transaction_Type, ilc.status, sd.Sub_Department_Name,ilc.Sub_Department_ID FROM tbl_item_list_cache ilc, tbl_sub_department sd WHERE sd.sub_department_id = ilc.sub_department_id AND ilc.payment_cache_id=$Payment_Cache_ID AND ilc.Transaction_Type = 'Cash' AND ilc.status = 'active' AND ilc.Check_In_Type = 'Laboratory' GROUP BY ilc.payment_cache_id,ilc.sub_department_id order by ilc.payment_cache_id desc limit 100")or die(mysqli_error($conn));
        while($row2=mysqli_fetch_assoc($select2)){
            $Sub_Department_ID=$row2['Sub_Department_ID'];
            echo "<tr><td id='thead' style='width:5%;' >".$temp."</td>";
        if(strtolower($row2['status']) == 'active'){
        ...
        echo "<td><a href='patientbillinglaboratory.php?section=Laboratory&Registration_ID=".$row['Registration_ID']."&Transaction_Type=".$row2['Transaction_Type']."&Payment_Cache_ID=".$row['Payment_Cache_ID']."&NR=True&Billing_Type=".$Billing_Type2."&Sub_Department_ID=".$Sub_Department_ID."&LaboratoryWorks=LaboratoryWorksThisPage' target='_parent' style='text-decoration: none;'>".$row2['Sub_Department_Name']."</a></td>";     
        echo "<td><a href='patientbillinglaboratory.php?section=Laboratory&Registration_ID=".$row['Registration_ID']."&Transaction_Type=".$row2['Transaction_Type']."&Payment_Cache_ID=".$row['Payment_Cache_ID']."&NR=True&Billing_Type=".$Billing_Type2."&Sub_Department_ID=".$Sub_Department_ID."&LaboratoryWorks=LaboratoryWorksThisPage' target='_parent' style='text-decoration: none;'>".$row2['Transaction_Date_And_Time']."</a></td>";     
...
<script>
    function Patient_List_Search(){
        ...
        myObjectSearchPatient.open('GET','Revenue_Center_Laboratory_List_Iframe.php?Patient_Name='+Patient_Name+'&Billing_Type='+Billing_Type+'&Patient_Type='+Patient_Type+'&Start_Date='+Start_Date+'&End_Date='+End_Date,true);
...
<script type="text/javascript">
    function Patient_List_Search_Via_Number(){
        ...
        myObjectSearchP.open('GET','Revenue_Center_Laboratory_List_Iframe.php?Patient_Number='+Patient_Number+'&Billing_Type='+Billing_Type+'&Patient_Type='+Patient_Type+'&Start_Date='+Start_Date+'&End_Date='+End_Date,true);
