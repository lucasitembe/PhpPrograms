...
if (isset($_GET['Section'])) {
    ...
    $Section_Link2 = "Session=" . $_GET['Section'] . "&";
...
if ($Sponsor_ID != '' && $Sponsor_Name != '') {
    ...
    $select_items = mysqli_query($conn,"select ilc.Quantity, ilc.Edited_Quantity, ilc.Price, ilc.Discount, ilc.Payment_Item_Cache_List_ID, ilc.Check_In_Type, Consultant, 
										ilc.Payment_Item_Cache_List_ID, ilc.ePayment_Status, ilc.Item_ID, ilc.Consultant_ID,Billing_Type,payment_type
    ...
    if ($no > 0) {
        $row3 = mysqli_fetch_array($select_items);
        $billing_Type = $row3['Billing_Type'];
        //generate receipt
        ...
        if ($create_receipt) {
            ...
            if ($num_rec > 0) {
                ...
                while ($rm = mysqli_fetch_array($select_items)) {
                    ...
                    if ($insert_item) {
                        $update = mysqli_query($conn,"update tbl_item_list_cache set Status = 'served', 
...
//                exit();
        ...
        header("Location: ./previewapprovedtransactionothers.php?{$Section_Link2}Section=Others&Patient_Payment_ID=$Patient_Payment_ID&Registration_ID=$Registration_ID&PreviewApprovedTransaction=PreviewApprovedTransactionThisPage&Billing_Type=$Billing_Type");
    } else {
        header("Location: ./approveothers.php?{$Section_Link}Registration_ID=$Registration_ID&Payment_Cache_ID=$Payment_Cache_ID&ApproveInvestigation=ApproveInvestigationThisPage");
    }
} else {
    header("Location: ./approveothers.php?{$Section_Link}Registration_ID=$Registration_ID&Payment_Cache_ID=$Payment_Cache_ID&ApproveInvestigation=ApproveInvestigationThisPage");
}
