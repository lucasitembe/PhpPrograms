...
<br/>
<fieldset style='overflow-y: scroll; height: 400px;' id='Previous_Fieldset_List'>
	...
	<?php 
	    ...
	    $sql_select = mysqli_query($conn,"select emp.Employee_Name, rq.Requisition_Description, rq.Requisition_ID, rq.Created_Date_Time, rq.Store_Issue, rq.Store_Need, rq.Sent_Date_Time, sd.Sub_Department_Name from
									tbl_requisition rq, tbl_sub_department sd, tbl_employee emp, tbl_requisition_items ri where
									rq.store_issue = sd.sub_department_id and 
									rq.requisition_status = 'submitted' and
									ri.Requisition_ID = rq.Requisition_ID and
									ri.Procurement_Status = 'active' and
									emp.Employee_ID = rq.Employee_ID and
									rq.Store_Issue = '$Sub_Department_ID' group by rq.Requisition_ID order by rq.Requisition_ID desc") or die(mysqli_error($conn));
...
<script>
	...
	if (Start_Date != null && Start_Date != '' && End_Date != null && End_Date != '') {
	    ...
	    myObjectGetPrevious.open('GET','Get_Requisitions_List.php?Start_Date='+Start_Date+'&End_Date='+End_Date,true);
