...
<?php
	...
	$check = mysqli_query($conn,"select Requisition_ID from tbl_reagents_requisition where Requisition_Status = 'pending' and requisition_id = '$Requisition_ID'") or die(mysqli_error($conn));
	$num_rows = mysqli_num_rows($check);
	
	if($num_rows > 0 ){
	    //delete selected record
	    $delete_details = mysqli_query($conn,"delete from tbl_reagents_requisition_items where Requisition_Item_ID = '$Requisition_Item_ID'") or die(mysqli_error($conn));        
	}else{
	    $delete_details = True;
	}
        if($delete_details){
