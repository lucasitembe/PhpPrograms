...
//    echo $_SESSION['Sub_Department_ID'];
//    exit;
...
//        exit;
	...
	if($no > 0){
		...
		$data.= "<td><input type='hidden' name='item_list_id[]' value='".$row['Payment_Item_Cache_List_ID']."' /><input type='text' name='discount[]' style='text-align:right;' value='".number_format($row['Discount'])."'></td>";
		...
		from tbl_item_list_cache ilc, tbl_Items its
			...
			ilc.Payment_Cache_ID = '$Payment_Cache_ID'  and
                                ilc.Transaction_Type = '$Transaction_Type' and
                                        ilc.Check_In_Type = 'Pharmacy'"); 
//
        $sql="UPDATE tbl_item_list_cache SET status='approved' WHERE Payment_Item_Cache_List_ID='".$id."'";
        mysqli_query($conn,$sql) or die(mysqli_error($conn));
