...
$htm .= '<br/><table width="100%" border=1 style="border-collapse: collapse;">
				<thead><tr>
					...
					<td style="text-align: right" width="12%"><span style="font-size: x-small;"><b>TOTAL CASH</b></span></td>
...
$get_categories = mysqli_query($conn,"select ic.Item_Category_ID, ic.Item_Category_Name from
    ...
    if ($num_cat > 0) {
        ...
        //get quantity, cash & credit transactions
        $get_Quantiry = mysqli_query($conn,"select ppl.Quantity, ppl.Price, ppl.Discount, pp.Billing_Type, pp.payment_type, pp.Fast_Track  from
        ...
        if ($_SESSION['systeminfo']['price_precision'] == 'yes') {
            $htm .= '<span style="font-size: x-small;">' . number_format($Total_Cash, 2) . '</span>';
        } else {
            $htm .= '<span style="font-size: x-small;">' . number_format($Total_Cash) . '</span>';
        }
        $htm .= "</td>";
