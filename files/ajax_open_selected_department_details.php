...
<?php
            ...
            $sql_select_list_of_department_that_has_value_result=mysqli_query($conn,"SELECT Department_Name,Department_ID FROM tbl_department WHERE Department_Status='active' AND Department_ID IN(SELECT Department_ID FROM tbl_sub_department WHERE Sub_Department_ID IN(SELECT Sub_Department_ID FROM tbl_patient_payment_item_list ppl WHERE Transaction_Date_And_Time BETWEEN '$start_date' AND '$end_date' AND finance_department_id='$finance_department_id' GROUP BY Sub_Department_ID) GROUP BY Department_ID)") or die(mysqli_error($conn));
