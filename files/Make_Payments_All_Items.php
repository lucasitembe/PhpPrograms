<?php
    @session_start();
    include("./includes/connection.php");
    $temp = 1;
    if(!isset($_SESSION['userinfo'])){
		@session_destroy();
		header("Location: ../index.php?InvalidPrivilege=yes");
    }

	if(isset($_GET['Payment_Cache_ID'])){
		$Payment_Cache_ID = $_GET['Payment_Cache_ID'];
	}else{
		$Payment_Cache_ID = '';
        }
            if(isset($_GET['auth_code'])){
                    $auth_code=$_GET['auth_code'];
            }else{
                    $auth_code="";
            }
            if(isset($_GET['terminal_id'])){
                    $terminal_id=$_GET['terminal_id'];
            }else{
                    $terminal_id="";
            }
            $manual_offline=$_GET['manual_offline'];
    if(isset($_SESSION['userinfo'])){
		if(isset($_SESSION['userinfo']['Revenue_Center_Works'])){
		    if($_SESSION['userinfo']['Revenue_Center_Works'] != 'yes'){
			header("Location: ./index.php?InvalidPrivilege=yes");
		    } else{
				@session_start();
				if(!isset($_SESSION['supervisor'])){
		                
		                    //get patient registration id for future use
		                    if(isset($_GET['Registration_ID'])){
		                        $Registration_ID = $_GET['Registration_ID'];
		                    }else{
		                        $Registration_ID = '';
		                    }
				    header("Location: ./receptionsupervisorauthentication.php?Registration_ID=$Registration_ID&InvalidSupervisorAuthentication=yes");
				}
		    } 
		}else{
		    header("Location: ./index.php?InvalidPrivilege=yes");
		}
    }else{
		@session_destroy();
	    header("Location: ../index.php?InvalidPrivilege=yes");
    }
    
    //get employee id
    if(isset($_SESSION['userinfo']['Employee_ID'])){
        $Employee_ID = $_SESSION['userinfo']['Employee_ID'];
    }else{
        $Employee_ID = 0;
    }
    
    //---get supervisor id 
        if(isset($_SESSION['supervisor'])) {
            if(isset($_SESSION['supervisor']['Session_Master_Priveleges'])){
                if($_SESSION['supervisor']['Session_Master_Priveleges'] = 'yes'){
                    $Supervisor_ID = $_SESSION['supervisor']['Employee_ID'];
                }else{
                    $Supervisor_ID = '';
                }
            }else{
                 $Supervisor_ID = '';
            }
        }else{
                 $Supervisor_ID = '';
        }
    //end of fetching supervisor id
    
    //get the folio number
    $get_folio = mysqli_query($conn,"select Folio_Number, Registration_ID from tbl_payment_cache where Payment_Cache_ID = '$Payment_Cache_ID'") or die(mysqli_error($conn));
    $fNumber = mysqli_num_rows($get_folio);
    
    if($fNumber > 0){
        while($Folio = mysqli_fetch_array($get_folio)){
            $Folio_Number = $Folio['Folio_Number'];
            $Registration_ID = $Folio['Registration_ID'];
        }
    }else{
        $Folio_Number = '';
        $Registration_ID = 0;
    }

    //get the last check in
    $select = mysqli_query($conn,"select Check_In_ID from tbl_check_in where Registration_ID = '$Registration_ID' order by Check_In_ID desc limit 1") or die(mysqli_error($conn));
    $num = mysqli_num_rows($select);
    if($num > 0){
        while ($data = mysqli_fetch_array($select)) {
            $Check_In_ID = $data['Check_In_ID'];
        }
    }else{
        $Check_In_ID = 0;
    }

    //get branch id
    if(isset($_SESSION['userinfo']['Branch_ID'])){
        $Branch_ID = $_SESSION['userinfo']['Branch_ID'];
    }else{
        $Branch_ID = 0;
    }
    
    if($Employee_ID != 0 && $Registration_ID != 0 && $Branch_ID != 0 && $Folio_Number != ''){
        //select billing type, Sponsor Name, Sponsor ID
        $select_details = mysqli_query($conn,"select Billing_Type, Sponsor_Name, Sponsor_ID from tbl_payment_cache
                                        where Payment_Cache_ID = '$Payment_Cache_ID'") or die(mysqli_error($conn));
        $num = mysqli_num_rows($select_details);
        if($num > 0){
            while($row = mysqli_fetch_array($select_details)){
                $Billing_Type = $row['Billing_Type'];
                $Sponsor_Name = $row['Sponsor_Name'];
                $Sponsor_ID = $row['Sponsor_ID'];
            }
        }else{
            $Billing_Type = '';
            $Sponsor_Name = '';
            $Sponsor_ID = 0;
        }


        include("./includes/Get_Patient_Transaction_Number.php");

        if($Billing_Type != '' && $Sponsor_Name != '' && $Sponsor_ID != 0){
            //insert data into tbl_patient_payments
            $Insert_Data_To_tbl_patient_payments = "insert into tbl_patient_payments(
                                registration_id,supervisor_id,employee_id,
                                payment_date_and_time,folio_number,Check_In_ID,
                                sponsor_id,sponsor_name,billing_type,receipt_date,branch_id,Patient_Bill_ID,terminal_id,auth_code,manual_offline)
                            values(
                                '$Registration_ID','$Supervisor_ID','$Employee_ID',
                                    (select now()),'$Folio_Number','$Check_In_ID',
                                        '$Sponsor_ID','$Sponsor_Name','$Billing_Type',(select now()),'$Branch_ID','$Patient_Bill_ID','$terminal_id','$auth_code','$manual_offline')";
                                        
            $result = mysqli_query($conn,$Insert_Data_To_tbl_patient_payments) or die(mysqli_error($conn));
            //get receipt number to use as a foreign key
        
            if($result){
                $select_Patient_Payment = mysqli_query($conn,"select Patient_Payment_ID, Payment_Date_And_Time from tbl_patient_payments
                                                        where Registration_ID = '$Registration_ID' and
                                                            Employee_ID = '$Employee_ID' and
                                                                Supervisor_ID = '$Supervisor_ID' order by Patient_Payment_ID desc limit 1") or die(mysqli_error($conn));
                $num_rows = mysqli_num_rows($select_Patient_Payment);
                if($num_rows > 0 ){
                    while($row = mysqli_fetch_array($select_Patient_Payment)){
                        $Patient_Payment_ID =  $row['Patient_Payment_ID'];
                        $Payment_Date_And_Time = $row['Payment_Date_And_Time'];
                    }
                    //get all items from
                    $select_items = mysqli_query($conn,"select ilc.Sub_Department_ID,ilc.Item_ID, ilc.Quantity, ilc.Edited_Quantity,ilc.Clinic_ID, ilc.Price, ilc.Discount, ilc.Check_In_Type, ilc.Patient_Direction,
                    								ilc.Edited_Quantity, ilc.Consultant, ilc.Consultant_ID, ilc.Payment_Item_Cache_List_ID
					                                from tbl_payment_cache pc, tbl_item_list_cache ilc where
					                                ilc.Payment_Cache_ID = pc.Payment_Cache_ID and
					                                (ilc.Status = 'active' or ilc.Status = 'approved') and
					                                ilc.Transaction_Type = 'Cash' and
					                                pc.Payment_Cache_ID = '$Payment_Cache_ID' and
					                                pc.Billing_Type = 'Outpatient Cash' and
					                                ilc.ePayment_Status = 'pending' order by ilc.Check_In_Type") or die(mysqli_error($conn));
                    
                    $num_details = mysqli_num_rows($select_items);
                    $Qty = 0;
                    if($num_details > 0){
                        while($row = mysqli_fetch_array($select_items)){
                            
                            //generate quantity
                        	if($row['Edited_Quantity'] > 0){
                        		$Qty = $row['Edited_Quantity'];
                        	}else{
                        		$Qty = $row['Quantity'];
                        	}

                            //get items data                            
                            $Check_In_Type = $row['Check_In_Type'];
                            $Item_ID = $row['Item_ID'];
                            $Discount = $row['Discount'];
                            $Price = $row['Price'];
                            $Quantity = $row['Quantity'];
                            $Consultant = $row['Consultant'];
                            $Consultant_ID = $row['Consultant_ID'];
                            $Payment_Item_Cache_List_ID = $row['Payment_Item_Cache_List_ID'];
                            $Clinic_ID = $row['Clinic_ID'];
                            $Sub_Department_ID = $row['Sub_Department_ID'];
                            
                            //insert selected item data    
                                $Insert_Data_To_tbl_patient_payment_item_list = "insert into tbl_patient_payment_item_list(
                                    check_In_type,item_id,discount,
                                        price,quantity,patient_direction,
                                            consultant,consultant_id,patient_payment_id,Transaction_Date_And_Time,ItemOrigin,Clinic_ID,Sub_Department_ID)
                                    values(
                                        '$Check_In_Type','$Item_ID','$Discount',
                                            '$Price','$Quantity','Others',
                                                '$Consultant','$Consultant_ID','$Patient_Payment_ID',(select now()),'Doctor','$Clinic_ID','$Sub_Department_ID')";
                            
                            $result = mysqli_query($conn,$Insert_Data_To_tbl_patient_payment_item_list) or die(mysqli_error($conn));
                            if($result){
                            	mysqli_query($conn,"update tbl_item_list_cache set 
                            					Status = 'paid', 
                            					Payment_Date_And_Time = '$Payment_Date_And_Time', 
                            					Patient_Payment_ID = '$Patient_Payment_ID' where
					    						Payment_Item_Cache_List_ID = '$Payment_Item_Cache_List_ID'") or die(mysqli_error($conn));
                            }

                        }
                        header("Location: ./patientbillingreview.php?Registration_ID=$Registration_ID&Patient_Payment_ID=$Patient_Payment_ID&PatientBilling=PatientBillingThisPage");
                    }
                }
            }
        }
        
    }
    
?>