<?php
	...
	if(isset($_GET['Sub_Department_ID']) && isset($_GET['Sub_Department_Type'])){
		$select = mysqli_query($conn,"select i.Item_ID from tbl_items i where i.Item_Type = 'Pharmacy' or i.Item_Type = 'Others'") or die(mysqli_error($conn));
		while($row = mysqli_fetch_array($select)){
			$Item_ID = $row['Item_ID'];
			//check if selected item is available (Based on sub department id submitted)
			$select_details = mysqli_query($conn,"select Item_ID from tbl_items_balance where
							Item_ID = '$Item_ID' and
			...
			if($num == 0){
				...
				$insert = mysqli_query($conn,"insert into tbl_items_balance(Item_ID,Sub_Department_ID,Sub_Department_Type)
