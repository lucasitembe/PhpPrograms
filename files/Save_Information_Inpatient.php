<?php
	...
	$insert = mysqli_query($conn,"INSERT into tbl_patient_payments(
							...
							Billing_Type, Receipt_Date, Branch_ID, Patient_Bill_ID) 
			...
			if($numb > 0){
				while ($rw = mysqli_fetch_array($slct)) {
					...
					$Clinic_ID = $rw['Clinic_ID'];
                                        $clinic_location_id = $rw['clinic_location_id'];
                                        $working_department = $rw['working_department'];
					$Hospital_Ward_ID = $rw['Hospital_Ward_ID'];

					$insert2 = mysqli_query($conn,"INSERT into tbl_patient_payment_item_list(
												Check_In_Type, Item_ID, Discount, Price, Quantity, 
												Patient_Direction, Consultant, Consultant_ID, Patient_Payment_ID, Transaction_Date_And_Time,Sub_Department_ID,Hospital_Ward_ID,finance_department_id,clinic_location_id) 

											VALUES ('$Check_In_Type','$Item_ID','$Discount','$Price','$Quantity',
												'others','$Employee_Name','$Employee_ID','$Patient_Payment_ID',(select now()),'$Sub_Department_ID','$Hospital_Ward_ID','$working_department','$clinic_location_id')") or die(mysqli_error($conn));
					if($insert2){
