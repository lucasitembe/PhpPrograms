...
<?php
    ...
    if(isset($_SESSION['userinfo'])){
        if($_SESSION['userinfo']['Pharmacy'] == 'yes'){
                echo "<a href='Laboratory_Control_Requisition_Sessions.php?New_Requisition=True' class='art-button-green'>NEW REQUISITION</a>";
        }
    }

    if(isset($_SESSION['userinfo'])){
        if($_SESSION['userinfo']['Pharmacy'] == 'yes'){
            echo "<a href='laboratorypendingrequisitions.php?PendingRequisitions=PendingRequisitionsThisPage' class='art-button-green'>PENDING REQUISITIONS</a>";
        }
    }

    if(isset($_SESSION['userinfo'])){
        if($_SESSION['userinfo']['Pharmacy'] == 'yes'){
            echo "<a href='laboratorypreviousrequisitions.php?PreviousRequisitions=PreviousRequisitionsThisPage' class='art-button-green'>PREVIOUS REQUISITIONS</a>";
        }
    }

    if(isset($_SESSION['userinfo'])){
        if($_SESSION['userinfo']['Pharmacy'] == 'yes'){
            echo "<a href='laboratory.php?LaboratoryWorkPage=LaboratoryWorkPageThisPage' class='art-button-green'>BACK</a>";
        }
    }
?>
...
<br/>
<fieldset> <legend align='right'><b>Requisition ~ <?php if(isset($_SESSION['Laboratory_ID'])){ echo $Sub_Department_Name; } ?></b></legend>
        <table width=100%>
		<tr>
                    <td width='12%' style='text-align: right;'>Requisition Number</td>
                    <td width=5%>
                        <?php if(isset($_SESSION['Laboratory_Requisition_ID'])){ ?>
                                <input type='text' name='Requisition_Number' size=6 id='Requisition_Number' readonly='readonly' value='<?php echo $_SESSION['Laboratory_Requisition_ID']; ?>'>
                        <?php }else{ ?>
		...
		<tr>
			...
			<td width='16%'>
			    ...
			    <?php
				if(isset($_SESSION['Laboratory_ID'])){
					echo '<option selected="selected">'.$Sub_Department_Name.'</option>';
				}else{
			...
			<td style='text-align: right;'>Department Issueing</td>
			<td id='Store_Issue_Area'>
			<?php
				if(isset($_SESSION['Laboratory_Requisition_ID']) && $_SESSION['Laboratory_Requisition_ID'] != '' && $_SESSION['Laboratory_Requisition_ID'] != null){
                                    $Requisition_ID = $_SESSION['Laboratory_Requisition_ID'];
					...
					$Current_Sub_Department = $_SESSION['Laboratory_ID'];
			...
			<td style='text-align: right;'>Requisition Personnel</td>
			<td>
...
<script>
	function Get_Item_Name(Item_Name,Item_ID){
		...
		myObjectGetItemName.open('GET','Laboratory_Check_Item_Selected.php?Item_ID='+Item_ID,true);
...
<script>
	function Get_Balance(){
		...
		myObjectGetBalance.open('GET','Laboratory_Get_Item_Expected_Balance.php?Item_ID='+Item_ID+'&ControlValue=Laboratory',true);
...
<script>
	function updateStoreIssueMenu2() {
		...
		myRequisitionObject.open('GET','Laboratory2_Get_Store_Issued.php',true);
...
<script>
	function updateRequisitionNumber(){
		...
		myObjectUpdateRequisition.open('GET','Laboratory_Update_Requisition_Number.php',true);
...
<script>
	function updateRequisitionDesc(){
		...
		myObjectUpdateDescription.open('GET','Laboratory_Change_Requisition_Description.php?Requisition_Description='+Requisition_Description,true);
...
<script>
	function Update_Item_Remark(Requisition_Item_ID,Item_Remark){
		...
		myObjectUpdateItemRemark.open('GET','Laboratory_Update_Item_Remark.php?Requisition_Item_ID='+Requisition_Item_ID+'&Item_Remark='+Item_Remark,true);
...
<script>
	function updateRequisitionDescription(){
		...
		myObjectUpdateDescription.open('GET','Laboratory_Update_Requisition_Description.php',true);
		myObjectUpdateDescription.send();
	}
</script>
<script>
	function updateRequisitionCreatedDate(){
		...
		myObjectUpdateCreatedDate.open('GET','Laboratory_Update_Requisition_Created_Date.php',true);
...
<script>
	function updateSubmitArea(){
		...
		myObjectUpdateSubmitArea.open('GET','Laboratory_Update_Submit_Area.php',true);
...
<script>
	function Get_Selected_Item(){
		...
		if (Item_ID != '' && Item_ID != null && Quantity != '' && Quantity != null && Quantity!= '' && Store_Issue != '' && Store_Issue != null) {
			...
			my_Object_Get_Selected_Item.open('GET','Laboratory_Requisition_Add_Selected_Item.php?Item_ID='+Item_ID+'&Quantity='+Quantity+'&Item_Remark='+Item_Remark+'&Store_Issue='+Store_Issue+'&Store_Need='+Store_Need+'&Requisition_Description='+Requisition_Description,true);
...
<script>
	function Confirm_Remove_Item(Item_Name,Requisition_Item_ID){
		...
		if (Confirm_Message == true) {
			...
			My_Object_Remove_Item.open('GET','Laboratory_Requisition_Remove_Item_From_List.php?Requisition_Item_ID='+Requisition_Item_ID,true);
				...
				<table width=100%>
					...
					<tr>
						<td>
							...
							<table width=100%>
							    <?php
								$result = mysqli_query($conn,"SELECT * FROM tbl_items where Item_Type = 'Pharmacy' order by Product_Name limit 100");
				...
				<table width=100%>
					...
					<script type='text/javascript'>
						function Submit_Requisition_Function(Requisition_ID){
						    document.location = 'Laboratory_Submit_Requisition.php?Requisition_ID='+Requisition_ID+'?&Laboratory=True';
						}
					</script>
					<script>
						function Confirm_Submit_Requisition(){
							...
							myObjectCheckItemNumber.open('GET','Requisition_Check_Number_Of_Items.php?Laboratory=True',true);
