...
<?php 
    ...
    $select_exempted_patient = mysqli_query($conn, "SELECT next_kin_phoneNo, tef.Nurse_Exemption_ID,Anaombewa, Patient_Bill_ID,exemptionstatus, payment_method,Patient_Name,pr.Gender,pr.Phone_Number, tef.Registration_ID,kiasikinachoombewamshamaha,  Employee_Name,Employee_Title, tef.Exemption_ID, tef.Employee_ID,  tef.created_at FROM tbl_employee e, tbl_temporary_exemption_form tef, tbl_sponsor sp, tbl_patient_registration pr, tbl_nurse_exemption_form nef WHERE  nef.Nurse_Exemption_ID=tef.Nurse_Exemption_ID AND  tef.Employee_ID=e.Employee_ID AND tef.Registration_ID=pr.Registration_ID AND  pr.Sponsor_ID = sp.Sponsor_ID  GROUP BY created_at ORDER BY  Exemption_ID DESC LIMIT 50 ") or die(mysqli_error($conn));
...
<fieldset>
    <div class="col-md-offset-3 col-md-6 col-md-offset-3"> 
        <input type="text" id="search_value" onkeyup="seach_exempted_patient()" class="form-control text-center" placeholder="~~~~Search by REG NO. or NAME~~~~">
    </div>
</fieldset>
<fieldset style='height:450px; overflow-y: scroll;'>
    <legend align=center>LIST OF PATIENT EXEMPTED</legend>    
<table class="table" style="height:700px; overflow-y: scroll;">
    <thead style="background-color: #CCC;">
        ...
        <th width="7%">STATUS(DEBIT/EXEMPTION)</th>
        <th width="10%">CREATED AT </th>
        <th width="17%">ACTION </th>
    </thead>
    <tbody id='search_responce'>
        ...
        <?php
            ...
            while($row = mysqli_fetch_assoc($select_exempted_patient)){
                ...
                $next_kin_phoneNo =$row['next_kin_phoneNo'];
                //$admin_date = '';  
                    $count_rw++;
    ...
    //mwisho ho ya gharama za hospital 
        ...
        ?>
                <tr> 
                    ...
                    <td><?php echo $Phone_Number ." / ".$next_kin_phoneNo; ?></td>                    
                    ...
                    <td><?php echo $Debt_status ." / ".$exemptionstatus; ?></td>
                ...
                ?>
                <tr style="background: #BDB5AC; height:50px;" >
                    <th colspan="5">TOTAL AMOUNT</th>
                    <th><?php echo number_format($totol_bill, 2); ?></th>
                    <th><?php echo number_format($total_Amount_paid, 2) ?></th>
                    <th><?php echo number_format($Total_deficity, 2) ?></th>
                    <th><?php echo number_format($Totalkiasikilichoombewa, 2); ?></th>
                    <th colspan="8"></th>
                </tr>
...
<script>
     function cancel_excemption(Exemption_ID){
         alert(Exemption_ID)
        if(confirm("Are you sure you want to cancel this exemption")){
            $.ajax({
                type:'POST',
                url:'Search_patient_exempted.php',
                data:{Exemption_ID:Exemption_ID, cancelexemption:''},
     ...
     function seach_exempted_patient(){
