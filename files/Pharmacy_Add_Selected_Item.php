...
if ($Employee_ID != 0 && $Registration_ID != 0 && $Item_ID != 0 && $Quantity != '' && $Employee_ID != 0 && $Billing_Type != '') {
    ...
    $select = "select * from tbl_pharmacy_items_list_cache
    ...
    if ($no > 0) {
        $delete_details = mysqli_query($conn,"delete from tbl_pharmacy_items_list_cache where Employee_ID = '$Employee_ID'") or die(mysqli_error($conn));
    }
    ...
    $insert_data = mysqli_query($conn,"INSERT INTO tbl_pharmacy_items_list_cache(
                                Claim_Form_Number, Billing_Type, Sponsor_Name,
                                Sponsor_ID, Item_ID, Price, Quantity, Discount,
                                Consultant_ID, Employee_ID, Registration_ID,Dosage,Fast_Track,Clinic_ID,clinic_location_id,finance_department_id)

                            values('$Claim_Form_Number','$Billing_Type','$Guarantor_Name',
                                '$Sponsor_ID_Sel','$Item_ID','$Price','$Quantity','$Discount',
                                '$Consultant_ID','$Employee_ID','$Registration_ID','$Dosage','$Fast_Track','$Clinic_ID','$clinic_location_id','$finance_department_id')") or die(mysqli_error($conn));
}
?>
...
$select_Transaction_Items = mysqli_query($conn,
        "select Item_Cache_ID, Product_Name, Price, Quantity, Discount, Registration_ID,Dosage
