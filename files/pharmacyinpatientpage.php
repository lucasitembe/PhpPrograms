...
if (isset($_SESSION['userinfo'])) {
    // if ($_SESSION['userinfo']['Pharmacy'] == 'yes') {
        if(isset($_GET['from_billing_work'])){
            $Check_In_ID=$_GET['Check_In_ID'];
            $from_billing_work="from_billing_work";
            ...
            $select_admision_ID = mysqli_query($conn, "SELECT Admision_ID from tbl_admission WHERE Registration_ID='$Registration_ID' AND Admission_Status IN('Admitted','pending') AND (Credit_Bill_Status = 'pending' or Cash_Bill_Status='pending') ORDER BY Admision_ID DESC LIMIT 1");
                        ...
                        if($admission_num > 0){
           ?>
               <a href='previewpatientbilldetails.php?Check_In_ID=<?= $Check_In_ID ?>&PreviewPatientBillDetails=PreviewPatientBillDetailsThisPage' class='art-button-green'>
                    BACK
               </a>
               <?php 
                        }else{
                            ?>                    
                    <a href='pendingbill.php?Check_In_ID=<?= $Check_In_ID ?>&Registration_ID=<?= $Registration_ID ?>PreviewPatientBillDetails=PreviewPatientBillDetailsThisPage' class='art-button-green'>
                    BACK
               </a>
                        <?php 
                        }
        }else if($nursecommunication =='fromnursecommunication'){
        ?>
        <a href='nursecommunicationpage.php?Registration_ID=<?php echo $Registration_ID; ?>&Admision_ID=<?php echo $Admision_ID; ?>&consultation_ID=<?php echo $consultation_ID; ?>' class='art-button-green'>
                    BACK
               </a>
        <?php
        }
...
<?php
    ...
    $check_sub_department_ward = mysqli_query($conn,"SELECT Ward_Store_ID FROM tbl_sub_department_ward WHERE sub_department_id='$SubDepWardID'");
    if (mysqli_num_rows($check_sub_department_ward)>0) {
        $data = mysqli_fetch_assoc($check_sub_department_ward);
        $Sub_Department_ID = $data['Ward_Store_ID'];
    }else{
        $Sub_Department_ID =0;
    }
    // die("select Sub_Department_Name from tbl_sub_department where Sub_Department_ID = '$Sub_Department_ID'");
    $select1 = mysqli_query($conn,"SELECT Sub_Department_Name from tbl_sub_department where Sub_Department_ID = '$Sub_Department_ID'") or die(mysqli_error($conn));
    ...
    $_SESSION['Pharmacy_ID']=$Sub_Department_ID;
...
<fieldset>  
    <legend style='background-color:#006400;color:white;padding:5px;' align=center><b><?php
            if (isset($_SESSION['Admission_Sub_Department_ID'])) {
                echo $Sub_Department_Name;
            }else{
                echo "Pharmacy not selected";
            }
            ?></b></legend>
    <center>
        <table width=100%>
            ...
            <tr>
                ...
                <td>
                    <select name='Billing_Type' id='Billing_Type' onchange="Sponsor_Warning()" style="width:100%">
                        ...
                        if (strtolower($Payment_Method) == 'cash') {
                            $Transaction_Type = 'Cash_Bill_Details';
                            
                        } else {
                            $Transaction_Type ='Credit_Bill_Details';
                        }
                        ?>
                    </select>
                </td>
            ...
            <tr>
                <td style='text-align: right;'>Ordered By</td>
                <td>
                    <select name='Consultant_ID' id='Consultant_ID' style="width:100%">
                        
                        <?php
                        $Employee_ID = $_SESSION['userinfo']['Employee_ID'];
                        $Select_Doctors = "select Employee_ID, Employee_Name from tbl_employee where Employee_ID='$Employee_ID'";
                    ...
                    <table width="100%">
                        <tr>
                            <td id="Transaction_Area">
                                <select id="Transaction_Mode" name="Transaction_Mode" onchange="Validate_Transaction_Mode()" class="form-control" style="width:100%">
                                    <?php
                ...
                <td><input type='text' name='Supervised_By' id='Supervised_By' disabled='disabled' value='<?php echo $Member_Number; ?>'></td> 
                    ...
                    <td style="text-align:right;color:green" id="sponsor_for_this_trans">
                        <b><span style="color:red">*</span>Temporary Sponsor</b>
                    </td>
                    <td>
                        <select id="new_sponsor_to_bill" class="form-control" style="width:100%">
                    ...
                    <td>
                     <select id='working_department' name='working_department'  style="width:100%">
                            ...
                            <?php 
                                $sql_select_working_department_result=mysqli_query($conn,"SELECT finance_department_code,finance_department_id,finance_department_name FROM tbl_finance_department WHERE enabled_disabled='enabled' AND finance_department_id='$bill_working_department'") or die(mysqli_error($conn));
...
<fieldset>
    <table width=100% id="Process_Buttons_Area">
        ...
        <input type="button" name="Filter" id="Filter" value="PREVIEW BILL " class="btn btn-success" onclick="Preview_Patient_Bill(<?php echo $Patient_Bill_ID; ?>,<?php echo $Folio_Number; ?>,<?php echo $Sponsor_ID; ?>,<?php echo $Check_In_ID; ?>,<?php echo $Registration_ID; ?>)">
        <a href="previewpatientbill.php?Patient_Bill_ID=<?php echo $Patient_Bill_ID; ?>&Folio_Number=<?php echo $Folio_Number; ?>&Sponsor_ID=<?php echo $Sponsor_ID; ?>&Check_In_ID=<?php echo $Check_In_ID; ?>&Registration_ID=<?php echo $Registration_ID; ?>&Transaction_Type=<?php echo $Transaction_Type; ?>" class="btn btn-warning" target="_blank">PREVIEW BILL IN PDF</a>
                    ...
                    if (strtolower($_SESSION['systeminfo']['Inpatient_Prepaid']) == 'yes') {
                        ...
                        if (strtolower($_SESSION['systeminfo']['Display_Send_To_Cashier_Button']) == 'yes') { //this setting allows system to display both (send to cashier) and (make payment) buttons
                           if(strtolower($_SESSION['systeminfo']['Mobile_Payment']) == 'yes'&&isset($_SESSION['configData']) && $_SESSION['configData']['ShowCreateEpaymentBillOrMakePaymentButton']=='makepayment'){
                            echo '<button class="art-button-green" type="button" onclick="Make_Payment(); clearContent();">MAKE PAYMENT & DISPENSE MEDICATION</button>';
                           }
                        }
                        ?>
                        <button class="art-button-green" type="button" onclick="Check_Patients_Bill('true');
                                            clearContent();">BILL & DISPENSE MEDICATIONS</button>
                        ...
                        <button class="art-button-green" type="button" onclick="Check_Patients_Bill();
                                            clearContent();">BILL & DISPENSE MEDICATION</button>
                                <?php
                            }
                        } else {
                            if ($Exemption == 'yes') {
                        ...
                        ?>
                        <button class="art-button-green" type="button" onclick="Check_Patients_Bill(); clearContent();">BILL & DISPENSE MEDICATION</button>
                        <?php
                    }
                }
...
//                        }
					 ...
					 where alc.Item_ID = t.Item_ID and
                        ...
                        <div id="Add_Pharmacy_Items" style="width:70%;" >
                            <table width=100% style='border-style: none;' class="table table-condensed table-striped">
                                <tr>
                                    <td width='45%'>
                                        <table width=100% style='border-style: none;' class="table table-condensed table-striped">
                                            <!-- <tr>
                                            ...
                                            <tr>
                                                <td>
                                                    <input type='text' id='Search_Value_item' name='Search_Value' autocomplete='off' onkeyup='getItemsListFilteredPharmacy(this.value)' placeholder='Enter Item Name'>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td>
                                                    <fieldset style='overflow-y: scroll; height: 380px;' name='mimi' id='Items_Fieldset_item'>
                                    ...
                                    <td>
                                        <table class="table table-condensed table-striped">
                                            ...
                                            <tr>
                                                ...
                                                <td >
                                                    <textarea name='Dosage' id='Dosage' rowspan='1' style="height: 30px;" placeholder='Dosage' autocomplete="off"></textarea>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td colspan=2 style='text-align: right;'>
                                                    <input type='button' name='Submit' id='Submit' class='art-button-green' value='ADD MEDICATION' onclick='Verify_Balance()'>
                                                </td>
                                            </tr>
                                        </table>
                                    </td>
...
<script type="text/javascript">
    ...
    function Add_More_Items(Patient_Bill_ID, Folio_Number, Sponsor_ID, Check_In_ID, Registration_ID) {
        ...
        var nursecommunication = '<?php echo $nursecommunication; ?>';
        ...
        myObjectDisplay.open('GET', 'Patient_Billing_Add_More_Items.php?Patient_Bill_ID=' + Patient_Bill_ID + '&Folio_Number=' + Folio_Number + '&Sponsor_ID=' + Sponsor_ID + '&Check_In_ID=' + Check_In_ID + '&Registration_ID=' + Registration_ID+'&nursecommunication='+nursecommunication, true);
    ...
    function Save_Information(Registration_ID) {
       ...
       var Check_In_ID = '<?php echo $Check_In_ID; ?>';
        ...
        if (sms == true) {
            ...
            myObjectSave.open('GET', 'Save_Information_Inpatient.php?Registration_ID=' + Registration_ID + '&Check_In_ID=' + Check_In_ID + '&Folio_Number=' + Folio_Number + '&Sponsor_ID=' + Sponsor_ID + '&Patient_Bill_ID=' + Patient_Bill_ID+'&Billing_Type='+Billing_Type, true);
            myObjectSave.send();
        }
    }
...
//        }
        ...
        if (Price != 0 && Item_ID != null && Item_ID != '' && Check_In_Type != null && Check_In_Type != '' && Registration_ID != '' && Registration_ID != null && Quantity != null && Quantity != '' && Quantity != 0) {
            ...
            myObjectAddSelectedItem.open('GET', 'Inpatient_Add_More_Selected_Item.php?Registration_ID=' + Registration_ID + '&Item_ID=' + Item_ID + '&Quantity=' + Quantity + '&Discount=' + Discount + '&Check_In_Type=' + Check_In_Type + '&Billing_Type=' + Billing_Type + '&Sponsor_ID=' + Sponsor_ID+"&Admision_ID="+Admision_ID+"&working_department"+working_department, true);
    ...
    function getItemsListFiltered(Item_Name, Sponsor_ID) {
        ...
        myObject.open('GET', 'Get_List_Of_Items_Filtered.php?Item_Category_ID=' + Item_Category_ID + '&Item_Name=' + Item_Name + '&Sponsor_ID=' + Sponsor_ID +'&nursecommunication='+nursecommunication, true);
        myObject.send();
    }
    ...
    function getItemsList(Item_Category_ID,Sponsor_ID) {
        ...
        myObject.open('GET', 'Get_List_Of_Items.php?Item_Category_ID=' + Item_Category_ID + '&Guarantor_Name=' + Guarantor_Name+'&Sponsor_ID='+Sponsor_ID+'&nursecommunication='+nursecommunication, true);
        myObject.send();
    }
    ...
    function Preview_Patient_Bill(Patient_Bill_ID, Folio_Number, Sponsor_ID, Check_In_ID, Registration_ID) {
            ...
            $.ajax({
            type: 'POST',
            url: 'Ajax_bill_summary_preview.php',
            data: {Patient_Bill_ID:Patient_Bill_ID,Folio_Number:Folio_Number,Sponsor_ID:Sponsor_ID,Check_In_ID:Check_In_ID, Registration_ID:Registration_ID,Transaction_Type:Transaction_Type,   billsummarypreview:''},
            success: function(responce){
                $("#billsummary").dialog({
                    title: 'PATIENT BILL PREVIEW SUMMARY',
                    width: '85%',
                    height: 700,
                    modal: true,
                });
                $("#billsummary").html(responce)
            }
        });
        }
</script>
        ...
        if(trans_type.value=="Manual"){
            ...
            if(result){            
               // document.location = "Departmental_Patient_Billing_Pharmacy_Page.php?Payment_Cache_ID=<?php echo $Payment_Cache_ID; ?>&Transaction_Type=<?php echo $Transaction_Type; ?>&Sub_Department_ID=<?php echo $Sub_Department_ID; ?>&Registration_ID=<?php echo $Registration_ID; ?>&Billing_Type=<?php echo $Billing_Type; ?>"+'&manual_offline=manual&approve_yes=yes';
                document.location = 'Inpatient_Pharmacy_Bill_Patient.php?Registration_ID=' + Registration_ID + '&Folio_Number=' + Folio_Number + '&Claim_Form_Number=' + Claim_Form_Number + '&Consultant_ID=' + Consultant_ID+"&post_payment=false"+"&manual_offline=manual";
      ...
      function offline_transaction(amount_required,reg_id){
        ...
        if(comf){
            ...
            $.ajax({
                type: 'GET',
                url: uri,
                data: {amount_required:amount_required,registration_id:reg_id,Folio_Number:Folio_Number,Claim_Form_Number:Claim_Form_Number,Consultant_ID:Consultant_ID},
          ...
          //}
                        ...
                        <script type='text/javascript'>
                            function Send_To_Cashier() {
                                ...
                                if (Guarantor_Name == 'NHIF' && (Claim_Form_Number == '' || Claim_Form_Number == null || Folio_Number == '' || Folio_Number == null)) {
                                    ...
                                    if (r == true) {
                                        document.location = 'Send_To_Cashier_Pharmacy.php?Section=Inpatient&Registration_ID=' + Registration_ID + '&Folio_Number=' + Folio_Number + '&Claim_Form_Number=' + Claim_Form_Number + '&Consultant_ID=' + Consultant_ID;
                                    }
                                }
                            }
                        </script>
                        ...
                        <script>
                            function Confirm_Remove_Item(Item_Name, Item_Cache_ID, Registration_ID) {
                                ...
                                if (Confirm_Message == true) {
                                    ...
                                    My_Object_Remove_Item.open('GET', 'Inpatient_Pharmacy_Remove_Item_From_List.php?Item_Cache_ID=' + Item_Cache_ID + '&Registration_ID=' + Registration_ID, true);
                        ...
                        <script>
                            function update_total() {
                                ...
                                My_Object_Update_Total.open('GET', 'Inpatient_Pharmacy_Update_Total.php?Registration_ID=' + Registration_ID, true);
                        ...
                        <script>
                            function update_Billing_Type() {
                                ...
                                My_Object_Update_Total.open('GET', 'Inpatient_pharmacy_update_Billing_Type.php?Registration_ID=' + Registration_ID + '&Guarantor_Name=' + Guarantor_Name, true);
                                My_Object_Update_Total.send();
                            }
                        </script>
                        <script>
                        ...
                        <script type="text/javascript">
                            function Get_Details_Verify(Item_Name, Item_ID) {
                                ...
                                myObjectVerify.open('GET', 'Pharmacy_Inpatient_Get_Details_Verify.php?Item_ID=' + Item_ID + '&Registration_ID=' + Registration_ID, true);
                                myObjectVerify.send();
                            }
                        </script>
                        ...
                        <script>
                            function update_process_buttons(Registration_ID) {
                                ...
                                my_Object_Update_Process.open('GET', 'Inpatient_Pharmacy_Update_Process_Button.php?Registration_ID=' + Registration_ID, true);
                        ...
                        <script>
                            $(document).ready(function () {
                                ...
                                $("#Add_Pharmacy_Items").dialog({autoOpen: false, width: '1150', height: 550, title: 'ADD CONSUMABLE ITEMS', modal: true});
                                ...
                                if (Registration_ID != '' && Registration_ID != null && Item_Name != '' && Item_Name != null && Item_ID != '' && Item_ID != null && Quantity != '' && Quantity != null && Billing_Type != '' && Billing_Type != null && Dosage != null && Dosage != '') {
                                    ...
                                    myObject2.onreadystatechange = function () {
                                        ...
                                        if (myObject2.readyState == 4) {
                                            ...
                                            document.getElementById('Item_Balance').value = '';
                                            document.getElementById("Total").value = '';
                                            document.getElementById("Search_Value_item").focus();
                        ...
                        <script>
                            function update_billing_type(Registration_ID) {
                                ...
                                myObjectUpdateBilling.open('GET', 'Inpatient_Pharmacy_Update_Billing_Type.php?Registration_ID=' + Registration_ID, true);
            ...
            var uri="varidate_dosage_duration.php";
             ...
             $.ajax({
                ...
                error: function(){
                        ...
                        <script>
                            function getItemsListFilteredPharmacy(Item_Name) {
                                document.getElementById("Item_Name").value = '';
                                document.getElementById("Item_ID").value = '';
                                document.getElementById("Dosage").value = '';
                                document.getElementById("Quantity").value = '';
                                ...
                                myObject.onreadystatechange = function () {
                                    ...
                                    if (myObject.readyState == 4) {
                                        //document.getElementById('Approval').readonly = 'readonly';
                                        document.getElementById('Items_Fieldset_item').innerHTML = data135;
                                    }
                                }; //specify name of function that will handle server response........
                                myObject.open('GET', 'Get_List_Of_Pharmacy_Filtered_Items.php?Item_Name=' + Item_Name + '&Guarantor_Name=' + Guarantor_Name+"&Sponsor_ID="+Sponsor_ID, true);
                                myObject.send();
                            }
                        </script>
                                ...
                                myObjectConfirm.onreadystatechange = function () {
                                    ...
                                    if (myObjectConfirm.readyState == 4) {
                                        ...
                                        if (feedback == 'yes') {
                                            ...
                                            alert("Sorry. Process Fail! Some medication detected with zore price.\nYou are not allowed to process medication with zero Price");
                                        }
                                    }
                                }; //specify name of function that will handle server response........
                        ...
                        <script type='text/javascript'>
                            function Bill_Patient(post_payment='') {
                                ...
                                if (Consultant_ID == '' || Consultant_ID == null) {
                                    ...
                                    var r = confirm("Are you sure you want to bill and dispense medication to " + Patient_Name + "?");
                                    if (r == true) {
                                        document.location = 'Inpatient_Pharmacy_Bill_Patient.php?Registration_ID=' + Registration_ID + '&Folio_Number=' + Folio_Number + '&Claim_Form_Number=' + Claim_Form_Number + '&Consultant_ID=' + Consultant_ID+"&post_payment="+post_payment+"&Check_In_Type=Pharmacy&from_billing_work="+from_billing_work;
                                    }
                                }
                            }
                        </script>
                        ...
                        <script type="text/javascript">
                            function Approval_Medication() {
                                ...
                                if (Consultant_ID == '' || Consultant_ID == null) {
                                    ...
                                    if (r == true) {
                                        document.location = 'Inpatient_Pharmacy_Send_To_Approval.php?Registration_ID=' + Registration_ID + '&Folio_Number=' + Folio_Number + '&Claim_Form_Number=' + Claim_Form_Number + '&Consultant_ID=' + Consultant_ID;
                                    }
                                }
                            }
                        </script>
                        <script>
                            function alertMessage() {
                                alert("Please select medication first!");
                                document.getElementById("Quantity").value = '';
                            }
                        </script>
                                ...
                                if (Guarantor_Name == 'NHIF' && (Claim_Form_Number == '' || Claim_Form_Number == null || Folio_Number == '' || Folio_Number == null)) {
                                    ...
                                    if(confirm("Are You sure you want to go to Mobile/Card Payment")){
                                        $.ajax({
                                            ...
                                            data:{Sub_Department_Name:Sub_Department_Name,Registration_ID:Registration_ID,Check_In_Type:Check_In_Type,Payment_Cache_ID:Payment_Cache_ID,Section:'Inpatient',Folio_Number:Folio_Number,Claim_Form_Number:Claim_Form_Number,Consultant_ID:Consultant_ID},
                                            success:function(data){
                                                if(data=="success"){
