...
<script type="text/javascript" language="javascript">
    ...
    if (sponsor=='NHIF') {
        ...
        document.getElementById("date2").setAttribute('readonly');
    }
    } 
                            ...
                            <tr>
                                ...
                                <td><select name='Guarantor_Name' id='Guarantor_Name' required='required' style='border-color: red' onchange='MemberNumberMandate(this.value);setVerify(this.value); disable_member_number(this.value);'>
                                    <option selected='selected'></option>
                                        <?php
                                            $data = mysqli_query($conn,"select * from tbl_sponsor where Support_Patient_From_Outside = 'yes' or Guarantor_Name = 'cash'") or die(mysqli_error($conn));
...
<script>
    function confirm_message(state){
        if(state.checked){
            if(confirm("Are you sure this is a dead body?")){
                state.checked = true;
            }else{
                state.checked = false;
            }
