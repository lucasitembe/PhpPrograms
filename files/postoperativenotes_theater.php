<?php
    ...
    $Sponsor_ID = mysqli_fetch_assoc(mysqli_query($conn, "SELECT pc.Sponsor_ID FROM tbl_payment_cache pc, tbl_item_list_cache ilc WHERE ilc.Payment_Item_Cache_List_ID = '$Payment_Item_Cache_List_ID' AND pc.Payment_Cache_ID = ilc.Payment_Cache_ID"))['Sponsor_ID'];
    ...
    if(isset($_GET['Registration_ID']) && isset($_GET['Patient_Payment_Item_List_ID']) && isset($_GET['Patient_Payment_ID'])){
        echo "<a href='perform_suggery_theater.php?Section=".$Section."&consultation_ID=".$consultation_ID."&Registration_ID=".$Registration_ID."&Patient_Payment_ID=".$Patient_Payment_ID."&Patient_Payment_Item_List_ID=".$Patient_Payment_Item_List_ID."&SearchListDoctorsPagePatientBilling=SearchListDirectCashPatientBillingThisPage' class='art-button-green'>BACK</a>";
    }else{
        echo "<a href='perform_suggery_theater.php?Section=".$Section."&consultation_ID=".$consultation_ID."&Registration_ID=".$Registration_ID."&Patient_Payment_ID=".$Patient_Payment_ID."&Patient_Payment_Item_List_ID=".$Patient_Payment_Item_List_ID."&SearchListDoctorsPagePatientBilling=SearchListDirectCashPatientBillingThisPage' class='art-button-green'>BACK</a>";
    }
...
?>
<fieldset>
    <legend align=center>POST OPERATIVE DOCUMENTATION</legend>
        <div class="box box-primary" style="height: 80px;overflow-y: scroll;overflow-x: hidden">
    ...
    <div class="box box-primary" style="display:grid;grid-template-columns:1fr 1fr;gap:2em;">
        <div class="left-data" style="margin-left:70px;">
            <div class="field">
                <label for=""><b>****</b></span>Surgery Name</label><br>
            ...
            <div class="field">
                ...
                <input type='text' class="form-control ehms_date_time" id="cutting_time" value='<?= $cutting_time ?>'  onkeyup="Change_Type_Of_Anesthetic()" readonly="readonly" style='text-align:center;background:#FFFFFF;width:80%;height:30px; margin:0px;'>
            </div>
            <div class="field">
                <label>Ending Cutting Time</label><br>
                <input type='text' class="form-control ehms_date_time" id="end_cutting_time" value='<?= $end_cutting_time ?>' onchange="Change_Type_Of_Anesthetic()" readonly="readonly" style='text-align:center;background:#FFFFFF;width:80%;height:30px;margin:0px;'>
            </div>
            ...
            <div class="field">
                ...
                <input type="text" id="duration_of_surgery" value="<?= $duration_of_surgery ?>" onkeyup="Change_Type_Of_Anesthetic()" name='duration_of_surgery' placeholder="Enter Surgery Duration" style="width: 80%; height: 30px;" readonly="readonly"/>
            </div>
...
<fieldset>
    ...
    <div class="data-table" style="display:grid;grid-template-columns:2fr 2fr 1fr;gap:2em;margin-top:30px;">
        ...
        <div class="field" style="margin-left:-110px;" >
            ...
            <input type="button" value="BIOPSY REQUEST FORM" class="art-button-green" onclick="Biopsy_request_from()" style="margin-left:50px; background: #990000;" >
        </div>
    </div>
</fieldset>
...
<fieldset >
    ...
    <tr>
        ...
        <td colspan="2">
            <textarea placeholder="Enter Remarks" id='post_operative_remarks' onkeyup='Update_remarks()'><?= $post_operative_remarks ?></textarea>
        </td>
    </tr>
    <tr>
        ...
        <button type="button" class="btn btn-primary form-control art-button hide" data-toggle="collapse" onclick='open_other_optical_surgery_detail(<?php echo $Registration_ID?>);' style="overflow:auto;">Fill Eye detail</button>
                <div  class="collapse" id='open_other_optical_surgery_detail' style="overflow:auto;">
                ...
                <div style="border:1px solid black;margin-top:10px;" >
                    <table class='table table-striped table-hover'>
                        ...
                        <input type="hidden" id='Payment_Item_Cache_List_ID' name='Payment_Item_Cache_List_ID' value="<?php echo $Payment_Item_Cache_List_ID;?>">
                            <td><input type="submit" name="save_optical_data" id="save_optical_data" value="DONE" class="art-button-green" style="text-align:right;"></td>
                        </tr>
                            ...
                            <td width="40%">
                                <fieldset style='overflow-y: scroll; height: 250px; background-color: white;' id='Assistant_Surgeon_Area'>
                                    ...
                                    <?php
                                        ...
                                        emp.Employee_ID = pri.Employee_ID and
                                        pri.Theater_Works = 'yes'  AND (emp.Employee_Type = 'Doctor' or emp.Employee_Type LIKE '%Surge%')  and emp.Account_Status = 'active' AND emp.employee_signature IS NOT NULL order by Employee_Name limit 100") or die(mysqli_error($conn));
...
<?php
// die("SELECT consultation_ID FROM Check_In_Details_ID WHERE Registration_ID='$Registration_ID' AND Admit_Status = 'admitted' ORDER BY Check_In_Details_ID DESC LIMIT 1");
// exit();
$consultation_ID_current = mysqli_fetch_assoc(mysqli_query($conn, "SELECT consultation_ID FROM tbl_check_in_details WHERE Registration_ID='$Registration_ID' AND Admit_Status = 'admitted' ORDER BY Check_In_Details_ID DESC LIMIT 1"))['consultation_ID'];
if(!empty($consultation_ID_current) || $consultation_ID_current < 1){
    $consultation_ID_current = mysqli_fetch_assoc(mysqli_query($conn, "SELECT consultation_ID FROM tbl_consultation WHERE Registration_ID='$Registration_ID' AND Process_Status = 'served' ORDER BY consultation_ID DESC LIMIT 1"))['consultation_ID'];
}
?>
<script type="text/javascript">
    function Update_External_Participant(Participant_Type){
        ...
        if(consultation_ID ===''){
            consultation_ID = '<?= $consultation_ID_current ?>';
        }
    ...
    function Submit_Information(){
        ...
        myObjectSubmit.onreadystatechange = function () {
            ...
            if (myObjectSubmit.readyState == 4) {
                ...
                // alert("Surgery Successfully Done")
              document.location = 'perform_suggery_theater.php?Section='+Section+'&Registration_ID='+Registration_ID+'&Patient_Payment_ID='+Patient_Payment_ID+'&Patient_Payment_Item_List_ID='+Patient_Payment_Item_List_ID+'&consultation_ID='+consultation_ID+'&SearchListDoctorsPagePatientBilling=SearchListDirectCashPatientBillingThisPage';
            }
        }; //specify name of function that will handle server response........

        myObjectSubmit.open('GET', 'Submit_Postoperative_Information.php?Payment_Item_Cache_List_ID='+Payment_Item_Cache_List_ID+'&Patient_Payment_Item_List_ID='+Patient_Payment_Item_List_ID+'&Registration_ID='+Registration_ID+'&Position='+Position+'&Type_Of_Anesthetic='+Type_Of_Anesthetic+'&Patient_Payment_ID='+Patient_Payment_ID+'&Sponsor_ID='+Sponsor_ID+'&post_operative_remarks='+post_operative_remarks, true);
        myObjectSubmit.send();
    }
</script>
<script>
    function Update_remarks() {
        ...
        $.ajax({
            type: "GET",
            url: "update_post_operative_remarks.php",
            data: "Payment_Item_Cache_List_ID="+Payment_Item_Cache_List_ID+"&consultation_ID="+consultation_ID+"&Registration_ID="+Registration_ID+"&post_operative_remarks="+post_operative_remarks,
...
<script type="text/javascript">
    function Biopsy_request_from(){
        ...
        myObjectPost.open('GET', 'biopsy_requesting_form.php?Payment_Item_Cache_List_ID='+Payment_Item_Cache_List_ID+'&Patient_Payment_Item_List_ID='+Patient_Payment_Item_List_ID+'&Registration_ID='+Registration_ID+'&consultation_ID='+consultation_ID, true);
        myObjectPost.send();
    }
</script>
<script type="text/javascript">
...
<script type="text/javascript">
    function Change_Description_Title(){
        ...
        if(Previous_Title == 'Procedure description and closure'){
            ...
            Temp_ID = 'Problems';
            Temp_Data = document.getElemajax_tb_result_formentById("Problems").value;
...
<script type="text/javascript">
    ...
    function Open_Labodatory_Dialogy2(){
        ...
        myObjectLab_Details.open('GET', 'Postoperative_Add_Laboratory_Items_biopsy.php?consultation_ID='+consultation_ID+'&Registration_ID='+Registration_ID, true);
        myObjectLab_Details.send();
    }
...
<script type="text/javascript">
    function getItemsList_Laboratory_biopsy(){
        document.getElementById("Search_Value").value = '';
        document.getElementById("Item_Name").value = '';
        document.getElementById("Item_ID").value = '';
        var Guarantor_Name = '<?php echo $Guarantor_Name; ?>';
        ...
        myObject.open('GET','Post_Operative_Get_List_Of_Laboratory_Items_List_biopsy.php?Item_Category_ID='+Item_Category_ID+'&Guarantor_Name='+Guarantor_Name+'&Sponsor_ID='+Sponsor_ID,true);
        myObject.send();
    }
</script>
...
<script type="text/javascript">
    function Get_Selected_Item_Laboratory2(Check_in_type="Laboratory"){
        ...
        if (Registration_ID != '' && Registration_ID != null && Item_Name != '' && Item_Name != null && Item_ID != '' && Item_ID != null && Sub_Department_ID != null && Sub_Department_ID != '') {
            ...
            myObjectLabs.open('GET', 'Post_Operative_Add_Laboratory_biopsy.php?Registration_ID=' + Registration_ID + '&Item_ID=' + Item_ID + '&Quantity=' + Quantity + '&Billing_Type=' + Billing_Type + '&Guarantor_Name=' + Guarantor_Name + '&Sponsor_ID=' + Sponsor_ID + '&Billing_Type=' + Billing_Type + '&Comment=' + Comment+'&Transaction_Type='+Transaction_Type+'&consultation_ID='+consultation_ID+'&Sub_Department_ID='+Sub_Department_ID+'&Check_in_type='+Check_in_type+'&payment_Status='+payment_Status, true);
            myObjectLabs.send();
...
<script>
    $(document).ready(function () {
        ...
        $("#Add_Preoperative").dialog({autoOpen: false, width: '80%', height: 500, title: 'Pre Operative Diagnosis (indication)', modal: true});
        $("#Add_Postoperative").dialog({autoOpen: false, width: '80%', height: 500, title: 'Post Operative Diagnosis (indication)', modal: true});
        $("#Biopsy_Form").dialog({autoOpen: false, width: '70%', height: 650, title: 'BIOPSY/HISTOLOGICAL EXAMINATION REQUESTING FORM', modal: true});
        ...
        $("#Investigation_Details2").dialog({autoOpen: false, width: '75%', height: 500, title: 'ADD BIOPSY TEST', modal: true});
