...
<?php
if (isset($_GET['DHS'])) { ?>
    <a href='ExemptionListtoDHS.php?Exemptionlist=ToDHSPage' class='art-button-green'>
        BACK
    </a>
<?php    
}else{ ?>
    <a href='temporaryExemptionList.php?Exemptionlist=ToSociawelfareUnit' class='art-button-green'>
        BACK
    </a>
<?php    
}
...
<?php 
    ...
    $exemptiondata  = mysqli_query($conn, "SELECT ushauriwabima,mode_of_treatment,taarifa_za_mgonjwa,patient_next_kin,next_kin_phoneNo,Next_kin_relationship, Anaombewa, Exemption_ID, tathiminiyamsaada,alishawahikupewa, kiasikinachoombewamshamaha, kiwango_cha_msamaha,previous_hospital_treatment, time_of_treatment,mapendekezomsamaha FROM tbl_temporary_exemption_form WHERE Registration_ID ='$Registration_ID' AND Exemption_ID='$exemptionID' ") or die(mysqli_error($conn));
// echo "<h1>".mysqli_num_rows($exemptiondata)."</h1>";
...
<fieldset>
    <legend align=center>TEMPORARY EXEMPTION FORM NO: <?php echo $exemptionID; ?></legend> 
    <hr>
    <table width=100% >
        <tr>
        ...
        <tr>
            ...
            <td style='text-align:right;'  width="25%"><b><input type="text" name="" disabled='disabled' id="" value="<?php echo $District. " ".$village." ".getVillageName($village);?>">  </b></td>
            <td style='text-align:right;'  width="15%"><b>Mwenyekiti/Balozi </b></td>
            <td style='text-align:right;'  width="15%"><b><input type="text" name="" disabled='disabled' id="" value="<?php echo getTribeName($Tribe)?>"> </b></td>
    ...
    <hr>
    <table  width="100%" style="border-top: 1px solid black; border-bottom:1px solid black; border-left:1px solid black;">
                    <caption  align="center" id="th"><h4 align="center" style=" padding-bottom:5px;"> FORM YA MAOMBI YA DHAMANA/ MSAMAHA WA MALIPO YA MATIBABU </h4></caption>
                    <tr>
                        <?php 
    ...
    <table width="100%" >
        ...
        <tr>
                        ...
                        <tr>
                            ...
                            <td> <span><b>Kiasi kinachoombewa</b></span><span><input class="form-control" style="display: inline; width:60%;" disabled name="kiasikinachoombewamshamaha" id="" value="<?php echo number_format($kiasikinachoombewamshamaha);?>" ></span></td>
    ...
    ?>
    <table width="100%" >
            <thead style='text-align:right;'><tr><th colspan="4">Mgonjwa amesajiliwa na</th></tr></thead>
            <tbody>
                <tr>
    ...
    <table width="100%">
    <caption  align="center" id="th"><h4 align="center" style=" padding-bottom:5px; fornt-weight: bold;"> TATHIMINI YA CA </h4></caption>

        <tbody>
        <?php
            $select_tathimini_ya_PHRO = mysqli_query($conn, "SELECT ep.tathiminiyaphro, ep.created_at, em.Employee_Name, em.Employee_Title, em.employee_signature FROM tbl_expemption_phro ep, tbl_employee em WHERE ep.Exemption_ID='$Exemption_ID' AND em.Employee_ID = ep.Employee_ID" ) or die(mysqli_error($conn));
            if((mysqli_num_rows($select_tathimini_ya_PHRO))>0){
                while($row = mysqli_fetch_assoc($select_tathimini_ya_PHRO)){
                    ...
                    $CaName = $row['Employee_Name'];
                    $CaEmployee_Title = $row['Employee_Title'];
                    $Cacreated_at = $row['created_at'];
                    $Caemployee_signature = $row['employee_signature'];
                           if($Caemployee_signature==""||$Caemployee_signature==null){
                            $casignature="_______________________";
                        }else{
                            $casignature="<img src='../esign/employee_signatures/$Caemployee_signature' style='height:25px'>";
                        }
                    echo "<tr><td style='text-align:right; font-weight: bold;'>Tathmini ya CA</td><td colspan='3'><input type='text' value='".$tathiminiyaphro."' readonly='readonly'></td></tr>";

                    ?>
                    <tr>
                        <td style='text-align:right;'><b> Jina Kamili</b></td>
                        <td><input type="text" disabled='disabled' value=" <?php  echo $CaName; ?>"></td>
                        <td style='text-align:right;'><b>Cheo</b></td>
                        <td><input type="text" disabled='disabled' value="<?php  echo $CaEmployee_Title; ?>"></td>
                    </tr>
                    <tr>
                        <td style='text-align:right;'> <b>Sahihi</b> </td>
                        <td><?php echo $casignature; ?></td>
                        <td style='text-align:right;'><b>Tarehe</b></td>
                        <td><input type="text" disabled='disabled' value="<?php echo $Cacreated_at;?>"></td>
                    </tr> 
                    <?php
                }
            }else{
        ?>
        <tr>
            <td style='text-align:right;'><b>TATHIMINI YA CA</b></td>
...
<?php
if(!empty($tathiminiyaphro)){
?>
<fieldset>
    <legend align="center">MAONI YA UTAWALA</legend>
    <table width="100%">
        <tr><h4>Maoni /idhinisho la Mkurugenzi</h4></tr>
        <tbody><input type="text" id="Exemption_ID" style="display: none;" value="<?php echo $Exemption_ID;?>">
            <?php
                $select_maoni_ya_DHS = mysqli_query($conn, "SELECT maoniDHS, sababudhs, emd.Employee_ID, Employee_Name, Employee_Title, emd.created_at, employee_signature from tbl_exemption_maoni_dhs emd, tbl_employee e where Exemption_ID='$Exemption_ID' AND emd.Employee_ID =e.Employee_ID" ) or die(mysqli_error($conn));
                ...
                <td colspan="3" style="width: 60%; height:150px; overflow-y:scroll">
                   <?php echo $sababudhs; ?>
                </td>

            </tr>
            <tr>
                <td width='20%' style='text-align:right; font-weight: bold;'>Jina la alieidhinisha:</td>
                <td width='20%'><?php echo $Employee_Name; ?> </td>
                <td width='20%' style='text-align:right; font-weight: bold;'>Cheo:</td>
                <td width='20%'><?php echo $Employee_Title; ?> </td>
                <td  style='text-align:right; font-weight: bold;' width='15%'>Sahihi:</td>
                <td width='20%'><?php echo $signature; ?></td>
                <td width='5%' style='text-align:right; font-weight: bold;'>Tarehe:</td>
                <td width='30%'><?php echo $created_at; ?></td>
            </tr>
            <tr>
                
                <td style='text-align:center' colspan='6'><a href="printexemptionform.php?Registration_ID=<?= $Registration_ID?>&Exemption_ID=<?=$Exemption_ID?>&created_at=<?=$created_at?>" target="_blank" rel="noopener noreferrer" class="art-button-green pull-right" onclick="checkphrovalue()">PRINT FORM</a> </td>
            </tr>
                <?php
                }else{
            ?>            
            <tr>
                ...
                <td colspan="3" >
                    <textarea class="form-control" name="sababudhs" id="sababuDHS2" disabled rows="2" cols="70"></textarea>
                </td>
                <td><button type="button" class="art-button-green pull-right" onclick="maoni_ya_DHS(<?php echo $Exemption_ID;?>,<?php echo $Registration_ID;?>)">JAZA </button></td>
...
<script>
    function tathimini_PHRO(Exemption_ID, Registration_ID){
        
        $.ajax({
            type:'POST',
            url:'exemption_tathimini_PHRO.php',
            data:{Exemption_ID:Exemption_ID,Registration_ID:Registration_ID,tathimin:''},
    ...
    function savetathimini(Exemption_ID){
        var tathimini = $("#tathimini").val();
        $.ajax({
            type:'POST',
            url:'ajax_exemptionform.php',
            data:{Exemption_ID:Exemption_ID, tathiminiyaphro:tathimini,tathiminPHRO:""},
            success:function(responce){
    ...
    function maoni_ya_DHS(Exemption_ID, Registration_ID){
        
        $.ajax({
            type:'POST',
            url:'exemption_maoni_ya_DHS.php',
            data:{Exemption_ID:Exemption_ID,Registration_ID:Registration_ID,maoniDHS:''},
            success:function (responce){
                $("#maoniDHS").dialog({
    ...
    function savemaoni(Exemption_ID){
        var maoniDHS = "";
        ...
        $.ajax({
            type:'POST',
            url:'ajax_exemptionform.php',
            data:{maoniyadhs:"",maoniDHS:maoniDHS,sababudhs:sababudhs,Exemption_ID:Exemption_ID },
            success:function(){
