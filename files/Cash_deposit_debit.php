...
if (isset($_SESSION['userinfo'])) {
    if ($_SESSION['userinfo']['Revenue_Center_Works'] == 'yes') {
        ?>
        <a href='Direct_Cash_debit.php?SearchListDirectCashPatientBilling=SearchListDirectCashPatientBillingThisPage' class='art-button-green'>
            BACK
        </a>
    <?php }
}
...
// die("SELECT Patient_Bill_ID, Grand_Total, pd.Debt_ID, Patient_debt_amount FROM tbl_debt_cash_deposit cd,  tbl_patient_debt pd WHERE pd.Debt_ID=cd.Debt_ID AND Registration_ID='$Registration_ID'");
$clear_patient_debit = mysqli_query($conn, "SELECT Patient_Bill_ID, Grand_Total, pd.Debt_ID, Patient_debt_amount FROM tbl_debt_cash_deposit cd,  tbl_patient_debt pd WHERE pd.Debt_ID=cd.Debt_ID AND Registration_ID='$Registration_ID'") or die(mysqli_error($conn));
        if(mysqli_num_rows($clear_patient_debit)>0){
            while($rows = mysqli_fetch_assoc($clear_patient_debit)){
                $Patient_Bill_ID_ondebit = $rows['Patient_Bill_ID'];
                $Debt_ID = $rows['Debt_ID'];
                $Patient_debt_amount = $rows['Patient_debt_amount'];
                $Registration_IDssssss = $rows['Registration_ID'];
                $Grand_Total = $rows['Grand_Total'];
            //    die("SELECT Price, Consultant_ID,Transaction_Date_And_Time  FROM tbl_patient_payment_item_list pil,tbl_patient_payments pp WHERE Patient_Bill_ID='$Patient_Bill_ID_ondebit' AND pil.Patient_Payment_ID=pp.Patient_Payment_ID AND Registration_ID='$Registration_ID' AND DATE(Transaction_Date_And_Time) = CURDATE()");
                $select_if_payment_done = mysqli_query($conn, "SELECT Price, Consultant_ID,Transaction_Date_And_Time  FROM tbl_patient_payment_item_list pil,tbl_patient_payments pp WHERE Patient_Bill_ID='$Patient_Bill_ID_ondebit' AND pil.Patient_Payment_ID=pp.Patient_Payment_ID AND Registration_ID='$Registration_ID' AND DATE(Transaction_Date_And_Time) = CURDATE()") or die(mysqli_error($conn));
                    ...
                    while($rwdata = mysqli_fetch_assoc($select_if_payment_done)){                       
                        ...
                        if($Price == $Grand_Total){
                            ...
                            if($clear_debit){
                                ...
                                $update_social_walfere = mysqli_query($conn, "UPDATE tbl_social_reduce_debt SET debit_status='Paid' WHERE  Debt_social_ID='$Debt_social_ID' ") or mysqli_error($conn);
                            ...
                            $amont_paid_toreduce_debit = mysqli_fetch_assoc(mysqli_query($conn, "SELECT Amount_deposited FROM tbl_social_reduce_debt srd, tbl_patient_debt_to_socialwalfare pds WHERE Patient_Bill_ID='$Patient_Bill_ID_ondebit' AND Registration_ID='$Registration_ID' AND srd.Debt_social_ID=pds.Debt_social_ID"))['Amount_deposited'];
                            $Debt_social_ID = mysqli_fetch_assoc(mysqli_query($conn, "SELECT  srd.Debt_social_ID FROM tbl_social_reduce_debt srd, tbl_patient_debt_to_socialwalfare pds WHERE Patient_Bill_ID='$Patient_Bill_ID_ondebit' AND Registration_ID='$Registration_ID' AND srd.Debt_social_ID=pds.Debt_social_ID"))['Debt_social_ID'];
                           
                            if($amont_paid_toreduce_debit == $Price){
                                $clear_debit_tblcdd = mysqli_query($conn, "UPDATE tbl_debt_cash_deposit SET debit_status='Paid', Paid_at='$Transaction_Date_And_Time' WHERE Debt_ID='$Debt_ID'") or die(mysqli_error($conn));
                                if($clear_debit_tblcdd){                                    
                                    $update_social_walfere = mysqli_query($conn, "UPDATE tbl_social_reduce_debt SET debit_status='Paid' WHERE  Debt_social_ID='$Debt_social_ID' ") or mysqli_error($conn);
                                    if($update_social_walfere){
                                        echo "<script>alert('Cash paid successful')</script>";

                                    }else{
                                        echo "Something 2 went wrong";
                ...
                <table width=100%>
                    ...
                    <table width=100%>
                        ...
                        <tr>
                            ...
                            <td>
                                <input type='text' name='Quantity' class="numberonly" id='Quantity' placeholder='Quantity' readonly autocomplete="off">
                            </td>
                            <td>
                                <input type='text' name='Amount' class="numberonly" id='Amount' placeholder='Amount' readonly autocomplete="off">
                            </td>
                            <td style='text-align: center;'>
                        ...
                        <tr>
                            ...
                            <td colspan="1" style="text-align: right;" id="Total_Area">
                                <?php
                            ...
                            <td style="text-align: right;" id="Payment_Button_Area" colspan="4">
...
?>
                                        ...
                                        <?php
                                                                                     echo " <input type='button' value='Create ePayment Bill' id='Pay_Via_Mobile' style='float:left' name='Pay_Via_Mobile' class='art-button-green' onclick='Pay_Via_Mobile_Function()'>";  
...
<script type="text/javascript">
    function Get_Item_Name(Item_Name, Item_ID) {
        ...
        document.getElementById("Item_Description").value = 'Malipo ya Deni';
        document.getElementById("Amount").value = '<?php echo $Debit_amount; ?>';
        //alert('<?php echo $Debit_amount; ?>');
    ...
    function create_epayment_mobile_card_bill(){
                                ...
                                var Clinic_ID = document.getElementById("Clinic_ID").value;
                                    ...
                                    if(confirm("Are You sure you want to go to Mobile/Card Payment")){
                                        $.ajax({
                                            type:'GET',
                                            url:'ajax_create_epayment_mobile_card_bill_cash_deposit_outpatient.php',
