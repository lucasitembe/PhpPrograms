...
if (isset($_POST['store']) && $_POST['store'] === 'form-six') {
    ...
    $query = "SELECT id FROM tbl_icu_form_six_records 
    ...
    if ($result){
        ...
        echo 'updating';
        $id = $result['id'];
        $query = "UPDATE `tbl_icu_form_six_records` SET `value` = '$value' 
                    WHERE `id` = '$id';";
        ...
        // insert
        echo 'inserting';
        $query = "INSERT INTO tbl_icu_form_six_records (record_id, employee_id, name, shift, value, created_at) 
...
// Retrieve form data
if (isset($_GET['retrieve']) && $_GET['retrieve'] === 'form-six') {
    $shift = clean($_GET['shift']);
    $form_id = clean($_GET['form_id']);

    $date = date('Y-m-d H:i:s');

    $query = "SELECT * FROM tbl_icu_form_six_records WHERE record_id = '$form_id' AND shift = '$shift'";
