...
if (isset($_GET['records_list'])) {
    ...
    $query = "SELECT id, created_at FROM tbl_icu_form_seven WHERE registration_id = $registrationId AND consultation_id = $consultationId ORDER BY id DESC";
        ...
        <tr>
            ...
            <td class="text-center">
                <a target="_blank" class="btn btn-primary btn-sm text-white"
                   href="form_seven_preview.php?record_id=<?= $record['id'] ?>&Registration_ID=<?= $registrationId ?>">
                    VIEW
                </a>
                <!--                <a target="_blank" class="btn btn-primary btn-sm text-white"-->
