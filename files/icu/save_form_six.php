...
if (isset($_POST['field_name']) && isset($_POST['registration_id'])){
  ...
  $select_patient_from_icu_from_seven = "SELECT * FROM tbl_icu_form_six WHERE patient_id = '$registration_id' AND date_time != NOW()";

  $insert_result = mysqli_query($conn,$select_patient_from_icu_from_seven);
  $num = mysqli_num_rows($insert_result);

  if ($num > 0) {
      $inset_icu_form_seven = "UPDATE tbl_icu_form_six SET $fiels_name = '$field_value' WHERE patient_id='$registration_id'";
    ...
    $insert_patient_detailes_in_icu_form_seven = "INSERT INTO tbl_icu_form_six(patient_id,date_time) VALUES('$registration_id',NOW())";
