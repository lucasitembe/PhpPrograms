...
if(isset($_POST['field_name']) && isset($_POST['registration_id'])){
  ...
  // echo $medic_id;
  echo $item_id;
  $select_patient_form_two_evee_opening="SELECT * FROM tbl_icu_form_two_even_opening WHERE patient_id='$registration_id' AND DATE(date_time)=CURDATE() AND item_id='$item_id'";

  $select_result = mysqli_query($conn,$select_patient_form_two_evee_opening);

    if(($num = mysqli_num_rows($select_result)) > 0){

      $update_eve_opening = "UPDATE tbl_icu_form_two_even_opening SET $fields_name = '$field_value' WHERE item_id='$item_id' AND patient_id='$registration_id' ";
    ...
    }else
    {
      ...
      $insert_eve_opening = "INSERT INTO tbl_icu_form_two_even_opening(patient_id,item_id,$fields_name,date_time) VALUES('$registration_id','$item_id','$field_value',NOW())";
