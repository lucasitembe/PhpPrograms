...
if (isset($_POST['field_name']) && isset($_POST['registration_id'])){
  ...
  $select_patient_from_icu_from_five = "SELECT * FROM tbl_icu_form_five WHERE patient_id = '$registration_id' AND date_time != NOW()";

  $insert_result = mysqli_query($conn,$select_patient_from_icu_from_five);
  $num = mysqli_num_rows($insert_result);

  if ($num > 0) {
      $inset_icu_form_five = "UPDATE tbl_icu_form_five SET $fiels_name = '$field_value' WHERE patient_id='$registration_id'";
    ...
    $insert_patient_detailes_in_icu_form_five = "INSERT INTO tbl_icu_form_five(patient_id,date_time) VALUES('$registration_id',NOW())";
