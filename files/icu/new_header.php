...
<div class="container-fluid pt-3">
            ...
            <?php if (isset($_SESSION['userinfo'])) { ?>
                <a href='./../logout.php' class='btn btn-primary' id="ConfirmLogout" onclick='return ConfirmLogout()'>LOGOUT</a>
            <?php } ?>
            <?php if (isset($_SESSION['userinfo']) && mysqli_num_rows($chktosee) == 1) { ?>
                <a type="button" class='btn btn-primary' id="" onclick='checkout()'>CHECK OUT</a>
            <?php } ?>
            <?php
            if (isset($_SESSION['userinfo'])) {
                if (isset($_SESSION['userinfo']['Setup_And_Configuration'])) {
                    ?>
                    <a href='../index.php?Bashboard=BashboardThisPage' class='btn btn-primary'>
                        DASHBOARD
                    </a>
                <?php }
            }  ?>
            ...
            if (isset($_SESSION['userinfo'])) {
                if (isset($_SESSION['userinfo']['Setup_And_Configuration'])) {
                    if ($_SESSION['userinfo']['Setup_And_Configuration'] == 'yes') {
                        ?>
                        <a href='../setupandconfiguration.php?BackToSetupAndConfiguration=BackTosetupAndConfigurationThisPage' class='btn btn-primary'>
                            SETUP & CONFIG
                        </a>
                    <?php }
                }
