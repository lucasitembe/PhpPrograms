		...
		preg.Member_Number, ilc.Transaction_Type, ilc.status, sd.Sub_Department_Name,ilc.Sub_Department_ID from
		    tbl_payment_cache pc, tbl_item_list_cache ilc, tbl_patient_registration preg, tbl_sub_department sd where
			    ...
			    preg.registration_id = pc.registration_id and
					...
					ilc.status ='Pending' and
                                           ilc.Procedure_Location='Me' and
                                            ilc.Check_In_Type = 'Procedure' and
                                            ilc.Consultant_ID='".$_SESSION['userinfo']['Employee_ID']."'
                                            AND ilc.Transaction_Date_And_Time between '$Date_From' and '$Date_To'    
...
//        $status="paid";
	...
	$age .= $diff->d." Days";
        ...
        //echo "<td><a href='pendingprocedurepatientinfo.php?section=Doctorlist&Registration_ID=".$row['Registration_ID']."&Transaction_Type=".$row['Transaction_Type']."&Payment_Cache_ID=".$row['Payment_Cache_ID']."&NR=True&ProcedureWorks=ProcedureWorksThisPage'  >".$row['Transaction_Date_And_Time']."</td>";
	
        }else{
            if($row['typeconsultant']=="OTHERS_CONSULT"){ 
                 $data .= "<td><a href='pendingprocedurepatientinfo.php?section=Doctorlist&typeconsultant=".$row['typeconsultant']."&Registration_ID=".$row['Registration_ID']."&Transaction_Type=".$row['Transaction_Type']."&Payment_Cache_ID=".$row['Payment_Cache_ID']."&NR=True&Billing_Type=".$row['Billing_Type']."&Sub_Department_ID=".$Sub_Department_ID."&statusMsg=".$statusMsg."&ProcedureWorks=ProcedureWorksThisPage' target='_parent' style='text-decoration: none;'>".ucwords(strtolower($row['Patient_Name']))."</a></td>";
...
/*                  $data .= "<td><a href='pendingprocedurepatientinfo.php?section=Doctorlist&typeconsultant=".$row['typeconsultant']."&Registration_ID=".$row['Registration_ID']."&Transaction_Type=".$row['Transaction_Type']."&Payment_Cache_ID=".$row['Payment_Cache_ID']."&NR=True&Billing_Type=".$row['Billing_Type']."&Sub_Department_ID=".$Sub_Department_ID."&statusMsg=".$statusMsg."&ProcedureWorks=ProcedureWorksThisPage' target='_parent' style='text-decoration: none;'>".$row['Member_Number']."</a></td>"; */
	...
	$temp++;
       ...
       $data.='</table></center>';
    ?>
<a href='doctorprocedurelist.php' class='art-button-green'>
        BACK
    </a>
