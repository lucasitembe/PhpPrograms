...
<?php
  if((isset($_GET['Section']) && $_GET['Section'] != 'Doctor') || (isset($_GET['Section']) && $_GET['Section'] != 'DoctorLab') || (isset($_GET['Section']) && $_GET['Section'] != 'DoctorRad')){
  }else{
   if(isset($_SESSION['userinfo'])){
        if($_SESSION['userinfo']['Nurse_Station_Works'] == 'yes'){ 
?>
...
<?php
   if(isset($_SESSION['userinfo'])){
        if($_SESSION['userinfo']['Nurse_Station_Works'] == 'yes'){ 
?>
    <a href='nurse_station_report.php' class='art-button-green'>
...
//$Registration_ID='313131';
 ...
 if(isset($_GET['Registration_ID']) && isset($_GET['Nurse_DateTime'])  ){ 
	...
	$select_Patients = mysqli_query($conn,"SELECT 
                                                    ...
                                                    Allegies,Special_Condition,bmi
								from 
...
if(isset($_GET['Section']) && $_GET['Section'] == 'Doctor'){
			...
			echo "<a href='nurse.php?Section=DoctorLab&Registration_ID=".$_GET['Registration_ID']."&Patient_Payment_ID=".$_GET['Patient_Payment_ID']."&Patient_Payment_Item_List_ID=".$_GET['Patient_Payment_Item_List_ID']."' class='art-button-green'>BACK</a>";
			...
			echo "<a href='Patientfile_Record_Detail.php?Section=DoctorsPerformancePateintSummary&Registration_ID=".$_GET['Registration_ID']."&Patient_Payment_ID=".$_GET['Patient_Payment_ID']."&Patient_Payment_Item_List_ID=".$_GET['Patient_Payment_Item_List_ID']."&PatientType=&listtype=FromRec&Doctor=yes' class='art-button-green'>BACK</a>";
			...
			$section="Section=DoctorsPerformancePateintSummary&Patient_Payment_ID=".$_GET['Patient_Payment_ID']."&Patient_Payment_Item_List_ID=".$_GET['Patient_Payment_Item_List_ID']."&";
                    ...
                    $section="Section=ManagementPatient&consultation_ID=".$row['consultation_ID']."&Registration_ID=".$_GET['Registration_ID']."&Employee_ID=".$_GET['Employee_ID']."&Date_From=".$_GET['Date_From']."&Date_To=".$_GET['Date_To']."&DoctorsPerformanceDetailThisPage=DoctorsPerformanceDetailsThisPage&";
                }else{
		    if(isset($_GET['section']) && strtolower($_GET['section']) == 'patient'){
			if(isset($_GET['Registration_ID'])){ $Registration_ID = $_GET['Registration_ID']; }
			else{ $Registration_ID = 0; }
			echo "<a href='Patientfile_Record_Detail.php?Registration_ID=".$Registration_ID."&section=Patient&PatientFile=PatientFileThisForm' class='art-button-green'>BACK</a>";
		    }else{
			echo "<a href='nursecommunicationpage.php?Bashboard=BashboardThisPage' class='art-button-green'>BACK</a>";
		    }
	     }
	?>	
<br/><br/>
...
?>
	<td>
		<div class="nurse_tabs" style="width:100px;height:30px;padding:0px;">
			<a href='chartgraph.php?<?php echo $section?>Registration_ID=<?php echo $_GET['Registration_ID']; ?>&Vital_ID=<?php echo $row['Vital_ID']; ?>&
	...
	<div><span style="font-size:14px;margin-left:4px;margin-right:2px;margin-top:5px;"><b>BMI</b></span>
			...
			disabled='disabled' style="width:100px">
				<span class="nurse_tabs" style="width:120px;height:30px;">	
					<a href='chartbmi.php?<?php echo $section?>Registration_ID=<?php echo $_GET['Registration_ID']; ?>&bmi=<?php echo $bmi; ?>&Nurse_DateTime=<?php echo filter_input(INPUT_GET,'Nurse_DateTime'); ?>'  class='art-button-green'>history</a>
				</span>
			</span>
	</div>
		<?php
	...
	<div><span style="font-size:14px;margin-left:4px;margin-right:2px;margin-top:10px;"><b>BMI</b></span>
		<span><input type="text" name="bmi" id="bmi" value="<?php echo$bmi; ?>" style="width:100px">
			<span class="nurse_tabs" style="width:100px;height:30px;">	
					<a href='#??<?php echo $section?>Registration_ID=<?php echo $Registration_ID; ?>&bmi=<?php echo $row['bmi']; ?>'  class='art-button-green'>history</a>
			</span>
		</span>
	</div>
	<?php
