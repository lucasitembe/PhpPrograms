...
<a href='opticalpendingtransactions.php?Section=Home&OpticalPendingTransactions=OpticalPendingTransactionsThisPage' class='art-button-green'>BACK</a>
...
<?php
    ...
    $select = mysqli_query($conn,"select Patient_Payment_Item_List_ID from tbl_consultation where consultation_ID = '$consultation_ID'") or die(mysqli_error($conn));
    ...
    $select = mysqli_query($conn,"select Folio_Number, Claim_Form_Number from tbl_patient_payments pp, tbl_patient_payment_item_list ppl where
...
<br/><br/>
<fieldset>  
    <legend align="right"><b>Optical ~ Pending Transaction</b></legend>
    <center>
        <table width=100%>
            <tr>
            ...
            <tr>
                ...
                <td><input type='text' name='Phone_Number' id='Phone_Number1' disabled='disabled' value='<?php echo $Phone_Number; ?>'></td>
            ...
            <tr> 
                ...
                <?php
                    //select consultant
                    if(strtolower($Session) == 'credit'){
                        $slct = mysqli_query($conn,"select Employee_Name, ilc.Consultant_ID from tbl_employee emp, tbl_item_list_cache ilc where
                        ...
                        if($slct > 0){
                            while ($dt = mysqli_fetch_array($slct)) {
                                $Doctor_Name = $dt['Employee_Name'];
                                $Doctor_ID = $dt['Consultant_ID'];
                            }
                        }else{
                            $Doctor_Name = '';
                            $Doctor_ID = 0;
                        ...
                        $slct = mysqli_query($conn,"select emp.Employee_ID, emp.Employee_Name 
                                                from tbl_employee emp, tbl_consultation c where 
...
<fieldset>
    <table width=100% id="Process_Buttons_Area">
        <?php
            //return all items to cache table
            if(strtolower($Session) == 'cash'){
                $select = mysqli_query($conn,"select * from tbl_optical_items_list_cache where consultation_ID = '$consultation_ID' and Registration_ID = '$Registration_ID'") or die(mysqli_error($conn));
                ...
                if($num_rows > 0 ){
                    ...
                    while ($dt = mysqli_fetch_array($select)) {
                        $Item_Cache_ID = $dt['Item_Cache_ID'];
                        $insert_sql = mysqli_query($conn,"insert into tbl_departmental_items_list_cache(
                                                Claim_Form_Number, Billing_Type, Sponsor_Name,
                                                Sponsor_ID, Item_ID, Price,Quantity,
                                                Consultant_ID, Employee_ID, Registration_ID,Comment,Sub_Department_ID,Type_Of_Check_In)
                                                
                                            values('".$dt['Claim_Form_Number']."','Outpatient Cash','".$dt['Sponsor_Name']."',
                                                '".$dt['Sponsor_ID']."','".$dt['Item_ID']."','".$dt['Price']."','".$dt['Quantity']."',
                                                '".$dt['Consultant_ID']."','".$Employee_ID."','".$Registration_ID."','".$dt['Comment']."','".$dt['Sub_Department_ID']."','Optical')");
                    }
                }
            }
                $select_Transaction_Items = mysqli_query($conn,"select Billing_Type from tbl_departmental_items_list_cache alc
                        ...
                        <fieldset id="Picked_Items_Fieldset"  style='overflow-y: scroll; height: 190px;'>
                            <center>
                                <table width =100% border=0>
                                    ...
                                    $total = 0;
                                    if(strtolower($Session) == 'credit'){
                                        $select_Transaction_Items = mysqli_query($conn,"select itm.Product_Name, ilc.Quantity, ilc.Edited_Quantity, ilc.Price, ilc.Discount, ilc.Sub_Department_ID, ilc.Doctor_Comment as Comment,
                                                                                    ilc.Payment_Item_Cache_List_ID, ilc.Check_In_Type, ilc.Payment_Item_Cache_List_ID, ilc.ePayment_Status
...
<script>
   function getItemsList(Item_Category_ID){
      ...
      myObject.open('GET','Optical_Get_List_Of_Departmental_Items_List.php?Item_Category_ID='+Item_Category_ID+'&Guarantor_Name='+Guarantor_Name+'&Type_Of_Check_In='+Type_Of_Check_In,true);
      myObject.send();
   }
</script>
        ...
        if(sms == true){
            ...
            myObject.onreadystatechange = function (){
                ...
                if (myObject.readyState == 4) {
                    ...
                    if(feedback == 'yes'){
                        document.location = 'Optical_Re_Send_To_Cachier.php?Registration_ID='+Registration_ID+'&consultation_ID='+consultation_ID;
                    }else{
