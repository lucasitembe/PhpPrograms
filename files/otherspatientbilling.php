...
<fieldset>  
            <legend align=right><b>DEPARTMENT PAYMENTS ~ OTHERS</b></legend>
        <center>
            <table width=100%> 
                <tr> 
                    <td>
...
<fieldset style='overflow-y:scroll;'>  
		...
		<tr>
			...
			<div id="patientItemsList" style='height:230px;'>
                            <center><b>List of Items </b></center>
                            <?php include "Patient_Billing_Others_Iframe.php";?>
                        </div>
			<!--
...
<script>
    function Make_Payment_Laboratory() {
	 var itemzero = document.getElementById('Item_price_zero').value;
            if (itemzero != '' ){
            alert(' \nYou have item with zero price.Receipt for item with zero price will not be created.');
            }
             ...
             var Confirm_Message = confirm("Are you sure you want to perform transaction?");
                if (Confirm_Message == true) {
                     if (itemzero != '' ){
                         alert('seen');
                    document.location = 'Patient_Billing_Others_Page.php?itemzero='+itemzero+'&Transaction_Type=<?php echo $Transaction_Type; ?>&Payment_Cache_ID=<?php echo $Payment_Cache_ID; ?>&Sub_Department_ID=<?php echo $Sub_Department_ID; ?>';
                    } else{
					 alert('seen2');
                     document.location = 'Patient_Billing_Others_Page.php?Payment_Cache_ID=<?php echo $Payment_Cache_ID ?>&Transaction_Type=<?php echo $Transaction_Type ?>&Sub_Department_ID=<?php echo $Sub_Department_ID?>&Registration_ID=<?php echo $Registration_ID ?>&Billing_Type=<?php echo $Temp_Billing_Type2 ?>';
                 }
                }
        var Confirm_Message = confirm("Are you sure you want to perform transaction?");
