...
getPatientFileSummery($consultation_ID,$Registration_ID,$Check_In_ID);
}
function getPatientFileSummery($consultation_ID,$Registration_ID,$Check_In_ID) {
    global $conn;
...
$select_patients = "SELECT 0 as Patient_Payment_Item_List_ID, 0 as Patient_Payment_ID,Employee_Name, c.consultation_ID,ch.employee_ID, ch.cons_hist_Date,e.Employee_Title,e.Phone_Number, consultation_histry_ID,ch.course_of_injuries, c.Registration_ID FROM tbl_consultation c,tbl_consultation_history ch, tbl_patient_payment_item_list ppl,tbl_employee e, tbl_patient_payments pp, tbl_check_in_details cd WHERE c.Patient_Payment_Item_List_ID = ppl.Patient_Payment_Item_List_ID and c.consultation_ID=ch.consultation_ID and e.Employee_ID=ch.employee_ID and cd.Check_In_ID = pp.Check_In_ID AND pp.Check_In_ID='$Check_In_ID' and pp.Patient_Payment_ID=ppl.Patient_Payment_ID AND c.Registration_ID='$Registration_ID' GROUP BY Employee_Name, c.consultation_ID";
...
$Employee_Title = $doctorsInfo['Employee_Title'];
$Employee_Title = ucwords(strtolower($Employee_Title));
$Phone_Number = $doctorsInfo['Phone_Number'];
...
$select_payment_cache = "SELECT *,ilc.Status FROM tbl_payment_cache pc,tbl_item_list_cache ilc,tbl_items i
				...
				AND i.Item_ID = ilc.Item_ID AND ilc.Consultant_ID=$doctorsID ";
...
if (!empty($past_medical_history)) {
    $data .= '<tr>
            <td  width="25%"  style="text-align:right;">Past Medical History</td>
            <td>
               ' . $past_medical_history . '
            </td>
        </tr>';
...
$data .= '  </table>
        ...
        <div id="observation">
            <table width=100% style="border: 0px;">';
            if (!empty($Gynocological_history)) {
                $data .= ' <tr>
                                <td  width="25%"  style="text-align:right;">Gynocological History</td>
                                </td>
                                <td>
                                    '.$Gynocological_history.'
                                </td>
            ...
            if (!empty($family_social_history)) {
                $data .= '<tr>
                            <td  width="25%"  style="text-align:right;">Family Social History</td>
                            <td>
                                '.$family_social_history .'
                            </td>
                        </tr>';
            ...
            if (!empty($local_examination)) {
                $data .= '<tr>
                        <td  width="25%"  style="text-align:right;">Local Examination</td>
                        <td>
                           ' . $local_examination . '
                        </td>
                    </tr>';
...
//INPATIENT DITAILS
$result = mysqli_query($conn,"SELECT Admission_Date_Time,Admission_Employee_ID,Employee_Name,Admission_Status,Hospital_Ward_Name,ad.Bed_Name,Kin_Name,ToBe_Admitted_Reason FROM tbl_admission ad    INNER JOIN tbl_check_in_details cd ON cd.Admission_ID = ad.Admision_ID     JOIN tbl_hospital_ward hw ON hw.Hospital_Ward_ID=ad.Hospital_Ward_ID    JOIN tbl_employee e ON e.Employee_ID=ad.Admission_Employee_ID    WHERE consultation_ID = '".$consultation_ID ."' AND ad.Registration_ID='".$Registration_ID ."' AND cd.Admit_Status='admitted'   ") or die(mysqli_error($conn));
...
$result_disc = mysqli_query($conn,"SELECT Discharge_Date_Time,Employee_Name,Discharge_Reason,ad.Admission_Status FROM tbl_admission ad
...
$rsDoc = mysqli_query($conn,"SELECT *, TIMESTAMPDIFF(DAY,Ward_Round_Date_And_Time,NOW()) AS days_past  FROM tbl_ward_round wr JOIN tbl_employee e ON wr.employee_ID=e.employee_ID  WHERE wr.consultation_ID='$consultation_ID' AND wr.Registration_ID=$Registration_ID  ORDER BY wr.Round_ID DESC LIMIT 50") or die(mysqli_error($conn));
// =====AND Process_Status ='served'
