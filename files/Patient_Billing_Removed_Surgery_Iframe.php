<?php
   ...
   // echo $Transaction_Status_Title; 
	...
	if($no > 0){
     ...
     //		        
		...
		echo "<td><input type='checkbox' class='paymentsugery' id='".$row['Payment_Item_Cache_List_ID']."'></td>";
	    ...
	    if($No_Of_Items > 0){
		 $dataAmount= "<td style='text-align: right;'><b> TOTAL : ".number_format($total)."</b>
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<button type='button' class='makesugeryPayment art-button'>Make Payment</button> 
                        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                       </td>";
	    }else{
    ...
    $(document).on('click','.makesugeryPayment',function(){
     var locate=$(location).attr('href');
     var datastring='';
     var num=1;
     var n = $(".paymentsugery:checked").length;
    if(n>0){
    if(confirm("Are you sure you want to perform this transaction ?")){
    $('.paymentsugery').each(function(){
    var id=$(this).attr('id');
  ...
  // alert(datastring);
        ...
        $.ajax({
        type:'POST', 
        url:'Patient_Billing_Removed_Surgery_Iframe.php',
