...
<?php
    if(isset($_SESSION['Reagent_Grn_Open_Balance_ID'])){
        ...
        $select_data = mysqli_query($conn,"select emp.Employee_Name, gob.Grn_Open_Balance_Description, gob.Created_Date_Time, sd.Sub_Department_Name from
                                        tbl_employee emp, tbl_reagent_grn_open_balance gob, tbl_sub_department sd where
...
<script>
	...
	if (Supervisor_Password != null && Supervisor_Password != '' && Supervisor_Username != null && Supervisor_Username != '') {
	    ...
	    myObjectAuthentication.open('GET','Reagent_Authentication_Submit_Open_Balance_Items.php?Supervisor_Username='+Supervisor_Username+'&Supervisor_Password='+Supervisor_Password,true);
...
<script>
	...
	myObjectSubmitOpenBalance.onreadystatechange = function (){
		...
		if (Feedback == 'yes') {
		    ...
		    document.location = "./index.php?Bashboard=BashboardThisPage";
		}else{
		    alert("PROCESS FAIL!! PLEASE TRY AGAIN.");
		}
	    }
	}; //specify name of function that will handle server response........
	myObjectSubmitOpenBalance.open('GET','Reagent_Submit_Open_Balance_Items.php',true);
...
<script>
	...
	myObjectUpdateDescription.open('GET','Reagent_Grn_Open_Balance_Update_Description.php?Grn_Description='+Grn_Description,true);
                                    ...
                                    <table width=100%>
                                        <tr>
                                            ...
                                            <td style='text-align: center; width: 10%;'>
                                                <input type='button' name='Submit' id='Submit' value='SAVE' class='art-button-green' onclick='Authenticate_Submit_Open_Balance_Items()'>
                                            </td>
                                        </tr>
                                    </table>
                                </fieldset>
