...
<fieldset style='overflow-y: scroll; height: 400px;' id='Items_Fieldset'>
    <legend align=right><b><?php if(isset($_SESSION['Laboratory_ID'])){ echo $Sub_Department_Name; }?>, Previous Consumptions prepared by : <?php echo $Employee_Name; ?></b></legend>
<table width="100%">
    <tr>
...
<?php
    $select = mysqli_query($conn,"select c.Consumption_ID, c.Consumption_Description, emp.Employee_Name, sd.Sub_Department_Name
                            ...
                            Consumption_Status = 'saved' order by Consumption_ID") or die(mysqli_error($conn));
    $num = mysqli_num_rows($select);
    if($num > 0){
        while ($data = mysqli_fetch_array($select)) {
?>
            <tr>
                ...
                <td width="">
                    <a href="" class="art-button-green">Process</a>
                </td>
            </tr>
<?php
        }
