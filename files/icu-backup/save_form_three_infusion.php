...
if(isset($_POST['field_name']) && isset($_POST['registration_id'])){
  ...
  // }
    ...
    if($num > 0){
      $update_medication = "UPDATE tbl_icu_formthree_infusion SET medication = '$field_value' WHERE medic_id='$medic_id' AND DATE(date_time) = CURDATE()";

      $update_infusion_hours = "UPDATE tbl_icu_form_three_infusion_hours SET $fiels_name = '$field_value' WHERE medic_id='$medic_id' AND patient_id = '$registration_id'";

      if($medication_updated  = mysqli_query($conn,$update_medication)) {
        echo "successfully Updated";
      }
      else if ($update_infusion_hrs = mysqli_error($conn$update_infusion_hours)) {
        echo "successfully updated";
      }
      else {
        echo mysqli_error($conn);
      ...
      $insert_medication_hours = "INSERT INTO tbl_icu_formthree_infusion(patient_id,medic_id,medication,date_time) VALUES('$registration_id','$medic_id','$field_value',NOW())";
      ...
      if($result_insert_medication = mysqli_query($conn,$insert_medication_hours)) {
        ...
        // echo $last_id;
        $insert_into_hour = "INSERT INTO tbl_icu_form_three_infusion_hours(medication_id,patient_id,date_time) VALUES('$medic_id','$registration_id',NOW())";
