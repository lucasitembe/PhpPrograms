<?php
 ...
 $diagnosisy_type_displ="";
                        if($diagnosis_type_to_display=="provisional_diagnosis"){
                           $diagnosisy_type_displ="PROVISIONAL DIAGNOSIS";
                        }else if($diagnosis_type_to_display=="diferential_diagnosis"){
                             $diagnosisy_type_displ="DIFFERENTIAL DIAGNOSIS";
                        }else if($diagnosis_type_to_display=="diagnosis"){
                           $diagnosisy_type_displ="FINAL DIAGNOSIS";
                        }else{
                             $diagnosisy_type_displ="DIAGNOSIS";
                        }
                    ?>
<?php
$htm ="<style>
...
$htm.="<br/>
    <table width=100%>
        ...
        <tr style='background:#cccccc'>
            ...
            <td><b>DISCHARGE</b></td>
            <td><b>DIED</b></td>
            <td><b>$diagnosisy_type_displ</b></td>
            <td><b>REMARKS</b></td>
...
?>
        <?php
            ...
            if($Sponsor_ID == 0){
                if(isset($_GET['Search_Value'])){
                    $select = mysqli_query($conn,"select sp.Guarantor_Name, pr.Registration_ID,ci.Check_In_Date_And_Time, pr.Patient_Name,pr.Date_Of_Birth, pr.Gender, pr.Tribe,pr.Region, pr.District, pr.village, pr.Phone_Number, ci.Type_Of_Check_In, ci.Check_In_Date_And_Time, emp.Employee_Name, ci.Check_In_ID
                                            from tbl_check_in_details chd,tbl_check_in ci, tbl_employee emp, tbl_patient_registration pr, tbl_patient_payments pp, tbl_sponsor sp where
                                            pr.Registration_ID = ci.Registration_ID and
                                            ci.Employee_ID = emp.Employee_ID and
                                            sp.Sponsor_ID = pr.Sponsor_ID and
                                            ci.Check_In_ID=chd.Check_In_ID and
                                            pr.Registration_ID=chd.Registration_ID and
                    ...
                    $select = mysqli_query($conn,"select sp.Guarantor_Name,ci.Check_In_Date_And_Time, pr.Registration_ID, pr.Patient_Name,pr.Date_Of_Birth, pr.Gender, pr.Tribe,pr.Region, pr.District, pr.village, pr.Phone_Number, ci.Type_Of_Check_In, ci.Check_In_Date_And_Time, emp.Employee_Name, ci.Check_In_ID
                                            from tbl_check_in_details chd,tbl_check_in ci, tbl_employee emp, tbl_patient_registration pr, tbl_patient_payments pp, tbl_sponsor sp where
                                            pr.Registration_ID = ci.Registration_ID and
                                            ci.Employee_ID = emp.Employee_ID and
                                            sp.Sponsor_ID = pr.Sponsor_ID and
                ...
                if(isset($_GET['Search_Value'])){
                    ...
                    $select = mysqli_query($conn,"select sp.Guarantor_Name,ci.Check_In_Date_And_Time, pr.Registration_ID, pr.Patient_Name,pr.Date_Of_Birth, pr.Gender, pr.Tribe, pr.Region, pr.District, pr.village, pr.Phone_Number, ci.Type_Of_Check_In, ci.Check_In_Date_And_Time, emp.Employee_Name, ci.Check_In_ID
                                            ...
                                            pr.Registration_ID=chd.Registration_ID and
                                            sp.Sponsor_ID = pr.Sponsor_ID and
                                            pr.Sponsor_ID = '$Sponsor_ID' and $pf3 $alive_diceased $visit_type $patient_gender $Type_Of_Check_In $Search_Patient_number  $rank $military_unit $patient_type $Admit_Status $age_filter
