...
<center><b>Physiotherapy Service List</b></center><link rel="stylesheet" type="text/css" href="css/jquery.datetimepicker.css"/>
...
.ie6 .art-post .art-layout-cell {border:none !important; padding:0 !important; }
    ...
    <script type='text/javascript'>
	    ...
	    mm.open('GET','physiotherapyQuantityUpdate.php?Payment_Item_Cache_List_ID='+Payment_Item_Cache_List_ID+'&Quantity='+Quantity,true);
	    mm.send();
        }
        function AJAXP() {
...
if($no > 0){
	...
	<td style='text-align:right;'>
		...
		<input type='text' value='<?php echo $Quantity;?>' readonly='readonly onkeyup="physiotherapyQuantityUpdate2('<?php echo $row["Payment_Item_Cache_List_ID"];?>',this.value)" style='text-align: center;' size=13>
	    <?php }else{ $Quantity = $row['Edited_Quantity']; ?>
		<input type='text' value='<?php echo $Quantity; ?>' readonly='readonly onkeyup="pharmacyQuantityUpdate2('<?php echo $row["Payment_Item_Cache_List_ID"];?>',this.value)" style='text-align: center;' size=13>
	    <?php } ?>
	</td>
	<?php
	...
	<td style='text-align:right;'>
	    <?php if($row['Edited_Quantity'] == 0){  $Quantity = $row['Quantity']; ?>
		<input type='text' value='<?php echo $Quantity;?>' onkeyup="physiotherapyQuantityUpdate('<?php echo $row["Payment_Item_Cache_List_ID"];?>',this.value)" style='text-align: center;' size=13>
	    <?php }else{ $Quantity = $row['Edited_Quantity']; ?>
		<input type='text' value='<?php echo $Quantity; ?>' onkeyup="physiotherapyQuantityUpdate('<?php echo $row["Payment_Item_Cache_List_ID"];?>',this.value)" style='text-align: center;' size=13>
	    <?php } ?>
	</td>
	<?php
	    echo "<td><input type='text' name='Sub_Total' id='Sub_Total' readonly='readonly' value='".number_format(($row['Price'] - $row['Discount']) * $Quantity)."' style='text-align:right;'></td>";	
	    echo "<td style='text-align: right;' width=8%><a href='RemovePhysiotherapyItem.php?Payment_Item_Cache_List_ID=".$row['Payment_Item_Cache_List_ID']."&Payment_Cache_ID=".$row['Payment_Cache_ID']."' class='art-button-green' target='_Parent'>Remove</a></td>";
	    $total = $total + (($row['Price'] - $row['Discount']) * $Quantity);
	    $temp++;
	}   echo "</tr>";
	...
	if($No_Of_Items > 0){
	    echo "<tr><td colspan=5 style='text-align: right;'><b> TOTAL : ".number_format($total)."</b></td><td colspan=2 style='text-align: right;'>
		    <a href='physiotherapyitems_Iframe2.php?Transaction_Type=".$Transaction_Type."&Payment_Cache_ID=".$Payment_Cache_ID."' class = 'art-button'>View Removed Services</a>
			</td></tr>";
	}else{
	    echo "<tr><td colspan=6 style='text-align: right;'><b> TOTAL : ".number_format($total)."</b></td><td></td></tr>";
	}
	...
	if($no > 0){
		...
		if($no > 0){
		    ...
		    <td style='text-align:right;'>
			<?php if($row['Edited_Quantity'] == 0){  $Quantity = $row['Quantity']; ?>
			    <input type='text' value='<?php echo $Quantity;?>' onkeyup="pharmacyQuantityUpdate('<?php echo $row["Payment_Item_Cache_List_ID"];?>',this.value)" style='text-align: center;' size=13>
			<?php }else{ $Quantity = $row['Edited_Quantity']; ?>
			    <input type='text' value='<?php echo $Quantity; ?>' onkeyup="pharmacyQuantityUpdate('<?php echo $row["Payment_Item_Cache_List_ID"];?>',this.value)" style='text-align: center;' size=13>
			<?php } ?>
		    </td>
