<?php
    ...
    if($Item_ID != 0 && $Item_ID != '' && $Store_Need != '' && $Quantity != 0 && $Store_Issue != 0){
        ...
        $select_details = mysqli_query($conn,"select * from tbl_requisition
                                        where Store_Need = '$Store_Need' and
        ...
        $num = mysqli_num_rows($select_details);
            if($num > 0){
                ...
                $_SESSION['General_Requisition_ID'] = $Requisition_ID;
                $insert_data2 = mysqli_query($conn,"insert into tbl_requisition_items(
                                                        Requisition_ID,Quantity_Required,Item_Remark,Item_ID,Container_Qty,Items_Qty)
                                                            
                                                    values('$Requisition_ID','$Quantity','$Item_Remark','$Item_ID','$Cont_Quantity','$Items_Quantity')") or die(mysqli_error($conn));
                        if($insert_data2){
                ...
                if($insert_data){
                    ...
                    if($no > 0 ){
                        while($row = mysqli_fetch_array($select_Requisition_ID)){
                            $Requisition_ID = $row['Requisition_ID'];
                            $_SESSION['General_Requisition_ID'] = $row['Requisition_ID'];
                        }
                        $_SESSION['General_Requisition_ID'] = $Requisition_ID;
                    }else{
                        $Requisition_ID = 0;
                    ...
                    if($Requisition_ID != 0){
                        ...
                        if($insert_data2){
                                ...
                                $select_Transaction_Items = mysqli_query($conn,"select itm.Product_Name, rqi.Quantity_Required, rqi.Item_Remark, rqi.Container_Qty, rqi.Items_Qty
                                                                        from tbl_requisition_items rqi, tbl_items itm where
                                                                            itm.Item_ID = rqi.Item_ID and
                                                                                rqi.Requisition_ID ='$Requisition_ID'") or die(mysqli_error($conn)); 
