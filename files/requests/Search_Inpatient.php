...
//              }else{
   ...
   $temp = 1;
    ...
    $filter_Date='AND  DATE(Payment_Date_And_Time)=DATE(NOW()) ';
    $navFilter= 'AND  DATE(pc.Payment_Date_And_Time)=DATE(NOW())';
        ...
        $filter_Date=" AND  Payment_Date_And_Time BETWEEN '". $Date_From."' AND '".$Date_To."'";
        ...
        if(!empty($Search_Patient_by_number)){
             $filter .=" AND pc.Registration_ID = '$Search_Patient_by_number'";
        }

        if(isset($_GET['patient_priority_status'])){
          if($_GET['patient_priority_status']=="urgent_patient"){
            $Filter_Priority = " AND Priority = 'Urgent'";
          }else{
            $Filter_Priority = '';
          }
      }else{
...
$select_Filtered_Patients="SELECT pr.Patient_Name,pr.Date_Of_Birth,pr.Gender,pr.Phone_Number,pr.Registration_ID as registration_number,'cache' as Status_From,pc.Registration_ID,pc.Sponsor_Name,pc.Receipt_Date as Required_Date,
                                           pc.Payment_Cache_ID as payment_id FROM tbl_patient_registration pr,tbl_payment_cache pc  WHERE  pr.Registration_ID=pc.Registration_ID $filter GROUP BY payment_id ORDER BY payment_id ASC  LIMIT 100";
   ...
   $result=mysqli_query($conn,$select_Filtered_Patients)or die(mysqli_error($conn));
       //date manipulation to get the patient age
        $Today_Date = mysqli_query($conn,"select now() as today");
            ...
            $num_rows=  mysqli_num_rows($result);
            if($num_rows>0){
              $urgent_patient = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(Registration_ID) AS urgent_patient FROM tbl_payment_cache WHERE Payment_Cache_ID IN(SELECT Payment_Cache_ID FROM tbl_item_list_cache WHERE Check_In_Type ='Laboratory' AND Status IN('active','paid') AND removing_status='No') $filter_Date GROUP BY Payment_Cache_ID"))['urgent_patient'];
                 ...
                 while($row = mysqli_fetch_array($result)){
                   ...
                   $other_details=  mysqli_query($conn,"SELECT Priority,Consultant,Transaction_Date_And_Time FROM tbl_item_list_cache WHERE Payment_Cache_ID='$Patient_Payment_ID' AND Check_In_Type ='Laboratory' AND (Status='active' OR Status='paid') AND removing_status='No' $Filter_Priority LIMIT 1") or die(mysqli_error($conn));  
                      ...
                      if($Priority=='Urgent'){
                         ...
                         $colo_urg="";
                          ...
                          if($row['Status'] =='Sample Collected'){
                                   ...
                                   <td>".$temp."</td>
                                   <td><a class='viewDetails' href='laboratory_sample_collection_details.php?Status_From=".$row['Status_From']."&patient_id=".$Registration_ID."&payment_id=".$row['payment_id']."&Required_Date=".$row['Required_Date']."&Date_From=".$Date_From."&Date_To=".$Date_To."&Sponsor=".$Sponsor."&subcategory_ID=".$subcategory_ID."$for_back_link' target='_parent' style='text-decoration: none;'>$priorities</a></td>
                                ...
                                echo "<td><a class='viewDetails'  href='laboratory_sample_collection_details.php?Status_From=".$row['Status_From']."&patient_id=".$Registration_ID."&payment_id=".$row['payment_id']."&Required_Date=".$row['Required_Date']."&Date_From=".$Date_From."&Date_To=".$Date_To."&Sponsor=".$Sponsor."&subcategory_ID=".$subcategory_ID."$for_back_link' target='_parent' style='text-decoration: none;$colo_urg'>".$Registration_ID."</a></td>";
...
<script type="text/javascript"> 
    function test(){
        ...
        $('#number_of_urgent_patien').html("<input type='button' class='art-button-green' style='background:red; font-weight: bold; border-radius: 10px; font-size: 15px;' onclick='filter_urgent()' value='"+num+" Urgent Patient'>");
    }
    test();
     //View button here
