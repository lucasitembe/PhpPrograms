    ...
    <center>
        ...
        while ($rows = mysqli_fetch_array($sql)) {
            ...
            <table border="1"  style="width:100%;" class="hiv_table" bgcolor="white">
                ...
                <?php
                $sql1 = mysqli_query($conn,"SELECT * FROM tbl_tests_specimen join tbl_laboratory_specimen on ref_specimen_ID=Specimen_ID WHERE tests_item_ID='" . $item_id . "' ");
                ?>
                           ...
                           // die("SELECT * FROM tbl_specimen_results WHERE payment_item_ID='" . $_POST['val'] . "' AND Specimen_ID='" . $rows1['ref_specimen_ID'] . "'");
                            ...
                            if ($num > 0) {
                                while ($rows1 = mysqli_fetch_array($sql1)) {
                                    $barcode = mysqli_query($conn,"SELECT * FROM tbl_specimen_results WHERE payment_item_ID='" . $_POST['val'] . "' AND Specimen_ID='" . $rows1['ref_specimen_ID'] . "'");
                                    $checkCollectionStatus = mysqli_fetch_assoc($barcode);
...
//                                            echo $CheckAvailability;
                                            ...
                                            <td style="border:1px solid #ccc;">
                                                <?php
                                                // $barcodeTxt='a';
                                                // $row7 = mysqli_fetch_assoc($barcode);
