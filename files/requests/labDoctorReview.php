    ...
    $filter = filter_input(INPUT_GET, 'filter');
     ...
     $validateQuery = "SELECT payment_item_ID FROM tbl_tests_parameters_results JOIN tbl_test_results ON test_result_ID=ref_test_result_ID WHERE Validated='Yes' GROUP BY test_result_ID";
        ...
        while ($results = mysqli_fetch_assoc($anwerQuery)) {
            echo $results['payment_item_ID']."<br>";
...
//         die($listItems);
    ...
    $query_cons = mysqli_query($conn,$sql) or die(mysqli_error($conn));
    ...
    if (mysqli_num_rows($query_cons) > 0) {
        ...
        while ($row = mysqli_fetch_array($query_cons)) {
            ...
            $select_qry = mysqli_query($conn,$sql_dgn) or die(mysqli_error($conn));
    ...
    //echo $sql;exit;
    $query_cons_inp = mysqli_query($conn,$sql) or die(mysqli_error($conn));

    $data .="<br/><br/><div class='myheader'>INPATIENT</div>";
    if (mysqli_num_rows($query_cons_inp) > 0) {
        ...
        $sn = 1;
        while ($row = mysqli_fetch_array($query_cons_inp)) {
            $Payment_Item_Cache_List_ID = $row['Payment_Item_Cache_List_ID'];
            $consultation_id = $row['consultation_id'];
            $Product_Name = $row['Product_Name'];
            $billing = $row['Billing_Type'];
