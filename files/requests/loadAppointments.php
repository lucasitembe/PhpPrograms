...
//    regID:regID,PatientName:PatientName
    ...
    $numberSpecimen="SELECT ap.Clinic,reg.Phone_Number,reg.Registration_ID,ap.doctor,ap.Set_BY,ap.date_time,reg.Patient_Name,ap.appointment_reason,ap.appointment_id FROM tbl_appointment ap,tbl_patient_registration reg WHERE ap.patient_No=reg.Registration_ID $filterclinic  AND ap.Status='1' $filterdoctor AND date_time BETWEEN '$fromDate' AND '$toDate' LIMIT 200  ";     
//   echo "inafika";
      $totalrevenue= mysqli_query($conn,$numberSpecimen);
        ...
        echo "<td style='text-align:left;'><a href='$link_to_procedure'>".$clinic_name."</a></td>";
        echo "<td style='text-align:left;'><a href='$link_to_procedure'>".$doctor_name."</a></td>";
