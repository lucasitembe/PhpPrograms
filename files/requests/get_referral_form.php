...
if (isset($_POST['referral_from'])) {
    ...
    if ($result) {
        if (isset($_POST['src']) && $_POST['src'] == 'inpat') { //admission_ID, consultation_ID
            $remfromdoct = mysqli_query($conn,"UPDATE tbl_admission SET Admission_Status='pending' WHERE  Admision_ID='$admission_ID'") or die(mysqli_error($conn));
            if (!$remfromdoct) {
                $error = true;
            }
        } else {
		...
		";
            $result2 = mysqli_query($conn,$querySignoff) or die(mysqli_error($conn));
    ...
    $result = mysqli_query($conn,"UPDATE tbl_referral_patient set status='served',proccessed_by='$employee_id',date_processed=NOW() where referr_id='$id'") or die(mysqli_error($conn));
    ...
    if (isset($_GET['src']) && $_GET['src'] == 'inpat') { //src=inpat admID, consID
        ...
        $getPatientDetails = mysqli_query($conn,"SELECT Patient_Name,Gender,TIMESTAMPDIFF(YEAR,Date_Of_Birth,CURDATE()) as age FROM tbl_patient_registration WHERE Registration_ID='$regid'") or die(mysqli_error($conn));
        $pat = mysqli_fetch_assoc($getPatientDetails);

        $getCompanyList = mysqli_query($conn,"SELECT * FROM tbl_company") or die(mysqli_error($conn));

        $getPatientDiagnopsis = mysqli_query($conn,"SELECT d.disease_name FROM tbl_ward_round w JOIN tbl_ward_round_disease wd
                                 ON wd.Round_ID=w.Round_ID AND wd.diagnosis_type='diagnosis'
                                 JOIN tbl_disease d ON d.disease_ID=wd.disease_ID
                                 WHERE w.consultation_ID='$consID'") or die(mysqli_error($conn));

        $getPatienclinicdetaiil = mysqli_query($conn,"SELECT w.Findings AS maincomplain,'' AS history_present_illness FROM tbl_ward_round w
                                 WHERE w.consultation_ID='$consID' AND Process_Status='served' ORDER BY Round_ID DESC LIMIT 1") or die(mysqli_error($conn));

        $patclin = mysqli_fetch_assoc($getPatienclinicdetaiil);

        $getMedics = mysqli_query($conn,"SELECT Product_Name,Doctor_Comment FROM tbl_payment_cache pc,tbl_item_list_cache ilc,tbl_items i
        ...
        $getPatientDiagnopsis = mysqli_query($conn,"SELECT d.disease_name FROM tbl_consultation c JOIN tbl_disease_consultation dc
                                 ON dc.consultation_ID=c.consultation_ID AND dc.diagnosis_type='diagnosis'
                                 JOIN tbl_disease d ON d.disease_id=dc.disease_id
                                 WHERE Patient_Payment_Item_List_ID='$pplID'") or die(mysqli_error($conn));

        $getPatienclinicdetaiil = mysqli_query($conn,"SELECT ch.maincomplain,ch.history_present_illness FROM tbl_consultation c JOIN  tbl_consultation_history ch
        ...
        $getMedics = mysqli_query($conn,"SELECT Product_Name,Doctor_Comment FROM tbl_payment_cache pc,tbl_item_list_cache ilc,tbl_items i
				WHERE pc.consultation_id = (SELECT consultation_ID FROM tbl_consultation where Patient_Payment_Item_List_ID='$pplID' ORDER BY consultation_ID DESC LIMIT 1)
				AND pc.Payment_Cache_ID = ilc.Payment_Cache_ID
				AND i.Item_ID = ilc.Item_ID
                                AND ilc.Status = 'dispensed'
				") or die(mysqli_error($conn));
    ...
    $Pharmacy = '';
    while ($phar = mysqli_fetch_array($getMedics)) {
        $Pharmacy.= ' ' . $phar['Product_Name'] . '[ ' . $phar['Doctor_Comment'] . ' ]' . ';   ';
    }

    $diagnosis = '';
    $i = 1;
    while ($dign = mysqli_fetch_array($getPatientDiagnopsis)) {
        if ($i == 1) {
    ...
    while ($row = mysqli_fetch_array($getCompanyList)) {
