...
if (isset($_POST['SavegeneralResult'])) {
   ...
   $testID = '';
    ...
    while ($row2 = mysqli_fetch_assoc($GetResults)) {
        ...
        $data .= '<input type="hidden" class="productID" value="' . $productID . '">';
    ...
    $resultSaved = mysqli_query($conn,"SELECT * FROM tbl_tests_parameters WHERE ref_item_ID='$productID'");
...
<script>
    ...
    $('.SubmitvalidatedResulttrone').click(function (e) {
...
<script>
    /* $('.SubmitvalidatedResulttr').click(function (e) {
		e.stopImmediatePropagation();
...
<script>
   /* /*  $('.validateSubmittedResulttr').click(function (e) {
	/* 	e.stopImmediatePropagation(); 
        $('.validated').attr('checked', true);
        var itemID = $('.productID').val();
        var patientID = $(this).attr('name');
...
//               alert(html);
    ...
    /* $('.validateResult').click(function (e) {
		e.stopImmediatePropagation();
...
<script type="text/javascript">
    ...
    $('.modificationStats').click(function (e) {
		e.stopImmediatePropagation();
        $('#labGeneral').fadeOut();
