<legend align="right;">OUTPATIENT BILL TRANSACTIONS</legend>
<?php
	session_start();
    ...
    if($Start_Date != null && $Start_Date != '' && $End_Date != '' && $End_Date != ''){
        ...
        $Date_Filter = 'and DATE(ilc.Transaction_Date_And_Time)=CURDATE()';
    }
...
<table width="100%">
    <?php
        ...
        if(isset($_GET['Patient_Name'])){
        	if($Sponsor_ID == 0 && $_GET['Patient_Name'] != null && $_GET['Patient_Name'] != ''){
    	        $get_details = mysqli_query($conn,"select ilc.Transaction_Date_And_Time, pc.Payment_Cache_ID, pr.Registration_ID, sp.Guarantor_Name, pr.Phone_Number, pr.Member_Number, pr.Gender, pr.Patient_Name, pr.Date_Of_Birth from
                                        ...
                                        $Date_Filter group by pr.Registration_ID, pc.Payment_Cache_ID order by ilc.Payment_Item_Cache_List_ID desc limit 50") or die(mysqli_error($conn));
            ...
            if($Sponsor_ID == 0 && $_GET['Patient_Number'] != null && $_GET['Patient_Number'] != ''){
                $get_details = mysqli_query($conn,"select ilc.Transaction_Date_And_Time, pc.Payment_Cache_ID, pr.Registration_ID, sp.Guarantor_Name, pr.Phone_Number, pr.Member_Number, pr.Gender, pr.Patient_Name, pr.Date_Of_Birth from
                                        ...
                                        (ilc.Check_In_Type = 'Laboratory' or ilc.Check_In_Type = 'Radiology')
                                        $Date_Filter group by pr.Registration_ID, pc.Payment_Cache_ID order by ilc.Payment_Item_Cache_List_ID desc limit 20") or die(mysqli_error($conn));
        ...
        if($nm > 0){
            ...
            ?>
                <tr>
                    <td><a href="approve_outpatient_inestigation.php?<?php echo $Section_Link; ?>Registration_ID=<?php echo $row['Registration_ID']; ?>&Payment_Cache_ID=<?php echo $row['Payment_Cache_ID']; ?>&ApproveInvestigation=ApproveInvestigationThisPage&Transaction_Date_And_Time=<?= $Transaction_Date_And_Time ?>" style="text-decoration: none;"><?php echo ++$temp; ?></td>
                    ...
                    <td><a href="approve_outpatient_inestigation.php?<?php echo $Section_Link; ?>Registration_ID=<?php echo $row['Registration_ID']; ?>&Payment_Cache_ID=<?php echo $row['Payment_Cache_ID']; ?>&ApproveInvestigation=ApproveInvestigationThisPage&Transaction_Date_And_Time=<?= $Transaction_Date_And_Time ?>" style="text-decoration: none;"><?php echo $age; ?></a></td>
