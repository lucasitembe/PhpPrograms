...
if (isset($_POST['payment_id'])) {
   ...
   if(mysqli_num_rows($sql_check_if_test_result)>0){
       ...
       $sql_update_validate_status_for_this_page_result=mysqli_query($conn,"UPDATE tbl_tests_parameters_results SET Validated='Yes',Saved='Yes',SavedBy='$Employee_ID',ValidatedBy='$Employee_ID',TimeValidate=NOW() WHERE ref_test_result_ID IN (SELECT test_result_ID FROM tbl_test_results WHERE payment_item_ID='$payment_id')") or die(mysqli_error($conn));
   }else{
       echo "new";
