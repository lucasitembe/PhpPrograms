<?php
    ...
    if($Employee_ID != 0 && $Payment_Item_Cache_List_ID != 0 && $consultation_ID != 0){
	    ...
	    $check = mysqli_query($conn,"select Disease_ID from tbl_ogd_post_operative_diagnosis where Ogd_Post_operative_ID = '$Ogd_Post_operative_ID' and Disease_ID = '$disease_ID'") or die(mysqli_error($conn));
	    $no = mysqli_num_rows($check);
	    if($no < 1){
	    	$insert_disease = mysqli_query($conn,"insert into tbl_ogd_post_operative_diagnosis(Disease_ID, Ogd_Post_operative_ID)
	    									values('$disease_ID','$Ogd_Post_operative_ID')") or die(mysqli_error($conn));
	    }
	}
?>
<table width="100%">
