...
if (isset($_POST['action'])) {
    if ($_POST['action'] == 'barcode') {
        ...
        if ($payingID == '') {
            ...
            $validateQuery = "SELECT * FROM tbl_tests_parameters_results JOIN tbl_test_results ON test_result_ID=ref_test_result_ID WHERE Validated='Yes' GROUP BY test_result_ID";
...
//              }else{
    ...
    $htm.="<td>
                ...
                <input type='button' class='searchresults' name='" . $row['Patient_Name'] . "' id='" . $row['Registration_ID'] . "' payment_id='" . $row['Payment_Cache_ID'] . "' value='Lab results' />
                </td>";
