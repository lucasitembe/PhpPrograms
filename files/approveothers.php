...
<?php
echo "<a href='otherscredits.php?" . $Section_Link . "ProcedureCredits=ProcedureCreditsThisPage' class='art-button-green'>BACK</a>";
?>
...
$select = mysqli_query($conn,"select consultation_id,Billing_Type from tbl_payment_cache where
...
<br/>
<fieldset>
    <legend align=right><b>OTHERS APPROVAL</b></legend>
    <center>
        <table width=100%> 
            <tr> 
                <td>
        ...
        <table width=100%>
            <tr>
                <td style='text-align: right;'>
                    <input type="button" name="Make_Payments" id="Make_Payments" value="APPROVE TRANSACTION" class="art-button-green" onclick="Validate_make_Payments()">
                </td>
            </tr> 
        </table>
    </center>
...
$select_items = mysqli_query($conn,"select itm.Product_Name, ilc.Quantity, ilc.Transaction_type, ilc.Edited_Quantity, ilc.Price, ilc.Discount, ilc.Payment_Item_Cache_List_ID, ilc.Check_In_Type, ilc.Payment_Item_Cache_List_ID, ilc.ePayment_Status
                ...
                echo $data['Quantity'];
                       ...
                       <input type="text"class="sub_t_txt"hidden="hidden" value="<?= $Total ?>" id="sub_total_txt<?= $Payment_Item_Cache_List_ID ?>">
                        ...
                        if ($no == 1) {
                          ...
                          ?>
                        <td>
                            <select style="float:right;height:25px" onchange="change_bill_type('<?= $Payment_Item_Cache_List_ID ?>')" id="billtype<?= $Payment_Item_Cache_List_ID ?>">
...
<script>
    ...
    function update_discount_price(Payment_Item_Cache_List_ID){
       ...
       $.ajax({
           type:"GET",
           url:"change_discount_price_for_approval_others.php",
...
<script type="text/javascript">
    function make_Payments(Payment_Cache_ID) {
        ...
        if (Confirm_Message == true) {
            ...
            document.location = 'Approve_Selected_Transaction_Others.php?' + Section_Link + 'Payment_Cache_ID=' + Payment_Cache_ID + '&Registration_ID=' + Registration_ID+"&Billing_Type="+Billing_Type;
        }
    }
</script>
<script>
