...
<?php
	...
	if(isset($_SESSION['userinfo'])){
		if($_SESSION['userinfo']['Admission_Works'] == 'yes'){
			...
			echo "<a href='admissiongrnissuenotelist.php?admissiongrnissuenoteList=admissiongrnissuenoteListThisPage' class='art-button-green'>NEW GRN&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style='background-color: red; border-radius: 8px; color: white; padding: 6px;'>".$number."</span></a>";
		}
	}

        if(isset($_SESSION['userinfo'])){
		if($_SESSION['userinfo']['Admission_Works'] == 'yes'){
			echo "<a href='admissionpreviousgrnlist.php?PreviousGrn=PreviousGrnThisPage' class='art-button-green'>PREVIOUS GRNs</a>";
		}
	}
	
	if(isset($_SESSION['userinfo'])){
		if($_SESSION['userinfo']['Admission_Works'] == 'yes'){
			echo "<a href='admissiongoodreceivingnote.php?GoodReceivingNote=GoodReceivingNoteThisPage' class='art-button-green'>BACK</a>";
		}
	}
?>
...
<?php
	...
	if(isset($_SESSION['Admission_Issue_ID'])){
		$Issue_ID = $_SESSION['Admission_Issue_ID'];
	}else{
		$Issue_ID = 0;
	}
...
<fieldset>
	<legend align="right">
		<?php if(isset($_SESSION['Admission_ID'])){ echo $Sub_Department_Name; }?>, GRN Against Issue Note
	</legend>
			...
			if($i == 0){
				...
				if($result){
					...
					echo "<script>
							...
							document.location = 'admissiongrnissuenote.php?GrnIssueNote=GrnIssueNoteThisPage';
						</script>";
				}
