...
<script language="javascript" type="text/javascript">
    function seadentalPatient(Patient_Name){
        document.getElementById('Seadental_Iframe').innerHTML = "<iframe width='100%' height=100% src='viewpatientsIframe.php?Patient_Name="+Patient_Name+"'></iframe>";
    }
</script>
...
<!--<br/>-->
<fieldset>  
            <legend align=right><b><?php if(isset($_SESSION['Dental'])){ echo $_SESSION['Dental']; }?></b></legend>
        <center>
            <table width=100%> 
                <tr> 
                    <td>
				...
				<?php
				    if(isset($_SESSION['Dental_Supervisor'])) {
					if(isset($_SESSION['Dental_Supervisor']['Session_Master_Priveleges'])){
					    if($_SESSION['Dental_Supervisor']['Session_Master_Priveleges'] = 'yes'){
						$Supervisor = $_SESSION['Dental_Supervisor']['Employee_Name'];
					    }else{
		...
		<tr>
		   <td style='text-align: center;'>
			<?php
				...
				$Sub_Department_Name = $_SESSION['Dental'];
			    }else{
				$Sub_Department_Name = '';
			    }
...
<!--					     <a href='Send_To_Cashier.php?Payment_Cache_ID=<?php echo $Payment_Cache_ID; ?>&Transaction_Type=<?php echo $Transaction_Type; ?>&Sub_Department_Name=<?php echo $Sub_Department_Name; ?>&Registration_ID=<?php echo $Registration_ID; ?>' class='art-button-green' onclick='return approved_Message2();'>
		...
		if($Transaction_Status_Title != 'APPROVED' && $Transaction_Status_Title != 'PAID'){
			...
			if(strtolower($Departmental_Collection) == 'yes'){
				...
				echo "<a href='Departmental_Patient_Billing_Dental_Page.php?Payment_Cache_ID=".$Payment_Cache_ID."&Transaction_Type=".$Transaction_Type."&Sub_Department_ID=".$Sub_Department_ID."&Registration_ID=".$Registration_ID."&Billing_Type=".$Billing_Type."' class='art-button-green'>
					Make Payment
				    </a>";
			    }
			}
...
<?php if($Transaction_Status_Title != 'DISPENSED'){ 
	...
	<?php if(strtoupper($Transaction_Status_Title) == 'PAID'){ ?>
	    <a href='dentalcashpatientlist.php?Payment_Cache_ID=<?php echo $Payment_Cache_ID; ?>&Transaction_Type=<?php echo $Transaction_Type; ?>' class='art-button-green'>Back To Main Page</a>
	<?php }else{ ?>
...
<?php }else{ ?>
	...
	<a href='dentalcreditpatientlist.php?Payment_Cache_ID=<?php echo $Payment_Cache_ID; ?>&Transaction_Type=<?php echo $Transaction_Type; ?>&Registration_ID=<?php echo $Registration_ID; ?>&Billing_Type=<?php echo $Billing_Type; ?>' class='art-button-green'>Back To Main Page</a>
    <?php } ?>
<?php }} ?>
		<?php
...
<fieldset>   
		...
		<tr>
		    <td>
			<iframe src='dentalitems_Iframe.php?Transaction_Type=<?php echo $Transaction_Type; ?>&Payment_Cache_ID=<?php echo $Payment_Cache_ID; ?>' width='100%' height=200px></iframe>
		    </td>
		</tr> 
	    </table>
        </center>
