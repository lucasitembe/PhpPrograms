<table width="100%">
	...
	<tr>
		...
		<td><b>FINANCE DEPARTMENT NAME</b></td>
...
?>
		...
		<td style="text-align: right;"><b><?php if($_SESSION['systeminfo']['price_precision']=='yes'){ echo $Currency_Code.'&nbsp;'.number_format($Grand_Total_Credit, 2); }else{ echo $Currency_Code.'&nbsp;'.number_format($Grand_Total_Credit); } ?></b></td>
                <td style="text-align: right;"><b><?php if($_SESSION['systeminfo']['price_precision']=='yes'){ echo $Currency_Code.'&nbsp;'.number_format($Grand_Total_Msamaha, 2); }else{ echo $Currency_Code.'&nbsp;'.number_format($Grand_Total_Msamaha); } ?></b></td>
		<!--<td style="text-align: right;"><b><?php if($_SESSION['systeminfo']['price_precision']=='yes'){ echo $Currency_Code.'&nbsp;'.number_format($Grand_Total_Credit + $Grand_Total_Cash+$Grand_Total_Msamaha, 2); }else{ echo $Currency_Code.'&nbsp;'.number_format($Grand_Total_Credit + $Grand_Total_Cash + $Grand_Total_Msamaha + $Grand_Total_Fast_Track); } ?></b>&nbsp;&nbsp;&nbsp;</td>-->
...
<?php
                ...
                ?>
							 ...
							 ?>
							<!-- <tr>
							 	<td><b><?=(++$temper);?></b></td> <td><a href=' generalgeneralledgerfilteredreport.php?Section=other_sources&Start_Date=<?=$Start_Date?>&End_Date=<?=$End_Date?>&Employee_ID=<?=$Employee_ID?>&GeneralLedgerCashAndCreditReport=GeneralLedgerCashAndCreditReportThisPage'  style='text-decoration:none'><b><label style='color: #0079AE;'>REVENUE FROM OTHER SOURCES</label></b></a></td><td style='text-align:center;'><b><?=$service_number;?></b></td><td style='text-align:right;'><b><?=number_format($total_rev_amount);?></b></td>
	...
	<tr>
		...
		<td style="text-align: right;"><b><?php if($_SESSION['systeminfo']['price_precision']=='yes'){ echo $Currency_Code.'&nbsp;'.number_format($all_Grand_Total_Credit, 2); }else{ echo $Currency_Code.'&nbsp;'.number_format($all_Grand_Total_Credit); } ?></b></td>
        <td style="text-align: right;"><b><?php if($_SESSION['systeminfo']['price_precision']=='yes'){ echo $Currency_Code.'&nbsp;'.number_format($all_Grand_Total_Msamaha, 2); }else{ echo $Currency_Code.'&nbsp;'.number_format($all_Grand_Total_Msamaha); } ?></b></td>
