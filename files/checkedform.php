...
<a href='Pre_OperativeCompleted.php?Registration_ID=<?php echo $_GET['Registration_ID'];?>&Nurse_DateTime=<?php echo $_GET['Nurse_DateTime'];?>' class='art-button-green'>
    BACK
</a>
...
<?php
 ...
 if(isset($_GET['Registration_ID']) && isset($_GET['Nurse_DateTime'])  ){ 
	...
	$select_Patients = mysqli_query($conn,"SELECT 
								...
								from tbl_patient_registration pr,tbl_sponsor sp,tbl_nurse n
								where  
	...
	$no = mysqli_num_rows($select_Patients);
	 ...
	 if($no>0){
			...
			//	  $Vital_Value = $row['Vital_Value'];	 ,tbl_nurse_vital nv	,Vital_Value  and n.Nurse_ID = nv.Nurse_ID
	}
	...
	//validation of nurse form
			 if(isset($_POST['submitnurseforms'])){       
        if(isset($_SESSION['userinfo'])){ 
           if(isset($_SESSION['userinfo']['Employee_ID'])){
                $Employee_ID = $_SESSION['userinfo']['Employee_ID'];
           }else{
            $Employee_ID = 0;
				...
				$$Nurse_DateTime= mysqli_real_escape_string($conn,$_POST['$Nurse_DateTime']);
				$Allergies = mysqli_real_escape_string($conn,$_POST['Allegies']);
	...
	where 
		...
		ppl.Patient_Direction = 'Direct To Clinic Via Nurse Station') ") or die(mysqli_error($conn));
...
<center>
	...
	<fieldset style="width:80%">
						<legend align=right><b>VITAL SIGNS</b></legend>
				<center>
		<table  class='hiv_table'  style="width:100%;">
			<tr>
				...
				<td width='8%' style="text-align:right;">Visit Date</td>
...
<hr>
			<table class='hiv_table' style="width:100%;text-align:right;margin-top:5px;" >
				<tr>
					<td rowspan='2'>
					<fieldset class='vital' style="height:250px;overflow:scroll;">
<?php
...
?>
	<td>
		<div class="nurse_tabs" style="width:100px;height:30px;padding:0px;">
			<a href='chartgraph.php?Registration_ID=<?php echo $Registration_ID; ?>&Vital_ID=<?php echo $row['Vital_ID']; ?>&
	...
	<div><span style="font-size:14px;margin-left:4px;margin-right:2px;"><b>BMI</b></span>
			<span><input type="text" name="bmi" id="bmi" value="<?php echo$bmi; ?>" disabled='disabled' style="width:100px">
				<span class="nurse_tabs" style="width:120px;height:30px;">	
					<a href='chartbmi.php?Registration_ID=<?php echo $Registration_ID; ?> &bmi=<?php echo $bmi; ?>&Nurse_DateTime=<?php echo filter_input(INPUT_GET,'Nurse_DateTime'); ?>'  class='art-button-green'>history</a>
				</span>
			</span>
	</div>
		<?php
...
?>	
	...
	<td>
		...
		<table width='100%'>
			<tr>
				<td width="20%" style="text-align:right;">Allegies</td>
				<td >
