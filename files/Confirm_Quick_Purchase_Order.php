<?php
    ...
    if($Sub_Department_Name != null && $Sub_Department_Name != ''){
        $sql_select = mysqli_query($conn,"select Sub_Department_ID from tbl_quick_purchase_order where
        ...
        if($num > 0){
            $sql_select2 = mysqli_query($conn,"select Sub_Department_ID from tbl_quick_purchase_order where
