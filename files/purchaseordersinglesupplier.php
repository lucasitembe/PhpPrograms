...
<?php
	...
	if(isset($_SESSION['Selected_Store_Order_ID'])){
	    ...
	    include("./procurement_cache_store_order.php");
...
<script>
	function display_submit_button() {
		document.getElementById("Submit_Button_Area").innerHTML = "<input type='button' name='Submit_Purchase_Order' id='Submit_Purchase_Order' value='APPROVE PURCHASE ORDER' class='art-button-green' onclick='Confirm_Submit_Order()'>";
	}
</script>
...
<script>
	function Submit_Order(){
		...
		document.location = 'submit_order.php?Store_Need='+Store_Need+'&Supplier_ID='+Supplier_ID+'&Order_Description='+Order_Description;
	}
</script>
<script>
...
<fieldset>
        ...
        <legend align='right'>
            <b>Purchase Order ~ <?php if(isset($_SESSION['Procurement_ID'])){ echo $Sub_Department_Name; }?> ~ Single Supplier</b>
        </legend>
				...
				<?php
					if(isset($_SESSION['Purchase_Order_ID'])){
						...
						if($no > 0){
							...
							$check_sub = mysqli_query($conn,"select Store_Need from tbl_purchase_cache where Store_Order_Id='$Selected_Store_Order_ID' limit 1") or die(mysqli_error($conn));
		...
		<td id='Supplier_ID_Area'>
			...
			<?php	
				if(isset($_SESSION['Purchase_Order_ID']) && $_SESSION['Purchase_Order_ID'] != '' && $_SESSION['Purchase_Order_ID'] != null){
					...
					?>
						<select name='Supplier_ID' id='Supplier_ID' required='required' style='width:100%'>
							<option selected='selected' value='<?php echo $Supplier_ID; ?>'><?php echo ucfirst($Supplier_Name); ?></option>
						</select>
					<?php
				}else{
                                        //check if this order already been in approval process
                                        ...
                                        echo "<select name='Supplier_ID' id='Supplier_ID' required='required' style='width:100%'>";
                                        if(mysqli_num_rows($sql_check_for_approval_process_result)>0){
                                            
                                            $Supplier_ID=mysqli_fetch_assoc($sql_check_for_approval_process_result)['Supplier_ID'];
                                            $select_Supplier = mysqli_query($conn,"select * from tbl_supplier where Supplier_ID='$Supplier_ID'") or die(mysqli_error($conn));
...
<?php
	$select_order_items = mysqli_query($conn,"select itm.Product_Name, pc.Quantity_Required, pc.Purchase_Cache_ID, pc.Price, pc.Container_Qty, pc.Items_Per_Container, pc.Item_ID, pc.Store_Need
										from tbl_purchase_cache pc, tbl_items itm where
										itm.Item_ID = pc.Item_ID and
										pc.Store_Order_Id='$Selected_Store_Order_ID'") or die(mysqli_error($conn)); 
...
<fieldset style='overflow-y: scroll; height: 260px;' id='Items_Fieldset_List'>
	<?php
		...
		echo '<tr><td width=4% style="text-align: center;">Sn</td>
				...
				<td width=5% '.$display_none.' class="Remove_Item">Remove</td></tr>';
	...
	<?php
		...
		?>
                        <td width=6% <?= $display_none ?> class="Remove_Item">
...
<script>
	function Confirm_Submit_Order() {
		...
		if (Supplier_ID != null && Supplier_ID != '' && Store_Need != null && Store_Need != '') {
