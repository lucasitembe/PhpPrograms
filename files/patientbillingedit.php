...
<script type='text/javascript'>
	...
	if (confirmDialogy) {
	    document.location = "./updatetransaction.php?Type_Of_Check_In="+Type_Of_Check_In+"&direction="+direction+"&Consultant="+Consultant+"&Item_ID="+Item_ID+"&Discount="+Discount+"&Quantity="+Quantity+"&Patient_Payment_Item_List_ID="+Patient_Payment_Item_List_ID; 
	}
    }
</script>
<!-- end of submit-->
        ...
        <center>
		    ...
		    <td> 
			    ...
			    <option selected='selected'>All Available</option>
                    ...
                    <td>
                        <select name='Item_Category' id='Item_Category' onchange='getItemList(this.value)' onchange='getPrice()' onkeypress='getPrice()'>
		    ...
		    <?php
			if(isset($_GET['Patient_Payment_Item_List_ID'])){
			    $select_item_name = mysqli_query($conn,"select * from tbl_patient_payments pp, tbl_patient_payment_item_list ppl, tbl_items itm
								where pp.Patient_Payment_ID = ppl.Patient_Payment_ID and
								itm.item_id = ppl.item_id and
								    ppl.Patient_Payment_Item_List_ID = '$Patient_Payment_Item_List_ID'");
			    while($row = mysqli_fetch_array($select_item_name)){
		    ...
		    ?>
                    ...
                    <td>
			<?php if($Product_Name == ''){ ?>
			    <select name='Item_Name' id='Item_Name' onchange='getPrice()' disabled='disabled'>
			<?php }else{ ?>
			    <select name='Item_Name' id='Item_Name' onchange='getPrice()' required='required'>
			<?php } ?>
			
			    <option selected='selected' value='<?php echo $Item_ID; ?>'><?php echo $Product_Name; ?></option>
				...
				//we get item id
                    ...
                    <td>
                        <input type='text' name='Discount' id='Discount' placeholder='Discount' value='<?php echo $Discount; ?>' onchange='getPrice()' onkeypress='getPrice()' value=0>
                    </td>
                    <td>
			<?php if($Price == '') { ?>
			    <input type='text' name='Price' id='Price' disabled='disabled' placeholder='Price' value='<?php echo $Price; ?>'>
			<?php }else{ ?>
			...
			<?php } ?>
                    ...
                    <td>
			<?php if($Product_Name == ''){ ?>
			    <input type='text' name='Quantity' id='Quantity' disabled='disabled' value='<?php echo $Quantity; ?>' required='required' placeholder='Quantity' onchange='getPrice()' onkeypress='getPrice()'>
			<?php }else{ ?>
			    <input type='text' name='Quantity' id='Quantity' value='<?php echo $Quantity; ?>' required='required' placeholder='Quantity' onchange='getPrice()' onkeypress='getPrice()'>
			<?php } ?>
                    </td>
                    <td>
                        <input type='text' name='Amount' id='Amount' value='<?php echo $Amount; ?>' placeholder='Amount' disabled='disabled'>
                    </td>
                    <td style='text-align: center;'>
			<?php if($Product_Name == ''){ ?>
			    <input type='button' name='submit' id='submit' disabled='disabled' onclick='return submitted()' value='UPDATE' class='art-button-green'>
                        <?php }else{ ?>
			    <input type='button' name='submit' id='submit' onclick='return submitted()' value='UPDATE' class='art-button-green'>
                        <?php } ?>
			<input type='hidden' name='submittedUpdateTransactionForm' value='true'/>
			<input type='hidden' name='Patient_Payment_Item_List_ID' id='Patient_Payment_Item_List_ID' value='<?php echo $Patient_Payment_Item_List_ID; ?>'>
                    </td>
                </tr>
            </table>   
        </center>
...
<fieldset>   
		...
		<tr>
			...
			<iframe src="Patient_Billing_Edit_Iframe.php?Patient_Payment_ID=<?php echo $Patient_Payment_ID; ?>&Patient_Payment_Item_List_ID=<?php echo $Patient_Payment_Item_List_ID; ?>" width='100%' height=210px></iframe>
		    </td>
		</tr> 
	    </table>
        </center>
