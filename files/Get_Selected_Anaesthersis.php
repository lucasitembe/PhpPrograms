<?php
	...
	if(isset($_GET['Employee_ID'])){
		$Nurse_ID = $_GET['Employee_ID']; //selected Anaesthetics
	}else{
		$Nurse_ID = '';
	}
    ...
    if($consultation_ID != 0 && $Registration_ID != 0 && $Employee_ID != '' && $Payment_Item_Cache_List_ID != ''){
	    ...
	    //get External_Anaesthetic if available
	    //get external participants
	    $select_external = mysqli_query($conn,"select External_Anaesthetic from tbl_post_operative_external_participant where Post_operative_ID = '$Post_operative_ID'") or die(mysqli_error($conn));
	    ...
	    if($External_Anaesthetic_Value != '' && $External_Anaesthetic_Value != ''){
	    	$Externals = '('.$External_Anaesthetic_Value.')**External';
	    }else{
	    	$Externals = '';
	    }

	    //check if selected Anaesthetics already added
	    ...
	    if($num_check < 1){
	    	$add = mysqli_query($conn,"insert into tbl_post_operative_participant(Post_operative_ID, Employee_ID, Employee_Type)
	    						values('$Post_operative_ID','$Nurse_ID','Anaesthetics')") or die(mysqli_error($conn));
	    	if($add){
	    		$select = mysqli_query($conn,"select Employee_Name from tbl_employee emp, tbl_post_operative_participant pop where
	    								...
	    								pop.Employee_Type = 'Anaesthetics'") or die(mysqli_error($conn));
