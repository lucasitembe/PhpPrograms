<?php
    ...
    $get_folio = mysqli_query($conn,"select Folio_Number,Check_In_ID from tbl_patient_payments where
    ...
    if($Employee_ID != 0 && $Registration_ID != 0 && $Branch_ID != 0 && $Folio_Number != ''){
        ...
        $select_details = mysqli_query($conn,"select Billing_Type, Sponsor_Name, Sponsor_ID, Claim_Form_Number from tbl_adhoc_items_list_cache
        ...
        if($Billing_Type != '' && $Sponsor_Name != '' && $Sponsor_ID != 0){
            ...
            if($result){
                ...
                if($num_rows > 0 ){
                    ...
                    //get all items from
                    $select_items = mysqli_query($conn,"select * from tbl_adhoc_items_list_cache
                    ...
                    if($num_details > 0){
                        ...
                        while($row = mysqli_fetch_array($select_items)){
                                            ...
                                            '$Check_In_Type','$Item_ID',
                                                '$Discount','$Price','$Quantity',
                                                    '$Patient_Direction','$Consultant',(select clinic_ID from tbl_clinic where clinic_name = '$Consultant')
                                                        ,'$Patient_Payment_ID',(select now()))";                   
                            }elseif(strtolower($Patient_Direction) == 'others' || strtolower($Patient_Direction) == 'direct to clinic via nurse station'){
							
							//echo $Patient_Direction.'<br/>'; 
                                $Insert_Data_To_tbl_patient_payment_item_list = "insert into tbl_patient_payment_item_list(
                                        check_In_type,item_id,discount,price,quantity,
                                                patient_direction,consultant,consultant_id,patient_payment_id,Transaction_Date_And_Time)
                                        values(
                            ...
                            mysqli_query($conn,$Insert_Data_To_tbl_patient_payment_item_list) or die(mysqli_error($conn));
                        }
                        //exit;
                        mysqli_query($conn,"delete from tbl_adhoc_items_list_cache where Employee_ID = '$Employee_ID'") or die(mysqli_error($conn));
