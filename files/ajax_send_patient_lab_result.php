...
if (isset($_POST['Payment_Item_Cache_List_ID'])) {
   ...
   $sql_update_validated_status_result=mysqli_query($conn,"UPDATE tbl_intergrated_lab_results SET sent_to_doctor='yes',sent_to_doctor_by='$Employee_ID' WHERE result_date='$result_date' AND sample_test_id='$Payment_Item_Cache_List_ID'") or die(mysqli_error($conn));
   ...
   if(mysqli_num_rows($sql_check_if_test_result)>0){
       ...
       $sql_update_validate_status_for_this_page_result=mysqli_query($conn,"UPDATE tbl_tests_parameters_results SET Submitted='Yes',Saved='Yes',SavedBy='$Employee_ID',TimeSubmitted=NOW() WHERE ref_test_result_ID IN (SELECT test_result_ID FROM tbl_test_results WHERE payment_item_ID='$Payment_Item_Cache_List_ID')") or die(mysqli_error($conn));
   }else{
       //echo "new";
       ...
       $sql_insert_validate_status_result=mysqli_query($conn,"INSERT INTO tbl_tests_parameters_results (ref_test_result_ID,parameter,Submitted,Saved,SavedBy,TimeSubmitted) VALUES('$test_result_ID','$ref_parameter_ID','Yes','Yes','$Employee_ID',NOW())") or die(mysqli_error($conn));
   }
}