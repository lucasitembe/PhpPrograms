					...
					<select name='Sub_Department_ID' id='Sub_Department_ID' required='Sub_Department_ID'>
						<?php
							...
							if(isset($_SESSION['Storage'])){
								...
								$select_sub_department_id = mysqli_query($conn,"select Sub_Department_ID from tbl_Sub_Department where
													Sub_Department_Name = '$Sub_Department_Name'") or die(mysqli_error($conn));
								$num = mysqli_num_rows($select_sub_department_id);
								if($num > 0){
									while($row = mysqli_fetch_array($select_sub_department_id)){
							...
							//display sub department
							if($Sub_Department_ID != 0){
						?>
							<option value='<?php echo $Sub_Department_ID; ?>'><?php echo $_SESSION['Storage']; ?></option>
						<?php
							}else{						
						?>
...
<script>
	...
	function Get_Selected_Item(){
		...
		if (Item_ID != '' && Item_ID != null && Quantity != '' && Quantity != null && Sub_Department_ID != '' && Sub_Department_ID != null && Buying_Price != '' && Buying_Price != null) {
			...
			my_Object_Get_Selected_Item.open('GET','Grn_Open_Balance_Add_Selected_Item.php?Item_ID='+Item_ID+'&Quantity='+Quantity+'&Item_Remark='+Item_Remark+'&Sub_Department_ID='+Sub_Department_ID+'&Grn_Description='+Grn_Description+'&Buying_Price='+Buying_Price,true);
				...
				<table width=100%>
					...
					<tr>
                                            <td>
                                                ...
                                                <table width=100%>
                                                    <?php
                                                        $result = mysqli_query($conn,"SELECT * FROM tbl_items where Item_Type <> 'Service' order by Product_Name");
