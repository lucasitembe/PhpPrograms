...
if (isset($_GET['fromDate'])) {
    ...
    $filter = " WHERE ilc.Transaction_Date_And_Time BETWEEN '$fromDate' AND '$toDate' AND ilc.Check_In_Type='Laboratory'";
    $filterPatient = " AND ilc.Transaction_Date_And_Time BETWEEN '$fromDate' AND '$toDate' AND ilc.Check_In_Type='Laboratory'";
}
...
if ($procedureStatus == 'All') {
    ...
    if($procedureStatus == 'removed'){
       $filter .="  AND ilc.Status = 'active' AND removing_status='Yes'";
       $filterPatient .="  AND ilc.Status = 'active' AND removing_status='Yes'";
    }else{
       $filter .="  AND ilc.Status = 'active'";
       $filterPatient .="  AND ilc.Status = 'active'"; 
    }
}
?>
...
$htm = "<table width ='100%'  class='nobordertable'>
		    ...
		    <tr><td style='text-align: center;'><span><b>PATIENT WITH LABORATORY TESTS ACTIVE / REMOVED</b></span></td></tr>
