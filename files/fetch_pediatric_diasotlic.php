  ...
  if ($result=mysqli_query($conn,$select_pediatric_graph)) {
    ...
    if (($num = mysqli_num_rows($result)) > 0) {
      ...
      while ($row = mysqli_fetch_assoc($result)) {
        ...
        $final_time_diasotlic=$TimeStamp_hour+(($TimeStamp_min/60));
        $d['final_time_diasotlic'] = $final_time_diasotlic;
        $d['time'] = $time;
        $d['blood_pressure_diasotlic'] = $row['blood_pressure_diasotlic'];
