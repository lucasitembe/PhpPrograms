...
if (isset($_GET['from_doctors_page']) && $_GET['from_doctors_page'] != "") {
    $from_doctors_page = $_GET['from_doctors_page'];
} else {
    $from_doctors_page = "";
}
if(isset($_GET['consultation_type'])){
    $consultation_type=$_GET['consultation_type'];
    $consultation_type2=$_GET['consultation_type'];
    $consultation_type="AND ch.consultation_type='$consultation_type'";
    if($consultation_type2=="All"){
        $consultation_type="";
    }     
}else{
    $consultation_type="";
...
if(isset($_GET['this_page_from']) && $_GET['this_page_from'] != "") {
...
$filter = "  ch.cons_hist_Date BETWEEN '$Date_From' AND '$Date_To' AND ch.employee_ID='$employee_ID'  AND  Saved='yes' $consultation_type $process_status ";
...
?>
<a target="_blank" href='./printdoctorsperformancesummary.php?Employee_ID=<?php echo $employee_ID ?>&Date_From=<?php echo $Date_From ?>&Date_To=<?php echo $Date_To ?>&Sponsor_ID=<?php echo $_GET['Sponsor_ID'] ?>' class='art-button-green'>
    PRINT
</a>
<?php
echo "<a href='./doctorsPerformanceSummaryFilter.php?Date_From=" . $_GET['Date_From'] . "&Date_To=" . $_GET['Date_To'] . "&Sponsor_ID=" . $_GET['Sponsor_ID'] ."&from_doctors_page=". $from_doctors_page ."' class='art-button-green'>
        BACK
    </a>
 <br/><br/>";
?>
...
<fieldset style='overflow-y:scroll; height:500px'>
    <center>
        ...
        <legend  align="right" style="background-color:#006400;color:white;padding:5px;">
            <br/>
            <form action='doctorsPerformanceSummaryFilter.php' method='post' name='myForm' id='myForm' onsubmit="return validateForm();" enctype="multipart/form-data"></legend>
...
while ($sponsor_rows = mysqli_fetch_assoc($sponsor_results)) {
    ...
    <legend align="center" style="background-color:#037DB0;color: white;padding: 5px;text-align: center">
        <b><?php echo $EmployeeName; ?> PERFORMANCE REPORT </b><br/>
...
foreach ($dataRange as $value) {
    ...
    if (mysqli_num_rows($consultationDateRange) > 0) {
        ...
        echo "<div class='daterange'>$thisDate $day<span style='float:right'>PATIENT NO. $noOfPatient </span></div>";
...
<script>
    function Show_Patient_File(Registration_ID) {
     // var printWindow= window.open("http://www.w3schools.com", "_blank", "toolbar=yes, scrollbars=yes, resizable=yes, top=500, left=500, width=400, height=400");
        window.open('Patientfile_Record_Detail_General.php?Section=Doctor&Registration_ID=' + Registration_ID + '&PatientFile=PatientFileThisForm',  '_blank');
        //winClose.close();
