...
//        
	...
	$age ='';
        ...
        // $select_emergency=mysqli_query($conn,"SELECT emergency from tbl_nurse where Registration_ID='$Registration_ID' order by Nurse_ID desc limit 1");
        $Today2=date("Y-m-d");
        $select_emergency=mysqli_query($conn,"SELECT visit_type FROM tbl_check_in WHERE Registration_ID='$Registration_ID' AND Visit_Date ='$Today2' and visit_type=2") or die(mysqli_error($conn));

        $get_emergency=mysqli_num_rows($select_emergency);
        if($get_emergency > 0){
