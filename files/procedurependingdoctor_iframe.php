...
$sqlq = " select ilc.Status, its.Product_Name,ilc.Transaction_Type,ilc.Payment_Item_Cache_List_ID, ilc.Payment_Cache_ID,ilc.remarks,pc.Billing_Type,ilc.Transaction_Type,ilc.payment_type,Require_Document_To_Sign_At_receiption
		from tbl_item_list_cache ilc,tbl_payment_cache pc, tbl_Items its , tbl_sponsor sp 		  
                where ilc.Payment_Cache_ID = pc.Payment_Cache_ID and
                    ...
                    ilc.status IN ('pending','not applicable')  and
...
while ($row = mysqli_fetch_array($select_Transaction_Items_Active)) {
    ...
    if ($transStatust) {
        ...
        if ($status == 'pending') {
            ...
            $displ .= '<option value="not applicable">Not Applicable</option>';
        } else if ($status == 'not applicable') {
            $displ .= '<option value="pending">Pending</option>'
                    . '<option value="served">Done</option>';
            $displ .= ' <option value="not applicable" selected="selected">Not Applicable</option>';
        }
        $displ .= '  </select>  
