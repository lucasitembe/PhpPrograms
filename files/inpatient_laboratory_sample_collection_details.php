...
if (isset($_SESSION['userinfo'])) {
    if ($_SESSION['userinfo']['Laboratory_Works'] == 'yes') {
      ...
      ";
        echo "<a href='inpatientSpecimentCollection.php?Date_From=".$Date_From."&Date_To=".$Date_To."' class='art-button-green'>BACK</a>";
    }
}
...
<table border="1"  style="width:100%; color:#fff;" class="hiv_table" bgcolor="#0079AE" >
    ...
    <center><b style='background:#0079AE;padding: 6px 9px 12px 9px;margin-top: 20%'><?php echo strtoupper($disp['Patient_Name'] . " | " . $disp['Gender'] . " | " . $disp['Guarantor_Name'] . " | " . $age); ?></b></center></td>
</tr>
</table>
    ...
    <center>
        ...
        <table border="1"  style="width:90%" class="hiv_table" bgcolor="white">
            ...
            while ($rows = mysqli_fetch_array($sql)) {
                ...
                <tr>
                    <td><?php echo $i; ?></td>
                    <td style="width:40%;"><input name="" type="" readonly='readonly' style="width:60%" value="<?php echo $rows['item_name']; ?>"></td>
                    <td>
                    ...
                    <td>
                    <input name="" class="paystatus" id="<?php echo $rows['Patient_Payment_Item_List_ID']; ?>" type="" readonly='readonly' style="width:98%" value="<?php
                    ...
                    <td style="border-left:1px solid #ccc;text-align:center">
                       ...
                       if($paymentstatus == "Not Billed" || $paymentstatus == "Paid" ){
                            $class_disable="";
                        }else{
                      ...
                      echo "<input type='checkbox' class='Patient_Payment_Item_List_ID_n_status'$checked $class_disable value='$Patient_Payment_Item_List_ID..kiunganishi$paymentstatus' name='item_id' onclick='Change_Status_unchecked(\"$Patient_Payment_Item_List_ID..kiunganishi$paymentstatus\",$Patient_Payment_Item_List_ID)' id='Patient_Payment_Item_List_ID'> ";
                        ?>
                    ...
                    <td style="border-left:1px solid #ccc;text-align:center">
                        <button style="width:150px;height:30px" class="viewSpecimen paid<?php echo $rows['Patient_Payment_Item_List_ID']; ?>" value="<?php echo $rows['Patient_Payment_Item_List_ID']; ?>" id="<?php echo $rows['item_id']; ?>"><b>Collect</b> Specimen</button>
                        ...
                        <input type="text" id="paymentListItem" style="display:none" value='<?php echo $rows['Patient_Payment_Item_List_ID']; ?>' />
                        <input type="text" id="rquiredDate" style="display:none" value='<?php echo $_GET['Required_Date']; ?>' />
                        <input type="text" id="statusFrom" style="display:none" value='<?php echo $rows['Status_From']; ?>' />
                        <input type="text" class="CollectDone" style="display:none" value='<?php echo $rows['Patient_Payment_Item_List_ID']; ?>' />

                        </button>
                        <a href='laboratory_setup_test.php?Item_ID=<?php echo $rows['item_id'] ?>&Status_From=<?php echo $rows['Status_From'] ?>&Patient_Payment_Item_List_ID=<?php echo $rows['Patient_Payment_Item_List_ID']; ?>&patient_id=<?php echo $patient_id ?>&payment_id=<?php echo $payment_id ?>&Required_Date=<?php echo $_GET['Required_Date'] ?>&LaboratorySetupTestThisPage=ThisPage'>
                            <button style="width:150px;height:30px">Define Specimen</button>
                        </a>
                    </td>
                    <td>
                        Send to Bill <input type="checkbox" <?= $class_disable_bill ?> class="send_to_bill_this_item"/>
                    </td>
                </tr>
            ...
            ?>
                <?php if($paymentstatus == "Not Billed" || $paymentstatus == "Paid" ){
                            $class_disable="";
...
<!--          <button class="art-button-green" id="itemIDAdd" onclick="Change_Status()"  style="margin-left:13px !important; color:white !important; font-weight:bold;" >Collect All</button>-->
                    <button class="art-button-green" id="itemIDAdd1" onclick="Change_Status1()"  style="margin-left:13px !important; color:white !important; font-weight:bold;" >Uncollect</button>
                    </td>
                    <td>
...
<fieldset>
    <table>
        ...
        <td>
            ...
            echo "<button class='art-button-green'>CREATE INPATIENT BILL</button>";
            echo "</center>";
            ?></td>
        </tr>
    </table>
      ...
      $("#bill_all").click(function (e){
       ...
       function Change_Status(){
        ...
        $.ajax({
            ...
            data:{selected_item:selected_item,Registration_ID:Registration_ID},
...
//                refresh_content()
    ...
    $('.viewSpecimen').click(function () {
        ...
        $.ajax({
            ...
            data: 'action=collect&id=' + id + '&paymentStatus=' + paymentStatus + '&trasanction=' + trasanction + '&patientId=' + patientId + '&paymentId=' + paymentId + '&paymentListItem=' + paymentListItem + '&rquiredDate=' + rquiredDate + '&statusFrom=' + statusFrom + '&val=' + val,
    ...
    $('#Filter').on('click',function(){
        ...
        $.ajax({
           ...
           data:'action=selectData&fromDate='+fromDate+'&toDate='+toDate+'&subcategory_ID='+subcategory_ID,
           cache:false,
           success:function(html){
             $('#iframeDiv').html(html);
           }
    ...
    $('#print_worksheet').on('click',function(){
       var fromDate=$('#Date_From').val();
        var toDate=$('#Date_To').val();
        var subcategory_ID=$('#subcategory_ID').val();
        window.open('Print_Worksheet.php?action=action&fromDate='+fromDate+'&toDate='+toDate+'&subcategory_ID='+subcategory_ID);
    });
...
// var printWindow= window.open("http://www.w3schools.com", "_blank", "toolbar=yes, scrollbars=yes, resizable=yes, top=500, left=500, width=400, height=400");
        var winClose = popupwindow('Patientfile_Record_Detail_General.php?Section=Doctor&Registration_ID=<?php echo $_GET['patient_id']; ?>&PatientFile=PatientFileThisForm', 'Patient File', 1300, 700);
        //winClose.close();
