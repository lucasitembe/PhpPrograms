...
if (isset($_SESSION['userinfo'])) {
    if ($_SESSION['userinfo']['Revenue_Center_Works'] == 'yes') {
        if(isset($_GET['Visitorsform']) && $_GET['Visitorsform']=='Visitorsform'){
        ?>
        <a href='visitorform.php?Registration_ID=<?= $Registration_ID?>Visitorsform=DirectDepartmentalListThisForm' class='art-button-green'>
            BACK
        </a>
    <?php } else {

        if(isset($_GET['from']) && $_GET['from'] == "depOPD") {
            echo "<a href='Direct_departmental_payment_outpatient.php?from=depOPD' class='art-button-green'>BACK</a>";
        } else {
            echo "<a href='directdepartmentalpayments.php?DirectDepartmentalList=DirectDepartmentalListThisForm' class='art-button-green'>
                BACK
            </a>";
        }
    }
...
//generate checkin id for outside patient
//check if patient has check in id
$Employee_ID = $_SESSION['userinfo']['Employee_ID'];
        ...
        <table width=100%>
            ...
            <tr>
                ...
                <td id="Type_Of_Check_In_Area">
                    <select name='Type_Of_Check_In' id='Type_Of_Check_In' required='required' style='width: 100%;' > 
            ...
            <tr> 
                ...
                <?php
                $Visit_Controler = 'new';
                //check if checked checked in today
                $slct = mysqli_query($conn,"select Visit_Date from tbl_check_in where Registration_ID = '$Registration_ID' order by Check_In_ID desc limit 1") or die(mysqli_error($conn));
                $slct_num = mysqli_num_rows($slct);
                if ($slct_num > 0) {
                    while ($dt = mysqli_fetch_array($slct)) {
                        $Visit_Date = $dt['Visit_Date'];
                    }
                } else {
                    $Visit_Date = '';
                }
                if ($slct_num > 0) {
                    //GET DURATION AND PROCEED KAMA NI CHINI YA SIKU MBILI
                    $date1 = new DateTime($Today);
                    $date2 = new DateTime($Visit_Date);
                    $diff = $date1->diff($date2);
                    $y = $diff->y;
                    $m = $diff->m;
                    $d = $diff->d;
                    if ($y == 0 && $m == 0 && $d <= 2) {
                        $Visit_Controler = 'continue';
                    }
                }
                ?>
                <td>
                    <select name="Visit_Type" id="Visit_Type" onchange="Specify_Visit_Type()">
                        <option selected="selected" value=""></option>
                        <?php
                        echo "<option>CT SCAN</option>";
                        if (strtolower($Support_Patient_From_Outside) == 'yes' || strtolower($Guarantor_Name) == 'cash') {
                            echo "<option>PATIENT FROM OUTSIDE</option>";
                        }
    ...
    function get_terminals(trans_type){
        ...
        if(trans_type.value=="Manual"){
            ...
            if(result){
                ...
                document.location = 'Departmental_Make_Payment.php?Registration_ID=' + Registration_ID + '&Claim_Form_Number=' + Claim_Form_Number + '&Consultant_ID=' + Consultant_ID + '&Visit_Type=' + Visit_Type + '&Visit_Controler=' + Visit_Controler+'&manual_offline=manual'; 
            }
        }else{
                $.ajax({
                type: 'GET',
      ...
      function offline_transaction(amount_required,reg_id){
               var Sponsor_ID = '<?php echo $Sponsor_ID; ?>';
        var uri = '../epay/directdepartmentalpaymentsoutpatientsOffline.php';
        ...
        if(comf){
            ...
            $.ajax({
                type: 'GET',
                url: uri,
                data: {amount_required:amount_required,registration_id:reg_id,Sponsor_ID:Sponsor_ID,Claim_Form_Number:Claim_Form_Number,Consultant_ID:Consultant_ID,Visit_Type:Visit_Type,Visit_Controler:Visit_Controler},
                     ...
                     $('#offlineProgressStatus').hide();
                        ...
                        <script type='text/javascript'>
                            function Make_Payment() {
                                ...
                                //    var r = confirm("Are you sure you want to make payment?\nClick OK to proceed or Cancel to stop process");
                                 //   if (r == true) {
                                   //     document.location = 'Departmental_Make_Payment.php?Registration_ID=' + Registration_ID + '&Claim_Form_Number=' + Claim_Form_Number + '&Consultant_ID=' + Consultant_ID + '&Visit_Type=' + Visit_Type + '&Visit_Controler=' + Visit_Controler;
                                  //  }
                                }
                            }
                        </script>
                            ...
                            function update_total() {
                                ...
                                My_Object_Update_Total.onreadystatechange = function () {
                                    ...
                                    if (My_Object_Update_Total.readyState == 4) {
                                        document.getElementById('Total_Area').innerHTML = "TOTAL : "+data600;
                                        document.getElementById('grand_total_txt').value=data600;
                                        //update_total();
                        ...
                        <script>
                            function Refresh_Items_Div() {
                                ...
                                myObjectRefreshDiv.open('GET', 'Refresh_Departmental_Payments_Div.php?Type_Of_Check_In=' + Type_Of_Check_In + '&Sponsor_ID=' + Sponsor_ID + '&Visit_Type=' + Visit_Type + '&Transaction_Mode=' + Transaction_Mode, true);
                        ...
                        <script>
                            function update_process_buttons(Registration_ID) {
                                ...
                                my_Object_Update_Process.open('GET', 'Departmental_Update_Process_Button.php?Registration_ID=' + Registration_ID, true);
                                ...
                                if (Amount <= 0 || Amount == null || Amount == '' || Amount == '0') {
                                    ...
                                    if (Payment_Mode != null && Payment_Mode != '') {
                                        if (Payment_Mode == 'Bank_Payment') {
                                            ...
                                            if (Confirm_Message == true) {
                                                document.location = 'Departmental_Prepare_Bank_Payment_Transaction.php?src=droup&Section=departmental&Registration_ID=' + Registration_ID;
                                            }
                        ...
                        <script>
                            function getItemsList(Item_Category_ID) {
                                ...
                                myObject.open('GET', 'Get_List_Of_Departmental_Items_List.php?Item_Category_ID=' + Item_Category_ID + '&Guarantor_Name=' + Guarantor_Name + '&Type_Of_Check_In=' + Type_Of_Check_In + '&Visit_Type=' + Visit_Type+"&Sponsor_ID="+Sponsor_ID, true);
                                myObject.send();
                            }
                        </script>
                        ...
                        <script type='text/javascript'>
                            function Save_Information() {
                                ...
                                if (Visit_Type == null || Visit_Type == '' || Consultant_ID == '' || Consultant_ID == null <?php
                                    if ($Claim_Number_Status == 'mandatory') {
                                        echo "|| (Claim_Form_Number == '' || Claim_Form_Number == null && Billing_Type == 'Outpatient Credit')";
                                    }
                                    ?>) {
                                    if (Consultant_ID == '' || Consultant_ID == null) {
                                        document.getElementById("Consultant_ID").focus();
                                    ...
                                    if (r == true) {
                                        document.location = 'Departmental_Save_Information.php?Registration_ID=' + Registration_ID + '&Claim_Form_Number=' + Claim_Form_Number + '&Consultant_ID=' + Consultant_ID + '&Visit_Type=' + Visit_Type + '&Visit_Controler=' + Visit_Controler;
                                    }
                                }
                            }
                        </script>
                       ...
                       function create_epayment_mobile_card_bill(){
                                ...
                                var Clinic_ID = document.getElementById("Clinic_ID").value;
                                    ...
                                    if(confirm("Are You sure you want to go to Mobile/Card Payment")){
                                        $.ajax({
                                            type:'GET',
                                            url:'ajax_create_epayment_mobile_card_bill_departmental_payment_outpatient.php',
