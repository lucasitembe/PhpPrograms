...
<?php
    if(isset($_SESSION['userinfo'])){
        if($_SESSION['userinfo']['Setup_And_Configuration'] == 'yes'){ 
?>
    <a href='edititemlist.php?EditItem=EditItemThisPage' class='art-button-green'>
        BACK
    </a>
<?php  } } ?>
    ...
    <tr>
        <td>
		 ...
		 <tr>
			...
			<?php } ?>
                        <a href='edititemlist.php?EditItem=EditItemThisPage' class='art-button-green'>CANCEL</a> 
...
<?php
    if(isset($_POST['submittedEditItemForm'])){
        ...
        ?>
            <script>
                alert("You must fill the Unit of Measure"); 
            </script>
        <?php
            exit;
        }
...
if(isset($_POST['Can_Be_Substituted_In_Doctors_Page'])) {
    $Can_Be_Substituted_In_Doctors_Page = 'yes';
    $Activity_done = 'Can Be Substituted In Doctors Page, ';
}else{
    $Can_Be_Substituted_In_Doctors_Page = 'no';
}
...
if(isset($_POST['consultation'])) {
    $consultation = 'yes';
    $Activity_done .= 'Consultation Item, ';
...
if(isset($_POST['Seen_On_Allpayments'])) {
    $Seen_On_Allpayments = 'yes';
    $Activity_done .= 'Can be seen in All Payment, ';
}else{
    $Seen_On_Allpayments = 'no';
}
if(isset($_POST['daily_add_automaticaly_in_inpatient_bill'])) {
    $daily_add_automaticaly_in_inpatient_bill = 'yes';
    $Activity_done .= 'Daily Add Automatically in Inpatient Bill, ';
}else{
    $daily_add_automaticaly_in_inpatient_bill = 'no';
}
...
if(isset($_POST['Free_Consultation_Item'])) {
    $Free_Consultation_Item = '1';
    $Activity_done .= 'Free Consultation Item, ';
...
// AUDIT TRAIL
	    ...
	    $Update_New_Item = "UPDATE tbl_items set
                ...
                Particular_Type = '$Particular_Type',Seen_On_Allpayments='$Seen_On_Allpayments', Free_Consultation_Item = '$Free_Consultation_Item'
	...
	if(!mysqli_query($conn,$Update_New_Item)){ 
            ...
            if(empty($Activity_done)){
                $Activity_done = "NOTHING WAS TICKED";
            }
            $audit_edit = mysqli_query($conn, "INSERT INTO tbl_item_audit(Item_ID, Activity_done, Employee_ID, Edited_Time) VALUES('$Item_ID', '$Activity_done', '$Current_Employee_ID', NOW())") or(mysqli_error($conn));
		    echo '<script>
