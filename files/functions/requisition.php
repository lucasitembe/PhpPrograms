<?php
    ...
    function Get_Requisition($Requisition_ID) {
        ...
        $Requisition_Result = Get_From("tbl_requisition", array("Requisition_ID", "=", $Requisition_ID), array(), 1);
    ...
    function List_Requisition($Sub_Department_ID, $Requisition_Status, $Start_Date, $End_Date, $Requisition_ID, $Limit = 0) {
        ...
        $Requisition_List_Result = Query_DB("SELECT Created_Date, Requisition_ID, Requisition_Status,
                    ...
                    (SELECT Sub_Department_Name FROM tbl_sub_department ssd WHERE ssd.Sub_Department_ID = ri.Store_Issue) as Store_Issuing,
    ...
    function Get_Requisition_Items($Requisition_ID) {
        ...
        $Requisition_Item_Result = Query_DB("SELECT i.Product_Name, ii.Requisition_Item_ID, ii.Item_ID,
                                                ii.Quantity_Required 	, ii.Item_Remark,
                                                im.Store_Need, im.Store_Issue,
                                                (SELECT Item_Balance FROM tbl_items_balance ib
                                                WHERE ib.Item_ID = ii.Item_ID
                                                AND ib.Sub_Department_ID = im.Store_Need) as Store_Need_Balance,
                                                (SELECT Item_Balance FROM tbl_items_balance ibb
                                                WHERE ibb.Item_ID = ii.Item_ID
                                                AND ibb.Sub_Department_ID = im.Store_Issue) as Store_Issue_Balance
                                                FROM tbl_requisition_items ii, tbl_items i, tbl_requisition im
                                                WHERE ii.Item_ID = i.Item_ID
                                                AND ii.Requisition_ID = im.Requisition_ID
                                                AND ii.Requisition_ID = '{$Requisition_ID}'");
        $hasError = $Requisition_Item_Result["error"];
        if (!$hasError) {
            $Requisition_Item_List = $Requisition_Item_Result["data"];
        } else {
            echo $Requisition_Item_Result["errorMsg"];
        }
