...
if (isset($_POST['payment_id'])) {
   ...
   if(mysqli_num_rows($sql_check_if_test_result)>0){
       echo "updatelkdokdjiojioer";
       //update validate status
       $sql_update_validate_status_for_this_page_result=mysqli_query($conn,"UPDATE tbl_tests_parameters_results SET Submitted='Yes',Saved='Yes',modified='Yes',SavedBy='$Employee_ID',TimeSubmitted=NOW() WHERE ref_test_result_ID IN (SELECT test_result_ID FROM tbl_test_results WHERE payment_item_ID='$payment_id')") or die(mysqli_error($conn));
   }else{
       echo "new";
       ...
       $sql_insert_validate_status_result=mysqli_query($conn,"INSERT INTO tbl_tests_parameters_results (ref_test_result_ID,parameter,modified,Submitted,Saved,SavedBy,TimeSubmitted) VALUES('$test_result_ID','$ref_parameter_ID','yes',Yes','Yes','$Employee_ID',NOW())") or die(mysqli_error($conn));
   }
}