<?php
	...
	$select = mysqli_query($conn,"select Registration_ID, Consultation_ID, Admision_ID from tbl_patient_progress where Progress_ID = '$Progress_ID'") or die(mysqli_error($conn));
		...
		if($num > 0){
			while ($data = mysqli_fetch_array($select)) {
				$htm .= '<table width="100%" border=1 style="border-collapse: collapse;">
						<tr>
							<td  width="60%" colspan="3"><span style="font-size: x-small;">'.++$temp.'<b>. Date And Time : </b>'.@date("d F Y H:i:s",strtotime($data['Checked_Date_Time'])).'</td>
							<td width="40%" style="text-align: left;" colspan="2"><b>Nurse Name : </b>'.ucwords(strtolower($data['Employee_Name'])).'</td>
						</tr>';
				$htm .=	'<tr>
