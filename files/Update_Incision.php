<?php
	...
	if(isset($_GET['Incision_Value'])){
		$Incision_Value = mysqli_real_escape_string($conn,$_GET['Incision_Value']);
	}else{
		$Incision_Value = '';
	}
	...
	if($Employee_ID != '' && $Payment_Item_Cache_List_ID != '' && $consultation_ID != 0){
	    ...
	    $update = mysqli_query($conn,"update tbl_post_operative_notes set Incision = '$Incision_Value' where Post_operative_ID = '$Post_operative_ID'") or die(mysqli_error($conn));
	    if($update){
	    	echo $Incision_Value;
	    }
	}
