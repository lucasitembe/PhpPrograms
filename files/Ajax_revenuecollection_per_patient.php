...
if(isset($_POST['Revenuecollectionfilter'])){
    ...
    echo "<table width='100%' class='table-hover'> <thead>
    <tr>
       ...
       echo " <td id='thead' ><b>SPONSOR</b></td>
...
echo "<tr><td colspan='16' style='background:green; color:white; text-align:center;'>INPATIENT RESULT</td></tr>";
    ...
    if(mysqli_num_rows($select_patient_details)>0){
        while($row = mysqli_fetch_assoc($select_patient_details)){
            ...
            echo "<tr>
                    ...
                    <td  style='box-sizing:border-box; '><h6>".ucfirst($patient_name)." ($Registration_ID)</h6></td>";
    ...
    if(mysqli_num_rows($sql_payments_amaount)>0){
        while($rwdt = mysqli_fetch_assoc($sql_payments_amaount)){
            ...
            $sql_select_cash_pay = mysqli_query($conn, "SELECT  sum((ptl.Price - ptl.Discount) * ptl.Quantity) as Amount from  tbl_patient_payment_item_list ptl, tbl_patient_payments pp WHERE pp.Registration_ID='$Registration_ID' and pp.Payment_Date_And_Time BETWEEN '$start_date' AND '$end_date' and ptl.Patient_Payment_ID = pp.Patient_Payment_ID AND Pre_Paid='0' AND pp.payment_type ='pre'  AND pp.Transaction_status <> 'cancelled'  AND Billing_Type IN ('Inpatient Cash')  $filter") or die(mysqli_error($conn));
            ...
            $sql_select_cash_pays_bill = mysqli_query($conn, "SELECT  sum((ptl.Price - ptl.Discount) * ptl.Quantity) as Amount from  tbl_patient_payment_item_list ptl, tbl_patient_payments pp WHERE pp.Registration_ID='$Registration_ID' and pp.Payment_Date_And_Time BETWEEN '$start_date' AND '$end_date' and ptl.Patient_Payment_ID = pp.Patient_Payment_ID AND Pre_Paid IN ('1', '0') AND pp.payment_type = 'post' AND pp.Transaction_status <> 'cancelled' $filter AND Billing_Type IN ( 'Inpatient Cash') ") or die(mysqli_error($conn));
    ...
    echo "
                ...
                echo "<td >".number_format($subtotal_cash)."</td>
                <td>".number_format($subtotal_credit)."</td>";
               
               echo " <td >".number_format($Total_Discount)."</td>
                                
        ";
    echo "</tr>";
    }
    }else{
        echo "<tr><td colspan='16' style='color:red; font-size:15px;'>No result found since $start_date to $end_date</td></tr>";
    }
    //===============+++ TOTAL FOR INPATIENT +++==============
    ...
    echo "<tr><td colspan='16' style='background:green; color:white; text-align:center;'>OUTPATIENT RESULT</td></tr>
    <tr>
        <td id='thead' ><b>SN</b></td>
        <td id='thead' ><b>PATIENT NAME (REG:#)</b></td>";
        // <td id='thead' ><b>DATE</b></td>
    ...
    if(mysqli_num_rows($select_patient_details)>0){
        while($row = mysqli_fetch_assoc($select_patient_details)){
            ...
            $nums++;
            echo "<tr>
                    <td  style='width:3%;box-sizing:border-box;'>".$nums."</td>";
                    // <td>$Payment_Date_And_Time</td>
                   echo " <td  style='width:15%;box-sizing:border-box; '><h6>".ucfirst($patient_name)." ($Registration_ID)</h6></td>";
                    // <td  >".$gender."</td>
                    // <td  style='width:5%;box-sizing:border-box; '>".$dob."</td>
                    echo "<td  >".($Guarantor_Name)."</td>";
    ...
    echo "
                ...
                echo "<td $background>".number_format($subtotal_cash)."</td>
                <td $background>".number_format($subtotal_credit)."</td>";
               
               echo " <td $background>".number_format($Total_Discount)."</td>
 ...
 echo "<tr><td colspan='11' style='background:yellow; color:white; font-size:20px;' ><h5><b>DEBIT BILLS PAID BY CASH DEPOSIT </b></h5></td><td colspan='5' style='background:yellow; color:white; font-size:20px;'><h5><b>".number_format($Waliolipamadeni, 2)."</b></h5></td><td></tr>";
    
    //================++ END PATIENT FOR DEPOSIT ++==============
    //=====================+ OVERALL TOTAL +====================== 

    echo "<tr><td colspan='16'></td></tr><tr>
