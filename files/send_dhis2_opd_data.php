...
<?php
	...
	if($group_num > 0){
		...
		while($details = mysqli_fetch_array($select_groups)){
			...
			//pushing to array
			array_push($dhis_data_array, setDHISopdData($dhis_data,array('report_type'=>'opd','dataset'=>'v6wdME3ouXu')));
	...
	//echo json_encode($dhis_data_array);
	$sent_count = 0;
	$failed_count = 0;
	foreach ($dhis_data_array as $data) {
		$response = sendDataToDHIS($data);
                
                //echo '<pre>';
    //print_r($dhis_data_array);    echo '<br/><br/>';;
	...
	echo "Sent entry: ".$sent_count;
	echo "<br>";
	echo "Failed entry: ".$failed_count;
