                ...
                <tbody>
                    <?php 
                        ...
                        if(mysqli_num_rows($sql_select_all_dispensing_unit_result)>0){
                            ...
                            //select all items
                                   $sql_select_all_items_with_price_result=mysqli_query($conn,"SELECT ppl.Sub_Department_ID,pp.payment_type,Billing_Type,Pre_Paid,Sponsor_ID,Discount,Price,Quantity FROM tbl_patient_payment_item_list ppl,tbl_patient_payments pp WHERE ppl.Patient_Payment_ID=pp.Patient_Payment_ID AND pp.Payment_Date_And_Time BETWEEN '$start_date' AND '$end_date' AND ppl.finance_department_id='$finance_department_id' AND ppl.Item_ID='$Item_ID' AND Sub_Department_ID='$Sub_Department_ID'") or die(mysqli_error($conn));
