...
if($Surgical_Type != 'All'){
    $filter .= " AND pc.Billing_Type LIKE '%$Surgical_Type%'";
}else{
    $filter .= "";
}
?>
        <style>
            ...
            $select_Filtered_Donors = mysqli_query($conn, "SELECT pc.Registration_ID, sd.Sub_Department_Name, sap.Anaesthesia_Type,  pr.Patient_Name, ilc.Transaction_Date_And_Time, pr.Date_Of_Birth, pr.Gender, 
            ...
            i.Item_ID = ilc.Item_ID AND em.Employee_ID = sap.Surgeon_filled $filter AND ilc.Payment_Item_Cache_List_ID = sap.Payment_Item_Cache_List_ID AND sap.Surgery_Status = 'Authorized' AND sap.Final_Decision = 'Accepted' ORDER BY ilc.Service_Date_And_Time ASC") or die(mysqli_error($conn));
            ...
            if(mysqli_num_rows($select_Filtered_Donors)>0){
                ...
                while($rows = mysqli_fetch_assoc($select_Filtered_Donors)){
                    ...
                    $Anaesthesia_Type_Selected = $rows['Anaesthesia_Type'];


                    if(!empty($Anaesthesia_Type_Selected)){
                        $Usingizi = $Anaesthesia_Type_Selected;
                    }else{
                        $Usingizi = "Not Specified";
                    }
                    ...
                    $age .= $diff->d . " Days";
                        ...
                        echo "<td>
                                <select id='assign".$Payment_Item_Cache_List_ID."' $condition class='Mark_This_Item' onchange='add_reason(".$Payment_Item_Cache_List_ID.")' style='text-align: center;width:100%;display:inline; height: 32px; border-radius: 5px; font-size: 12px;'>
                                    <option value='Accept'>Fit</option>
                                    <option value='Reject'>Not Fit</option>
                                </select>
                            </td>
                            <td>
                                <select id='Type_Of_Anesthetic".$Payment_Item_Cache_List_ID."' $condition class='Mark_This_Item' onchange='add_reason(".$Payment_Item_Cache_List_ID.")' style='text-align: center;width:100%;display:inline; height: 32px; border-radius: 5px; font-size: 12px;'>";

                                if(!empty($Anaesthesia_Type_Selected)){
                                    echo "<option selected='selected'>".$Anaesthesia_Type_Selected."</option>";
                                }else{
                ...
                echo "<tr><td style='font-size: 17px; color: #bd0d0d; font-weight: bold; text-align: center;' colspan='13'>NO PATIENT SELECTED FOR THIS OPERATION, PLEASE SELECT TO PREPARE SURGERY LIST</td></tr>";
            }

            if(mysqli_num_rows($select_Filtered_Donors)>0){
                echo '<tr>
                            <td colspan="11" style="text-align: center;"><input type="button" name="Submit_Information" id="Submit_Information" value="APPROVE SURGERY LIST"  style="font-weight: bold; padding: 10px; border-radius: 7px;" class="art-button-green" onclick="save_surgery_list()"></td></tr>
                ';
