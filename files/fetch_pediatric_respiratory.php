  ...
  if ($result=mysqli_query($conn,$select_pediatric_graph)) {
    ...
    if (($num = mysqli_num_rows($result)) > 0) {
      ...
      while ($row = mysqli_fetch_assoc($result)) {
        ...
        $final_time_respiratory_rate=$TimeStamp_hour+(($TimeStamp_min/60));
        $d['final_time_respiratory_rate'] = $final_time_respiratory_rate;
        $d['time'] = $time;
        $d['respiratory_rate'] = $row['respiratory_rate'];
