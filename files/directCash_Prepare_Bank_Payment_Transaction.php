...
$get_spo = mysqli_query($conn,"select pr.Sponsor_ID from tbl_patient_registration pr where
...
//move data
                $move = mysqli_query($conn,"insert into tbl_bank_items_detail_cache(Check_In_Type, Billing_Type, Sponsor_ID, 
													...
													Transaction_Date_And_Time, Transaction_Date,Branch_ID,comments,Clinic_ID,finance_department_id,clinic_location_id)

													VALUES ('$Check_In_Type','$Billing_Type','$Sponsor_ID',
															'$Item_ID','$Discount','$Price','$Quantity',
															'Others',$Employee_ID,
															'$Employee_ID','$Registration_ID','$Transaction_ID',
			   												(select now()), (select now()), '$Branch_ID','$comments','$Clinic_ID','$finance_department_id','$clinic_location_id')") or die(mysqli_error($conn));
            ...
            if (!$_SESSION['HAS_ERROR']) {
                ...
                header("Location: ./crdbtransactiondetails.php?Section=$Section&CRDBTransactionDetails=CRDBTransactionDetailsThisPage&Sponsor_ID=$Sponsor_ID");
            } else {
