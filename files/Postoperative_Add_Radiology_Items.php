<?php
    ...
    if(empty($Sponsor_ID)){
        $select = mysqli_query($conn,"SELECT sp.Sponsor_ID, sp.payment_method, sp.Guarantor_Name from tbl_patient_registration pr, tbl_sponsor sp where
    ...
    <tr>
        <td width=40%>
            <table width=100% style='border-style: none;'>
                    ...
                    <td>
                        ...
                        <select name='Item_Category_ID' id='Item_Category_ID' onchange='getItemsList_Laboratory(this.value)' onchange='Calculate_Amount()' onkeypress='Calculate_Amount()'>
                            ...
                            $data = mysqli_query($conn,"SELECT cat.Item_Category_Name, cat.Item_Category_ID
                                                    ...
                                                    i.Item_ID IN (SELECT Item_ID FROM tbl_item_price WHERE Sponsor_ID = '$Sponsor_ID' AND Items_Price <> 0) AND
                                                    i.Can_Be_Sold = 'yes' 
                ...
                <tr>
                    <td>
                        <input type='text' id='Search_Value' name='Search_Value' autocomplete='off' onkeyup='getItemsListFiltered_Labolatory(this.value)' placeholder='Enter Item Name'>
                    </td>
                </tr>
                <tr>
                    <td>
                        <fieldset style='overflow-y: scroll; height: 330px;' id='Items_Fieldset'>
                            <table width=100%>
                                <?php
//                                    $result = mysqli_query($conn,"select Product_Name, Item_ID from tbl_items where Consultation_Type = 'Radiology' and Status = 'Available' and Can_Be_Sold = 'yes' order by Product_Name limit 150");
                                    
                                    
                                     $SQL_SELECT = "SELECT t.Item_ID, t.Product_Name  from tbl_items t, tbl_item_subcategory s, tbl_item_category c, tbl_item_price ip
                    where t.Item_Subcategory_ID = s.Item_Subcategory_ID and
                       ...
                       (t.Consultation_Type='Radiology') and
                             t.Status='Available' AND ip.Sponsor_ID='$Sponsor_ID' and ip.Items_Price<>'0' order by t.Product_Name LIMIT 50";
                                   $result= mysqli_query($conn,$SQL_SELECT) or die(mysqli_error($conn));
                                    while ($row = mysqli_fetch_array($result)) {
                                        echo "<tr><td style='color:black; border:2px solid #ccc;text-align: left;' width=5%>";
                                ?>
                                        <input type='radio' name='selection' id='<?php echo $row['Item_ID']; ?>' value='<?php echo $row['Product_Name']; ?>' onclick="Get_Item_Name_Laboratory(this.value,<?php echo $row['Item_ID']; ?>,'Radiology'); Get_Item_Price(<?php echo $row['Item_ID']; ?>, '<?php echo $Guarantor_Name; ?>');">
                                <?php
        ...
        <td>
            <table width="100%">
                        ...
                        <select name='Sub_Department_ID' id='Sub_Department_ID'>
                            <?php
                                $select = mysqli_query($conn,"select Sub_Department_Name, Sub_Department_ID from tbl_department dep, tbl_sub_department sdep
                                                        ...
                                                        Department_Location = 'Radiology'") or die(mysqli_error($conn));
                                $num = mysqli_num_rows($select);
                                if($num > 1){
                ...
                <tr>
                    ...
                    <td style='text-align: right;'>
                        <input type='button' name='Submit' id='Submit' class='art-button-green' value='ADD INVESTIGATION' onclick='Get_Selected_Item_Laboratory("Radiology")'>
                    </td>
                </tr>
            </table><br/>
            ...
            <?php
                ...
                $select = mysqli_query($conn,"select ilc.Payment_Item_Cache_List_ID, i.Product_Name, ilc.Transaction_Type, ilc.Price, ilc.Quantity, ilc.Status from tbl_item_list_cache ilc, tbl_items i where
                                        ...
                                        Check_In_Type = 'Radiology'") or die(mysqli_error($conn));
