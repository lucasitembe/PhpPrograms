				...
				FROM 	
					...
					WHERE 
						...
						a.Discharge_Reason_ID = dr.Discharge_Reason_ID AND
                                                a.Discharge_Employee_ID = e.Employee_ID AND
...
while ($row = mysqli_fetch_array($results)) {
    ...
    echo '<tr><td>' . $temp . '</td>';
    echo "<td style='font-size:13px;'><a target='_blank' href='print_patient_discharge_summery.php?consultation_ID=".$row['consultation_ID']."&Registration_ID=".$row['Registration_ID']."' title='Click to go to discharge patient Summer'>" . ucfirst($row['Patient_Name']) . "</a></td>";
