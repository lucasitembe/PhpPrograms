	...
	$Link_Control = 'true';
	  ...
	  if($transaction_channel != "All"){
       if($transaction_channel=="CRDB"){
           $transaction_channel_filter=" AND pp.manual_offline != 'Mobile Online'";
	   }else if($transaction_channel=="CRDBmobile"){
		   $transaction_channel_filter ="and pp.payment_direction = 'to_crdb'";
       }else if($transaction_channel=="NMB"){
		  //  $table_card_mobile = ",tbl_card_and_mobile_payment_transaction crd ";
			$transaction_channel_filter ="and pp.payment_direction = 'to_nmb'";
...
<legend align="center" style="background-color:#006400;color:white;padding:5px;">
		...
		<?php echo "<b>All Employee Performance Report</b>"; ?>
...
<form action='#' method='post' name='myForm' id='myForm' onsubmit="return validateForm();" enctype="multipart/form-data">
    <table width=100%>
        <tr>
            <td style='text-align: center;' width=5%>From</td>
            <td style="text-align: center; border: 1px #ccc solid;width: 15%;">
                <input type='text' autocomplete='off' name='Date_From' id='date_From' style="background-color:#eeeeee;" required='required'>
            </td>
            <td style='text-align: center;' width=5%>To</td>
            <td style="text-align: center; border: 1px #ccc solid;width: 15%">
                <input type='text' name='Date_To' autocomplete='off' id='date_To' style="background-color:#eeeeee;" required='required'>
            </td>

            <td width = '15%' style='text-align: center;'><b>Transaction Type</b></td>
            <td width = '6%'>
			...
			<td>
				...
				<a href='allEmployeePerformanceExcelReport.php?Date_From=<?php echo $Date_From; ?>&Date_To=<?php echo $Date_To; ?>&Transaction_Type=<?php echo $Transaction_Type; ?>&transaction_channel=<?php echo $transaction_channel;?>&EmployeePerformance=EmployeePerformanceThisForm' class='art-button-green' target='_blank'>EXCEL PREVIEW</a>
			    <?php } ?>
...
if($Transaction_Type == 'All'){
        $select_Filtered_Employees = mysqli_query($conn, 
                "SELECT pp.Patient_Payment_ID, pp.Payment_Date_And_Time, pp.Billing_Type, pp.Transaction_status, sum((price*quantity)-(discount*quantity)) as Total, pp.payment_type, pp.Pre_Paid,pp.Payment_Code,pp.payment_mode,pp.auth_code,pp.manual_offline from tbl_patient_payments pp, tbl_patient_payment_item_list ppl where pp.Patient_Payment_ID = ppl.Patient_Payment_ID and  pp.Payment_Date_And_Time between '$Date_From' and '$Date_To' group by pp.Patient_Payment_ID order by pp.Payment_Date_And_Time") or die(mysqli_error($conn)); 
    }elseif($Transaction_Type == 'Cash'){
        $select_Filtered_Employees = mysqli_query($conn, 
                "SELECT pp.Patient_Payment_ID, pp.Payment_Date_And_Time, pp.Billing_Type, pp.Transaction_status, sum((price*quantity)-(discount*quantity)) as Total, pp.payment_type, pp.Pre_Paid,pp.Payment_Code,pp.payment_mode,pp.auth_code,pp.manual_offline from tbl_patient_payments pp, tbl_patient_payment_item_list ppl where pp.Patient_Payment_ID = ppl.Patient_Payment_ID $transaction_channel_filter and pp.Billing_Type in ('Outpatient Cash','Inpatient Cash') and pp.payment_type = 'pre' and pp.Payment_Date_And_Time between '$Date_From' and '$Date_To' group by pp.Patient_Payment_ID order by pp.Payment_Date_And_Time") or die(mysqli_error($conn)); 
    }elseif($Transaction_Type == 'Credit'){
        $select_Filtered_Employees = mysqli_query($conn,"SELECT pp.Patient_Payment_ID, pp.Payment_Date_And_Time, pp.Billing_Type, pp.Transaction_status, sum((price*quantity)-(discount*quantity)) as Total, pp.payment_type, pp.Pre_Paid,pp.Payment_Code,pp.payment_mode,pp.auth_code,pp.manual_offline from tbl_patient_payments pp, tbl_patient_payment_item_list ppl
        ...
        $select_Filtered_Employees = mysqli_query($conn, 
                "select pp.Patient_Payment_ID, pp.Payment_Date_And_Time, pp.Billing_Type, pp.Transaction_status, sum((price*quantity)-(discount*quantity)) as Total, pp.payment_type, pp.Pre_Paid,pp.Payment_Code,pp.payment_mode,pp.auth_code,pp.manual_offline from tbl_patient_payments pp, tbl_patient_payment_item_list ppl
                    where pp.Patient_Payment_ID = ppl.Patient_Payment_ID and
                            pp.Transaction_status = 'cancelled' and
                                pp.Payment_Date_And_Time between '$Date_From' and '$Date_To' group by pp.Patient_Payment_ID order by pp.Payment_Date_And_Time") or die(mysqli_error($conn)); 
    }else{
        $select_Filtered_Employees = mysqli_query($conn, 
                "SELECT pp.Patient_Payment_ID, pp.Payment_Date_And_Time, pp.Billing_Type, pp.Transaction_status, sum((price*quantity)-(discount*quantity)) as Total, pp.payment_type, pp.Pre_Paid,pp.Payment_Code,pp.payment_mode,pp.auth_code,pp.manual_offline from tbl_patient_payments pp, tbl_patient_payment_item_list ppl where pp.Patient_Payment_ID = ppl.Patient_Payment_ID and  pp.employee_id = 0 and
                            pp.Payment_Date_And_Time between '$Date_From' and '$Date_To' group by pp.Patient_Payment_ID order by pp.Payment_Date_And_Time") or die(mysqli_error($conn)); 
    }
    //declare all total
    $Cash_Total = 0;
    ...
    while($row = mysqli_fetch_array($select_Filtered_Employees)){
        ...
        if(((strtolower($row['Billing_Type']) == 'outpatient cash'&& $row['Pre_Paid']==0) ||(strtolower($row['Billing_Type']) =="patient from outside"  && $row['Pre_Paid'] == '0') || (strtolower($row['Billing_Type']) == 'inpatient cash' && $row['payment_type'] == 'pre')) && strtolower($row['Transaction_status']) != 'cancelled'  && ($row['auth_code'] != '' || $row['manual_offline'] = 'manual' || ($row['Payment_Code'] != '' && ($row['payment_mode']== 'CRDB' || $row['payment_mode']== 'CRDB..' )))){
...
<!-- END OF CALCULATION-->
        ...
        <center>
            <table width=100% border=0>
                <tr>
                    <td>
                        <iframe width='100%' height=330px src="All_Employee_Performance_Report_Ifrmae.php?Section=<?php echo $Section; ?>&Date_From=<?php echo $Date_From; ?>&Date_To=<?php echo $Date_To; ?>&Transaction_Type=<?php echo $Transaction_Type; ?>&transaction_channel=<?php echo $transaction_channel; ?>"></iframe>
                    </td>
                </tr>
            </table>
	    <table width = 90% class='hide'>
	    <tr> 
