	    ...
	    <table width=100% style='border-style: none;'>
		  ...
		  <td>
			...
			<?php
				if(strtolower($Type_Of_Check_In) == 'laboratory' || strtolower($Type_Of_Check_In) == 'radiology' || strtolower($Type_Of_Check_In) == 'procedure'|| strtolower($Type_Of_Check_In) == 'optical' || strtolower($Type_Of_Check_In) == 'surgery' || strtolower($Type_Of_Check_In) == 'admission'){
			   		if(strtolower($Visit_Type) == 'ct scan'){
			   			$result = mysqli_query($conn,"select * FROM tbl_items i INNER JOIN tbl_item_price ip ON i.Item_ID=ip.Item_ID where Consultation_Type = '$Type_Of_Check_In' AND Status='Available' AND Ct_Scan_Item = 'yes' AND ip.Sponsor_ID='$Sponsor_ID' and ip.Items_Price<>'0' order by Product_Name limit 100");
			   		}else{
			   			$result = mysqli_query($conn,"select * FROM tbl_items i INNER JOIN tbl_item_price ip ON i.Item_ID=ip.Item_ID where Consultation_Type = '$Type_Of_Check_In' AND Status='Available' AND Ct_Scan_Item <> 'yes' AND ip.Sponsor_ID='$Sponsor_ID' and ip.Items_Price<>'0' order by Product_Name limit 100");
			   		}
				} else {
					$result = mysqli_query($conn,"select * FROM tbl_items i INNER JOIN tbl_item_price ip ON i.Item_ID=ip.Item_ID where 
						Consultation_Type <> 'laboratory' and  Consultation_Type <> 'radiology' and 
						Consultation_Type <> 'procedure' and Consultation_Type <> 'Optical' and 
						Consultation_Type <> 'Surgery' and Status = 'Available' and 
						Consultation_Type <> 'Pharmacy' and Consultation_Type <> 'Admission' and Ct_Scan_Item <> 'yes' AND ip.Sponsor_ID='$Sponsor_ID' and ip.Items_Price<>'0' order by Product_Name limit 100") or die(mysqli_error($conn));
				}
	    ...
	    <table width=100% border=0>
          ...
          <td>
             <select name="Sub_Department_ID" id="Sub_Department_ID">
				...
				//if(strtolower($Type_Of_Check_In) == 'others'){
				...
				//}else{
                    $select = mysqli_query($conn,"select sd.Sub_Department_ID, sd.Sub_Department_Name from
                                            ...
                                            $Consultation_Condition and Sub_Department_Status = 'active'") or die(mysqli_error($conn));
