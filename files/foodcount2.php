...
<?php  } } ?>
 ...
 <fieldset style="width:80%;height:360px;overflow:scroll;">
  ...
  $select_Food=mysqli_query($conn,"SELECT * 
			...
			WHERE ft.Food_Time='LUNCH'  AND DATE(Trans_Date_Time)='".DATE("Y-m-d")."' AND 
	ft.Food_Menu_ID=fm.Food_Menu_ID AND pr.Registration_ID=ft.Registration_ID GROUP BY  fm.Food_Menu_ID") or die (mysqli_error($conn));
...
// AND DATE(Trans_Date_Time)='".DATE("Y-m-d")."' 
//Food_Standard

$select_Name=mysqli_query($conn,"SELECT * 
			FROM 
				tbl_food_transaction ft,tbl_food_menu fm,tbl_patient_registration pr	
			WHERE ft.Food_Time='LUNCH'  AND 
			DATE(Trans_Date_Time)='".DATE("Y-m-d")."' AND 
