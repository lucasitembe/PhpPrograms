...
$select_Filtered_Patients = "
				...
				FROM 	
					...
					tbl_admission a,
                                        tbl_employee e
					WHERE 
					a.Admision_ID NOT IN(SELECT ma.Admision_ID FROM tbl_mortuary_admission ma WHERE ma.Admision_ID=a.Admision_ID) AND
						a.registration_id = pr.registration_id AND
						a.registration_id = pr.registration_id AND
						pr.Sponsor_ID = sp.Sponsor_ID AND 
						hw.Hospital_Ward_ID = a.Hospital_Ward_ID AND
                                                a.Admission_Employee_ID = e.Employee_ID
                                                $filter ";
...
$htm = '<table width ="100%" border="0" style="background-color:white;" class="nobordertable">
          ...
          <tr>
             <td  style="text-align:center"><b>ADMITTED PATIENT LIST</b><br/> </td>
          </tr>';
