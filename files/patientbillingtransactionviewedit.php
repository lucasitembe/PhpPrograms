...
<?php
    if(isset($_SESSION['userinfo'])){
        if(($_SESSION['userinfo']['Modify_Cash_information'] == 'yes') || ($_SESSION['userinfo']['Modify_Credit_Information'] == 'yes')){ 
?>
    <a href='Viewedittransactionlist.php?Section=GeneralLedger&ModifyTransaction=ModifyTransactionThisPage' class='art-button-green'>
        BACK
    </a>
<?php  } } ?>
...
<!--<br/>-->
<fieldset>  
            <legend align=right><b>Edited Transaction</b></legend>
        <center> 
            <table width=100%> 
                <tr> 
                    <td>
...
<fieldset>   
		...
		<tr>
			...
			<div style="width:100%; height:210px;overflow-x:hidden;overflow-y:scroll" >
			   <?php
			     include 'Patient_Billing_View_Edit_Transaction_Iframe.php';
			   ?>
			</div>
		    </td>
		</tr>
                ...
                <td style="float:right">
                    <input type="button" value="Print" class='art-button-green' onclick="Print_Receipt_Payment('<?php echo $Patient_Payment_ID;?>')" name="<?php echo $Patient_Payment_Item_List_ID;?>" ppi="<?php echo $Patient_Payment_ID;?>">
                </td>
	...
	<td id='Items_Areaz'>
	    <fieldset style='overflow-y: scroll; height: 300px;' id='Items_Area'>
			<table width="100%" id="displayItem">
			    <?php
					$result = mysqli_query($conn,"SELECT Item_ID, Product_Name FROM tbl_items order by Product_Name LIMIT 100") or die(mysqli_error($conn));
					while($row = mysqli_fetch_array($result)){
					    echo "<tr>
...
<!--<script>
   $('#Print_List').on('click',function(){
     var Patient_Payment_ID=$(this).attr('ppi');
     var Registration_ID=$(this).attr('name');
     window.open('PrintEditedItems.php?Patient_Payment_ID='+Patient_Payment_ID+'&Registration_ID='+Registration_ID+'');
   });
</script>-->
...
function Print_Receipt_Payment(Patient_Payment_ID){
 //alert(Patient_Payment_ID);
    // var printWindow= window.open("http://www.w3schools.com", "_blank", "toolbar=yes, scrollbars=yes, resizable=yes, top=500, left=500, width=400, height=400");
       var winClose=popupwindow('PrintEditedItems.php?Patient_Payment_ID='+Patient_Payment_ID+'&IndividualSummaryReport=IndividualSummaryReportThisForm', 'Receipt Patient', 530, 400);
      //winClose.close();
