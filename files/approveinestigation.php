...
if (isset($_GET['Payment_Cache_ID'])) {
    ...
    $select_Patient = mysqli_query($conn,"select * from tbl_payment_cache pc, tbl_patient_registration pr, tbl_employee emp, tbl_sponsor sp where  pc.Registration_ID = pr.Registration_ID and
...
<br/>
<fieldset>
    <legend align=right><b>INVESTIGATION APPROVAL</b></legend>
    <center>
        <table width=100%> 
            <tr> 
                <td>
        ...
        <table width=100%>
            <tr>
                <td style='text-align: right;'>
                    <input type="button" name="Make_Payments" id="Make_Payments" value="APPROVE INVESTIGATIONS" class="art-button-green" onclick="Validate_make_Payments()">
                </td>
            </tr> 
        </table>
    </center>
...
<fieldset style='overflow-y: scroll; height: 200px;' id='Items_Fieldset_List'>
    <center>
        <table>
            ...
            $select_items = mysqli_query($conn,"SELECT ilc.Item_ID, itm.Product_Name,ilc.Transaction_type, ilc.Quantity, ilc.Edited_Quantity, ilc.Price, ilc.Discount, ilc.Payment_Item_Cache_List_ID, ilc.Check_In_Type, ilc.Payment_Item_Cache_List_ID, ilc.ePayment_Status
            ...
            if ($no > 0) {
                ...
                while ($data = mysqli_fetch_array($select_items)) {
                  ...
                  $count_rows++;
                    ?>
                 <input type="text"hidden="hidden" value="<?= $count_rows ?>" id="number_of_rows"/>
                    <tr>
                        <td><?php echo ++$temp; ?></td>
                        ...
                        if ($no == 1) {
                          ...
                          //  echo '<td><button type="button" style="display:none" class="removeItemFromCache art-button" onclick="removeitem(' . $data["Payment_Item_Cache_List_ID"] . ')">Remove</button></td>';
                        }
                        ?>
                        <td style="text-align: center;">
            ...
            <tr><td colspan="7" style="text-align: right;"><b>TOTAL</b></td><td style="text-align: right;" id="grand_total_td"><b><?php echo (($_SESSION['systeminfo']['price_precision'] == 'yes') ? number_format($Grand_Total) : number_format($Grand_Total)); ?></td></tr>
        </table>
    </center>
</fieldset>
<?php
...
<script type="text/javascript">
    function Validate_make_Payments() {
        ...
        myObjectMakePayment.open('GET', 'Validate_Make_Payments_All.php?Payment_Cache_ID=' + Payment_Cache_ID+'&items='+items, true);
    ...
    function make_Payments(Payment_Cache_ID) {
        ...
        if (Confirm_Message == true) {
            $.ajax({
               ...
               data:{Section_Link:Section_Link,Payment_Cache_ID:Payment_Cache_ID, Registration_ID:Registration_ID, items:items},
               success:function(responce){
                location.reload()
               } 
            })
           // document.location = 'Approve_Selected_Transaction_Investigation.php?' + Section_Link + 'Payment_Cache_ID=' + Payment_Cache_ID + '&Registration_ID=' + Registration_ID + '&items=' +items;
        }
    }
</script>
<script>
