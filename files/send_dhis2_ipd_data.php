...
<?php
	...
	if($group_num > 0){
		...
		while($details = mysqli_fetch_array($select_groups)){
                        $dhis_ipd_element_id = $details['dhis_ipd_element_id'];
			...
			if($disease_num > 0){
				...
				while ($dise = mysqli_fetch_array($select_diseases)) {
					...
					if($num_total_details > 0){
                        ...
                        //setting up dhis element 
                        $dhis_data = array(
			'Name' => $disease_group_name,
			'dataElement' => $dhis_ipd_element_id,
			'orgUnit' => $orgUnit,
			'period' => $period,
			'dataValues' => array(
				11,21,31,41,51,61,71,81,19,101,
				),
			);
			//pushing to array
			array_push($dhis_data_array, setDHISipdData($dhis_data,array('report_type'=>'ipd','dataset'=>'qpcwPcj8D6u')));
