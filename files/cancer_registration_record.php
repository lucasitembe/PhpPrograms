...
<a href="Cancer_patient_record.php?Registration_ID=<?php echo $Registration_ID; ?>&section=Patient&PatientFile=PatientFileThisForm&fromPatientFile=true&this_page_from=<?= $this_page_from; ?>" class="art-button-green">BACK</a>
...
<br>
    ...
    <table width = '100%'>
        ...
        <?php
     $resultPat = mysqli_query($conn,"SELECT * FROM tbl_treatment_delivery tr,tbl_machine_setup_delivery ms WHERE ms.Registration_ID = '$Registration_ID' GROUP BY wedge_date_time ORDER BY wedge_date_time DESC LIMIT 1") or die(mysqli_error($conn));

         $i = 1;
        while ($row = mysqli_fetch_array($resultPat)) {
            
                $date_and_time=$row['wedge_date_time'];
                $Registration_ID=$row['Registration_ID'];
...
?>
            ...
            <td>
            <center>
                <a href='cancer_radiotherapy_review.php?Registration_ID=<?php echo $Registration_ID; ?>&date_and_time=<?php echo $date_and_time; ?>&section=Patient&PatientFile=PatientFileThisForm&fromPatientFile=true&this_page_from=patient_record'><button class='art-button button_pro'>Review</button></a> 
                </a>
            </center>
            </td>
            </tr>
