<?php
	...
	if(isset($_GET['Employee_ID'])){
		$Endoswrist_ID = $_GET['Employee_ID']; //selected Endoswrist
	}else{
		$Endoswrist_ID = '';
	}
	...
	// die("SELECT pc.consultation_id, pc.Payment_Cache_ID from 
...
//            $Item_ID = mysqli_fetch_assoc(mysqli_query($conn,"SELECT Item_ID FROM tbl_item_list_cache WHERE Item_ID='$Item_ID'"))['Item_ID']; 
            $Patient_Payment_Item_List_ID = mysqli_fetch_assoc(mysqli_query($conn,"SELECT Patient_Payment_Item_List_ID FROM tbl_patient_payment_item_list WHERE Item_ID='$Item_ID'"))['Patient_Payment_Item_List_ID'];
             ...
             $check_consultation_id = mysqli_query($conn, "SELECT Registration_ID,Patient_Payment_Item_List_ID FROM tbl_consultation WHERE Registration_ID='$Registration_ID' AND Patient_Payment_Item_List_ID='$Patient_Payment_Item_List_ID'");
...
//         $consultation_ID=$mysql_consultation_ID;
	...
	//   die($consultation_ID."+".$Registration_ID."+".$Employee_ID."+".$Payment_Item_Cache_List_ID);
    ...
    if($consultation_ID != 0 && $Registration_ID != 0 && $Employee_ID != '' && $Payment_Item_Cache_List_ID != ''){
		...
		// die("SELECT Git_Post_operative_ID from tbl_git_post_operative_notes where Payment_Item_Cache_List_ID = '$Payment_Item_Cache_List_ID'");
	    ...
	    //check if selected Endoswrist already added
	    ...
	    if($num_check < 1){
	    	$add = mysqli_query($conn,"INSERT into tbl_git_post_operative_participant(Git_Post_operative_ID, Employee_ID, Employee_Type)
	    						values('$Git_Post_operative_ID','$Endoswrist_ID','Endoswrist')") or die(mysqli_error($conn));
	    	if($add){
