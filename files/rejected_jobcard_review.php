...
<style type="text/css">
    ...
    .row tr:nth-child(odd){
        ...
        <a href='Rejected_jobcard_list.php?section=Engineering_Works=Engineering_WorksThisPage' class='art-button-green'>
            BACK
        </a>
        ...
        <form action='' method='POST' name='' id='myForm' >
		    ...
		    <td width='10%' style='text-align: right;'>Department Requesting</td>
                ...
                <tr>
                    <td style='text-align: right; width:15%;'><b>Complaints / Faults:</b></td>
                     ...
                     ?>
                            ...
                            <td width="100%" height="20%" colspan='5'>
							<textarea id='defects' onkeyup='update_jobcard(<?= $jobcard_ID ?>)'> <?php echo $defects ?> </textarea>
                            ...
                            <td width="100%" height="20%" colspan='5'>
							<textarea id='diagnosis_action' onkeyup='update_jobcard(<?= $jobcard_ID ?>)'><?php echo $diagnosis_action?> </textarea>
                ...
                ?>
                            ...
                            <td width="100%" height="20%" colspan='5'>
							<textarea id='Comments' onkeyup='update_jobcard(<?= $jobcard_ID ?>)'> <?php echo $Comments ?> </textarea>
							
                    </td>
                            </tr>
                            <tr>
                                <td colspan='6' style='text-align: center;'><input type='submit' name='' class='btn btn-primary' onclick='save_changes()' value='    SAVE CHANGES    '></td>
                            </tr>
                                        <tr >
                    ...
                    <!-- <button type="button" name="add_item" class="btn btn-primary" onclick="mantainance_drugs()" >Add Items  </button> -->
                        <table class="table" id='spare_list'>
                            <tr>
                                <th width="2%">S/N</th>
                                <th style='text-align: left;'>ITEM(s) ORDERED</th>
                                <th width="7%">UOM</th>
                                <th width="7%">ITEM CODE</th>
                                <th width="5%">QUANTITY</th>
                                <th width="10%">ESTIMATED PRICE</th>
                            ...
                            <?php 
                                ...
                                if($no > 0){
                                    while($row = mysqli_fetch_assoc($spare_used)){
                                        ...
                                        <td><input type='text'  id='Quantity$Item_ID' placeholder='Enter Quantity' value='$Quantity' onkeyup='update_maintanance_Quantity2($jobcard_ID, $Item_ID)'></td>
                                        <td><input type='text' id='Price$Item_ID' value='$Price' placeholder='Enter The Price' onkeyup='update_maintanance_time2($jobcard_ID,$Item_ID)'></td>

                                        <td><input type='text' name='subtotal' readonly='readonly' style='text-align:right;' value='".number_format($subtotal,2)."' placeholder='Total Price'></td>
                                        <td><button class='btn btn-danger' type='button' name='removemaintanance' onclick='remove_maintanance2($jobcard_ID, $Item_ID, $Employee_ID)'>X</button></td></tr>";
                        ...
                        $Grand_total = $total_labor + $Sub_Total;
                    ...
                    <div id="drugdialog"></div>
                            ...
                            $previous_status = mysqli_query($conn,"SELECT status, Certified_by, Certified_at, Authorised_at, Authorised_by, Authorize_Comment, Certify_Comment, Approved_at, Approved_by, Rejection_Reason from tbl_jobcards where jobcard_ID='$jobcard_ID'");
                            ...
                            if ($no > 0) {
                                ...
                                while ($data2 = mysqli_fetch_array($previous_status)) {
                                    ...
                                    $Rejection_Reason = $data2['Rejection_Reason'];
                                    $Approved_by = $data2['Approved_by'];
                                    $Approved_at = $data2['Approved_at'];
                                    ...
                                    <?php
                                        ...
                                        <td style="text-align: right; width:15%;"> Certified by: </td>
                                            ...
                                            <td style="text-align: right; width:15%;"> Authorizor's Comments: </td>
                                            <td colspan="5">
                                            <textarea id="Certify_Comment" readonly="readonly" name="Certify_Comment"> <?php echo $Authorize_Comment?></textarea></tr><tr>
                                            <td style="text-align: right; width:15%;"> Rejected by: </td>
                                            <td colspan="2">
                                            <input type="text" readonly="readonly" value="<?php echo $Authorizer; ?>">
                                            </td>
                                            <td style="text-align: right; width:15%;"> Rejected at: </td>
                                            <td colspan="2">
                                            <input type="text" readonly="readonly" value="<?php echo $Approved_at; ?>">
                                            </td>
                                            </tr><tr>
                                            <td style="text-align: right; width:15%; color: red; font-weight: bold;">****Rejection Reason: </td>
                                            <td colspan="5">
                                            <textarea id="Certify_Comment" readonly="readonly" name="Certify_Comment"> <?php echo $Rejection_Reason?></textarea>
                            ...
                            <tr>
                                <?php
                                    ...
                                    }elseif($jobcard_status == 'Rejected'){
                                        if ($_SESSION['userinfo']['Authorize_Job'] == 'yes'){ ?>

                                            <td style='text-align: right; width:15%; font-weight: bold;'> Justify JobCard </td>
                                            <td width='100%' height='20%' colspan='5'>
                                            <textarea name='Authorize_Comment' required></textarea>
                                        </td>
                                        </tr>
                                        <tr>
                                        <td colspan='3'></td>
...
<script>
        function update_maintanance_time2(Jobcard_ID, Item_ID){
        ...
        function update_maintanance_Quantity2(Jobcard_ID, Item_ID){
        ...
        function remove_maintanance2(Jobcard_ID, Item_ID, Employee_ID){
            if(confirm("This Item will be removed from the list, Are you sure you want to continue?")){
                $.ajax({
                    type: 'POST',
                    url: 'add_jobcard_items.php',
        ...
        function add_maintanance(Item_ID){
            var Jobcard_ID = '<?php echo $jobcard_ID ?>';
       ...
       $.ajax({
                ...
                success:function(responce){
                    $("#drugdialog").dialog({
                        title: 'ADD CONSUMED SPARE FOR REQUISION NO: <?php echo $jobcard_ID?>',
                        width: 800, 
                        height: 600, 
                        modal: true
                        });
...
<script>
    function update_jobcard(Jobcard_ID){
        var Comments = $("#Comments").val();
        var defects = $("#defects").val();
        var diagnosis_action = $("#diagnosis_action").val();
        var Employee_ID = '<?= $Employee_ID ?>';


        if(Jobcard_ID != undefined){
            $.ajax({
                url: "ajax_update_jobcard.php",
                type: "post",
                data: {Comments:Comments,defects:defects,diagnosis_action:diagnosis_action,Jobcard_ID:Jobcard_ID,Employee_ID:Employee_ID},
                cache: false,
                success: function(responce){
    ...
    function save_changes() {
        alert("Jobcard Changes was Saved Successfully!");        
    }
</script>
