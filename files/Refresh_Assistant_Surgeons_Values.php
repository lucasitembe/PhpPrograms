    ...
    if($consultation_ID != 0 && $Registration_ID != 0 && $Employee_ID != '' && $Payment_Item_Cache_List_ID != ''){
	    ...
	    if($num_external > 0){
	        while ($ext = mysqli_fetch_array($select_external)) {
	            $External_Assistant_Surgeon = $ext['External_Assistant_Surgeon'];
	        }
	    }else{
	        $External_Assistant_Surgeon = '';
	    }

	    if($External_Assistant_Surgeon != '' && $External_Assistant_Surgeon != ''){
	    	$Externals = '('.$External_Assistant_Surgeon.')**External';
	    }else{
	    	$Externals = '';
	    }
