...
<?php
	...
	$select_Filtered_Patients = mysqli_query($conn,
		...
		from tbl_patient_registration pr, tbl_sponsor sp where
			...
			pr.Patient_Name like '%$Patient_Name%' and
				...
				pr.Phone_Number like '%$Phone_Number%'and Diseased='yes' order by Registration_ID Desc limit 200") or die(mysqli_error($conn));
    }elseif($Patient_Name != '' && $Patient_Name != null && ($Patient_Number == '' || $Patient_Number == null) && ($Phone_Number == '' || $Phone_Number == null)){
	$select_Filtered_Patients = mysqli_query($conn,
            "select pr.Patient_Name, pr.Date_Of_Birth, pr.Gender, pr.Registration_ID, pr.Phone_Number, pr.Member_Number, sp.Guarantor_Name
		from tbl_patient_registration pr, tbl_sponsor sp where
		    pr.sponsor_id = sp.sponsor_id and
			pr.Patient_Name like '%$Patient_Name%' and Diseased='yes' order by Registration_ID Desc limit 200") or die(mysqli_error($conn));
    }elseif($Patient_Number != '' && $Patient_Number != null && ($Patient_Name == '' || $Patient_Name == null) && ($Phone_Number == '' || $Phone_Number == null)){
	$select_Filtered_Patients = mysqli_query($conn,
            "select pr.Patient_Name, pr.Date_Of_Birth, pr.Gender, pr.Registration_ID, pr.Phone_Number, pr.Member_Number, sp.Guarantor_Name
		from tbl_patient_registration pr, tbl_sponsor sp where
		    pr.sponsor_id = sp.sponsor_id and
			pr.Registration_ID = '$Patient_Number' and Diseased='yes' order by Registration_ID Desc limit 200") or die(mysqli_error($conn));
	...
	echo "<td><a href='visitorform.php?Registration_ID=".$row['Registration_ID']."&PatientBilling=PatientBillingThisForm' target='_parent' style='text-decoration: none;'>".$row['Registration_ID']."</a></td>";
