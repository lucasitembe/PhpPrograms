...
if (isset($_GET['Payment_Cache_ID']) && isset($_GET['Registration_ID'])) {
    ...
    $sql_select_list_of_patient_sent_to_cashier_result = mysqli_query($conn, "SELECT pr.Phone_Number,pr.Registration_ID,pr.Patient_Name,pr.Date_Of_Birth,pr.Gender,pr.Sponsor_ID,pc.Payment_Date_And_Time,pc.Payment_Cache_ID FROM tbl_patient_registration pr,tbl_payment_cache pc WHERE pr.Registration_ID=pc.Registration_ID AND Billing_Type IN('Inpatient Cash','Outpatient Cash') $filter ORDER BY Payment_Date_And_Time DESC LIMIT 1") or die(mysqli_error($conn));
    ...
    $check_status = mysqli_fetch_assoc(mysqli_query($conn, "SELECT extenal_server_status FROM `tbl_epay_server_config` ORDER BY setup_id DESC LIMIT 1"))['extenal_server_status'];
            
            if($check_status == 'not active'){
                $epay_server = '../../mobile_processing_payfolder/ajax_card_and_mobile_epayment.php';
            }else{
                $epay_server = 'http://192.168.112.50/mobile_processing_payfolder/ajax_card_and_mobile_epayment.php';
            }
...
<a href="#" class="art-button-green" onclick="mytransactions()">POS PENDING TRANSACTION</a>
...
<?php if ($itemfrom == "Pharmacy") {
    ...
    ?>
    <a href="patient_sent_to_cashier.php" class="art-button-green">BACK</a>
    <?php }
?>
<div id="transactions" style="width:50%;margin-top: 5px;" >
...
<fieldset style="height:100px;overflow-y: hidden;overflow-x: hidden">
    <legend align='center'><b>PATIENT PAYMENT</b> <?= $Sub_Department_Name ?></legend>
...
<fieldset>
    <label id="sangila_code_feedback" class="col-md-4" style="color:red;font-size:20px">SANGIRA CODE</label><div class="col-md-2"><input type="button" onclick="print_all_sangira()" value="PRINT SANGIRA CODE" class="art-button-green" style="height:35px!important"/></div><label class="col-md-6">GRAND TOTAL AMOUNT FOR SELECTED ITEMS</label>
...
<fieldset>
    ...
    <div class="col-md-2">
        <input type="text" placeholder="Phone Number" onkeyup="validate_number()" maxlength="10"  value="<?php echo $PhoneNumber; ?>" class="form-control" style="font-weight: bold;font-size:24px" id="patient_phone_number"/>
    </div>
	...
	<div class="col-md-1" id="progress_dialog2" style="display:none">
        <img id="progressStatus2"  src="images/waiting3.gif" style="margin-top: -10px;border: 0;width: 190px; height:100px; ">
    </div>
...
<fieldset style="padding-left:150px;<?= $hide_e_pay ?>">
    ...
    <div class="col-md-2 pull-right <?= $g_pesa_payment ?>">
        <input type="button" class="art-button-green pull-right" onclick="process_g_pesa_payment()" style="width: 90%" value="G-PESA" />
    </div>
...
<!--<div class="col-md-2"><input type="button" value="ISSUE CARD" class="art-button-green" style="width: 90%"/></div>-->
    ...
    <div class="col-md-2 pull-right <?= $crdb_card_pay ?>">
        <input type="button" class="art-button-green pull-right " onclick="crdb_card_makepayment2()" style="width:90%;background: green" value="Demo Card Payment" />
    </div>
</fieldset>
...
<div id="Submit_data">
    <center><b>You are About to Create Transaction for Selected Items above, This Action is <span style='font-size: 22px; color: red;'>IRREVERSIBLE</span>, Are you Sure you want to Proceed?</b></center><br/>
    <table width="100%">
        <tr>
            <td style="text-align: right;">
                <input type="text" name="payment_direction2" id="payment_direction2" style='display: none;'>
                <input type="button" name="Submit_Information" id="Submit_Information" value="YES" class="art-button-green" onclick="make_epayment_confirm()">
...
<script>
 function mytransactions() {
            $.ajax({
                type: 'POST',
                url: 'ajax_fetch_mytransaction.php',
                data: {},
                success: function (data) {
                    $("#transactions").dialog("open");
...
function cancel_crdb_transaction(billID) {
            $.ajax({
                type: 'POST',
                url: 'ajax_fetch_mytransaction.php',
                data: {billID: billID, delete: "delete"},
                success: function (data) {
                    $("#transactiondata").html(data);
                }
            });
...
//        alert(Payment_Cache_ID);
            $.ajax({
                type: 'POST',
                url: 'ajax_filter_items_by_department.php',
                data: {Check_In_Type: Check_In_Type, Payment_Cache_ID: Payment_Cache_ID},
                success: function (data) {
...
//                        ecr_paycode(data) 
        ...
        function manual_payment_complete_payment_process(Payment_Code, Transaction_ID) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    var html = "<div class='pull-right'>&nbsp;&nbsp;<input type='button' class='art-button hide' value='PRINT DETAIL RECEIPT' onclick='Print_Receipt_Payment_new(\"" + Payment_Code + "\")'></div>";
        ...
        function process_g_pesa_payment(payment_direction = "gpesa") {
            //////////////////////////////////////////////////////////////
            var selected_Payment_Item_Cache_List_ID = [];
            var grand_total_price = 0;
            $(".Payment_Item_Cache_List_ID:checked").each(function () {
...
<?php $Employee_ID = $_SESSION['userinfo']['Employee_ID']; ?>
                    ...
                    var epay_server = '<?= $epay_server ?>'
                    $("#progress_dialog").show();
                    $.ajax({
                        type: 'POST',
                        url: epay_server+'mobile_processing_payfolder/mobile_g_pesa_epayment.php',
...
//           alert(Payment_Item_Cache_List_ID+"==>"+grand_total_price)
            ...
            if (selected_Payment_Item_Cache_List_ID.length > 0) {
                $(".select_item_column").css("background", "white");
                var payment_direction = payment_direction;
                if(payment_direction === 'to_nmb'){
                    var Bank = 'NMB';
                }else if(payment_direction === 'to_crdb'){
                    var Bank = 'CRDB';
                }

                document.getElementById("payment_direction2").value = payment_direction;

            $("#Submit_data").dialog({autoOpen: false, width: '60%', height: 150, title: 'CONFIRM THIS TRANSACTION TO '+Bank, modal: true});
        ...
        function make_epayment_confirm() {
       $("#Submit_data").dialog("close");
            var selected_Payment_Item_Cache_List_ID = [];
            var grand_total_price = 0;
            var payment_direction = $("#payment_direction2").val();
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            if (data == "fail") {
                                ...
                                $("#patient_bill_dialog").dialog("close");
                                setTimeout(function () {
                                    read_return_controlnumber(data)
                                }, 3000);
...
//                    $("#progress_dialog").hide();
        ...
        function crdb_card_makepayment2() {
            var selected_Payment_Item_Cache_List_ID = [];
            var grand_total_price = 0;
            $(".Payment_Item_Cache_List_ID:checked").each(function () {
                selected_Payment_Item_Cache_List_ID.push($(this).val());
                ...
                if (validate <= 0) {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            if (data == "already_created") {
//                        alert("Selected Item(s) Arleady Sent To POS");
                                if (confirm("Selected Item(s) Arleady Sent To POS,Are you sure you want to clear and resend?")) {
                                    crdb_card_makepayment();
                                } else {
                                    $("#progress_dialog").hide();
                                }
                            } else {
...
//                    $("#progress_dialog").hide();
        ...
        function confirmTransaction(transactionID) {
            $.ajax({
                type: 'POST',
                url: 'ajax_check_crdb_transaction.php',
                data: {
                ...
                success: function (data) {
                    if (data == "fail") {
                        $("#progress_dialog").hide();
                        alert("Transaction time out Click ok to re-synchronize.");
                        ...
                        Print_Receipt_Payment_new(data);
						$("#progress_dialog2").hide();
						$("#progressStatus").hide();
...
//                        ecr_paycode(data) 
                        ...
                        var html = "<div class='pull-right'><input type='button' value='RE-SYNCHRONIZE' class='art-button-green' style='display: none;' onclick='sync_epayments_force(\"" + Registration_ID + "\",\"" + Transaction_ID + "\",\"" + Payment_Code + "\")'>&nbsp;&nbsp;<input type='button' class='art-button-green' value='PRINT DETAIL RECEIPT' onclick='Print_Receipt_Payment_new(\"" + Payment_Code + "\")'></div>";
                        $("#print_and_synchronyzation_btn").html(html);
//                        console.log(html);
                        ecr_paycode(data)
                    } else {
...
<script>
    $(document).ready(function () {
        $("#transactions").dialog({autoOpen: false, width: '85%', height: 400, title: 'POS PENDING TRANSACTIONS', modal: true});
