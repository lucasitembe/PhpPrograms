...
if (isset($_SESSION['userinfo'])) {
    if ($_SESSION['userinfo']['Procurement_Works'] == 'yes') {
        ...
        $Select_Pending_Order_Number = mysqli_query($conn,"select po.Purchase_Order_ID from  tbl_purchase_order po,tbl_purchase_order_items poi,tbl_sub_department sd where
...
if (isset($_SESSION['userinfo'])) {
    if ($_SESSION['userinfo']['Procurement_Works'] == 'yes') {
        echo "<a href='purchaseoutstandingorders.php?ProcurementWork=ProcurementWorkThisPage' class='art-button-green'>BACK</a>";
    }
}
?>
...
<script>
    function Submit_Order() {
        ...
        var datastring = 'Supplier_ID=' + Supplier_ID + '&Store_Need=' + Store_Need + '&Order_Description=' + Order_Description + '&Purchase_Order_ID=<?php echo $Purchase_Order_ID; ?>';

        $.ajax({
            type: "POST",
            url: "submit_new_purchase_order.php",
            dataType: 'JSON',
            data: datastring,
            success: function (result) {
                if (result.status == '0') {
                    ...
                    document.location = 'purchaseordereprocess.php?action=new&Purchase_Order_ID=' + result.data;
                }
            }
        });
    }
...
<script>
    function Confirm_Remove_Item(Item_Name, Order_Item_ID) {
        ...
        if (Confirm_Message == true) {
            ...
            My_Object_Remove_Item.open('GET', 'discard_purchase_order.php?type=single&Order_Item_ID=' + Order_Item_ID, true);
...
<form action='#' method='post' name='myForm' id='myForm' >
    ...
    if(isset($_GET['action']) && $_GET['action']=='new'){
      $select_order_items = mysqli_query($conn,"SELECT poi.Order_Item_ID,itm.Product_Name, poi.Quantity_Required, poi.Price,
                                              poi.Containers_Required, poi.Items_Per_Container_Required,poi.Quantity_Received, poi.Item_ID, poi.Buying_Price,'' AS OutStanding
									   FROM  tbl_purchase_order po,tbl_purchase_order_items poi, tbl_items itm
									   WHERE 
                                                                            itm.Item_ID = poi.Item_ID AND
                                                                            po.Purchase_Order_ID = poi.Purchase_Order_ID AND
								            poi.Purchase_Order_ID ='$Purchase_Order_ID'") or die(mysqli_error($conn));
    ...
    <fieldset style='overflow-y: scroll; height: 260px;' id='Items_Fieldset_List'>
        ...
        echo '<tr>
				...
				<td width=7% style="text-align: right;">Quantity Required</td>
                                <td width=7% style="text-align: right;">OutStanding</td>
        ...
        while ($row = mysqli_fetch_array($select_order_items)) {
            ...
            ?>
            <td>
                <input type='text' id='Container_<?php echo $row['Order_Item_ID']; ?>' readonly="readonly" value='<?php echo $row['Containers_Required']; ?>' style='text-align: right;' oninput="Update_Quantity('<?php echo $row['Order_Item_ID']; ?>')">
            </td>
            <td>
                <input type='text' id='Items_<?php echo $row['Order_Item_ID']; ?>' readonly="readonly" value='<?php echo $row['Items_Per_Container_Required']; ?>' style='text-align: right;' oninput="Update_Quantity('<?php echo $row['Order_Item_ID']; ?>')">
            </td>		
            <td>
                <input type='text' readonly="readonly" value='<?php echo $row['Quantity_Required']; ?>' style='text-align: right;' >
            </td>
            <td>
                <input type='text' id='QR<?php echo $row['Order_Item_ID']; ?>'  readonly="readonly"  value='<?php echo $row['OutStanding']; ?>' style='text-align: right;' oninput="Update_Quantity2(this.value,<?php echo $row['Order_Item_ID']; ?>)">
            </td>
            <td>
                <input type='text' id='Balance_<?php echo $row['Order_Item_ID']; ?>' value='<?php echo $Item_Balance; ?>' style='text-align: right;' readonly="readonly">
            </td>
            <td>
                <input type='text' id='<?php echo $row['Order_Item_ID']; ?>' name='<?php echo $row['Order_Item_ID']; ?>' value='<?php echo $row['Price']; ?>' style='text-align: right;' oninput="Update_Price(this.value,<?php echo $row['Order_Item_ID']; ?>)">
            </td>
            <?php
            echo "<td><input type='text' name='Sub_Total" . $row['Order_Item_ID'] . "' id='Sub_Total" . $row['Order_Item_ID'] . "' readonly='readonly' value='" . number_format($row['Quantity_Required'] * $row['Price'], 2) . "' style='text-align: right;'></td>";
            //echo "<td><input type='text' value='".$row['Item_Remark']."'></td>";
            ?>
            <td width=6%>
                <input type='button' name='Remove_Item' id='Remove_Item' value='X' class='art-button-green' onclick='Confirm_Remove_Item("<?php echo str_replace("'", "", $row['Product_Name']); ?>",<?php echo $row['Order_Item_ID']; ?>)'>
            </td>
            <td width=6%>
        ...
        echo '<tr><td style="text-align:right" colspan="8"><b>Grand Total: </b></td><td style="text-align:right"><b>' . number_format($total, 2) . '</b></td></tr></table>';
        ?>
    </fieldset>
            ...
            <tr>
                ...
                <td width="15%" id="Submit_Button_Area">
                    <?php if(isset($_GET['action']) && $_GET['action']=='new'){
                        ...
                        <input type='button' name='' id='' class='art-button-green' value='CREATE PURCHASE ORDER' onclick='Confirm_Submit_Order(<?php echo $Supplier_ID ?>)'>
                    <?php } ?>
                </td>
    ...
    <script>
        function Confirm_Submit_Order(Supplier_ID) {
            var Store_Need = document.getElementById("Store_Need").value;
            var Curr_Supplier_ID = document.getElementById("Supplier_ID").value;

            if (Curr_Supplier_ID == Supplier_ID) {
                if (!confirm("Are you sure you want to resend order to the selected supplier?")) {
                    exit;
                }
            }

            if (Store_Need != null && Store_Need != '') {
                var Confirm_Message = confirm("Are you sure you want to create new purchase order?.");
                if (Confirm_Message == true) {
                    Submit_Order();
                }
            } else {
    ...
    <script>
        function Update_Supplier(Supplier_ID, Order_Item_ID) {
            if (Supplier_ID != null && Supplier_ID != '' && Order_Item_ID != null && Order_Item_ID != '') {
                if (window.XMLHttpRequest) {
                ...
                myObjectUpdatePrice.open('GET', 'Update_Supplier_Purchase_Cache.php?Order_Item_ID=' + Order_Item_ID + '&Supplier_ID=' + Supplier_ID, true);
                myObjectUpdatePrice.send();
            }
        }

        function Update_Price(Price, Order_Item_ID) {

            if (Price != null && Price != '' && Order_Item_ID != null && Order_Item_ID != '') {
                if (window.XMLHttpRequest) {
                    myObjectUpdatePrice = new XMLHttpRequest();
                } else if (window.ActiveXObject) {
                    myObjectUpdatePrice = new ActiveXObject('Micrsoft.XMLHTTP');
                ...
                myObjectUpdatePrice.onreadystatechange = function () {
                    data2000 = myObjectUpdatePrice.responseText;
                    if (myObjectUpdatePrice.readyState == 4) {
                        document.getElementById(Order_Item_ID).value = data2000;
                        update_sub_total(Order_Item_ID);
                    }
                }; //specify name of function that will handle server response........
                myObjectUpdatePrice.open('GET', 'Update_Price_Purchase.php?Order_Item_ID=' + Order_Item_ID + '&Price=' + Price, true);
                myObjectUpdatePrice.send();
            }
        }
    </script>
    ...
    <script>
        function Update_Quantity2(Quantity, Order_Item_ID) {
            if (Quantity != '' && Quantity != null && Order_Item_ID != null && Order_Item_ID != '') {
                document.getElementById("Container_" + Order_Item_ID).value = 1;
                document.getElementById("Items_" + Order_Item_ID).value = Quantity;
                if (window.XMLHttpRequest) {
                ...
                myObjectUpdateQuantity.onreadystatechange = function () {
                    ...
                    if (myObjectUpdateQuantity.readyState == 4) {
                        document.getElementById("QR" + Order_Item_ID).value = data2090;
                        update_sub_total(Order_Item_ID);
                    }
                }; //specify name of function that will handle server response........
                myObjectUpdateQuantity.open('GET', 'Update_Quantity_Purchase2.php?Order_Item_ID=' + Order_Item_ID + '&Quantity=' + Quantity, true);
    ...
    <script>
        function Update_Quantity(Order_Item_ID) {
            ...
            var Items_Per_Container = document.getElementById("Items_" + Order_Item_ID).value;
            if (Order_Item_ID != null && Order_Item_ID != '') {
                ...
                myObjectUpdateQuantity.open('GET', 'Update_Quantity_Purchase.php?Order_Item_ID=' + Order_Item_ID + '&Containers=' + Containers + '&Items_Per_Container=' + Items_Per_Container, true);
                myObjectUpdateQuantity.send();
            }
        }
    </script>
    ...
    <script>
        function update_sub_total(Order_Item_ID) {
            var Price = document.getElementById(Order_Item_ID).value;
            var Quantity = document.getElementById("QR" + Order_Item_ID).value;
            document.getElementById("Sub_Total" + Order_Item_ID).value = (Price * Quantity);
        }
    </script>
