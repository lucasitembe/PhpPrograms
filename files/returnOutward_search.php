...
<?php
    ...
    $Purchase_History_List = Get_ReturnOutward_Item_History($Supplier_ID, $Classification, null, $Start_Date, $End_Date, 0);
...
//            $Title .= "<td width=15% style='text-align: left;'><b>PRICE</b></td>";
        ...
        foreach($Purchase_History_List as $Purchase_History){
           ...
           if(empty($lastprice)){
            ...
            $Sub_Total = $buying * $Purchase_History['Quantity_Returned'];
            ...
            if(array_key_exists($Purchase_History['Item_ID'], $Item_List)){
                ...
                $Product_Quantity_List[$Purchase_History['Item_ID']] += $Purchase_History['Quantity_Returned'];
//                $buying_price=$buying;
            }else{
                $Item_List[$Purchase_History['Item_ID']] = $Sub_Total;
...
//                $buying_price=0;
        ...
        if ($Report_Type == "SUMMARY") {
            echo $Title;
            $temp = 1;
//            $returnInward=  mysqli_query($conn,"SELECT * FROM tbl_return_inward tri JOIN tbl_return_inward_items ti ON tri.Inward_ID=ti.Inward_ID JOIN ");
//            while ($row=  mysqli_fetch_assoc($returnInward)){
//               echo "<tr>";
//               echo "<td ><b> {$temp}. </b></td>";
//               echo "<td > mm</td>";
//               echo "<td style='text-align: right;'>nn</td>";
//               echo "</tr>";
//               
//               $temp++; 
//                
