...
require_once './functions/items.php';
    ...
    if($Sponsor_ID != '' && $Sponsor_Name != ''){
		...
		$select_items = mysqli_query($conn,"SELECT ilc.Sub_Department_ID,ilc.Quantity,ilc.Clinic_ID,ilc.finance_department_id,ilc.Edited_Quantity, ilc.Price, ilc.Discount, ilc.Payment_Item_Cache_List_ID, ilc.Check_In_Type, Consultant, 
	                                    ...
	                                    (ilc.Check_In_Type = 'Procedure' or ilc.Check_In_Type = 'Surgery' or ilc.Check_In_Type='Nuclearmedicine') and
	                                    pc.Payment_Cache_ID = '$Payment_Cache_ID' and
	                                    pc.Registration_ID = '$Registration_ID' and Payment_Item_Cache_List_ID='$Payment_Item_Cache_List_ID' AND
    	...
    	if(mysqli_num_rows($select_items) > 0){
			...
			while ($rm = mysqli_fetch_array($select_items)) {
				...
				$select_patient_payment= mysqli_query($conn, "SELECT pp.Patient_Payment_ID, pp.Payment_Date_And_Time FROM tbl_patient_payments pp, tbl_patient_payment_item_list ppil WHERE pp.Patient_Payment_ID=ppil.Patient_Payment_ID AND pp.Registration_ID='$Registration_ID' AND (ppil.Check_In_Type = 'Procedure' or ppil.Check_In_Type = 'Surgery' or ppil.Check_In_Type='Nuclearmedicine')  AND pp.Employee_ID='$Employee_ID' AND Check_In_ID='$Check_In_ID'   AND Receipt_Date='$Today' ORDER BY pp.Patient_Payment_ID DESC LIMIT 1") or die(mysqli_error($conn));
    ...
    header("Location: ./approveprocedure.php?{$Section_Link}Registration_ID=$Registration_ID&Payment_Cache_ID=$Payment_Cache_ID&ApproveInvestigation=ApproveInvestigationThisPage");
}
?>
