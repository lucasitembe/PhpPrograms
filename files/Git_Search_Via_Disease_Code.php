...
<table width="100%">
	<?php
		...
		echo $Title;
		if($subcategory_ID == 0){
			$select = mysqli_query($conn,"select disease_ID, disease_code, disease_name from tbl_disease where disease_code like '%$Disease_Code%' order by disease_name limit 200") or die(mysqli_error($conn));
		}else{
			$select = mysqli_query($conn,"select disease_ID, disease_code, disease_name from tbl_disease where disease_code like '%$Disease_Code%' and subcategory_ID = '$subcategory_ID' order by disease_name limit 200") or die(mysqli_error($conn));
		}
