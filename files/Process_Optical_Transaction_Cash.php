<?php
	...
	//get sponsor name
	$select = mysqli_query($conn,"select Guarantor_Name from tbl_sponsor sp, tbl_patient_registration pr where
	...
	//get Billing_Type
	$select = mysqli_query($conn,"select Billing_Type from tbl_optical_items_list_cache where Item_Cache_ID = '$Item_Cache_ID'") or die(mysqli_error($conn));
	...
	if(strtolower($Billing_Type) == 'inpatient cash' && strtolower($Guarantor_Name) != 'cash'){
		$payment_type = 'pre';
	}else{
		$payment_type = 'post';
	}
	//get Patient_cache_id
    ...
    if($Sponsor_ID != '' && $Sponsor_Name != ''){
    	...
    	if($no > 0){
		    ...
		    $create_receipt = mysqli_query($conn,"insert into tbl_patient_payments(
											...
											Receipt_Date,Branch_ID,Patient_Bill_ID,Check_In_ID,payment_type)
	                                    
									values('$Registration_ID','$Supervisor_ID','$Employee_ID',
										(select now()),'$Folio_Number','$Claim_Form_Number',
										'$Sponsor_ID','$Sponsor_Name','$Billing_Type',
										(select now()),'$Branch_ID','$Patient_Bill_ID','$Check_In_ID','$payment_type')") or die(mysqli_error($conn));
