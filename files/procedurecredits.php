...
<fieldset style='overflow-y: scroll; height: 380px; background-color: white;' id='Patient_List_Area'>
    <legend align="right;">PROCEDURE, NUCLEAR MEDICINE & SURGERY TRANSACTIONS</legend>
    <table width="100%">
...
//                                    group by pr.Registration_ID, pc.Payment_Cache_ID order by ilc.Payment_Item_Cache_List_ID desc limit 200") or die(mysqli_error($conn));
        ...
        if($nm > 0){
            while ($row = mysqli_fetch_array($get_details)) {
               ...
               $query_result= mysqli_query($conn,"SELECT Payment_Cache_ID FROM tbl_item_list_cache WHERE Check_In_Type IN ('Procedure', 'Nuclearmedicine',  'Surgery') AND ePayment_Status = 'pending' AND Status = 'active' AND Payment_Cache_ID='$Payment_Cache_ID'") or die(mysqli_error($conn));
...
<script type="text/javascript">
    ...
    function Filter_Patient(){
        ...
        if(Start_Date != null && Start_Date != '' && End_Date != null && End_Date != ''){
            ...
            myObjectFilter.open('GET','Procedure_Credit_Filter.php?Section='+Section+'&Sponsor_ID='+Sponsor_ID+'&Start_Date='+Start_Date+'&End_Date='+End_Date, true);
            myObjectFilter.send();
        }else{
...
<script type="text/javascript">
    function Search_Patient(){
        ...
        if(outpatient_bill_credit_trns_approval=="approve_outpatient_bill_transaction"){
            search_url="procedure_surgery_outpatient_bill.php"; 
        }else{
            search_url="Procedure_Credit_Filter_Specific.php";
        }
...
<script type="text/javascript">
    function Search_Patient_Via_Number(){
        ...
        myObjectFilter.open('GET','Procedure_Credit_Filter_Specific.php?Section='+Section+'&Sponsor_ID='+Sponsor_ID+'&Start_Date='+Start_Date+'&End_Date='+End_Date+'&Patient_Number='+Patient_Number, true);
        myObjectFilter.send();
    }
</script>
