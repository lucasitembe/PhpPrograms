...
<?php
    ...
    if (isset($_SESSION['Stock_Taking_ID'])) {
        $Stock_Taking_ID = $_SESSION['Stock_Taking_ID'];
    }
...
<form action='#' method='post' name='myForm' id='myForm' >
            ...
            <tr>
                ...
                <td width='16%'>
                    <select name='Stock_Taking_Location'  class="Stock_Taking_Location" id='Stock_Taking_Location' style="width:100%">
                        <?php
                            if (!empty($Stock_Taking_ID)) {
                                $Sub_Department = Get_Sub_Department($Stock_Taking['Sub_Department_ID']);
    ...
    function Add_To_Stock_Taking(Item_ID){
        ...
        if ((Stock_Taking_Location > 0 || Stock_Taking_Location != "") &&
            ...
            if (Stock_Taking_ID == "new") {
                ...
                myDisposalObject.open('GET', 'stocktaking_check_item_exists.php?Stock_Taking_ID=' + Stock_Taking_ID
                    + '&Item_ID=' + Item_ID, true);
                myDisposalObject.send();
            }
        } else {
            Error_Message("PLEASE SELECT <b>STOCK TAKING DATE</b> AND <b>STOCK TAKING LOCATION</b> ABOVE TO CONTINUE");
        }
    }
    ...
    function Add_To_Stock_Taking_Item(Item_ID){
        ...
        if (Stock_Taking_ID > 0) {
            ...
            myItemsDisposalItemObject.open('GET', 'stocktaking_add_stocktaking_item.php?Stock_Taking_ID=' + Stock_Taking_ID
                + '&Item_ID=' + Item_ID, true);
            myItemsDisposalItemObject.send();
        }
    }

    function Confirm_Remove_Item (Item_Name, Stock_Taking_Item_ID) {
        Stock_Taking_ID = document.getElementById("Stock_Taking_ID").value;
        ...
        if (Confirm_Message == true) {
            ...
            myObject.open('GET', 'stocktaking_remove_item.php?Stock_Taking_Item_ID=' + Stock_Taking_Item_ID
                + "&Stock_Taking_ID=" + Stock_Taking_ID, true);
            myObject.send();
        }
    }

    function Update_Under_Quantity (Under_Quantity, Stock_Taking_Item_ID) {
        ...
        if ( Under_Quantity > 0 && (Store_Balance - Under_Quantity) < 0 ) {
            Error_Message("THIS TRANSACTION IS NOT ALLOWED");
            Under_Quantity = Store_Balance;
            document.getElementById("Under_Quantity_"+Stock_Taking_Item_ID).value = Store_Balance;
        }

        if (Under_Quantity > 0) {
            Balance_After_Disposal = Store_Balance - Under_Quantity;
        ...
        if (Under_Quantity >= 0) {
            ...
            myObject.open('GET', 'stocktaking_update_under_quantity.php?Stock_Taking_Item_ID=' + Stock_Taking_Item_ID
    ...
    function Update_Over_Quantity (Over_Quantity, Stock_Taking_Item_ID) {
        Store_Balance = parseInt(document.getElementById("Store_Balance_"+Stock_Taking_Item_ID).value);
        if(Over_Quantity == "") { Over_Quantity = 0; }
        Over_Quantity = parseInt(Over_Quantity);

        if (Over_Quantity > 0) {
            Balance_After_Disposal = Store_Balance + Over_Quantity;
            document.getElementById("Balance_After_Stock_Taking_"+Stock_Taking_Item_ID).value = Balance_After_Disposal;
        }
        ...
        if (Over_Quantity >= 0) {
            ...
            myObject.open('GET', 'stocktaking_update_over_quantity.php?Stock_Taking_Item_ID=' + Stock_Taking_Item_ID
    ...
    function Update_Item_Remark(Item_Remark, Stock_Taking_Item_ID) {
        if (window.XMLHttpRequest) {
            myObject = new XMLHttpRequest();
        } else if (window.ActiveXObject) {
            myObject = new ActiveXObject('Micrsoft.XMLHTTP');
        ...
        myObject.open('GET', 'stocktaking_update_item_remark.php?Stock_Taking_Item_ID=' + Stock_Taking_Item_ID
                    ...
                    $.ajax({
                        ...
                        success: function (feedback) {
                            if (feedback == '1') {
                                ...
                                Error_Message("Unable To Submit Document. Please check again and remove any item that has Zero(0) on both Under and Over Quantity!!");
                                //location.reload();
                            } else {
                                if (feedback == '0') {
