...
if(isset($_GET['Billing_Type'])){
    ...
    if($Sponsor_ID != '' && $Sponsor_Name != ''){
    	...
    	if($no > 0){
		    ...
		    header("Location: ./previewapprovedtransaction.php?{$Section_Link}Section=Pharmacy&Patient_Payment_ID=$Patient_Payment_ID&Registration_ID=$Registration_ID&PreviewApprovedTransaction=PreviewApprovedTransactionThisPage");
		}else{
		    header("Location: ./approvepharmacy.php?{$Section_Link}Registration_ID=$Registration_ID&Payment_Cache_ID=$Payment_Cache_ID&ApproveInvestigation=ApproveInvestigationThisPage");
		}
	}else{
		header("Location: ./approvepharmacy.php?{$Section_Link}Registration_ID=$Registration_ID&Payment_Cache_ID=$Payment_Cache_ID&ApproveInvestigation=ApproveInvestigationThisPage");
	}
